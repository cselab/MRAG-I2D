...
public:
  ...
  virtual void Write(GridType &inputGrid, string fileName) {
    ...
    // Calling Base-Class _Write function, but now with a pointer to the binary
    // File.
    ...
    if (status == 0)
      ...
      cout << "Something went wrong writing to binary file " << fileNameBinary
           << endl;
      abort();
    }
  }
  ...
  virtual void Read(GridType &inputGrid, string fileName) {
    ...
    // Calling Base-Class _Read function, but now with a pointer to the binary
    // File.
    ...
    if (status == 0)
      ...
      cout << "Something went wrong reading from binary file " << fileNameBinary
           << endl;
      abort();
    }
  }
