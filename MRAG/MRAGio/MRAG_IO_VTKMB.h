...
public:
  ...
  void Write(GridType &inputGrid, BoundaryInfo &bInfo, string fileName) {
    ...
    for (int i = 0; i < vInfo.size(); i++) {
      ...
      fwrapper << "		<DataSet dataset=\"" << i
               << "\" group=\"0\" file=\"" << fileName << "_" << i << ".vti\""
               << "/>" << endl;
    }

    // Close wrapper
    fwrapper << "	</vtkMultiBlockDataSet>" << endl << "</VTKFile>" << endl;
    fwrapper.close();
  }

  void VTI_Dump(BlockInfo &info, TLab &lab, const char *fileroot, int bidx,
                int Lmax) {
    // Open output file
    char filename[255];
    ...
    unsigned int totalNumberOfPoints =
        (TBlock::sizeX + 1) * (TBlock::sizeY + 1) * (TBlock::sizeZ + 1);
    ...
    imageData->SetDimensions(TBlock::sizeX + 1, TBlock::sizeY + 1,
                             TBlock::sizeZ + 1);
      ...
      for (int bz = 0; bz < TBlock::sizeZ + 1; bz++)
        for (int by = 0; by < TBlock::sizeY + 1; by++)
          for (int bx = 0; bx < TBlock::sizeX + 1; bx++) {
            const float rValue =
                lab(bx, by, bz).giveMe(ichannel + iChannelStart, h);
            *vpts++ = rValue;
          }
