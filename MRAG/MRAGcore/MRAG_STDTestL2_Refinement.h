...
class MRAG_STDTestL2_Refinement // LEVEL 2: correct data representation, correct
                                // data-processing
{
protected:
  ...
  MRAG_STDTestL2_Refinement() : grid(2, 2), refiner() {
    ...
    for (int i = 0; i < vInfo.size(); i++) {
      ...
      for (int iy = 0; iy < Block::sizeY; iy++)
        for (int ix = 0; ix < Block::sizeX; ix++) {
          ...
          block(ix, iy) =
              -1. / (2. * M_PI) *
              (cos(2. * M_PI * (x[0] + 0.5 * info.h[0])) -
               cos(2. * M_PI * (x[0] - 0.5 * info.h[0]))) /
              info.h[0]; // sin((x[0])*2.0*M_PI);//*cos((x[1])*2.0*M_PI);
        }
    }
  }
  ...
  void _some_refinements() {
    for (int iRef = 0; iRef < 1; iRef++) {
      ...
      for (int i = 0; i < vInfo.size(); i++)
        shouldBeRefined.insert(
            vInfo[i].blockID); /* everything should be refined */
        ...
        for (int ix = 0; ix < Block::sizeX; ix++) {
          ...
          if (abs_error > 0.1) {
            printf("Funny error at ix=%d,iy=%d,l=%d  e=%f, block=%f\n", ix, iy,
                   info.level, exact_value, block(ix, iy));
          }
        }
