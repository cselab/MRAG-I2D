...
template <typename WaveletsType, typename BlockType> class MRAG_BBInfoCreator {
  ...
  template <typename T>
  inline _MRAG_GHOSTSCREATION_ALLOCATOR<T> allocator() const {
  ...
  struct BastardGhost {
    ...
    typedef Matrix3D<BastardGhost *, true, _MRAG_GHOSTSCREATION_ALLOCATOR>
        MatrixOfRequests;
    ...
    vector<BastardGhost *, _MRAG_GHOSTSCREATION_ALLOCATOR<BastardGhost *>>
        customers;
    ...
    void resolvedLeaf(bool bKnown, int iPoolIndex) {
      ...
      status =
          bKnown ? BastardGhost_ResolvedKnown : BastardGhost_ResolvedUnknown;

      for (int c = 0; c < customers.size(); c++)
        customers[c]->resolved(
            this, status == BastardGhost_ResolvedKnown ? true : false);
    }
    ...
    template <typename FrontType, typename BufferLayer, typename LayerAllocator,
              template <typename V, typename A> class BufferType>
    void generateRequests(SmartBlockFinder &smartFinder,
                          BufferType<BufferLayer, LayerAllocator> &buffer,
                          FrontType &newFront, int minLevel, int lying_level) {
      assert(status == BastardGhost_Unresolved);
      ...
      int s[3] = {
          B::shouldProcessDirectionX
              ? (bAnalysis ? index.i[0] * 2 - W::HaSupport[1] + 1
                           : (int)ceil((index.i[0] + W::HsSupport[0]) * 0.5))
              : 0,
          B::shouldProcessDirectionY
              ? (bAnalysis ? index.i[1] * 2 - W::HaSupport[1] + 1
                           : (int)ceil((index.i[1] + W::HsSupport[0]) * 0.5))
              : 0,
          B::shouldProcessDirectionZ
              ? (bAnalysis ? index.i[2] * 2 - W::HaSupport[1] + 1
                           : (int)ceil((index.i[2] + W::HsSupport[0]) * 0.5))
              : 0};

      int e[3] = {
          B::shouldProcessDirectionX
              ? (bAnalysis
                     ? index.i[0] * 2 - W::HaSupport[0] + 1
                     : 1 + (int)floor((index.i[0] + W::HsSupport[1] - 1) * 0.5))
              : 1,
          B::shouldProcessDirectionY
              ? (bAnalysis
                     ? index.i[1] * 2 - W::HaSupport[0] + 1
                     : 1 + (int)floor((index.i[1] + W::HsSupport[1] - 1) * 0.5))
              : 1,
          B::shouldProcessDirectionZ
              ? (bAnalysis
                     ? index.i[2] * 2 - W::HaSupport[0] + 1
                     : 1 + (int)floor((index.i[2] + W::HsSupport[1] - 1) * 0.5))
              : 1};
        ...
        for (i[1] = s[1]; i[1] < e[1]; i[1]++)
          for (i[0] = s[0]; i[0] < e[0]; i[0]++) {
            ...
            /*i[0]*((bAnalysis || !B::shouldProcessDirectionX)?1:2),
                                     i[1]*((bAnalysis ||
               !B::shouldProcessDirectionY)?1:2), i[2]*((bAnalysis ||
               !B::shouldProcessDirectionZ)?1:2)* /);//i[1], i[2]); */

            const double w = vWeights[0][i[0] - s[0]] *
                             vWeights[1][i[1] - s[1]] *
                             vWeights[2][i[2] - s[2]];
            ...
            typename BufferLayer::const_iterator it =
                buffer[request_block_level - minLevel].find(key);
            ...
            matRequests->Access(i[0] - s[0], i[1] - s[1], i[2] - s[2]) =
                request;

            if (request->status == BastardGhost::BastardGhost_Unresolved ||
                request->status == BastardGhost_Unresolved_PendingRequests) {
    ...
    void collect(WeightsSet &wSet, MappingW2I &mapW2I, vector<IndexWP> &info,
      ...
      if (matRequests == NULL) // leaf
      {
        ...
        const int n[3] = {(int)matRequests->getSize()[0],
        ...
        for (i[2] = 0; i[2] < n[2]; i[2]++)
          for (i[1] = 0; i[1] < n[1]; i[1]++)
            for (i[0] = 0; i[0] < n[0]; i[0]++) {
              ...
              if (request != NULL)
                request->template collect<iPass>(
                    wSet, mapW2I, info, weightsPool, wX * vWeights[0][i[0]],
                    wY * vWeights[1][i[1]], wZ * vWeights[2][i[2]], bVerbose);
            }
      }
    }

    BastardGhost(const int block_level_, const int point_index[3])
        : status(BastardGhost_Unresolved),
          candidate_status(BastardGhost_ResolvedKnown),
          block_level(block_level_), nMissingReferences(0), index(),
          iPoolIndex(0), customers(), matRequests(NULL) {
    ...
    BastardGhost(const BastardGhost &b)
        : status(BastardGhost_Unresolved),
          candidate_status(BastardGhost_ResolvedKnown),
          block_level(b.block_level), nMissingReferences(0), index(),
          iPoolIndex(0), customers(), matRequests(NULL) {
    ...
    ~BastardGhost() {
      ...
      if (vWeights[0] != NULL)
        allocator<double>().deallocate(vWeights[0], nWeightSize[0]);
      if (vWeights[1] != NULL)
        allocator<double>().deallocate(vWeights[1], nWeightSize[1]);
      if (vWeights[2] != NULL)
        allocator<double>().deallocate(vWeights[2], nWeightSize[2]);
  ...
  private:
    // forbidden
    BastardGhost &operator=(const BastardGhost &) {
      abort();
      return *this;
    }
  };
