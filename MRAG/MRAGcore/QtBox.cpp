...
QtBox::QtBox(const unsigned int _maxLevel, const double _width,
             const double _minVertex[2], const int _invLevel, QtBox *_parent,
             const unsigned int _idxX, const unsigned int _idxY)
    : MAX_N_LEVELS(_maxLevel), width(_width), invLevel(_invLevel),
      parent(_parent), idxX(_idxX), idxY(_idxY), bEmpty(true) {
...
I3 QtBox::getIndices() { return I3(idxX, idxY, invLevel); }
  ...
  if (_splitCondition()) {
    ...
    for (unsigned int i = 0; i < 2; i++)
      for (unsigned int j = 0; j < 2; j++) {
        ...
        _getChildBoxDimensions(minVertex, width, childMinVertex, childWidth,
                               coordIdx);
        const unsigned int idx = i + 2 * j;
        children[idx] = new QtBox(MAX_N_LEVELS, childWidth, childMinVertex,
                                  invLevel + 1, this, 2 * pix + i, 2 * piy + j);
      ...
      for (unsigned int i = 0; i < 2; i++)
        for (unsigned int j = 0; j < 2; j++) {
          ...
          _getChildBoxDimensions(minVertex, width, childMinVertex, childWidth,
                                 coordIdx);
          children[i + 2 * j] =
              new QtBox(MAX_N_LEVELS, childWidth, childMinVertex, invLevel + 1,
                        this, 2 * pix + i, 2 * piy + j);
        }
    }
...
bool QtBox::isMyLeftNeighbor(const QtBox *leftNeigh) {
  ...
  const double leftNeighMaxx =
      modf((leftNeigh->minVertex[0] + leftNeigh->width), &intpart);
  ...
  if (!(touchLeftSide && withinHeight)) {
    printf("Not LEFT neighbor\n");
    exit(0);
  }
...
bool QtBox::isMyRightNeighbor(const QtBox *rightNeigh) {
  ...
  if (!(touchRightSide && withinHeight)) {
    printf("Not RIGHT neighbor\n");
    exit(0);
  }
...
bool QtBox::isMyBottomNeighbor(const QtBox *bottomNeigh) {
  double intpart;
  const double myMiny = minVertex[1];
  const double bottomNeighMaxy =
      modf((bottomNeigh->minVertex[1] + bottomNeigh->width), &intpart);
  ...
  if (!(touchBottomSide && withinWidth)) {
    printf("Not BOTTOM neighbor\n");
    exit(0);
  }
...
bool QtBox::isMyTopNeighbor(const QtBox *topNeigh) {
  ...
  if (!(touchTopSide && withinWidth)) {
    printf("Not TOP neighbor\n");
    exit(0);
  }
...
bool QtBox::isTopLeftCornerNeighbor(const QtBox *cornerNeigh) {
  ...
  const double cornerNeighMiny = periodicY
                                     ? (cornerNeigh->minVertex[1] + offsetY)
                                     : (cornerNeigh->minVertex[1]);
  const double cornerNeighMaxy =
      periodicY ? (cornerNeigh->minVertex[1] + cornerNeigh->width + offsetY)
                : (cornerNeigh->minVertex[1] + cornerNeigh->width);
  ...
  const double cornerNeighMinx = periodicX
                                     ? (cornerNeigh->minVertex[0] + offsetX)
                                     : (cornerNeigh->minVertex[0]);
  const double cornerNeighMaxx =
      periodicX ? (cornerNeigh->minVertex[0] + cornerNeigh->width + offsetX)
                : (cornerNeigh->minVertex[0] + cornerNeigh->width);
  ...
  if (!(withinHeight && withinWidth)) {
    printf("Not TOP-LEFT CORNER neighbor\n");
    exit(0);
  }
...
bool QtBox::isTopRightCornerNeighbor(const QtBox *cornerNeigh) {
  ...
  if (!(withinHeight && withinWidth)) {
    printf("Not TOP-RIGHT CORNER neighbor\n");
    exit(0);
  }
...
bool QtBox::isBottomLeftCornerNeighbor(const QtBox *cornerNeigh) {
  ...
  if (!(withinHeight && withinWidth)) {
    printf("Not BOTTOM-LEFT CORNER neighbor\n");
    exit(0);
  }
...
bool QtBox::isBottomRightCornerNeighbor(const QtBox *cornerNeigh) {
  ...
  if (!(withinHeight && withinWidth)) {
    printf("Not BOTTOM-RIGHT CORNER neighbor\n");
    exit(0);
  }
...
void QtBox::twoChildren(unsigned int q0, unsigned int q1,
                        std::vector<QtBox *> &two) {
  if (bEmpty) {
    two.push_back(this);
    return;
  } else {
...
void QtBox::leftmostChildren(std::vector<QtBox *> &two) {
  twoChildren(2, 0, two);
}
void QtBox::rightmostChildren(std::vector<QtBox *> &two) {
  twoChildren(3, 1, two);
}
void QtBox::bottommostChildren(std::vector<QtBox *> &two) {
  twoChildren(0, 1, two);
}
void QtBox::topmostChildren(std::vector<QtBox *> &two) {
  twoChildren(2, 3, two);
}

void QtBox::bottomLeftChildren(std::vector<QtBox *> &one) {
  oneChildren(0, one);
}
void QtBox::bottomRightChildren(std::vector<QtBox *> &one) {
  oneChildren(1, one);
}
...
void QtBox::_getChildBoxDimensions(const double parentMinVertex[2],
                                   double parentWidth, double childMinVertex[2],
                                   double &childWidth,
                                   const unsigned int coordIdx[2]) {
