 ...
 * T&). Macro is to be used at the top level where you would also define a
 * class. To be used for BlockFWT where result of Project is used to guide
 * refinements/compressions.
 * @param name      Name used for the created class
 * @param f         Global function f of the form template <typename T, int i>
 * inline Real f(const T&t). Provides the result returned by static function
 * Real Project(const T&). With i one can number a set of projectors.
 * @see MRAG::BlockFWT
 */
#define make_projector(name, f)                                                \
  class name {                                                                 \
  public:                                                                      \
    template <typename T, int i> inline static Real Project(const T &t) {      \
      return f<T, i>(t);                                                       \
    }                                                                          \
  };
