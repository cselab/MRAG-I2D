...
struct P {
  ...
  Real evaluate_rho(Real t) { return rho + drho_dt * t; }
};
...
RGBA convertToRGBA(P &p) {
  RGBA c(max(0.0, 2.0 * (p.rho + 2.223) / 4.445 - 0.5),
         1.0 - 2.0 * fabs((p.rho + 2.223) / 4.445 - 0.5),
         max(0.0, 1.0 - 2.0 * (p.rho + 2.223) / 4.445), 0);
  // RGBA c(max(0.0, 2.0*(p.rho+2.223)/4.445 - 0.5),  1.0
  // - 2.0*fabs((p.rho+2.223)/4.445 - 0.5),min(1.0,
  // 0.0+2.0*(p.rho+2.223)/4.445),0);
  return c;
}
...
class STDTestL3_Diffusion // LEVEL 3: correct 2D simulations, correct
                          // data-processing
{
...
#pragma mark IC
  ...
  static Real _ic_test1(Real x[3], Real time) {
    ...
    return (exp(-4.0 * M_PI * M_PI * time) * sin(2.0 * M_PI * x[0]) *
                sin(2.0 * M_PI * x[1]) +
            exp(-4.0 * M_PI * M_PI * time * beta * beta) *
                sin(2.0 * M_PI * beta * x[0]) * sin(2.0 * beta * M_PI * x[1]));
  }

  static Real _ic_test2(Real x[3], Real time) {
    return (exp(-8.0 * M_PI * M_PI * time) *
            (sin(2.0 * M_PI * x[0]) + sin(2.0 * M_PI * x[1])));
  }

  static Real _ic_test3(Real x[3], Real time) {
    return (sin(2.0 * M_PI * x[0]) * sin(2.0 * M_PI * x[1]) *
            exp(-8 * M_PI * M_PI * time)); // exp(-time);//
  }

  template <typename Grid> static void _ic(Grid &grid) {
...
#pragma mark Simulation
  void _step_computeRHS(vector<BlockInfo> &vInfo) {
    ...
    lab.prepare(grid.getBlockCollection(), grid.getBoundaryInfo(), steStart,
                steEnd);
    ...
    for (int i = 0; i < vInfo.size(); i++) {
      ...
      for (int iy = 0; iy < B::sizeY; iy++)
        for (int ix = 0; ix < B::sizeX; ix++) //-block(ix,iy).rho;
          block(ix, iy).tmp =
              -8 * M_PI * M_PI * lab(ix, iy).rho * (1 - cazzo_viscosity) +
              cazzo_viscosity * factor *
                  (lab(ix - 1, iy).rho + lab(ix + 1, iy).rho +
                   lab(ix, iy - 1).rho + lab(ix, iy + 1).rho -
                   4.0 * lab(ix, iy).rho);
    }
  }
  ...
  void _step_computeRHS_TR(vector<BlockInfo> &vInfo, double t, double dt) {
    ...
    for (int i = 0; i < vInfo.size(); i++) {
      ...
      for (int iy = 0; iy < B::sizeY; iy++)
        for (int ix = 0; ix < B::sizeX; ix++)
          block(ix, iy).tmp = -8 * M_PI * M_PI * lab(ix, iy).evaluate_rho(t) *
                                  (1 - cazzo_viscosity) +
                              cazzo_viscosity * factor *
  ...
  void _step(Real t, Real dt) {
    ...
    while (true) {
      ...
      const bool bContinue =
          stSorter.getBlocks(level, currDeltaT, currTime, vInfo, type);
...
public:
  STDTestL3_Diffusion(double tol = 1e-3)
      : toleranceIC(tol), mu(1.0), ti(0.0), beta2(3.0),
        grid(blocksPerDimension, blocksPerDimension,
             b3D ? blocksPerDimension : 1),
        refiner(resJump), compressor(resJump), lab(), blockfwt(),
        viewer(false, true), t(0), stSorter() {
    ...
    Science::AutomaticCompression<0, 0>(grid, blockfwt, toleranceIC, -1, NULL,
                                        _ic);
  }
...
public:
  ...
  void Step(double factorDT = 1.0) {
    ...
    const double F = 1. / 8;
    const double dt =
        factorDT * F * dx * dx / cazzo_viscosity; //(4.0*mu);//8e-1;

    printf("Time: %f\n", getTime());

    //	Science::AutomaticRefinement< 0,0 >(grid, blockfwt,
    //toleranceIC/2,maxLevel);
  ...
  static void runTests(int argc, char **argv, bool bVisual = true) {
    ...
    if (bVisual) {
      ...
      double tols[11] = {(bDoTimestepAnalysis ? 1e-4 : 1e-1),
                         5e-2,
                         1e-2,
                         5e-3,
                         1e-3,
      ...
      for (int iTimeStep = 0; iTimeStep < (bDoTimestepAnalysis ? 6 : 1);
           iTimeStep++)
...

          {
            ...
            if (bDoTimestepAnalysis)
              fprintf(f, "%e\t%e\t%e\t%e\t%e\t%e\n", factorDT, error[1],
                      factorDT, avgLengthScale, avgResolution, error[1]);
            else
              fprintf(f, "%e\t%e\t%e\t%e\t%e\t%e\n", tols[iRun], avgLengthScale,
                      avgResolution, error[1], log(avgResolution),
                      log(error[1]));
...

      {
        ...
        if (bDoTimestepAnalysis)
          fprintf(
              fgnuplot,
              "plot \"Error.dat\" using 1:2  title 'cazz0' with linespoint\n");
        else
          fprintf(fgnuplot,
                  "plot \"Error.dat\" using 1:4  title 'error vs compression "
                  "threshold' with linespoint, \"Error.dat\" using 2:4 title "
                  "'error vs average element length' with linespoint , "
                  "\"Error.dat\" using 3:4 title 'error vs 1./sqrt(N)' with "
                  "linespoint\n");
