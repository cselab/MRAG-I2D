...
float getmemusage() {
  ...
  pscallstrm << "ps -x -p " << mypid
             << " -o rss"; // returns size in kb (1024 byte)
  ...
  if (fp != NULL) {
    while (fgets(psres, buf_length, fp) != NULL) {
      ...
      if (cur_string[cur_string.size() - 1] != '\n') {
        std::cout
            << std::endl
            << "could not interpretate result of ps-call. check buf_length"
            << std::endl;
        break;
      }
    ...
    pclose(fp);
    if (psretlist.size() > 0 &&
        (psretlist[0].find("RSS", 0) != std::string::npos)) {
      kbused = atoi(psretlist[1].c_str());
      mbused = float(kbused) / float(1024.0);
    } else {
      std::cout << "could interpretate result of ps-call: does ps exist on "
                   "your system?"
                << std::endl;
      mbused = -1.0;
    }
    ...
    std::cout
        << std::endl
        << "could not open pipe to process: does popen() exist on your system?"
        << std::endl;
    mbused = -1.0;
  }
...
protected:
  bool run(int nallocs, int nBlocks) {
    ...
    for (int i = 0; i < nallocs; ++i) {
      ...
      try {
        MRAG::Grid<Wavelets, Block> *grid =
            new MRAG::Grid<Wavelets, Block>(nBlocks, nBlocks, 1, false);
        ...
        if (mbused > 0.0) {
          ...
          std::cout << "relative mismatch: " << fabs(mbused / mbshould)
                    << std::endl;

        } else {
          throw "could not determine Memory usage from System. [getmemusage "
                "returned -1.0] ";
          break;
        }
    ...
    std::cout << std::endl
              << " ------------MEMORY GROWTH REPORT:  Increments -------------"
              << std::endl;
    std::cout << "[ ";
    for (int i = 0; i < nallocs; ++i) {
      std::cout << mbincs[i] << " ";
