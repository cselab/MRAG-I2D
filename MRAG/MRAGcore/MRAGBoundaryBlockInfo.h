...
struct PointIndex {
  ...
  bool operator<(const PointIndex culo) const {
    return blockID < culo.blockID ||
           (blockID == culo.blockID && index < culo.index);
  }
};
...
private:
  ...
  int _get_diff_res() {
    ...
    for (vector<GridNode>::iterator it = myneighbors.begin();
...
public:
  ...
  BoundaryInfoBlock(const int block_size_[3], const GridNode &b,
                    const vector<GridNode *> &neighbors)
      : node(b), neighbors(neighbors), indexPool(), weightsPool(), ghosts(),
        dependentBlockIDs(), state(BBIState_Initialized), nLocks(1),
        encodedInstructionSizes(), encodedInstructionItemsWs(),
        encodedInstructionItemsPts(), vBlockID_encodedPointIndices3D(),
        vBlockID_Points(), bCompressed(false) {
...
struct BoundaryInfo {
  ...
  void check() {
    for (map<int, BoundaryInfoBlock *>::iterator it =
  ...
  set<int> block_to_erase() {
    ...
    for (map<int, BoundaryInfoBlock *>::iterator itBBI =
             boundaryInfoOfBlock.begin();
         itBBI != boundaryInfoOfBlock.end(); ++itBBI) {
