...
Grid<WaveletType, BlockType>::Grid(int nBlocksX, int nBlocksY, int nBlocksZ,
                                   BlockCollection<BlockType> *collection,
                                   bool bVerbose)
    : m_blockAtLevel(),
      m_blockCollection((collection != NULL)
                            ? *collection
                            : *(new BlockCollection<BlockType>())),
      m_bCollectionOwner(collection == NULL), m_blockCollapser(),
      m_blockSplitter(), m_hierarchy(), m_neighborhood(), m_setInvalidBBInfo(),
      m_boundaryInfo(), m_mapGhost2Node(), m_ghostNodes(), // nodes
      m_status(eGridStatus_Initialized), m_bVerbose(bVerbose),
      m_refRefiner(NULL), m_refCompressor(NULL) {
  m_blockCollapser =
      new BlockCollapser<WaveletType, BlockCollection<BlockType>>;
...
Grid<WaveletType, BlockType>::Grid(int nBlocksX, int nBlocksY, int nBlocksZ,
    ...
    : m_blockAtLevel(),
      m_blockCollection((collection != NULL)
                            ? *collection
                            : *(new BlockCollection<BlockType>())),
      m_bCollectionOwner(collection == NULL), m_blockCollapser(NULL),
      m_blockSplitter(NULL), m_hierarchy(), m_neighborhood(),
      m_setInvalidBBInfo(), m_boundaryInfo(), m_mapGhost2Node(),
      m_ghostNodes(), // nodes
      m_status(eGridStatus_Initialized), m_bVerbose(bVerbose),
      m_refRefiner(NULL), m_refCompressor(NULL) {
  m_blockCollapser =
...
template <typename WaveletType, typename BlockType>
void Grid<WaveletType, BlockType>::_setup(int nBlocksX, int nBlocksY,
                                          int nBlocksZ) {
  ...
  // create blocks
  std::vector<int> vIDs =
      m_blockCollection.create(nBlocksX * nBlocksY * nBlocksZ);
  assert(vIDs.size() > 0);

  // create hierarchy
  m_hierarchy.clear();
  MRAG::MRAGridHelpers::_createChildren(vIDs, m_hierarchy, NULL, 0, nBlocks, 0,
                                        iMaxLevel);
  assert(vIDs.size() == 0);
...
void Grid<WaveletType, BlockType>::_dispose() {
  ...
  // 2.
  for (HierarchyType::iterator it = m_hierarchy.begin();
...
float Grid<WaveletType, BlockType>::getMemorySize() const {
  ...
  memsize += m_hierarchy.size() * (sizeof(HierarchyType::key_type) +
                                   sizeof(HierarchyType::mapped_type));

  memsize += m_neighborhood.size() * (sizeof(NeighborhoodType::key_type) +
                                      sizeof(NeighborhoodType::mapped_type));
...
vector<BlockInfo> Grid<WaveletType, BlockType>::getBlocksInfo() const {
  ...
  for (typename HierarchyType::const_iterator it = m_hierarchy.begin();
    ...
    if (node != NULL && !node->isEmpty) {
      ...
      const Real h[3] = {(Real)(dilate * m_vSize[0] / BlockType::sizeX),
                         (Real)(m_vProcessingDirections[1]
                                    ? dilate * m_vSize[1] / BlockType::sizeY
                                    : 1),
                         (Real)(m_vProcessingDirections[2]
                                    ? (dilate * m_vSize[2] / BlockType::sizeZ)
                                    : 1)};

      const Real p[3] = {
          (Real)(m_vPosition[0] + node->index[0] * dilate * m_vSize[0] +
                 (WaveletType::bIsCellCentered ? 0.5 * h[0] : 0)),
          (Real)(m_vPosition[1] + node->index[1] * dilate * m_vSize[1] +
                 (WaveletType::bIsCellCentered ? 0.5 * h[1] : 0)),
          (Real)(m_vPosition[2] + node->index[2] * dilate * m_vSize[2] +
                 (WaveletType::bIsCellCentered ? 0.5 * h[2] : 0))};

      result.push_back(
          BlockInfo(node->blockID, node->index, node->level, p, h));
    }
  }
...
template <typename WaveletType, typename BlockType>
vector<BlockInfo>
Grid<WaveletType, BlockType>::getNeighborsInfo(const vector<BlockInfo> &vInfo,
                                               bool bConsiderGhosts) const {
  // 1. put all input blockIDs into a set
  // 2. construct map: blockID -> GridNode
  // 3. use it for BlockInfo -> GridNode
  // 4. use GridNode to get the neighbors
  // 5. select those neighbors that are not inside BlockInfo and forget about
  // ghosts

  if (bConsiderGhosts == true) {
    printf("Grid<WaveletType, BlockType>::getNeighborsInfo: "
           "bConsiderGhosts=true is not implemented yet. Aborting.\n");
    abort();
  }
  ...
  for (vector<BlockInfo>::const_iterator it = vInfo.begin(); it != vInfo.end();
       ++it)
    inputIDs.insert(it->blockID);

  // 2.
  map<int, GridNode *> leafs;
  ...
  for (typename HierarchyType::const_iterator it = m_hierarchy.begin();
    ...
    if (node != NULL && !node->isEmpty)
      leafs[it->first->blockID] = it->first;
  }

  // 3.
  set<GridNode *> to_process;

  for (vector<BlockInfo>::const_iterator it = vInfo.begin(); it != vInfo.end();
       ++it) {
    GridNode *mynode = leafs[it->blockID];
    NeighborhoodType::const_iterator itNeighbors = m_neighborhood.find(mynode);

    assert(itNeighbors != m_neighborhood.end());

    vector<GridNode *> neighbors = itNeighbors->second;

    for (vector<GridNode *>::const_iterator itN = neighbors.begin();
         itN != neighbors.end(); ++itN) {
      const bool bIsInput = inputIDs.find((*itN)->blockID) != inputIDs.end();
      const bool bIsGhost = m_neighborhood.find(*itN) != m_neighborhood.end();

      if (!bIsInput && !bIsGhost)
        to_process.insert(*itN);
    }
  }
  ...
  for (set<GridNode *>::const_iterator it = to_process.begin();
       it != to_process.end(); ++it) {
    const GridNode *node = *it;

    // const double dilate = pow(2.0, -node->level);
    const Real dilate = pow(2.0, -node->level);

    const Real h[3] = {
        dilate * m_vSize[0] / BlockType::sizeX,
        m_vProcessingDirections[1] ? dilate * m_vSize[1] / BlockType::sizeY : 1,
        m_vProcessingDirections[2] ? (dilate * m_vSize[2] / BlockType::sizeZ)
                                   : 1};

    const Real p[3] = {
        m_vPosition[0] + node->index[0] * dilate * m_vSize[0] +
            (WaveletType::bIsCellCentered ? (Real)0.5 * h[0] : 0),
        m_vPosition[1] + node->index[1] * dilate * m_vSize[1] +
            (WaveletType::bIsCellCentered ? (Real)0.5 * h[1] : 0),
        m_vPosition[2] + node->index[2] * dilate * m_vSize[2] +
            (WaveletType::bIsCellCentered ? (Real)0.5 * h[2] : 0)};
...
template <typename WaveletType, typename BlockType>
vector<BlockInfo> Grid<WaveletType, BlockType>::getInteriorNeighborsInfo(
    const vector<BlockInfo> &vInfo, bool bConsiderGhosts) const {
  // same method as getNeighborsInfo but now returning only ghosts (== interior
  // neighbors, not across domain boundaries)
  ...
  if (bConsiderGhosts == true) {
    printf("Grid<WaveletType, BlockType>::getInteriorNeighborsInfo: "
           "bConsiderGhosts=true is not implemented yet. Aborting.\n");
    abort();
  }
  ...
  for (vector<BlockInfo>::const_iterator it = vInfo.begin(); it != vInfo.end();
    ...
    for (vector<GridNode *>::const_iterator itN = neighbors.begin();
      ...
      if (!bIsInput && bIsGhost) // want only ghosts, not neighbors now
        to_process.insert(*itN);
    }
  }
...
template <typename WaveletType, typename BlockType>
void Grid<WaveletType, BlockType>::setBlockSplitter(
    BlockSplitter<WaveletType, BlockCollection<BlockType>> *splitter) {
  if (splitter == NULL) {
    printf("void Grid<WaveletType, BlockType>::setBlockSplitter: NULL as "
           "arguments!\n");
    abort();
  }
...
Grid<WaveletType, BlockType>::refine(const set<int> &blocksToRefine) {
  ...
  // 1. iterate over the nodes, look which one has an associated block to be
  // refined
  ...
  for (typename HierarchyType::const_iterator it = m_hierarchy.begin();
    ...
    const bool bFound =
        blocksToRefine.find(node->blockID) != blocksToRefine.end();
  ...
  printf("vRefinementReport size: %ld\n", vRefinementReport.size());
    ...
    for (int i = 0; i < nRefinements; i++) {
      ...
      for (int c = 0; c < nChildren; c++) {
        GridNode *node = new GridNode(false, parent, blockIDs[c],
                                      vNodes[c]->index[0], vNodes[c]->index[1],
  ...
  for (vector<GridNode *>::const_iterator itNewNode = newNodes.begin();
    ...
    NeighborhoodType::const_iterator itNeighbors =
        m_neighborhood.find(*itNewNode);
...
template <typename WaveletType, typename BlockType>
void Grid<WaveletType, BlockType>::setBlockCollapser(
    BlockCollapser<WaveletType, BlockCollection<BlockType>> *collapser) {
  if (collapser == NULL) {
    printf("void Grid<WaveletType, BlockType>::setBlockCollapser: NULL as "
           "arguments!\n");
    abort();
  }
...
Grid<WaveletType, BlockType>::compress(const set<int> &blocksToCompress,
  ...
  for (typename HierarchyType::const_iterator it = m_hierarchy.begin();
    ...
    GridNode *node = it->first;
    if (node != NULL)
      node->shouldBeCompressed = (node->isEmpty)
                                     ? false
                                     : (blocksToCompress.find(node->blockID) !=
                                        blocksToCompress.end());
  }
  ...
  BoundaryInfo &bInfo = getBoundaryInfo();
  CompressionResult result = m_blockCollapser->collapse(
      m_blockCollection, bInfo, *plan, vBlockCollapseInfo);
  ...
  // 6.
  for (int i = 0; i < vToCollapse.size(); i++)
    _collapse(m_hierarchy, vToCollapse[i].node,
              mapCollapseIDBlockID[vToCollapse[i].collapseID]);
  delete plan;
...
template <typename WaveletType, typename BlockType>
BoundaryInfo &
Grid<WaveletType, BlockType>::getBoundaryInfo(int *output_stencil_start,
                                              int *output_stencil_end) {
  // 0. couple of checks
  // 1. compute the stencil used in the grid
  // 2. fill the output
  // 3. if m_bAlreadyComputedBoundaryInfo is true, it means that the work has
  // already been done
...
template <typename WaveletType, typename BlockType>
BoundaryInfo *Grid<WaveletType, BlockType>::createBoundaryInfo(
    const int requested_stencil_start[3],
    const int requested_stencil_end[3]) const {
  _checkResolutionJumpCondition(_computeMaxLevelJump(), requested_stencil_start,
                                requested_stencil_end);
  ...
  _computeBoundaryInfo(*binfo, requested_stencil_start, requested_stencil_end,
                       vNodes);
...
struct Body_CreateBoundaryInfo {
  ...
  Body_CreateBoundaryInfo(const NeighborhoodType &neighborhood,
                          BoundaryInfo &binfo,
  ...
  void operator()(const BlockedRange &r) const {
    const int block_size[3] = {BlockType::sizeX, BlockType::sizeY,
                               BlockType::sizeZ};

    const int requested_stencil_start[3] = {m_binfo.stencil_start[0],
                                            m_binfo.stencil_start[1],
                                            m_binfo.stencil_start[2]};

    const int requested_stencil_end[3] = {
        m_binfo.stencil_end[0], m_binfo.stencil_end[1], m_binfo.stencil_end[2]};

    MRAG_BBInfoCreator<WaveletType, BlockType> bbcreator(
        requested_stencil_start, requested_stencil_end, block_size);
    // printf("=============:: %d %d\n", r.begin(), r.end());
    for (int i = r.begin(); i != r.end(); i++) {
      vWorkingList[i]->bbi = bbcreator.createBoundaryInfoBlock(
          *vWorkingList[i]->node, vWorkingList[i]->neighbors);
      // vWorkingList[i]->bbi->node =
      // const_cast<GridNode*>(vWorkingList[i]->node);
      // vWorkingList[i]->bbi->neighbors = vWorkingList[i]->neighbors;
    }
  }
...
void Grid<WaveletType, BlockType>::_computeBoundaryInfo(
  ...
  Body_CreateBoundaryInfo<WaveletType, BlockType> body(
      m_neighborhood, binfo, requested_stencil_start, requested_stencil_end,
      vNodesToCompute);
...
template <typename WaveletType, typename BlockType>
int Grid<WaveletType, BlockType>::_computeMaxLevel(
    const HierarchyType &hierarchy) const {
  int lmax = 0;
...
void Grid<WaveletType, BlockType>::_computeMaxStencilUsed(
  ...
  const int support_needed_collapse[2] = {1 - WaveletType::HaSupport[1],
                                          -WaveletType::HaSupport[0]};

  const int wavelet_left_supp = std::min(
      -1,
      std::min(support_needed_fwt[0],
               std::min(support_needed_split[0], support_needed_collapse[0])));
  const int wavelet_right_supp = std::max(
      2, std::max(support_needed_fwt[1], std::max(support_needed_split[1],
                                                  support_needed_collapse[1])));

  stencil_start[0] = m_vProcessingDirections[0]
                         ? std::min(wavelet_left_supp, m_maxStencil[0][0])
                         : 0;
  stencil_start[1] = m_vProcessingDirections[1]
                         ? std::min(wavelet_left_supp, m_maxStencil[0][1])
                         : 0;
  stencil_start[2] = m_vProcessingDirections[2]
                         ? std::min(wavelet_left_supp, m_maxStencil[0][2])
                         : 0;

  stencil_end[0] = m_vProcessingDirections[0]
                       ? std::max(wavelet_right_supp, m_maxStencil[1][0])
                       : 1;
  stencil_end[1] = m_vProcessingDirections[1]
                       ? std::max(wavelet_right_supp, m_maxStencil[1][1])
                       : 1;
  stencil_end[2] = m_vProcessingDirections[2]
                       ? std::max(wavelet_right_supp, m_maxStencil[1][2])
                       : 1;
}

template <typename WaveletType, typename BlockType>
void Grid<WaveletType, BlockType>::_checkResolutionJumpCondition(
    int maxJump, const int *client_stencil_start,
    const int *client_stencil_end) const {
  int stencil_start[3] = {0, 0, 0};
  int stencil_end[3] = {0, 0, 0};
  ...
  const int w[3] = {std::max((int)abs(stencil_start[0]), -1 + stencil_end[0]),
                    std::max((int)abs(stencil_start[1]), -1 + stencil_end[1]),
                    std::max((int)abs(stencil_start[2]), -1 + stencil_end[2])};

  const int minimalBlockSize[3] = {std::max(1, w[0] << maxJump),
                                   std::max(1, w[1] << maxJump),
                                   std::max(1, w[2] << maxJump)};

  const bool bAbort = BlockType::sizeX < minimalBlockSize[0] ||
                      BlockType::sizeY < minimalBlockSize[1] ||
                      BlockType::sizeZ < minimalBlockSize[2];

  if (bAbort) {
    printf("Grid::_checkResolutionJumpCondition: ABORT! Blocksize + resolution "
           "jump condition not satisfied for this stencil width!\n");
    printf("stencil width = %d %d %d, minimalBlockSize = %d %d %d, jump-res = "
           "%d, used block size = %d %d %d\n",
           w[0] + 1, w[1] + 1, w[2] + 1, minimalBlockSize[0],
           minimalBlockSize[1], minimalBlockSize[2], maxJump, BlockType::sizeX,
           BlockType::sizeY, BlockType::sizeZ);
...
void Grid<WaveletType, BlockType>::_computeNeighborhood(
  ...
  // 0.
  {
    ...
    for (NeighborhoodType::iterator it1 = neighborhood.begin();
         it1 != neighborhood.end(); it1++)
      it1->second.clear();
  ...
  const int nPeriodicBoundaries = (int)(m_vPeriodicDirection[0]) +
                                  (int)(m_vPeriodicDirection[1]) +
                                  (int)(m_vPeriodicDirection[2]);

  MRAG::Matrix3D<B> &bucket =
      *(new MRAG::Matrix3D<B>(m_vProcessingDirections[0] ? reference_size : 1,
  ...
  {
    int nAdded = 0;
    for (HierarchyType::const_iterator it = hierarchy.begin();
         it != hierarchy.end(); it++) {
      if (it->first == NULL || it->first->isEmpty)
        continue;
      ...
      const int finest_start[3] = {
          m_vProcessingDirections[0] ? ((node.index[0] << level_difference))
                                     : 0,
          m_vProcessingDirections[1] ? ((node.index[1] << level_difference))
                                     : 0,
          m_vProcessingDirections[2] ? ((node.index[2] << level_difference))
                                     : 0};

      const int finest_end[3] = {
          m_vProcessingDirections[0]
              ? (((node.index[0] + 1) << level_difference))
              : 1,
          m_vProcessingDirections[1]
              ? (((node.index[1] + 1) << level_difference))
              : 1,
          m_vProcessingDirections[2]
              ? (((node.index[2] + 1) << level_difference))
              : 1};

      const FlattenedBlock *block =
          new FlattenedBlock(finest_start, finest_end, &node);
      pool.push_back(block);

      const int reference_start[3] = {finest_start[0] >> level_offset,
      ...
      const int reference_end[3] = {
          reference_start[0] +
              std::max((int)1,
                       (finest_end[0] >> level_offset) - reference_start[0]),
          reference_start[1] +
              std::max((int)1,
                       (finest_end[1] >> level_offset) - reference_start[1]),
          reference_start[2] +
              std::max((int)1,
                       (finest_end[2] >> level_offset) - reference_start[2])};

      nAdded += (reference_end[2] - reference_start[2]) *
      ...
      if (nPeriodicBoundaries > 0) {
        ...
        if (node.index[0] != 0 && node.index[1] != 0 && node.index[2] != 0 &&
            node.index[0] != n - 1 && node.index[1] != n - 1 &&
            node.index[2] != n - 1)
          continue;
        ...
        for (int code = 0; code < 27; code++) {
          ...
          const int d[3] = {-(code % 3 - 1), -((code / 3) % 3 - 1),
                            -((code / 9) % 3 - 1)};
          int idx[3] = {node.index[0] + d[0], node.index[1] + d[1],
                        node.index[2] + d[2]};
          ...
          if (vProcessAndPeriodic[0])
            idx[0] = (idx[0] + n) % n;
          if (vProcessAndPeriodic[1])
            idx[1] = (idx[1] + n) % n;
          if (vProcessAndPeriodic[2])
            idx[2] = (idx[2] + n) % n;
          ...
          if (idx[0] < 0 || idx[0] >= n || idx[1] < 0 || idx[1] >= n ||
            ...
            continue;
          ghosts++;
          GridNode *ghostNode = new GridNode(
              node.isEmpty, node.parent, node.blockID, node.index[0] - d[0] * n,
              node.index[1] - d[1] * n, node.index[2] - d[2] * n, node.level);
    ...
    for (int iz = 0; iz < nZ; iz++)
      for (int iy = 0; iy < nY; iy++)
        for (int ix = 0; ix < nX; ix++) {
          // current block-list
          if (iz != 0 && iy != 0 && ix != 0 && iz != nZ - 1 && iy != nY - 1 &&
              ix != nX - 1)
            ix = nX - 1;

          assert(iz == nZ - 1 || iy == nY - 1 || ix == nX - 1 || ix == 0 ||
                 iy == 0 || iz == 0);
          ...
          for (int i = 0; i < 27; i++) {
            ...
            if (m_vProcessingDirections[0] && m_vPeriodicDirection[0])
              idx[0] = (idx[0] + nX) % nX;
            if (m_vProcessingDirections[1] && m_vPeriodicDirection[1])
              idx[1] = (idx[1] + nY) % nY;
            if (m_vProcessingDirections[2] && m_vPeriodicDirection[2])
              idx[2] = (idx[2] + nZ) % nZ;
            ...
            for (int iElement = 0; iElement < nElements; iElement++) {
              if (ghostNodes[(GridNode *)v[iElement]->target][i] == NULL)
                continue;
              ...
              const int finest_start[3] = {
                  m_vProcessingDirections[0]
                      ? ((g.index[0] << level_difference))
                      : 0,
                  m_vProcessingDirections[1]
                      ? ((g.index[1] << level_difference))
                      : 0,
                  m_vProcessingDirections[2]
                      ? ((g.index[2] << level_difference))
                      : 0};

              const int finest_end[3] = {
                  m_vProcessingDirections[0]
                      ? (((g.index[0] + 1) << level_difference))
                      : 1,
                  m_vProcessingDirections[1]
                      ? (((g.index[1] + 1) << level_difference))
                      : 1,
                  m_vProcessingDirections[2]
                      ? (((g.index[2] + 1) << level_difference))
                      : 1};

              // printf("TOOO FASST\n");
              // FlattenedBlock * block = new FlattenedBlock(s, e,
              // v[iElement]->target);
              FlattenedBlock *block = new FlattenedBlock(
                  finest_start, finest_end,
                  ghostNodes[(GridNode *)v[iElement]->target][i]);
              pool.push_back(block);
              bucket.Access(ix, iy, iz).push_back(block);
            }
          }
            ...
            if (&currentlist != &neighbors)
              for (int b2 = 0; b2 < n2; b2++)
                for (int b1 = 0; b1 < n1; b1++)
                  FlattenedBlock::compareAndAdd(
                      *currentlist[b1], *neighbors[b2], neighborhood,
                      nComparisons, nNeighborsConnections);
            else
              for (int b2 = 0; b2 < n2; b2++)
                for (int b1 = 0; b1 <= b2; b1++)
                  FlattenedBlock::compareAndAdd(
                      *currentlist[b1], *currentlist[b2], neighborhood,
                      nComparisons, nNeighborsConnections);
          }
        }

    printf("Done N=%dx%dx%d Compared:%d Found:%d\n", nX, nY, nZ, nComparisons,
           nNeighborsConnections);
  }
