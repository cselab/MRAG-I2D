...
private:
  template <typename Wavelets, typename Block, typename Projector,
            bool bWriteDetails, int nChannels_, typename BlockLabType>
  friend class BlockFWT;

  enum FWTReport_Status {
    FWTReport_Undefined,
    FWTReport_Open,
    FWTReport_Concluded
  };
  ...
  template <int iChannel> void send(int code, double val) {
    assert(m_status == FWTReport_Open);

    char idx[3] = {(char)(code & 1), (char)((code >> 1) & 1),
                   (char)((code >> 2) & 1)};
  ...
  double _getDetailMaxMag(int iChannel) const {
...
public:
  FWTReport(const FWTReport &report) : m_status(report.m_status) {
  ...
  double getCoeffMinMag(int ix, int iy = 0, int iz = 0) const {
    assert(m_status == FWTReport_Concluded);
    return m_matMinValues[iChannel]->Read(ix, iy, iz);
  }

  template <int iChannel>
  double getCoeffMaxMag(int ix, int iy = 0, int iz = 0) const {
    assert(m_status == FWTReport_Concluded);
    return m_matMaxValues[iChannel]->Read(ix, iy, iz);
  }

  template <int iChannel> double getDetailMaxMag() const {
    return _getDetailMaxMag(iChannel);
  }

  double getOverAll_DetailMaxMag() const {
 ...
 * Projector needs to provide a static function Real Project(const T&) which
 * should work for T = Block::ElementType. Easiest way to do this is by using
 * macro #make_projector.
 * @see Science::AutomaticRefinement, Science::AutomaticCompression,
 * MRAGHelperMacros.h, #make_projector
 */
template <typename Wavelets, typename Block,
          typename Projector = dummy_projector, bool bWriteDetails = true,
          int nChannels = 1, typename BlockLabType = BlockLab<Block>>
class BlockFWT {
  typedef Wavelets W;
...
private:
  ...
  enum eBlockFWT_State {
    eBlockFWT_NotReady,
    eBlockFWT_Initialized,
    eBlockFWT_Transformed,
    eBlockFWT_Reported
  };
  ...
  Real _filter(const int dx, const int dy, const int dz) const {
    const int s[3] = {!Block::shouldProcessDirectionY ? 0
                      : bWaveletX                     ? Wavelets::GaSupport[0]
                                                      : Wavelets::HaSupport[0],
                      !Block::shouldProcessDirectionY ? 0
                      : bWaveletY                     ? Wavelets::GaSupport[0]
                                                      : Wavelets::HaSupport[0],
                      !Block::shouldProcessDirectionZ ? 0
                      : bWaveletZ                     ? Wavelets::GaSupport[0]
                                                      : Wavelets::HaSupport[0]};

    const int e[3] = {!Block::shouldProcessDirectionX ? 1
                      : bWaveletX                     ? Wavelets::GaSupport[1]
                                                      : Wavelets::HaSupport[1],
                      !Block::shouldProcessDirectionY ? 1
                      : bWaveletY                     ? Wavelets::GaSupport[1]
                                                      : Wavelets::HaSupport[1],
                      !Block::shouldProcessDirectionZ ? 1
                      : bWaveletZ                     ? Wavelets::GaSupport[1]
                                                      : Wavelets::HaSupport[1]};

    const int o[3] = {!Block::shouldProcessDirectionX ? 0 : dx * 2,
                      !Block::shouldProcessDirectionY ? 0 : dy * 2,
                      !Block::shouldProcessDirectionZ ? 0 : dz * 2};

    Real result = 0;
    for (int iz = s[2]; iz < e[2]; iz++) {
      const Real wZ = (!Block::shouldProcessDirectionZ)
                          ? 1
                          : (bWaveletZ ? m_weightsGa[iz] : m_weightsHa[iz]);
      for (int iy = s[1]; iy < e[1]; iy++) {
        const Real wY = (!Block::shouldProcessDirectionY)
                            ? 1
                            : (bWaveletY ? m_weightsGa[iy] : m_weightsHa[iy]);
        const Real wZwY = wZ * wY;

        const Element *ptr =
            &m_blockLab.read(o[0] - s[0], o[1] - iy, o[2] - iz);

        for (int ix = s[0]; ix < e[0]; ix++, ptr--)
          result += (Projector::template Project<Element, iChannel>(*ptr)) *
                    (wZwY * (bWaveletX ? m_weightsGa[ix] : m_weightsHa[ix]));
      }
    }
    ...
    for (int iz = 0; iz < nZ; iz++)
      for (int iy = 0; iy < nY; iy++)
        for (int ix = 0; ix < nX; ix++) {
          ...
          block.wtdata(values[0] = _filter<0, 0, 0, iChannel>(ix, iy, iz), 0,
                       ix, iy, iz);
          block.wtdata(values[1] = _filter<1, 0, 0, iChannel>(ix, iy, iz), 1,
                       ix, iy, iz);

          if (Block::shouldProcessDirectionY) {
            block.wtdata(values[2] = _filter<0, 1, 0, iChannel>(ix, iy, iz), 2,
                         ix, iy, iz);
            block.wtdata(values[3] = _filter<1, 1, 0, iChannel>(ix, iy, iz), 3,
                         ix, iy, iz);

            if (Block::shouldProcessDirectionZ) {
              block.wtdata(values[4] = _filter<0, 0, 1, iChannel>(ix, iy, iz),
                           4, ix, iy, iz);
              block.wtdata(values[5] = _filter<1, 0, 1, iChannel>(ix, iy, iz),
                           5, ix, iy, iz);
              block.wtdata(values[6] = _filter<0, 1, 1, iChannel>(ix, iy, iz),
                           6, ix, iy, iz);
              block.wtdata(values[7] = _filter<1, 1, 1, iChannel>(ix, iy, iz),
                           7, ix, iy, iz);
            }
          }

          const int n = !Block::shouldProcessDirectionY   ? 2
                        : !Block::shouldProcessDirectionZ ? 4
                                                          : 8;
          for (int i = 0; i < n; i += 2) {
  ...
  template <int iFirst, int iLast, int iSize> class MultiChannel_FWT {
  public:
    inline static void
    multichannel_fwt(const BlockInfo &info, Block &block,
                     BlockFWT<Wavelets, Block, Projector, bWriteDetails,
                              nChannels, BlockLabType> &blockfwt) {
      blockfwt._fwt<iFirst>(info, block);
      MultiChannel_FWT<iFirst + 1, iLast, iSize - 1>::multichannel_fwt(
          info, block, blockfwt);
    }
  };

  template <int iFirst, int iLast> class MultiChannel_FWT<iFirst, iLast, 0> {
  public:
    inline static void
    multichannel_fwt(const BlockInfo &info, Block &block,
                     BlockFWT<Wavelets, Block, Projector, bWriteDetails,
...
public:
  BlockFWT()
      : m_blockLab(), m_state(eBlockFWT_NotReady), m_refCollection(NULL),
        m_report(nReportSizeX, nReportSizeY, nReportSizeZ), m_weightsHa(NULL),
        m_weightsGa(NULL) {
  ...
  void prepare(const BlockCollection<Block> &collection,
  ...
  template <int iChannel> void fwt(const BlockInfo &info) {
    assert(m_state == eBlockFWT_Initialized ||
  ...
  void multichannel_fwt(const BlockInfo &info) {
    ...
    MultiChannel_FWT<iFirstChannel, iLastChannel,
                     iLastChannel - iFirstChannel + 1>::multichannel_fwt(info,
                                                                         block,
                                                                         *this);
  ...
  multichannel_fwt(const vector<MRAG::BlockInfo> &vInfos,
    ...
    MultiChannelFWT_Body<iFirstChannel, iLastChannel> body(vInfos, collection,
                                                           boundaryInfo);
