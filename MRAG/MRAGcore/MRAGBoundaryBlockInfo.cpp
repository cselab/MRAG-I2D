...
void BoundaryInfoBlock::_decompress() {
  ...
  // 2.
  {
    ...
    // A.
    {
      ...
      const vector<ReconstructionInfo>::const_iterator itGEnd = ghosts.end();
      for (vector<ReconstructionInfo>::iterator itG = ghosts.begin();
           itG != itGEnd; itG++) {
        const vector<IndexWP>::iterator itItemEnd = itG->end();

        for (vector<IndexWP>::iterator itItem = itG->begin();
             itItem != itItemEnd; itItem++)
    ...
    // B.
    {
      ...
      for (vector<ReconstructionInfo>::iterator itG = ghosts.begin();
        ...
        for (vector<IndexWP>::iterator itItem = itG->begin();
             itItem != itItemEnd; itItem++, itSource++)
...
void BoundaryInfo::erase(int blockID, bool bCritical) {
  ...
  BoundaryInfoBlock *ptr = it->second;
  ...
  // memset(ptr, 0, sizeof(BoundaryInfoBlock));
}
} // namespace MRAG
