...
// toDo: check loops for fast access. (which index is varying faster in c++ [][]
// arrays?)
...
struct tAdVector {
  ...
  BType &
  operator()(int i,
             int j = 0) // idea is to access with (vecComponent,t=0,t=-1...)
  {
    assert(i < VecDim);
    assert(-j < tStencil);
    return data[i][-j];
  ...
  void update() {
    ...
    for (int j = tStencil - 1; j > 0;
         --j) // start at oldest value, and overwrite with newer values.
    {
      for (int i = 0; i < VecDim; ++i) {
        data[i][j] = data[i][j - 1];
      }
...
template <typename NType, int VecDim, int SpaceDim, int tStencil,
          typename someType>
tAdVector<NType, VecDim, SpaceDim, tStencil>
operator*(const tAdVector<NType, VecDim, SpaceDim, tStencil> &p, someType v) {
...
template <typename NType, int VecDim, int SpaceDim, int tStencil>
tAdVector<NType, VecDim, SpaceDim, tStencil>
operator*(const tAdVector<NType, VecDim, SpaceDim, tStencil> &p, float v) {
  tAdVector<NType, VecDim, SpaceDim, tStencil> t;
...
make_projector(burgers_projector, burgers_projector_impl)
    ...
    class MRAG_STDTestL3_VectorAdvection // LEVEL 3: a "Real Life" Advection
                                         // problem
{
protected:
  ...
  // Destructor:
  ~MRAG_STDTestL3_VectorAdvection() { delete myBoundaries; }
  ...
  static BaseType _solShockNWave(BaseType x, BaseType t) {
    ...
    if ((xdash < BaseType(0.0)) || xdash > shock_loc)
      res = BaseType(0.0);
    else if (t < BaseType(2.0) && t <= xdash && xdash <= shock_loc)
      res = 1.0;
    else
      res = xdash / t;
    return res;
  ...
  // Update Velocities (currently just a constant velocity (yeah I know, its a
  // memory killer))
  ...
  BaseType _updateVelocities(vector<BlockInfo> &vInfo,
                             BaseType forceCFL = BaseType(0.5)) {
    ...
    for (int i = 0; i < vInfo.size(); i++) {
      ...
      for (int iy = 0; iy < Block::sizeY; iy++)
        for (int ix = 0; ix < Block::sizeX; ix++) {
          ...
          // velocity set to: value of 1st vector component at old iteration
          // step (-1).
          lab(ix, iy).velocity[0] = lab(ix, iy)(0, -1);
          lab(ix, iy).velocity[1] = lab(ix, iy)(0, -1);
          minDeltaT = std::min(minDeltaT,
                               forceCFL * h[0] / fabs(lab(ix, iy).velocity[0]));
          minDeltaT = std::min(minDeltaT,
                               forceCFL * h[1] / fabs(lab(ix, iy).velocity[1]));
    ...
    // debug:
    // std::cout << "Calling updateVelocities, while forcing CFL="<< forceCFL <<
    // " mindT " << minDeltaT <<std::endl;
    std::cout << "Calling updateVelocities, maxVeL=" << maxvel << std::endl;

    return minDeltaT;
  }

  BaseType _advectwFluxes(vector<BlockInfo> &vInfo,
                          BaseType forceCFL = BaseType(0.5), int fType = 0,
                          int icType = 9) {
    ...
    for (int i = 0; i < vInfo.size(); i++) {
      ...
      for (int iy = 0; iy < Block::sizeY; iy++)
        for (int ix = 0; ix < Block::sizeX; ix++) {
          ...
          minDeltaT =
              std::min(minDeltaT, forceCFL * h[0] /
                                      fabs(block(ix, iy).velocity[0] + tiny));
          minDeltaT =
              std::min(minDeltaT, forceCFL * h[1] /
                                      fabs(block(ix, iy).velocity[1] + tiny));
        }
    }
    ...
    lab.prepare(grid.getBlockCollection(), *myBoundaries,
                myBoundaries->getStencilStart(), myBoundaries->getStencilEnd());
      ...
      for (int iy = 0; iy < Block::sizeY; iy++)
        for (int ix = 0; ix < Block::sizeX; ix++) {
          ...
          for (int ik = 0; ik < vDim; ++ik) // loop over vector components
          {
            ...
            lsum = 0.0;
            for (int d = 0; d < sDim; ++d) {
              bool r_gt_m = (lab(ix, iy).velocity[d] >
                             lab(ix + sel[d][0], iy + sel[d][1]).velocity[d]);
              bool l_gt_m = (lab(ix - sel[d][0], iy - sel[d][1]).velocity[d] >
                             lab(ix, iy).velocity[d]);
              Fl[d] = pow(lab(ix - sel[d][0], iy - sel[d][1]).velocity[d],
                          2.0); // currently burgers-flux is hardcoded.
            ...
            lab(ix, iy)(ik, 0) =
                lab(ix, iy)(ik, -1) - preFac[0] * (rsum - lsum);
          }
        }
      lab.flush(info);
    }

    // std::cout << "mxdif was: " << mxdif << "mxvel was: " << mxvel << " minvel
    // was:" << mnvel << std::endl;
  ...
  void _some_advection_1stOrder(BaseType dt) // upwinding advection
  {
    ...
    lab.prepare(grid.getBlockCollection(), *myBoundaries,
                myBoundaries->getStencilStart(), myBoundaries->getStencilEnd());
    // lab.prepare(grid.getBlockCollection(),
    // grid.getBoundaryInfo(),steStart,steEnd);
      ...
      for (int iy = 0; iy < Block::sizeY; iy++)
        for (int ix = 0; ix < Block::sizeX; ix++) {
          ...
          for (int ik = 0; ik < vDim; ++ik) // loop over vector components
          {
            ...
            dud[0] = 1.0 / h[0] *
                     (lab(ix, iy)(ik, -1) - lab(ix + sel[0], iy)(ik, -1));
            dud[1] = 1.0 / h[1] *
                     (lab(ix, iy)(ik, -1) - lab(ix, iy + sel[1])(ik, -1));
            // dud[2]=1.0/h[2]*(lab(ix,iy)(ik,-1)-lab(ix,iy+sel[2])(ik,-1));
            lab(ix, iy)(ik) = lab(ix, iy)(ik, -1) -
                              dt * (fabs(lab(ix, iy).velocity[0]) * dud[0] +
                                    fabs(lab(ix, iy).velocity[1]) * dud[1]);
            ...
            // sel[0]: selector for upwinding scheme, based on local velocity
            // field.

            // does not work (would require some auxillary-arrays, so hardcoding
            // is better)

            // for (int sd=0; sd<sDim;++sd) //loop over space directions.
            //  {
            //   lab(ix,iy)(ik)-=dt*
            //   fabs(lab(ix,iy).velocity[sd])/h[sd]*(lab(ix+sel[sd][0],iy+sel[sd][1])(ik,-1)
            //   - lab(ix+sel[sd][0]-1,iy+seld[sd][1]-1)(ik,-1));
            //  }
    ...
    // update "oldG" (dont need a lab here, but direct access to
    // BlockCollection)
    for (int i = 0; i < vInfo.size(); i++) {
      BlockInfo &info2 = vInfo[i];
      Block &block = grid.getBlockCollection()[info2.blockID];
  ...
  void _some_advection_2ndOrder(BaseType dt) // upwinding advection
  {
    ...
    std::cout << " s: " << int(myBoundaries->stencil_start[0]) << " "
              << int(myBoundaries->stencil_start[1]) << " "
              << int(myBoundaries->stencil_start[2]) << " " << std::endl;
    std::cout << " e: " << int(myBoundaries->stencil_end[0]) << " "
              << int(myBoundaries->stencil_end[1]) << " "
              << int(myBoundaries->stencil_end[2]) << " " << std::endl;
    lab.prepare(grid.getBlockCollection(), *myBoundaries,
                myBoundaries->getStencilStart(), myBoundaries->getStencilEnd());
    // lab.prepare(grid.getBlockCollection(),
    // grid.getBoundaryInfo(),steStart,steEnd);
      ...
      for (int iy = 0; iy < Block::sizeY; iy++)
        for (int ix = 0; ix < Block::sizeX; ix++) {
          ...
          for (int ik = 0; ik < vDim; ++ik) // loop over vector components
          {
            ...
            dud[0] = 1.0 / (2.0 * h[0]) *
                     (3.0 * lab(ix, iy)(ik, -1) -
                      4.0 * lab(ix + sel[0], iy)(ik, -1) +
                      lab(ix - (sel[0] * 2), iy)(ik, -1));
            dud[1] = 1.0 / (2.0 * h[1]) *
                     (3.0 * lab(ix, iy)(ik, -1) -
                      4.0 * lab(ix, iy + sel[1])(ik, -1) +
                      lab(ix, iy - (sel[1] * 2))(ik, -1));
            // dud[2]=1.0/(2.0*h[2])*(3.0*lab(ix,iy)(ik,-1)-4.0*lab(ix,iy+sel[2])(ik,-1)+lab(ix,iy-(sel[2]*2))(ik,-1));
            // calculation based on interim results. (the same for all (OrderN)
            // stencils)
            lab(ix, iy)(ik) = lab(ix, iy)(ik, -1) -
                              dt * (fabs(lab(ix, iy).velocity[0]) * dud[0] +
                                    fabs(lab(ix, iy).velocity[1]) * dud[1]);
    ...
    for (int i = 0; i < vInfo.size(); i++) {
      ...
      for (int iy = 0; iy < Block::sizeY; iy++) {
        for (int ix = 0; ix < Block::sizeX; ix++) {
          ...
          for (int ik = 0; ik < vDim; ++ik) {
            abs_error +=
                pow(fabs(block(ix, iy)(ik) - block(ix, iy).ic[ik]), 2.0);
          }
          ...
          if (abs_error > 0.5 || abs_error < 0.0) {
            ...
            printf("Funny error at ix=%d,iy=%d,l=%d  ini=%f, current=%f\n", ix,
                   iy, info.level, block(ix, iy).ic, block(ix, iy).cur);
          }
#endif
        }
      }
  ...
  BaseType getError(BaseType t, int solCase = 0, double avgNumElems) {
    ...
    if (t == BaseType(0.0))
      exoutf.open("exact.dat");
    if (t < BaseType(0.6) && t > BaseType(0.4))
      exoutf.open("exact-t0.5.dat");
      ...
      for (int iy = 0; iy < Block::sizeY; iy++) {
        for (int ix = 0; ix < Block::sizeX; ix++) {
          ...
          for (int ik = 0; ik < vDim; ++ik) {
            ...
            for (int d = 0; d < sDim; ++d) {
              ...
              case 5: {
                if ((x[d] <= b1) || (x[d] >= bs))
                  exact[ik] = -1.0;
                else if ((x[d] > b1) && (x[d] <= b2))
                  exact[ik] = -1.0 + 2.0 * (x[ik] - b1) / (b2 - b1);
                else if ((x[d] > b1) && (x[d] < bs))
                  exact[ik] = 1.0;
                else
                  exact[ik] = 5.0;
                break;
              ...
              case 8: {
                ...
                if ((x[ds] < 1.0 / 4.0) || x[ds] > shock_loc)
                  exact[ik] = 0.0;
                else if (t < 2.0 && t < (3.0 / 4.0) * x[ds] &&
                         x[ds] <= shock_loc)
                  exact[ik] = 1.0;
                else
                  exact[ik] = 4.0 * (x[ds] - (1.0 / 4.0)) / t;
                break;
              }
              case 9: {
                /*
                BaseType shock_loc=BaseType(1.0)+BaseType(0.5)*t;
                BaseType xdash=BaseType(4.0)*x[0]-BaseType(1.0); //mapping to
                -1..3 if ((xdash<BaseType(0.0)) || xdash>shock_loc)
                exact[ik]=BaseType(0.0); else if (t<BaseType(2.0) && t<=xdash &&
                xdash <= shock_loc) exact[ik]=1.0; else exact[ik]=xdash/t;
                */
          ...
          if (t == BaseType(0.0) || (t < BaseType(0.6) && t > BaseType(0.4))) {
            exoutf << x[0] << " " << x[1] << " " << exact[0] << " "
                   << block(ix, iy)(0, 0) << std::endl;
          }
          abs_error = sqrt(abs_error);
          ...
          if (abs_error > Linfty)
            Linfty = abs_error;
          if (abs_error > 0.0 && t == BaseType(0.0))
            std::cout << "err. = " << abs_error << " at: " << x[0] << " "
                      << x[1] << std::endl;
        }
      }
    }

    std::cout << "hx, Linfty, Ltwo, abs/avgNumPoints = "
              << 1.0 / (pow(2.0, maxLevel)) << " " << Linfty << " " << Ltwo
              << " " << sumAbs / avgNumElems << std::endl;
    exoutf.close();
    return Linfty;
  }
  ...
  virtual bool run(int LevelOfRefinements, int NumOfBlocks, int OrderFD,
                   int NumberOfSteps) {
  ...
  void burgers2D(int orderFD = 4, bool WithSTR = false, float tmax = 1.0,
                 int maxLevel = 3) {
    ...
    int ictype = 9; // NOTE: if you use ictype 9: it is going to use a mapping
                    // from -1...3 to 0...1 to be compatible with an exact
                    // solution defined on that domain.

    std::cout << "putting initial Condition: " << std::endl;
    _some_ic(grid, ictype);
    if (adapt < 1e6) {
      Science::AutomaticRefinement<0, 0>(grid, blockfwt, toleranceIC / 2,
                                         maxLevel);
    ...
    std::cout << "myBoundaries->stest[0] "
              << int(myBoundaries->stencil_start[0])
              << " end[0]: " << int(myBoundaries->stencil_end[0]) << std::endl;
    std::cout << "myBoundaries->_smarttest[0] "
              << myBoundaries->getStencilStart()[0] << " end[0] "
              << myBoundaries->getStencilEnd()[0] << std::endl;

    szX = Block::sizeX;
    szY = Block::sizeY;
    szZ = Block::sizeZ;
    curAvg = vInfo.size() * szX * szY * szZ;
    std::cout << "Error at t= 0 (IC)" << getError(tcur, ictype, curAvg)
              << std::endl;
    ...
    while (tcur < tmax && iter < maxiter) {
      ...
      if (WithSTR) // SpaceTimeRefinement
      {
        ...
        case 1: {
          ...
          std::cout << "t= " << tcur << " iter= " << iter
                    << " moving with timestep: " << dt << std::endl;
          _some_advection_1stOrder(dt);
          break;
        }
        case 2: {
      ...
      tcur += dt;
      std::cout << "Error at t= " << tcur << " "
                << getError(tcur, ictype, avgNumElements) << std::endl;
...
public:
  static void runTests(int LevelOfRefinements = 2, int NumOfBlocks = 2,
                       int OrderFD = 2, int NumberOfSteps = 1) {
    ...
    // usage example:
    //	MRAG::MRAG_STDTestL3_VectorAdvection<MRAG::Wavelets_Interp2ndOrder,
    //MRAG::Block<misc::tAdVector<double,3,2,2>,16,16,1> >::runTests(2,4,1,1);
  }

  // usage:
  //	MRAG::MRAG_STDTestL3_VectorAdvection<MRAG::Wavelets_Interp2ndOrder,
  //MRAG::Block<misc::tAdVector<double,3,2,2>,16,16,1> >::runBurgers2D(8,2);
  static void runBurgers2D(int NumOfBlocks = 2, int maxRefinementLevel) {

    MRAG_STDTestL3_VectorAdvection<Wavelets, Block> test(NumOfBlocks);
    // void burgers2D(int orderFD=4, bool WithSTR=false, float tmax=1.0, int
    // maxLevel = 3)
