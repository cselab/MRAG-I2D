...
class MRAG_STDTestL1_Generator {
  ...
  inline static void _test() {
    ...
    //	MRAG_STDTestL1_BlockCollection< Wavelets, Block<float, nX,nY,nZ>
    //>::runTests(); 	MRAG_STDTestL1_memtest< Wavelets, Block<float, nX,nY,nZ>
    //>::runTests(2000,20);
    MRAG_STDTestL1_BoundaryInfo<Wavelets, Block<float, nX, nY, nZ>>::runTests(
        argc, argv, false);
  }

  template <typename Wavelets, int n> class TEST {
  public:
  ...
  template <typename Wavelets> class TEST<Wavelets, 8> {
  public:
    static void generate() { printf("End of the Testing\n"); }
  };
