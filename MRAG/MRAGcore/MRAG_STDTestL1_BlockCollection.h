...
protected:
  void create(BlockCollection<Block> &bc, set<int> &blockIDSet,
              unsigned int howmany) {
    vector<int> newBlocks = bc.create(howmany);
    vector<int>::iterator it;
    for (vector<int>::iterator it = newBlocks.begin(); it != newBlocks.end();
         ++it) {
  ...
  void remove(BlockCollection<Block> &bc, std::set<int> &blockIDSet,
              unsigned int howmany) {
  ...
  bool run() {
    try {
      /*
       ...
       131072 blocks, memorySize = 10.4998 vec.size = 131072 while nChunkSize is
       3 262144 blocks, memorySize = 21.0002 vec.size = 262144 while nChunkSize
       is 3 524288 blocks, memorySize = 41.9998 vec.size = 524288 while
       nChunkSize is 3
