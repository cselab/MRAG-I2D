  ...
  public:
    ...
    CartesianIndex decode(const int hilbertcode, const int level) const {
      ...
      for (int l = level - 1; l >= 0; l--) {
        ...
        const int peano = 0 * (hilbert == rule0) + 1 * (hilbert == rule1) +
                          2 * (hilbert == rule2) + 3 * (hilbert == rule3);
    ...
    int encode(const int ix, const int iy, const int level) const {
      ...
      for (int l = level - 1; l >= 0; l--) {
        ...
        const int hilbert = rule0 * (peano == 0) + rule1 * (peano == 1) +
                            rule2 * (peano == 2) + rule3 * (peano == 3);
...
#pragma omp parallel for
      for (int i = 0; i < N; i++) {
        const int ix = blocksinfo[i].index[0]
                       << (mylevel - blocksinfo[i].level);
        const int iy = blocksinfo[i].index[1]
                       << (mylevel - blocksinfo[i].level);
...
public:
  Grid_Hilbert2D(int nBlocksX, int nBlocksY = 1, int nBlocksZ = 1,
                 BlockCollection<BlockType> *collection = NULL,
                 bool bVerbose = true)
      : Grid<WaveletType, BlockType>(nBlocksX, nBlocksY, nBlocksZ, collection,
                                     bVerbose) {}

  Grid_Hilbert2D(int nBlocksX, int nBlocksY, int nBlocksZ,
                 const int maxStencil[2][3],
                 BlockCollection<BlockType> *collection = NULL,
                 bool bVerbose = true)
      : Grid<WaveletType, BlockType>(nBlocksX, nBlocksY, nBlocksZ, maxStencil,
                                     collection, bVerbose) {}
