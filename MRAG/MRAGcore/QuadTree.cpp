...
QuadTree::QuadTree(const unsigned int _maxLevel, const double _width,
                   const double _minVertex[2])
    : MAX_N_LEVELS(_maxLevel) {
...
void QuadTree::traverse(QtBox **cell, unsigned int &nextLevel,
                        const unsigned int xLocCode,
                        const unsigned int yLocCode) {
  //-------------------------------------------------------------------------------
  // BUG: in their code wrong indexing
  //-------------------------------------------------------------------------------
  while (!(*cell)->bEmpty) {
    const unsigned int childBranchBit = 1 << (nextLevel);
    const unsigned int childIndexX =
        (((xLocCode)&childBranchBit) >> (nextLevel));
    const unsigned int childIndexY =
...
void QuadTree::traverseToLevel(QtBox **cell, unsigned int &nextLevel,
                               const unsigned int xLocCode,
                               const unsigned int yLocCode,
...
void QuadTree::getCommonAncestor(QtBox **cell, unsigned int &cellLevel,
                                 const unsigned int binaryDiff) {
  //-------------------------------------------------------------------------------
...
QtBox *QuadTree::locateRegion(const double v0[2], const double v1[2]) {
  ...
  while (!(xDiff & (1 << level)) && level)
    level--;
  while (!(yDiff & (1 << minLevel)) && (minLevel > level))
    minLevel--;
  minLevel++;
...
std::pair<QtBox *, bool>
QuadTree::locateLeftNeighborGELevelPeriodic(QtBox *cell) {
...
std::pair<QtBox *, bool>
QuadTree::locateRightNeighborGELevelPeriodic(QtBox *cell) {
  //----Get cell's x and y locational codes and the x locational code of the
  //----cell's right neighbors
  const unsigned int binaryCellSize = 1 << cell->level;
  const bool rightEdge =
      ((cell->xLocCode + binaryCellSize) >= (1 << ROOT_LEVEL));
  ...
  const unsigned int yLocCode = cell->yLocCode;
  const unsigned int xRightLocCode =
      rightEdge ? xLocCode : (xLocCode + binaryCellSize);
...
std::pair<QtBox *, bool>
QuadTree::locateBottomNeighborGELevelPeriodic(QtBox *cell) {
...
std::pair<QtBox *, bool>
QuadTree::locateTopNeighborGELevelPeriodic(QtBox *cell) {
  //----Get cell's x and y locational codes and the x locational code of the
  ...
  const unsigned int yTopLocCode =
      topEdge ? yLocCode : (yLocCode + binaryCellSize);
...
std::vector<std::pair<QtBox *, bool>>
QuadTree::locateLeftNeighborsPeriodic(QtBox *cell) {
  ...
  for (std::vector<QtBox *>::iterator it = cellChildren.begin();
       it < cellChildren.end(); it++)
...
std::vector<std::pair<QtBox *, bool>>
QuadTree::locateRightNeighborsPeriodic(QtBox *cell) {
  std::vector<QtBox *> cellChildren;
...
std::vector<std::pair<QtBox *, bool>>
QuadTree::locateBottomNeighborsPeriodic(QtBox *cell) {
  std::vector<QtBox *> cellChildren;
...
std::vector<std::pair<QtBox *, bool>>
QuadTree::locateTopNeighborsPeriodic(QtBox *cell) {
  std::vector<QtBox *> cellChildren;
...
std::vector<std::pair<QtBox *, bool>>
QuadTree::locateAllNeighborsPeriodic(QtBox *cell) {
  ...
  std::pair<QtBox *, bool> left = locateLeftNeighborGELevelPeriodic(cell);
  {
    ...
    std::vector<std::pair<QtBox *, bool>> tagged;
    for (std::vector<QtBox *>::iterator it = leftChildren.begin();
         it < leftChildren.end(); it++)
      tagged.push_back(std::pair<QtBox *, bool>((*it), left.second));
    total.push_back(tagged);
#ifndef NDEBUG
    for (unsigned int j = 0; j < leftChildren.size(); j++) {
      cell->isMyLeftNeighbor(leftChildren[j]);
    }
#endif
  }
  ...
  std::pair<QtBox *, bool> right = locateRightNeighborGELevelPeriodic(cell);
  {
    ...
    std::vector<std::pair<QtBox *, bool>> tagged;
    for (std::vector<QtBox *>::iterator it = rightChildren.begin();
         it < rightChildren.end(); it++)
      tagged.push_back(std::pair<QtBox *, bool>((*it), right.second));
    total.push_back(tagged);
#ifndef NDEBUG
    for (unsigned int j = 0; j < rightChildren.size(); j++) {
      cell->isMyRightNeighbor(rightChildren[j]);
    }
#endif
  }
  ...
  std::pair<QtBox *, bool> bottom = locateBottomNeighborGELevelPeriodic(cell);
  {
    ...
    std::vector<std::pair<QtBox *, bool>> tagged;
    for (std::vector<QtBox *>::iterator it = bottomChildren.begin();
         it < bottomChildren.end(); it++)
      tagged.push_back(std::pair<QtBox *, bool>((*it), bottom.second));
    total.push_back(tagged);
#ifndef NDEBUG
    for (unsigned int j = 0; j < bottomChildren.size(); j++) {
      cell->isMyBottomNeighbor(bottomChildren[j]);
    }
#endif
  }
  ...
  std::pair<QtBox *, bool> top = locateTopNeighborGELevelPeriodic(cell);
  {
    ...
    std::vector<std::pair<QtBox *, bool>> tagged;
    for (std::vector<QtBox *>::iterator it = topChildren.begin();
         it < topChildren.end(); it++)
      tagged.push_back(std::pair<QtBox *, bool>((*it), top.second));
    total.push_back(tagged);
#ifndef NDEBUG
    for (unsigned int j = 0; j < topChildren.size(); j++) {
      cell->isMyTopNeighbor(topChildren[j]);
    }
#endif
  }
  ...
  // Top-left corner (it gives you a corner ONLY if it is not a lateral block!!)
  {
    ...
    if (first[0] == second[0]) {
      std::vector<std::pair<QtBox *, bool>> tagged;
      tagged.push_back(
          std::pair<QtBox *, bool>(first[0], (lt.second || tl.second)));
      total.push_back(tagged);
#ifndef NDEBUG
      for (unsigned int j = 0; j < first.size(); j++) {
        cell->isTopLeftCornerNeighbor(first[j]);
      }
#endif
    }
  }

  // Bottom-left corner (it gives you a corner ONLY if it is not a lateral
  // block!!)
  {
    std::pair<QtBox *, bool> lb =
        locateBottomNeighborGELevelPeriodic(left.first);
    std::vector<QtBox *> first;
    lb.first->topRightChildren(first);

    std::pair<QtBox *, bool> bl =
        locateLeftNeighborGELevelPeriodic(bottom.first);
    std::vector<QtBox *> second;
    ...
    if (first[0] == second[0]) {
      std::vector<std::pair<QtBox *, bool>> tagged;
      tagged.push_back(
          std::pair<QtBox *, bool>(first[0], (lb.second || bl.second)));
      total.push_back(tagged);
#ifndef NDEBUG
      for (unsigned int j = 0; j < first.size(); j++) {
        cell->isBottomLeftCornerNeighbor(first[j]);
      }
#endif
    }
  }

  // Top-right corner (it gives you a corner ONLY if it is not a lateral
  // block!!)
  {
    ...
    if (first[0] == second[0]) {
      std::vector<std::pair<QtBox *, bool>> tagged;
      tagged.push_back(
          std::pair<QtBox *, bool>(first[0], (ru.second || ur.second)));
      total.push_back(tagged);
#ifndef NDEBUG
      for (unsigned int j = 0; j < first.size(); j++) {
        cell->isTopRightCornerNeighbor(first[j]);
      }
#endif
    }
  }

  // Bottom-right corner (it gives you a corner ONLY if it is not a lateral
  // block!!)
  {
    std::pair<QtBox *, bool> rb =
        locateBottomNeighborGELevelPeriodic(right.first);
    std::vector<QtBox *> first;
    rb.first->topLeftChildren(first);

    std::pair<QtBox *, bool> br =
        locateRightNeighborGELevelPeriodic(bottom.first);
    std::vector<QtBox *> second;
    ...
    if (first[0] == second[0]) {
      std::vector<std::pair<QtBox *, bool>> tagged;
      tagged.push_back(
          std::pair<QtBox *, bool>(first[0], (rb.second || br.second)));
      total.push_back(tagged);
#ifndef NDEBUG
      for (unsigned int j = 0; j < first.size(); j++) {
        cell->isBottomRightCornerNeighbor(first[j]);
      }
#endif
    }
  }
  ...
  for (std::vector<std::vector<std::pair<QtBox *, bool>>>::iterator it =
           total.begin();
       it < total.end(); it++)
    for (std::vector<std::pair<QtBox *, bool>>::iterator it2 = (*it).begin();
         it2 < (*it).end(); it2++)
...
std::pair<std::vector<I3>, std::vector<std::pair<I3, I3>>>
QuadTree::locateNeighborhoodMRAG(QtBox *cell) {
  std::vector<std::pair<I3, I3>> ghosts;
  std::vector<I3> adjacent;
  std::vector<std::pair<QtBox *, bool>> tagged =
      locateAllNeighborsPeriodic(cell);

  for (std::vector<std::pair<QtBox *, bool>>::iterator it = tagged.begin();
       it < tagged.end(); it++) {
    if ((*it).second)
      ghosts.push_back(
          std::pair<I3, I3>((*it).first->getIndicesTranslated(cell),
                            (*it).first->getIndices())); // Ghosts
    else
      adjacent.push_back((*it).first->getIndices()); // Adjacent
  }

  std::pair<std::vector<I3>, std::vector<std::pair<I3, I3>>> outputMRAG(
      adjacent, ghosts);

  return outputMRAG;
}

std::vector<std::pair<std::vector<I3>, std::vector<std::pair<I3, I3>>>>
QuadTree::getNeighborsMRAG(std::vector<I3> const &input) {
  std::vector<std::pair<std::vector<I3>, std::vector<std::pair<I3, I3>>>>
      output;
  ...
  // Prepare output
  for (std::vector<QtBox *>::iterator it = leaves.begin(); it < leaves.end();
       it++)
