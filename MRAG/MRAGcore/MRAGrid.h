...
public:
  ...
  vector<BlockInfo> getNeighborsInfo(const vector<BlockInfo> &vInfo,
                                     bool bConsiderGhosts = false) const;
  vector<BlockInfo>
  getInteriorNeighborsInfo(const vector<BlockInfo> &vInfo,
                           bool bConsiderGhosts = false) const;
  virtual vector<BlockInfo> getBlocksInfoAtLevel(int level) const {
    return m_blockAtLevel[level];
  }
  ...
  virtual BoundaryInfo &getBoundaryInfo(int *output_stencil_start = NULL,
