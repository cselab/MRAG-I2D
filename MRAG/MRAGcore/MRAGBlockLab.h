 ...
 * the lab we are not changing the original data (unless we use
 * BlockLab::flush()).
 * @see MRAG::Block, BlockLab::prepare(), BlockLab::load()
 */
template <typename BlockType> class BlockLab {
...
protected:
  ...
  template <typename T> void _release_vector(T *&t, int n) {
    if (t != NULL)
...
public:
  BlockLab()
      : m_state(eMRAGBlockLab_Uninitialized), m_cacheBlock(NULL),
        m_weightPool(NULL), m_weightPoolSize(0), m_valuePool(NULL),
        m_valuePoolSize(0), m_refCollection(NULL), m_refBoundaryInfo(NULL) {
   ...
   * boundary. Defines how many ghosts we will get in extended block.
   */
  void prepare(const BlockCollection<BlockType> &collection,
               const BoundaryInfo &boundaryInfo, const int stencil_start[3],
    ...
    if (m_cacheBlock == NULL ||
      ...
      m_cacheBlock =
          allocator<Matrix3D<ElementType, true, _MRAG_BLOCKLAB_ALLOCATOR>>()
              .allocate(1);

      new (m_cacheBlock)
          Matrix3D<ElementType, true, _MRAG_BLOCKLAB_ALLOCATOR>();
  ...
  template <typename Processing> void inspect(Processing &) {}
  ...
  void load(const BlockInfo &info) {
    ...
    // 0.
    assert(m_state == eMRAGBlockLab_Prepared ||
    ...
    // 1.
    {
      ...
      for (int iz = 0; iz < nZ; iz++)
        for (int iy = 0; iy < nY; iy++) {
          ptrDestination = &m_cacheBlock->Access(0 - m_stencilStart[0],
                                                 iy - m_stencilStart[1],
                                                 iz - m_stencilStart[2]);
    ...
    // 2.
    {
      ...
      /* here we are going to do this:
       for(int i=0; i<nValuePoolSize; i++)
                m_valuePool[i] =
       collection[bbinfo.getIndexPool()[i].blockID][bbinfo.getIndexPool()[i].index];
  ...
  void flush(BlockInfo &info) {
    ...
    // 1.
    {
      ...
      for (int iz = 0; iz < nZ; iz++)
        for (int iy = 0; iy < nY; iy++)
          for (int ix = 0; ix < nX; ix++)
            block(ix, iy, iz) = m_cacheBlock->Access(ix - m_stencilStart[0],
                                                     iy - m_stencilStart[1],
                                                     iz - m_stencilStart[2]);
    }
   ...
   * BlockLab::prepare().
   *
   * @param ix    Index in x-direction (stencil_start[0] <= ix <
   * BlockType::sizeX + stencil_end[0] - 1).
   * @param iy    Index in y-direction (stencil_start[1] <= iy <
   * BlockType::sizeY + stencil_end[1] - 1).
   * @param iz    Index in z-direction (stencil_start[2] <= iz <
   * BlockType::sizeZ + stencil_end[2] - 1).
   */
...
private:
  ...
  BlockLab &operator=(const BlockLab &) {
    abort();
    return *this;
  }
};
