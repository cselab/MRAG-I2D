...
struct SmartBlockFinder {
  template <typename T>
  inline static _MRAG_GHOSTSCREATION_ALLOCATOR<T> allocator() {
    return _MRAG_GHOSTSCREATION_ALLOCATOR<T>();
  }
...
public:
  ...
  PointIndex *findData(const int level, I3 &i,
                       const GridNode **node = NULL) const;

private:
  // forbidden
  SmartBlockFinder(const SmartBlockFinder &)
      : vNeighborhood(0), nNeighborhoodSize(0), minLevel(-1), maxLevel(-1),
        referenceLevel(-1), refCenterNode(NULL), result(NULL) {
    abort();
  }

  SmartBlockFinder &operator=(const SmartBlockFinder &) {
    abort();
    return *this;
  }
};
...
inline SmartBlockFinder::SmartBlockFinder(const GridNode &b,
  ...
  int nTotalElements = 0;
  {
    ...
    for (int code = 0; code < 27; code++) {
      ...
      const int size_chunk[3] = {d[0] == 0 ? (block_size[0] > 1 ? length : 1)
                                           : (block_size[0] > 1 ? 1 : 0),
                                 d[1] == 0 ? (block_size[1] > 1 ? length : 1)
                                           : (block_size[1] > 1 ? 1 : 0),
                                 d[2] == 0 ? (block_size[2] > 1 ? length : 1)
                                           : (block_size[2] > 1 ? 1 : 0)};

      const int nBunchOfElements =
          size_chunk[0] * size_chunk[1] * size_chunk[2];
  ...
  // creation of the neighborhood
  {
    ...
    nNeighborhoodSize = (block_size[0] > 1 ? length + 2 : 1) *
                            (block_size[1] > 1 ? length + 2 : 1) *
                            (block_size[2] > 1 ? length + 2 : 1) -
                        (block_size[0] > 1 ? length : 1) *
                            (block_size[1] > 1 ? length : 1) *
                            (block_size[2] > 1 ? length : 1);
    ...
    for (int i = 0; i < neighbors.size(); i++) {
      ...
      // 2. compute the intersection between n_finestlevel and the box (-1,
      // length+1)
      ...
      const int s_n_finest[3] = {
          block_size[0] > 1
              ? (n.index[0] << (maxLevel - n.level)) - s_b_finest[0]
              : 0,
          block_size[1] > 1
              ? (n.index[1] << (maxLevel - n.level)) - s_b_finest[1]
              : 0,
          block_size[2] > 1
              ? (n.index[2] << (maxLevel - n.level)) - s_b_finest[2]
              : 0};
      ...
      for (d[2] = s_intersection[2]; d[2] < e_intersection[2]; d[2]++)
        for (d[1] = s_intersection[1]; d[1] < e_intersection[1]; d[1]++)
          for (d[0] = s_intersection[0]; d[0] < e_intersection[0]; d[0]++) {
            const int dir[3] = {(int)floor(d[0] / (double)length),
            ...
            const int size_chunk[3] = {
                dir[0] == 0 ? (block_size[0] > 1 ? length : 1)
                            : (block_size[0] > 1 ? 1 : 0),
                dir[1] == 0 ? (block_size[1] > 1 ? length : 1)
                            : (block_size[1] > 1 ? 1 : 0),
                dir[2] == 0 ? (block_size[2] > 1 ? length : 1)
                            : (block_size[2] > 1 ? 1 : 0)};
            ...
            const int dest_index = nSubStart[code] + dest_pos[0] +
                                   row_size * dest_pos[1] +
                                   slice_size * dest_pos[2];

            assert(nSubStart[code] <= dest_index &&
                   dest_index < nSubStart[code] + size_chunk[0] *
                                                      size_chunk[1] *
                                                      size_chunk[2]);
  ...
  for (int c = 0; c < 27; c++) {
    const int length = (1 << (maxLevel - b.level));
    const int dir[3] = {(c % 3) - 1, (c / 3 % 3) - 1, (c / 9 % 3) - 1};

    const int size_chunk[3] = {dir[0] == 0 ? (block_size[0] > 1 ? length : 1)
                                           : (block_size[0] > 1 ? 1 : 0),
                               dir[1] == 0 ? (block_size[1] > 1 ? length : 1)
                                           : (block_size[1] > 1 ? 1 : 0),
                               dir[2] == 0 ? (block_size[2] > 1 ? length : 1)
    ...
    for (int i = nSubStart[c]; i < n + nSubStart[c]; i++) {
      ...
      if (vNeighborhood[i] != NULL && c != 13)
        if (bVerbose)
          printf("vNeighborhood[%d]->level = %d\n", i, vNeighborhood[i]->level);
    }
  }
}

inline const GridNode *
SmartBlockFinder::findLyingNode(const int level, const int index[3]) const {
  ...
  const int candidate_i[3] = {
      block_size[0] > 1
          ? (int)floor((index[0] << level_difference) / (double)block_size[0])
          : 0,
      block_size[1] > 1
          ? (int)floor((index[1] << level_difference) / (double)block_size[1])
          : 0,
      block_size[2] > 1
          ? (int)floor((index[2] << level_difference) / (double)block_size[2])
          : 0};

  // 2.
  const int i[3] = {
      candidate_i[0] < -1 ? -1
                          : (candidate_i[0] > length ? length : candidate_i[0]),
      candidate_i[1] < -1 ? -1
                          : (candidate_i[1] > length ? length : candidate_i[1]),
      candidate_i[2] < -1
          ? -1
          : (candidate_i[2] > length ? length : candidate_i[2])};
  ...
  // 3.
  const int dir[3] = {(int)floor(i[0] / (double)length),
  ...
  const int s[3] = {dir[0] < 0 ? -1 : (dir[0] == 0 ? 0 : length),
                    dir[1] < 0 ? -1 : (dir[1] == 0 ? 0 : length),
                    dir[2] < 0 ? -1 : (dir[2] == 0 ? 0 : length)};
  ...
  for (int i = 0; i < 3; i++) {
    assert((result->index[i] << level_diff_gridnode) * block_size[i] <=
               (index[i] << level_difference) +
                   (refCenterNode->index[i] << level_diff_centernode) *
                       block_size[i] ||
           block_size[i] == 1);
    assert(((result->index[i] + 1) << level_diff_gridnode) * block_size[i] >
               (index[i] << level_difference) +
                   (refCenterNode->index[i] << level_diff_centernode) *
                       block_size[i] ||
           block_size[i] == 1);
...
inline int SmartBlockFinder::findLyingLevel(const int level,
                                            const int index[3]) const {
  return findLyingNode(level, index)->level;
}

inline PointIndex *
SmartBlockFinder::findData(const int level, I3 &ghost,
                           const GridNode **node_output) const {
  ...
  if (lying_level != level)
    return NULL;

  const int d[3] = {
      ghost.i[0] - block_size[0] * node->index[0] +
          (int)floor(dilate * block_size[0] * refCenterNode->index[0]),
      ghost.i[1] - block_size[1] * node->index[1] +
          (int)floor(dilate * block_size[1] * refCenterNode->index[1]),
      ghost.i[2] - block_size[2] * node->index[2] +
          (int)floor(dilate * block_size[2] * refCenterNode->index[2])};
  ...
  result->index =
      d[0] + d[1] * block_size[0] + d[2] * block_size[0] * block_size[1];
