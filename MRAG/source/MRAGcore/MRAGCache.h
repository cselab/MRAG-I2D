...
template <typename KeyType, typename CachedType> class Cache {
public:
  Cache(void)
      : pcache(NULL), ticket(0), in_cache(), victims(), owner(false),
        valid(false) {}
...
private:
  struct SlotInfo {
    ...
    SlotInfo(int ticket_, int slot_, const KeyType *key_ = NULL,
             bool valid = false)
        : is_valid(valid), ticket(ticket_), slot(slot_), key(key_) {}
...
template <typename KeyType, typename CachedType>
CachedType &Cache<KeyType, CachedType>::get(const KeyType key,
                                            bool &bCacheHit) {
  assert(valid);
  ...
  typename map<KeyType, const int>::const_iterator itInCache =
      in_cache.find(key);
  ...
  if (bCacheHit) {
    ...
    victims.push(
        SlotInfo(ticket++, slot_info.slot, &in_cache.find(key)->first, true));
    victims.pop();
...
template <typename KeyType, typename CachedType>
void Cache<KeyType, CachedType>::_rebuildVictims(
    priority_queue<SlotInfo> &v) const {
