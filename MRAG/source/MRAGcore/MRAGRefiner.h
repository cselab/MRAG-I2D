...
protected:
  ...
  bool _isFound(const C &c, const K &k) const {
...
public:
  ...
  Refiner(int nMaxLevelJump = 1, int nMaxLevel = -1)
      : m_nMaxLevelJump(nMaxLevelJump), m_nMaxLevel(nMaxLevel) {}
  ...
  virtual RefinementPlan *createPlan(const HierarchyType &hierarchy,
    ...
    // 3.
    {
      ...
      do {
        ...
        {
          new_victims.clear();
          for (VictimSet::iterator victim = old_victims.begin();
               victim != old_victims.end(); victim++) {
            ...
            {
              // look for the parent's children. the one with the same blockID
              // is the one. remove then the ghosts from the vicims

              HierarchyType::const_iterator itParent =
                  hierarchy.find((*victim)->parent);
              ...
              for (iChild = 0; iChild < children.size(); iChild++)
                if (children[iChild]->blockID == (*victim)->blockID)
                  break;

              assert(children[iChild]->blockID == (*victim)->blockID);

              NeighborhoodType::const_iterator itRealNode =
                  neighborhood.find(children[iChild]);
            ...
            for (vector<GridNode *>::const_iterator n = neighbors.begin();
              ...
              const bool bJumpInLevelTooHigh =
                  (abs(node.level - new_victim_level) > m_nMaxLevelJump);
    ...
    {
      int nNewChildren = 0;
      for (VictimSet::const_iterator victim = victims.begin();
           victim != victims.end(); victim++) {
