...
struct BitStream {
  ...
  BitStream(const BitStream &b)
      : bits(), words32(b.words32), word32_index(b.word32_index),
        bits_end(b.bits_end) {
    assert(bits == NULL);
    bits = b.bits;
  }
  ...
  inline void append_bits(unsigned int new_bits, unsigned int nof_bits) {
    ...
    if (word32_index + (unsigned int)ceil((nof_bits + bits_end - 1) / 32.) >=
        words32) {
      words32 *= 2;
  ...
  inline unsigned int get_bits(unsigned int bit_start,
                               unsigned int nof_bits) const {
    assert(nof_bits <= 32);
  ...
  unsigned int getLength() const { return 32 * (word32_index) + bits_end; }

private:
  inline void _CopyToTheEnd(unsigned int &ob, unsigned int nb,
                            unsigned int be) {
