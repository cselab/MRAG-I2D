...
Matrix2D<DataType>::Matrix2D(unsigned int nSizeX, unsigned int nSizeY,
                             DataType *data)
    : m_pData(NULL), m_nElements(0), m_bDataOwner(data == NULL) {
  m_vSize[0] = nSizeX;
  m_vSize[1] = nSizeY;
...
template <class DataType>
inline DataType &Matrix2D<DataType>::Access(unsigned int ix,
...
template <class DataType>
inline DataType &Matrix2D<DataType>::LinAccess(unsigned int i) const {
  assert(i < m_nElements);
  return m_pData[i];
}

template <class DataType>
inline unsigned int Matrix2D<DataType>::getNumberOfElements() const {
  return m_nElements;
}
...
template <class DataType>
inline Matrix2D<DataType> &Matrix2D<DataType>::operator=(DataType val) {
