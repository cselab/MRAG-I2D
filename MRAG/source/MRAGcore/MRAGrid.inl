...
#pragma mark Class Management
	...
	template <typename WaveletType, typename BlockType>
	vector<BlockInfo> Grid<WaveletType, BlockType>::getNeighborsInfo(const vector<BlockInfo>& vInfo, bool bConsiderGhosts) const
	{
		...
		//5. select those neighbors that are not inside BlockInfo and forget about ghosts
		
		if (bConsiderGhosts == true)
		{
			printf("Grid<WaveletType, BlockType>::getNeighborsInfo: bConsiderGhosts=true is not implemented yet. Aborting.\n");
			abort();
		}
		...
		for(vector<BlockInfo>::const_iterator it = vInfo.begin(); it != vInfo.end(); ++it)
		{
			...
			for(vector<GridNode *>::const_iterator itN = neighbors.begin(); itN != neighbors.end(); ++itN)
			{
		...
		for(set<GridNode *>::const_iterator it = to_process.begin(); it != to_process.end(); ++it)
		{
			...
			const Real p[3] = {
				m_vPosition[0] + node->index[0]*dilate*m_vSize[0] + (WaveletType::bIsCellCentered?(Real)0.5*h[0] : 0),
				m_vPosition[1] + node->index[1]*dilate*m_vSize[1] + (WaveletType::bIsCellCentered?(Real)0.5*h[1] : 0),
				m_vPosition[2] + node->index[2]*dilate*m_vSize[2] + (WaveletType::bIsCellCentered?(Real)0.5*h[2] : 0)
			};
	...
	template <typename WaveletType, typename BlockType>
	vector<BlockInfo> Grid<WaveletType, BlockType>::getInteriorNeighborsInfo(const vector<BlockInfo>& vInfo, bool bConsiderGhosts) const
	{
        // same method as getNeighborsInfo but now returning only ghosts (== interior neighbors, not across domain boundaries)
		...
		if (bConsiderGhosts == true)
		{
			printf("Grid<WaveletType, BlockType>::getInteriorNeighborsInfo: bConsiderGhosts=true is not implemented yet. Aborting.\n");
			abort();
		}
...
#pragma mark Compression
	...
	CompressionResult Grid<WaveletType, BlockType>::compress(const set<int>& blocksToCompress, int& nCollapsedBlocks)
	{
		...
		CompressionResult result = m_blockCollapser->collapse(m_blockCollection, bInfo,  *plan, vBlockCollapseInfo);
...
#pragma mark Boundary Info Computation
	...
	struct Body_CreateBoundaryInfo
	{
		...
		void operator()(const BlockedRange& r) const
		{
			...
			for(int i=r.begin(); i!=r.end(); i++)
			{
				...
				//vWorkingList[i]->bbi->node = const_cast<GridNode*>(vWorkingList[i]->node);
	...
	void Grid<WaveletType, BlockType>::_computeBoundaryInfo(BoundaryInfo& binfo, const int requested_stencil_start[3], const int requested_stencil_end[3], vector<GridNode*>& vNodesToCompute ) const
	{
		...
		tbb::parallel_for(tbb::blocked_range<size_t>(0, n), body, tbb::auto_partitioner());
#else
