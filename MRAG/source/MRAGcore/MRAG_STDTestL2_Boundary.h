...
class MRAG_STDTestL2_Boundary // LEVEL 2: correct data representation at
                              // boundaries
{
protected:
  ...
  // Destructor:
  ~MRAG_STDTestL2_Boundary() { delete bInfo; }
    ...
    for (int i = 0; i < vInfo.size(); i++) {
      ...
      for (int iy = 0; iy < Block::sizeY; iy++)
        for (int ix = 0; ix < Block::sizeX; ix++) {
          ...
          block(ix, iy).cur = sin((x[0] + cOffset) * 2.0 * M_PI) *
                              cos((x[1] + cOffset) * 2.0 * M_PI);
          block(ix, iy).setIC();
          block(ix, iy).update();
        }
    }
  ...
  void _some_computation(int dt, int ux,
                         int uy) //"integer-advection (obsolete)"
  {
    ...
    std::cout << "start:" << steStart[0] << " " << steStart[1] << " "
              << steStart[2] << " " << std::endl;
    std::cout << "end:" << steEnd[0] << " " << steEnd[1] << " " << steEnd[2]
              << std::endl;
    std::cout << "shifting data: lab(ix+" << dt * ux << ",iy+" << dt * uy
              << ")=oldlab(ix,iy)" << std::endl;
#endif mq_debug
...
#ifdef mq_debug
          std::cout << "current,old,diff:" << lab(ix, iy).cur << " "
                    << lab(ix, iy).old << " "
                    << lab(ix, iy).cur - lab(ix, iy).old << std::endl;
#endif
  ...
  void _some_advection_1stOrder(double dt, double ux,
                                double uy) // upwinding advection
  {
    int steStart[3] = {-1, -1, 0};
    int steEnd[3] = {2, 2, 1};

    std::cout << "Doing in a 1st order upwinding step with: ux= " << ux
              << " uy= " << uy << std::endl;
      ...
      for (int iy = 0; iy < Block::sizeY; iy++)
        for (int ix = 0; ix < Block::sizeX; ix++) {
          ...
          lab(ix, iy).cur =
              lab(ix, iy).old -
              dt * (fabs(ux) / h[0] *
                        (lab(ix + selx, iy).old - lab(ix + selx - 1, iy).old) +
                    fabs(uy) / h[1] *
                        (lab(ix, iy + sely).old - lab(ix, iy - sely - 1).old));
        }
      lab.flush(info);
    }
    ...
    for (int i = 0; i < vInfo.size(); i++) {
      ...
      for (int iy = 0; iy < Block::sizeY; iy++)
        for (int ix = 0; ix < Block::sizeX; ix++) {
          block(ix, iy).update();
#ifdef mq_debug
          std::cout << "[chkUpd:]current,old,diff:" << block(ix, iy).cur << " "
                    << block(ix, iy).old << " "
                    << block(ix, iy).cur - block(ix, iy).old << std::endl;
#endif
        }
    }
  }

  void _some_advection_2ndOrder(double dt, double ux,
                                double uy) // upwinding advection
  {
    ...
    std::cout << "Doing in a 2ndorder upwinding step with: ux= " << ux
              << " uy= " << uy << std::endl;
    ...
    for (int i = 0; i < vInfo.size(); i++) {
      ...
      for (int iy = 0; iy < Block::sizeY; iy++)
        for (int ix = 0; ix < Block::sizeX; ix++) {
          ...
          dudx = 1.0 / (2.0 * h[0]) *
                 (3.0 * lab(ix, iy).old - 4.0 * lab(ix + selx, iy).old +
                  lab(ix - (selx * 2), iy).old);
          dudy = 1.0 / (2.0 * h[1]) *
                 (3.0 * lab(ix, iy).old - 4.0 * lab(ix, iy + sely).old +
                  lab(ix, iy - (sely * 2)).old);
          lab(ix, iy).cur =
              lab(ix, iy).old - dt * (fabs(ux) * dudx + fabs(uy) * dudy);
        }
      lab.flush(info);
    }
...
public:
  static void runTests(int LevelOfRefinements = 2, int NumOfBlocks = 2,
    ...
    // usage example:
    // MRAG::MRAG_STDTestL2_Boundary<MRAG::Wavelets_Interp2Order,
    // MRAG::Block<misc::memNum<double>,16,16,1> >::runTests(2,4,2,1);
  }
};
...
template <typename NType> struct memNum {
  NType cur;
  NType old;
  NType ic;
