...
private:
  ...
  void checkJumpResolution() {
    for (HierarchyType::const_iterator it = grid.m_hierarchy.begin();
         it != grid.m_hierarchy.end(); it++) {
  ...
  template <typename Grid> int _getID(int ix, int iy, int level, Grid &g) {
  ...
  MRAG_STDTestL1_BoundaryInfo(int nBlocks)
      : viewer(true), compressor(), refiner(2),
        grid(Block::sizeX > 1 ? 2 : 1, Block::sizeY > 1 ? 2 : 1,
             Block::sizeZ > 1 ? 2 : 1) {
...
template <typename Wavelets, typename Block>
MRAG_STDTestL1_BoundaryInfo<Wavelets, Block>
    *MRAG_STDTestL1_BoundaryInfo<Wavelets, Block>::singletone = NULL;
} // namespace MRAG
#endif
