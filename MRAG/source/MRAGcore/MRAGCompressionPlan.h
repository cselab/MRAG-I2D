...
struct CompressionPlan {
  ...
  ~CompressionPlan() { delete[] vCollapseArray; }

private:
  CompressionPlan(const CompressionPlan &)
      : nCollapses(0), vCollapseArray(NULL) {
    abort();
  }
  CompressionPlan &operator=(const CompressionPlan &) {
    abort();
    return *this;
  }
};
