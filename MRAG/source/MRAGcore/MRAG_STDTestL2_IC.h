...
private:
  typedef double (MRAG::ICClass::*InitialConditionPointer)(double, double,
                                                           double, double);
  ...
  void _writeVectors() {
    ...
    for (int i = 0; i < Convergence_L2_Error_Vector.size(); ++i) {
      fprintf(myfile, "%lf\t%lf\n", Convergence_NumPoint_Vector[i],
              Convergence_L2_Error_Vector[i]);
    }
  ...
  void _ic(InitialConditionPointer icPointer2Function) {
    ...
    for (int i = 0; i < vInfo.size(); i++) {
      ...
      for (int iy = 0; iy < Block::sizeY; iy++)
        for (int ix = 0; ix < Block::sizeX; ix++) {
          const double x[2] = {start[0] + (ix)*d[0], start[1] + (iy)*d[1]};
          block(ix, iy) = (initialConditionObj.*icPointer2Function)(
              (x[0] + cOffset), (x[1] + cOffset), 0.0, 0.0);
        }
    }
  }
  ...
  void _error(InitialConditionPointer icPointer2Function, double &L2_error,
    ...
    for (int i = 0; i < vInfo.size(); i++) {
      ...
      for (int iy = 0; iy < Block::sizeY; iy++)
        for (int ix = 0; ix < Block::sizeX; ix++) {
          ...
          const double x[2] = {start[0] + (ix)*d[0], start[1] + (iy)*d[1]};
          L2_error += pow((block(ix, iy) -
                           (initialConditionObj.*icPointer2Function)(
                               (x[0] + cOffset), (x[1] + cOffset), 0.0, 0.0)),
                          2.0);
        }
    }
  ...
  bool run(double epsilon, int numberOfLevels) {
    try {
      ...
      cout << "Start Test with epsilon: " << epsilon
           << ", numberOfLevels: " << numberOfLevels << endl;

      // set a pointer to the initial condition function contained in the class
      // "ICClass"
