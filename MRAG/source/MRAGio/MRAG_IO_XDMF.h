...
namespace MRAG {
template <typename E> inline Real _xdmf_get(E &e, int c, Real h) {
  return e.giveMe(c, h);
}
...
public:
  ...
  Frame Write(GridType &grid, const std::string &fileName, double time = 0.0,
              int step = 0, const char *const *names = NULL) {
    ...
    for (int c = 0; c < nChannels; c++) {
      char nm[64];
      if (names != NULL)
        snprintf(nm, sizeof nm, "%s", names[c]);
      else
        snprintf(nm, sizeof nm, "channel%d", c);
    ...
    for (int c = 0; c < nChannels; c++)
      fprintf(xdmf,
              ...
              "        </DataItem>\n"
              "      </Attribute>\n",
              fr.name[c].c_str(), ncells, (long)sizeof(Real),
              fr.file[c].c_str());
    fprintf(xdmf, "    </Grid>\n"
                  "  </Domain>\n"
                  "</Xdmf>\n");
    ...
    for (int i = 0; i < (int)vInfo.size(); i++) {
      ...
      for (int iy = 0; iy < by; iy++)
        for (int ix = 0; ix < bx; ix++) {
          double x[2];
          info.pos(x, ix, iy);
          const float u0 = x[0] - off * h, v0 = x[1] - off * h, u1 = u0 + h,
                      v1 = v0 + h;
    ...
    for (size_t i = 0; i < frames.size(); i++) {
      ...
      for (size_t c = 0; c < f.name.size(); c++)
        fprintf(m,
                ...
                "          </DataItem>\n"
                "        </Attribute>\n",
                f.name[c].c_str(), f.ncells, (long)sizeof(Real),
                f.file[c].c_str());
      fprintf(m, "      </Grid>\n");
    }
    fprintf(m, "    </Grid>\n"
