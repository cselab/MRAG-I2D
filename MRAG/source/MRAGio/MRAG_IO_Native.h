 ...
 *
 *	Changed by Manfred Quack on 09/09/09: _WriteBlock and _ReadBlock
 * functions (to facilitate _Binary version)
 *
 */
...
public:
  ...
  // Typedefs
  typedef MRAG::IO_BaseClass<TWavelets, TBlock, TProjector, nChannels>
      SuperClass;
...
protected:
  // void _Write( GridType & inputGrid, string fileName, FILE*
  // binaryOutputFile=NULL ); void _Read( GridType & inputGrid, string fileName,
  // FILE* binaryInputFile=NULL );

  // virtual:
  // virtual void _WriteBlock(TBlock& block, std::ofstream & outputstream,FILE *
  // outputBinary); virtual void _ReadBlock(TBlock& block, std::ifstream &
  // inputstream,FILE * inputBinary);
  ...
  virtual void _WriteBlock(TBlock &block, std::ofstream &outputstream,
                           FILE *outputBinary) {
    outputstream.precision(16);
    for (int iz = 0; iz < TBlock::sizeZ; iz++)
      for (int iy = 0; iy < TBlock::sizeY; iy++)
        for (int ix = 0; ix < TBlock::sizeX; ix++) {
          outputstream << TProjector::template Project<ElementType, nChannels>(
                              block(ix, iy, iz))
                       << "  ";
        }
  }
  ...
  virtual void _ReadBlock(TBlock &block, std::ifstream &inputstream,
                          FILE *inputBinary) {
    for (int iz = 0; iz < TBlock::sizeZ; iz++)
      for (int iy = 0; iy < TBlock::sizeY; iy++)
        for (int ix = 0; ix < TBlock::sizeX; ix++) {
  ...
  void _Write(GridType &inputGrid, string fileName,
              FILE *binaryOutputFile = NULL) {
    // Open output file
    fileName += ".txt";
    ...
    for (HierarchyType::const_iterator it = inputGrid.m_hierarchy.begin();
         it != inputGrid.m_hierarchy.end(); it++) {
    ...
    // Loop over hierarchy and print out information to reconstruct grid
    // topology
    ...
    for (HierarchyType::const_iterator it = inputGrid.m_hierarchy.begin();
         it != inputGrid.m_hierarchy.end(); it++) {
      if (it->first == NULL) {
        // If the node is the root I print out fake values for level, isEmpty,
        // blockID, etc. They will not be used is just to mantain the format
        // regular
        ...
        for (unsigned int i = 0; i < it->second.size(); i++) {
          output << mappingNodes[it->second[i]] << "  ";
        }
        output << endl;
        output << endl;
      } else {
  ...
  void _Read(GridType &inputGrid, string fileName,
             FILE *binaryInputFile = NULL) {
    inputGrid._dispose();
    ...
    if (numberOfNodes == 0) {
      printf("numberOfNodes == 0, this is a failure.\nDid you compile this "
             "program in Debug Mode with GCC <4.0? STD C++ DEBUG NOT SUPPORTED "
             "with that compiler\n");
      abort();
    }
    input >> rootPosition;
    for (unsigned int i = 0; i < numberOfNodes; i++) {
      ...
      mapNodeIDToNode[nodeID] = new GridNode(isEmpty, NULL, blockID, index[0],
                                             index[1], index[2], level);
    ...
    // allocate blocks, attach them to the leaves
    {
      ...
      for (map<int, GridNode *>::const_iterator it = mapNodeIDToNode.begin();
           it != mapNodeIDToNode.end(); it++)
    ...
    // Read block data
    {
      ...
      for (int i = 0; i < nBlocks; i++) {
        ...
        TBlock &block =
            inputGrid.getBlockCollection().lock(mapCounterToBlockID[blockID]);
