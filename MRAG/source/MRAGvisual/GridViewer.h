...
private:
  ...
  template <typename Block> void _Setup() {
    if (m_texBlock != NULL)
      return;

    m_texBlock = new Texture2D(Block::sizeX, Block::sizeY,
                               m_bRGBA ? GL_RGBA32F_ARB : GL_LUMINANCE32F_ARB,
                               GL_NEAREST);
  }

  template <typename Block>
  void _drawBlockContent(Block &block, const short int index[3],
                         short int level) {
    ...
    if (!m_bRGBA) {
      ...
      glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, Block::sizeX,Block::sizeY, GL_RED,
      GL_FLOAT, buf); delete [] buf;*/
    } else {
      RGBA *buf = new RGBA[n];
      for (int i = 0; i < n; i++)
        buf[i] = convertToRGBA(block[i]);
      glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, Block::sizeX, Block::sizeY,
    ...
    if (!m_bCollocated) {
      ...
      glTexCoord2f(Block::sizeX / (double)m_texBlock->size[0],
                   Block::sizeY / (double)m_texBlock->size[1]);
  ...
  template <typename Block>
  void _drawBlockSkeleton(const short int index[3], short int level,
                          const bool bDrawBlockElements,
                          const float *vColorBoundary = NULL,
  ...
  void _drawGridPoint(A block_index[3], B_ level, C point_index[3],
                      const float *vColorBoundary = NULL, double factor = 1.) {
    const double h[2] = {pow(2., -level), pow(2., -level)};
    ...
    const int n[2] = {Block::sizeX + 1, Block::sizeY + 1};
    const double d[2] = {h[0] / (n[0] - 1), h[1] / (n[1] - 1)};
    const double point_center[2] = {
        start[0] + (point_index[0] + (m_bCollocated ? 0 : 0.5)) * d[0],
        start[1] + (point_index[1] + (m_bCollocated ? 0 : 0.5)) * d[1]};
  ...
  void _drawConnection(A block_index[3], B_ level, C point_index[3],
                       A block_index2[3], B_ level2, C point_index2[3],
                       const float *vColorBoundary = NULL) {
...

    {
      ...
      const double start[2] = {block_index[0] * h[0], block_index[1] * h[1]};
      ///	const double end[2] =
      ///{(block_index[0]+1)*h[0],(block_index[1]+1)*h[1]};
      const int n[2] = {Block::sizeX + 1, Block::sizeY + 1};
      const double d[2] = {h[0] / (n[0] - 1), h[1] / (n[1] - 1)};
      const double point_center[2] = {
          start[0] + (point_index[0] + (m_bCollocated ? 0 : 0.5)) * d[0],
    ...
    }
    {
      ...
      //	const double end[2] =
      //{(block_index2[0]+1)*h[0],(block_index2[1]+1)*h[1]};
      const int n[2] = {Block::sizeX + 1, Block::sizeY + 1};
      const double d[2] = {h[0] / (n[0] - 1), h[1] / (n[1] - 1)};
      const double point_center[2] = {
          start[0] + (point_index2[0] + (m_bCollocated ? 0 : 0.5)) * d[0],
          start[1] + (point_index2[1] + (m_bCollocated ? 0 : 0.5)) * d[1]};
...
public:
   ...
   * @param bCollocated   Optional (def = false): true if we are working on a
   * collocated grid
   * @param bRGBA         Optional (def = false): true if convertToRGBA function
   * used to get color for each element in grid.
   * @see GridViewer::drawContent()
   */
  GridViewer(bool bCollocated = false, bool bRGBA = false)
      : m_texBlock(NULL), m_bCollocated(bCollocated), m_bRGBA(bRGBA) {}

  /**
   * Draws content of all blocks in the grid.
   * Requires:
   * - function MRAG::Visual::RGBA convertToRGBA(Block::ElementType & p)
   * returning color of a grid-element. (only if bRGBA set to true in
   * constructor)
   * - operator Real() returning floating point number representing a
   * grid-element
   */
  template <typename Wavelets, typename Block>
  void drawContent(Grid<Wavelets, Block> &grid,
   ...
   */
  template <typename Wavelets, typename Block>
  void drawSketch(Grid<Wavelets, Block> &grid,
                  bool bDrawBlockElements = false) {
    glBegin(GL_LINES);
    glColor3f(0, 1, 0);
    glVertex2f(0, 0);
    glVertex2f(0, 1);
  ...
  void drawNeighborsOfABlock(MRAG::Grid<Wavelets, Block> &grid) {
    ...
    for (int i = 0; i < it->second.size(); i++) {
      ...
      _drawBlockSkeleton<Block>(node.index, node.level, true, vContent,
                                vBoundary);
    }
  ...
  template <typename Wavelets, typename Block>
  void drawGhosts(MRAG::Grid<Wavelets, Block> &grid,
                  const BlockCollection<Block> &collection,
    ...
    BlockInfo &blockinfo =
        vInfo[counter]; //*mapBlockIDToBlockInfo[10];// ////;////
    printf("BLOCKID = ======= %d\n", blockinfo.blockID);
    ...
    _drawBlockSkeleton<Block>(blockinfo.index, blockinfo.level, true, vBlock,
                              vContent);
    ...
    for (int iz = 0; iz < nZ; iz++)
      for (int iy = 0; iy < nY; iy++)
        for (int ix = 0; ix < nX; ix++) {
          ...
          _drawGridPoint<Block>(blockinfo.index, blockinfo.level, idx,
                                vContent);
        }

    BoundaryInfoBlock &bbinfo =
        *boundaryInfo.boundaryInfoOfBlock[blockinfo.blockID];
    bbinfo.lock();
    ...
    for (int icode = 0; icode < 27; icode++) {
      ...
      const int e[3] = {code[0] < 1 ? (code[0] < 0 ? 0 : nX)
                                    : nX + boundaryInfo.stencil_end[0] - 1,
                        code[1] < 1 ? (code[1] < 0 ? 0 : nY)
                                    : nY + boundaryInfo.stencil_end[1] - 1,
                        code[2] < 1 ? (code[2] < 0 ? 0 : nZ)
                                    : nZ + boundaryInfo.stencil_end[2] - 1};

      assert(bbinfo.boundary[icode].nGhosts ==
             (e[2] - s[2]) * (e[1] - s[1]) * (e[0] - s[0]));
      ...
      for (int iz = s[2]; iz < e[2]; iz++)
        for (int iy = s[1]; iy < e[1]; iy++)
          for (int ix = s[0]; ix < e[0]; ix++, currentghost++) {
            ...
            double sum = 0;
            for (int i = 0; i < n; i++)
              sum += bbinfo.weightsPool[vWP[i].weights_index[0]] *
                     bbinfo.weightsPool[vWP[i].weights_index[1]] *
                     bbinfo.weightsPool[vWP[i].weights_index[2]];
            printf("Ghost number: %d (start: %d nGhosts: %d, code=%d)\n",
                   ghost_counter, currentghost, bbinfo.boundary[icode].nGhosts,
                   icode);
            ghost_counter++;
            const bool bOk = fabs(sum - 1.0) < 0.000001;

            if (!bOk) {
              printf("Failed: block %d %d %d l=%d, ghost: %d %d %d weight: %f "
                     "(Contributions: %d)\n",
                     blockinfo.index[0], blockinfo.index[1], blockinfo.index[2],
                     blockinfo.level, ix, iy, iz, sum, n);
            } else {
              /*		printf("PASSED!! block %d %d %d l=%d, ghost: %d
                 %d %d weight: %f (Contributions: %d)\n", blockinfo.index[0],
                 blockinfo.index[1],  blockinfo.index[2],  blockinfo.level, ix,
                 iy,  iz, sum, n);*/
            }
            assert(bOk);
            for (int i = 0; i < n; i++) {
              const int blockID =
                  bbinfo.getIndexPool()[vWP[i].point_index].blockID;
              const int pointID =
                  bbinfo.getIndexPool()[vWP[i].point_index].index;
              ...
              // mapBlockIDToBlockInfo[blockID]->index;
              _drawGridPoint<Block>(mapBlockIDToBlockInfo[blockID]->index,
                                    mapBlockIDToBlockInfo[blockID]->level,
                                    point_index, vConnecions, 1.0);
              _drawConnection<Block>(mapBlockIDToBlockInfo[blockID]->index,
                                     mapBlockIDToBlockInfo[blockID]->level,
                                     point_index, blockinfo.index,
                                     blockinfo.level, pt, vConnecions);

              if (!bOk) {
                printf("(Failed) Contribution %d: block %d %d %d l=%d, point "
                       "%d %d %d, w=%f %f %f\n",
                       i, mapBlockIDToBlockInfo[blockID]->index[0],
                       mapBlockIDToBlockInfo[blockID]->index[1],
                       mapBlockIDToBlockInfo[blockID]->index[2],
                       mapBlockIDToBlockInfo[blockID]->level, point_index[0],
                       point_index[1], point_index[2],
                       bbinfo.weightsPool[vWP[i].weights_index[0]],
                       bbinfo.weightsPool[vWP[i].weights_index[1]],
                       bbinfo.weightsPool[vWP[i].weights_index[2]]);
              }
              ...
              // const double w =
              // (weight_pool[iw[0]]*weight_pool[iw[1]]*weight_pool[iw[2]]);
              // ghost +=
              // (value_pool[vWP[i].point_index]*w);//((weight_pool[iw[0]]*weight_pool[iw[1]]*weight_pool[iw[2]])*value_pool[vWP[i].point_index]);
            }

            const int pt[3] = {ix, iy, iz};
            const float vColor[3] = {bOk ? 1 : 0.5, bOk ? 0 : 0.5,
                                     bOk ? 0 : 0.5};
            _drawGridPoint<Block>(blockinfo.index, blockinfo.level, pt, vColor,
                                  1.0);
            //	const int point_index[3]= {ix, iy, iz};
            //	_drawGridPoint<Block>(blockinfo.index, blockinfo.level,
            //point_index, vBoundary);
          }
    }
    bbinfo.release();
