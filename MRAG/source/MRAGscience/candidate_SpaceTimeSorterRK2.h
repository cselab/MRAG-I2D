...
class SpaceTimeSorterRK2 : public SpaceTimeSorter {
  //
public:
  // int estimateNofBlockOperations(int nRecursionsPerLevel, int startLevel)
  // const; virtual bool getBlocks(int& level, double& timeStep, double&
  // currentTime, vector<BlockInfo>& blocksInfo,  ETimeInterval& type);
      ...
      if (l < m_maxLevel) {
        ...
        for (int i = m_nRecursions - 1; i >= 0; i--)
          m_stackRecursions.push(
              StackItem(l + 1, t + (i + 0) * children_dt, ETimeInterval_Start));
      }
    }
  ...
  int estimateNofBlockOperations(int nRecursionsPerLevel,
