...
namespace MRAG
{
	namespace Science
	{
		...
		int AutomaticRefinement(Grid& g, BlockFWT& fwt, const double dAbsoluteTolerance, 
								 ...
								 MRAG::Profiler* profiler=NULL, void (*fillGrid)(Grid& g)=NULL, set<int> * dont_refine=NULL)
		{
			int loopCounter = 0;
			...
			do
			{
				...
				else 
				{
					if (iMaxLoops != 1)
					{
						printf("Can't be that you provide set<int> * dont_refine and iMaxLoops is not 1.\n");
						abort();
					}
					
					for(int i=0; i<vBlocksToFWT.size(); i++)
						if (vReports[i].getOverAll_DetailMaxMag() >= dAbsoluteTolerance && dont_refine->find(vBlocksToFWT[i].blockID) == dont_refine->end())
							shouldBeRefined.insert(vBlocksToFWT[i].blockID); // i was right
						else
							niceGuys.insert(vBlocksToFWT[i].blockID); //i was wrong
				}
				...
				nRefinedBlocks += g.getBlocksInfo().size() - oldnumblocks;//result.nCollapsedParentBlocks;
                                 ...
                                 set<int> * dont_compress=NULL)
		{
			...
			{ 
                ...
                else
                {
					if (iMaxLoops != 1)
					{
						printf("Can't be that you provide set<int> * dont_compress and iMaxLoops is not 1.\n");
						abort();
					}
					
					for(int i=0; i<vBlocksToFWT.size(); i++)
						if (vReports[i].getOverAll_DetailMaxMag() < dAbsoluteTolerance && dont_compress->find(vBlocksToFWT[i].blockID) == dont_compress->end())
