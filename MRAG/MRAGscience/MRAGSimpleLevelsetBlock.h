...
template <typename DataType, int cNarrowBand = 4, int cSizeX = 16,
          int cSizeY = 1, int cSizeZ = 1>
struct SimpleLevelsetBlock {
  ...
  inline DataType &operator()(int ix, int iy = 0, int iz = 0) {
    if (cDoDebugChecks) {
      ...
      assert((ElementType *)&data[iz][iy][ix] ==
             (ElementType *)data + cSizeX * cSizeY * iz + cSizeX * iy + ix);
    }
...
private:
  inline Real _eta(Real phi, const int maxPhi) const {
    //
    ...
    if (phimapped < gamma) {
      if (phimapped < (gamma - 1.0)) {
        ...
        eta = pow((double)phimapped - gamma, 2.0) *
              (2.0 * phimapped + gamma - 3.0 * beta);
      }
    } else {
      eta = 0.0;
    }
