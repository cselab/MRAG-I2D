...
public:
  Refiner_SpaceExtension(int nMaxLevelJump = 1, int nMaxLevel = -1)
      : Refiner(nMaxLevelJump), m_nMaxLevel(nMaxLevel) {}
  ...
  virtual RefinementPlan *createPlan(const HierarchyType &hierarchy,
    ...
    // 1.
    for (NeighborhoodType::const_iterator itS = neighborhood.begin();
