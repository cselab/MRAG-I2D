...
public:
  virtual bool getBlocks(int &level, double &timeStep, double &currentTime,
                         vector<BlockInfo> &blocksInfo, ETimeInterval &type);
};

bool SpaceTimeSorterRK3::getBlocks(int &level, double &timeStep,
                                   double &currentTime,
  ...
  if (type == ETimeInterval_Start) {
    ...
    m_stackRecursions.push(
        StackItem(l, t + 0.5 * dt, ETimeInterval_Intermediate2));
