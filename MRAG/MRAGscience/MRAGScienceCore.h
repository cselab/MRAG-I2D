 ...
 * @param fwt                   MRAG::BlockFWT instance for the wavelet
 * transforms.
 * @param dAbsoluteTolerance    Tolerance in terms of "energy" for the
 * refinement.
...
int AutomaticRefinement(Grid &g, BlockFWT &fwt, const double dAbsoluteTolerance,
  ...
  do {
    ...
    if (dont_refine == NULL) {
      ...
      if (iMaxLoops != 1) {
        printf("Can't be that you provide set<int> * dont_refine and iMaxLoops "
               "is not 1.\n");
        abort();
      }
      ...
      for (int i = 0; i < vBlocksToFWT.size(); i++)
        if (vReports[i].getOverAll_DetailMaxMag() >= dAbsoluteTolerance &&
            dont_refine->find(vBlocksToFWT[i].blockID) == dont_refine->end())
          shouldBeRefined.insert(vBlocksToFWT[i].blockID); // i was right
        else
          niceGuys.insert(vBlocksToFWT[i].blockID); // i was wrong
    }
  ...
  printf("AutoRef::refine: the grid has %zu BLOCKS.\n",
         g.getBlocksInfo().size());
 ...
 * Works just like MRAG::Science::AutomaticRefinement() but compressing
 * (coarsening) the grid instead of refining it. Also the arguments are
 * essentially the same besides:
 * - dAbsoluteTolerance is now the tolerance in terms of "energy" for the
 * compression.
...
int AutomaticCompression(Grid &g, BlockFWT &fwt,
  ...
  do {
    ...
    if (dont_compress == NULL) {
      ...
      if (iMaxLoops != 1) {
        printf("Can't be that you provide set<int> * dont_compress and "
               "iMaxLoops is not 1.\n");
        abort();
      }
      ...
      for (int i = 0; i < vBlocksToFWT.size(); i++)
        if (vReports[i].getOverAll_DetailMaxMag() < dAbsoluteTolerance &&
            dont_compress->find(vBlocksToFWT[i].blockID) ==
                dont_compress->end())
          shouldBeCompressed.insert(vBlocksToFWT[i].blockID); // i was right
        else
          niceGuys.insert(vBlocksToFWT[i].blockID); // i was wrong
    }
...
RefinementResult AutomaticRefinementForLevelsets(
  ...
  printf("AutomaticRefinementForLevelsets\n");
  ...
  do {
    ...
    for (vector<BlockInfo>::iterator it = vBlocksToFWT.begin();
      ...
      fwt.template multichannel_fwt<0, 0>(*it);
      // printf("getOverAll_DetailMaxMag()=%e >= dAbsoluteTolerance=%e ----->
      // %s\n", fwt.getReport().getOverAll_DetailMaxMag(),
      // dAbsoluteTolerance,fwt.getReport().getOverAll_DetailMaxMag() >=
      // dAbsoluteTolerance ? "to refine!": "skept");
      if (fwt.getReport().getOverAll_DetailMaxMag() >= dAbsoluteTolerance)
    ...
    printf("AutomaticRefinement:: Refined %lu, FWT skipped blocks:%d\n",
           shouldBeRefined.size(), nSkippedBlocks);
