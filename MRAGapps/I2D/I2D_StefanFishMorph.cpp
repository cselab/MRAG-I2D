...
I2D_StefanFishMorph::StefanFishMorph::StefanFishMorph(
    double xm, double ym, double _D, double _T, double phase, double tau,
    double angle_rad, vector<double> WIDTH, vector<double> BASELINE,
    vector<double> CURVATURE, double angleInSpace_rad, double eps,
    const int LMAX, const bool isSharp)
    : StefanFish(xm, ym, _D, _T, phase, tau, angle_rad, BASELINE, CURVATURE,
  ...
  if (this->WIDTH[this->WIDTH.size() - 1] < dx) {
    printf("WIDTH[1] < dx!\n");
    abort();
  }
  ...
  printf("LMAX=%d\n", LMAX);
  printf("WIDTH\n");
  for (unsigned int i = 0; i < this->WIDTH.size(); i++) {
    printf("%f, ", this->WIDTH[i]);
  }
  printf("BASELINE\n");
  ...
  for (int i = 0; i < N; i++) {
    ...
    W[i] = _getWidth(S[i]);
    if (W[i] < 0.0) {
      printf("X-CROSSING IN THE WIDTH!\n");
      abort();
    }
  }
}

double I2D_StefanFishMorph::StefanFishMorph::_coxDeBoorRecursion(
    const int j, const int d, const double *t, const double u) const {
...
double I2D_StefanFishMorph::StefanFishMorph::_getWidth(const double &ss) const {
  ...
  if ((s >= 0.0) && (s <= end)) {
    ...
    width = yr[idx] +
            (yr[idxPlus] - yr[idx]) * (s - xr[idx]) / (xr[idxPlus] - xr[idx]);
  }

  return width;
}

I2D_StefanFishMorph::I2D_StefanFishMorph(
    ...
    const Real angle, vector<double> WIDTH, vector<double> BASELINE,
    ...
    const int ID)
    : I2D_StefanFish(parser, grid, _xm, _ym, D, T, phase, tau, angle, BASELINE,
                     CURVATURE, angleInSpace, eps, Uinf, penalization, LMAX,
                     ID) {
  assert(shape != NULL);
  if (shape != NULL) {
    delete shape;
  ...
  shape =
      new StefanFishMorph(_xm, _ym, D, T, phase, tau, angle, WIDTH, BASELINE,
