...
bool I2D_StefanFish::StefanFish::_checkCurvatureIsTheSame(
    double const *newControlK, double const *controlK, unsigned int n) {
  double sum = 0.0;
...
void I2D_StefanFish::StefanFish::_applyTravellingWave(
    const double t, const double tau, const double tauDerivative,
    const double T, const double TDerivative, const double *s,
    const double *currentCurvature, const double *currentBaseline,
    const double *derivativeCurvature, const double *derivativeBaseline,
    double *xt, double *xt_dt, const unsigned int nn, bool boolWaveStarted) {
  const double offset = 0.0; //-(EXTENSION/2.0)*EPS;
  const double ll = 1.0;
  if (boolWaveStarted) {
    for (unsigned int i = 0; i < nn; i++) {
      const double argument =
          2.0 * M_PI * (t / T - (s[i] + offset) / ll * tau) + phase;
      const double argumentDerived = 2.0 * M_PI *
                                     ((T - t * TDerivative) / (T * T) -
                                      (s[i] + offset) / ll * mTauDerivative);
      // const double argumentDerived = 2.0*M_PI*( 1.0/T -
      // (s[i]+offset)/ll*mTauDerivative );
      xt[i] = currentBaseline[i] + currentCurvature[i] * sin(argument);
      xt_dt[i] = derivativeBaseline[i] +
                 derivativeCurvature[i] * sin(argument) +
                 currentCurvature[i] * cos(argument) * argumentDerived;
    }
  } else {
    for (unsigned int i = 0; i < nn; i++) {
...
void I2D_StefanFish::StefanFish::_interpolateCurvature(
    const double *x, const double *y, const unsigned int n, const double *xx,
    double *yy, const unsigned int nn) {
  I2D_Interpolation1D iterpolator;
  const double offset = 0.0; //-(EXTENSION/2.0)*EPS;
  iterpolator.naturalCubicSpline(x, y, n, xx, yy, nn, offset);
}

void I2D_StefanFish::StefanFish::_schedulerCurvature(
    double t, double timeTransition, double *curvature, int kdim, double *s,
    int N, double *current_K_profile, double *current_dKdt_profile) {
  // Curvature checks
  bool curvaturesStartEndEqual =
      _checkCurvatureIsTheSame(mCurvatureStart, mCurvatureEnd, kdim);
  bool curvaturesEndCurrentEqual =
      _checkCurvatureIsTheSame(mCurvatureEnd, curvature, kdim);
  ...
  if (curvaturesStartEndEqual && curvaturesEndCurrentEqual) {
    ...
    _interpolateCurvature(mControlPoints, mCurrentCurvature, kdim, s,
                          current_K_profile, N);
  ...
  if (curvaturesStartEndEqual && !curvaturesEndCurrentEqual) {
    ...
    for (int i = 0; i < kdim; i++) {
      ...
      double dummy = 0.0;
      _cubicInterpolation(mCurvatureTimeStart, mCurvatureTimeEnd, t,
                          mCurvatureStart[i], mCurvatureEnd[i],
                          mCurrentCurvature[i], dummy);
    }

    // Compute curvature & curvature derivative profile
    double curvStartLong[N];
    double curvEndLong[N];
    _interpolateCurvature(mControlPoints, mCurvatureStart, kdim, s,
                          curvStartLong, N);
    _interpolateCurvature(mControlPoints, mCurvatureEnd, kdim, s, curvEndLong,
                          N);
    for (int i = 0; i < N; i++)
      _cubicInterpolation(mCurvatureTimeStart, mCurvatureTimeEnd, t,
                          curvStartLong[i], curvEndLong[i],
                          current_K_profile[i], current_dKdt_profile[i]);
  ...
  if (!curvaturesStartEndEqual && !curvaturesEndCurrentEqual) {
    ...
    for (int i = 0; i < kdim; i++) {
      ...
      double dummy = 0.0;
      _cubicInterpolation(
          mCurvatureTimeStart, mCurvatureTimeEnd, t, mCurvatureStart[i],
          mCurvatureEnd[i], mCurrentCurvature[i],
          dummy); // HERE CUBIC INTERPOLATION WITH SLOPE (to be implemented)!!!
    }
    ...
    for (int i = 0; i < N; i++)
      _cubicInterpolation(
          mCurvatureTimeStart, mCurvatureTimeEnd, t, curvStartLong[i],
          curvEndLong[i], current_K_profile[i],
          current_dKdt_profile[i]); // HERE CUBIC INTERPOLATION WITH SLOPE (to
                                    // be implemented)!!!
...
void I2D_StefanFish::StefanFish::_schedulerBaseline(
    double t, double timeTransition, double *baseline, int kdim, double *s,
    int N, double *current_B_profile, double *current_dBdt_profile) {
  // Curvature checks
  bool baselinesStartEndEqual =
      _checkCurvatureIsTheSame(mBaselineStart, mBaselineEnd, kdim);
  bool baselinesEndCurrentEqual =
      _checkCurvatureIsTheSame(mBaselineEnd, baseline, kdim);
  ...
  if (baselinesStartEndEqual && baselinesEndCurrentEqual) {
    ...
    // Compute curvature & curvature derivative profile
    _interpolateCurvature(mControlPoints, mCurrentBaseline, kdim, s,
                          current_B_profile, N);
  ...
  if (baselinesStartEndEqual && !baselinesEndCurrentEqual) {
    ...
    for (int i = 0; i < kdim; i++) {
      ...
      double dummy = 0.0;
      _cubicInterpolation(mBaselineTimeStart, mBaselineTimeEnd, t,
                          mBaselineStart[i], mBaselineEnd[i],
                          mCurrentBaseline[i], dummy);
    }

    // Compute curvature & curvature derivative profile
    double baselineStartLong[N];
    double baselineEndLong[N];
    _interpolateCurvature(mControlPoints, mBaselineStart, kdim, s,
                          baselineStartLong, N);
    _interpolateCurvature(mControlPoints, mBaselineEnd, kdim, s,
                          baselineEndLong, N);
    for (int i = 0; i < N; i++)
      _cubicInterpolation(mBaselineTimeStart, mBaselineTimeEnd, t,
                          baselineStartLong[i], baselineEndLong[i],
                          current_B_profile[i], current_dBdt_profile[i]);
...
void I2D_StefanFish::StefanFish::_schedulerT(double t, double timeTransition,
                                             double _T, double &current_T,
                                             double &current_T_derivative) {
  // Tau checks
  ...
  if (TStartEndEqual && !TEndCurrentEqual) {
    ...
    mTEnd = _T;
    double dummy = 0.0;
    _cubicInterpolation(mTTimeStart, mTTimeEnd, t, mTStart, mTEnd, mTCurrent,
                        mTDerivative);
...
void I2D_StefanFish::StefanFish::_schedulerTau(double t, double timeTransition,
                                               double tau, double &current_tau,
                                               double &current_tau_derivative) {
  // Tau checks
  ...
  if (tauStartEndEqual && !tauEndCurrentEqual) {
    ...
    mTauEnd = tau;
    double dummy = 0.0;
    _cubicInterpolation(mTauTimeStart, mTauTimeEnd, t, mTauStart, mTauEnd,
                        mTauCurrent, mTauDerivative);
...
void I2D_StefanFish::StefanFish::_solveFrenetInHeadRefFrame(
    const double ds, const double *k, double *rX, double *rY, double *tanX,
    double *tanY, double *norX, double *norY, const unsigned int n) {
  I2D_Frenet2D frenetSolver;
  frenetSolver.solve(ds, k, rX, rY, tanX, tanY, norX, norY, n);
}

void I2D_StefanFish::StefanFish::_solveVelProfileInHeadRefFrame(
    const double ds, const double *kt, const double *kt_dt, const double *tanX,
    const double *tanY, const double *norX, const double *norY, double *vX,
    double *vY, double *vTanX, double *vTanY, double *vNorX, double *vNorY,
    const unsigned int n) {
  I2D_Frenet2D frenetSolver;
  frenetSolver.solveVelProfile(ds, kt, kt_dt, tanX, tanY, norX, norY, vX, vY,
                               vTanX, vTanY, vNorX, vNorY, n);
}
...
void I2D_StefanFish::StefanFish::updateInSpace(const double t) {
  ...
  _schedulerBaseline(t, mTransitionTime, baseline, 6, S, N,
                     mBaselineInterpolated, mBaselineDerivative);
  _schedulerCurvature(t, mTransitionTime, curvature, 6, S, N, K,
                      mCurvatureDerivative);
  ...
  double wavetime = t;
  _startWave(t);
  if (mBoolWaveStarted == true) {
    wavetime = t - mTimeStartWave;
  }
  _applyTravellingWave(wavetime, current_tau, current_tau_derivative, current_T,
                       current_T_derivative, S, K, mBaselineInterpolated,
                       mCurvatureDerivative, mBaselineDerivative, KT, DKDT, N,
                       mBoolWaveStarted);
  ...
  _solveVelProfileInHeadRefFrame(DS, KT, DKDT, TANX, TANY, NORX, NORY, VX, VY,
                                 VTANX, VTANY, VNORX, VNORY, N);
...
I2D_StefanFish::StefanFish::StefanFish(
    double xm, double ym, double _D, double _T, double phase, double tau,
    double angle_rad, vector<double> BASELINE, vector<double> CURVATURE,
    double angleInSpace_rad, double eps, const int LMAX, const bool isSharp)
    : tau(tau), mTransitionTime(_T),
      Fish(xm, ym, _D, _T, phase, angle_rad, angleInSpace_rad, eps, LMAX,
           isSharp) {
  ...
  for (unsigned int i = 0; i < this->BASELINE.size(); i++) {
    printf("%f, ", this->BASELINE[i]);
  }
  printf("\nCURVATURE\n");
  for (unsigned int i = 0; i < this->CURVATURE.size(); i++) {
    printf("%f, ", this->CURVATURE[i]);
  }
  printf("\n--------------SUMMARY STEFANFISH--------------\n");

  if (this->BASELINE.size() != this->CURVATURE.size() ||
      this->BASELINE.size() != 6) {
    printf("Something wrong with BASELINE or CURVATURE!\n");
    abort();
  }
...
I2D_StefanFish::I2D_StefanFish(
    ArgumentParser &parser, Grid<W, B> &grid, const Real _xm, const Real _ym,
    const Real D, const Real T, const Real phase, const Real tau,
    const Real angle, vector<double> BASELINE, vector<double> CURVATURE,
    const Real angleInSpace, const Real eps, const Real Uinf[2],
    I2D_PenalizationOperator &penalization, const int LMAX, const int ID,
    RL::RL_TabularPolicy **policy, const int seed)
    : I2D_CarlingFish(parser, grid, _xm, _ym, D, T, phase, angle, angleInSpace,
                      eps, Uinf, penalization, LMAX, ID, policy, seed) {
  assert(shape != NULL);
  if (shape != NULL) {
    delete shape;
    shape = NULL;
  ...
  shape = new StefanFish(_xm, _ym, D, T, phase, tau, angle, BASELINE, CURVATURE,
                         angleInSpace, eps, LMAX, isSharp);
