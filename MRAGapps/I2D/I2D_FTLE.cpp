...
static const int maxParticleStencil[2][3] = {-4, -4, 0, +5, +5, +1};
...
struct FTLE {
  ...
  FTLE(double T, map<long int, vector<Real>> &particles,
       map<I3, unsigned int> &code)
      : T(T), particles(particles), code(code) {}
  ...
  inline Real _computeFTLE(const Real J[2][2], const Real T) const {
    ...
    const Real t29 = t9 + 0.2e1 * t4 * t2 + 0.2e1 * t6 * t2 - 0.2e1 * t2 * t8 +
                     t16 - 0.2e1 * t4 * t6 + 0.2e1 * t8 * t4 + t21 +
                     0.2e1 * t6 * t8 + t24 + 0.8e1 * t5 * t1 * t7 * t3;
    const Real t29bis = (t29 >= 0) ? t29 : 0.0;
    const Real t30 = sqrt(t29bis);
    const Real lambda_max =
        t2 / 0.2e1 + t4 / 0.2e1 + t6 / 0.2e1 + t8 / 0.2e1 + t30 / 0.2e1;
...
I2D_FTLE::I2D_FTLE(const int argc, const char **argv)
    : parser(argc, argv), t_completed(-1.0) {
  printf("////////////////////////////////////////////////////////////\n");
  printf("////////////            FTLE         ///////////////\n");
  printf("////////////////////////////////////////////////////////////\n");
...
void I2D_FTLE::_mapping(map<string, double> &mapping) {
  ...
  // for(map< string, double >::iterator it=mapping.begin(); it!=mapping.end();
...
void I2D_FTLE::_createDtsSet(const map<string, double> &subset,
                             vector<Real> &dts) {
  dts.clear();
  unsigned int counter = 0;
  for (map<string, double>::const_iterator it = subset.begin();
       it != subset.end(); ++it) {
...
void I2D_FTLE::_createParticleSet(string restart,
                                  map<long int, vector<Real>> &particles,
                                  map<I3, unsigned int> &code) {
  particles.clear();
  code.clear();
...
void I2D_FTLE::_extractSubset(double tStart, double tEnd,
                              const map<string, double> &mapping,
                              map<string, double> &subset) {
  subset.clear();
  for (map<string, double>::const_iterator it = mapping.begin();
       it != mapping.end(); ++it)
    if (it->second >= tStart && it->second < tEnd)
      subset[it->first] = it->second;
}

void I2D_FTLE::_FTLE(string initialField, double T,
                     map<long int, vector<Real>> &particles, vector<Real> &dts,
                     map<I3, unsigned int> &code) {
...
double I2D_FTLE::_advectParticles(map<long int, vector<Real>> &particles,
                                  vector<Real> &dts,
                                  map<string, double> &subset) {
  unsigned int counter = 0;
  double T = 0.0;
  for (map<string, double>::const_iterator it = subset.begin();
    ...
    for (map<long int, vector<Real>>::const_iterator it = particles.begin();
      ...
      if (p[0] >= 0.0 && p[0] < 1.0 && p[1] >= 0.0 && p[1] < 1.0) {
        ...
        Real startBlock[2] = {
            myblock.i[0] * dx,
            myblock.i[1] * dx}; /// actual location of the start of the block
        Real endBlock[2] = {
            startBlock[0] + dx,
            startBlock[1] +
                dx}; /// actual location of the end limits of the block
        const bool isInBlock = (p[0] >= startBlock[0] && p[0] < endBlock[0] &&
                                p[1] >= startBlock[1] && p[1] < endBlock[1]);
        if (!isInBlock) {
          printf("p[0]=%e, p[1]=%e\n", p[0], p[1]);
...
void I2D_FTLE::_computeFwdFTLE(double tStart, double tEnd, int idx,
