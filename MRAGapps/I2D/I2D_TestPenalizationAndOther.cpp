...
I2D_TestPenalizationAndOther::I2D_TestPenalizationAndOther(const int argc,
                                                           const char **argv)
  ...
  Real pos[2] = {0.45, 0.5};
  obstacle = new I2D_CircularObstacleOperator(
      *grid, 1 / 16., pos,
      sqrt(2.) * 2. / FluidBlock2D::sizeX * pow(0.5, parser("-lmax").asInt()));
  obstacle->characteristic_function();
  //

  //_dump("fadeout");

  if (parser("-dumpfreq").asInt() > 0)
    _dump("ic_penalization");
...
set<int> I2D_TestPenalizationAndOther::_getBoundaryBlockIDs() {
  ...
  for (vector<BlockInfo>::iterator it = vInfo.begin(); it != vInfo.end();
       it++) {
    const bool bX =
        it->index[0] == 0; // || it->index[0] == pow(2, it->level)-1;
    const bool bY =
        false; // it->index[1] == 0 || it->index[1] == pow(2, it->level)-1;
...
void I2D_TestPenalizationAndOther::_ic_omega(Grid<W, B> &grid) {
  ...
  I2D_CircularObstacleOperator obstacle(
      grid, 1 / 16., pos, sqrt(2.) * 2. / FluidBlock2D::sizeX * pow(0.5, 5));
  obstacle.characteristic_function();
  // exit(0);
  ...
  /*
   ...
   pow(p[2]-0.35, 2)); const bool bInside =  radius < 0.15;

   b(ix, iy, iz).omega[0] = bInside;
   b(ix, iy, iz).omega[1] = 1;//sqrt(pow(p[0]-0.25, 2) + pow(p[1]-0.25, 2) +
   pow(p[2]-0.25, 2))<0.1; b(ix, iy, iz).omega[2] = 0;//p[2];
   }
   }*/
}
