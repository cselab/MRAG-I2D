...
protected:
  ...
  //			|
  //			*x2
  bool _isRight(const double x1[2], const double dir[2],
                const double x2[2]) const;

  int _discretizeRange(const double value, const double minvalue,
                       const double maxvalue, const int levels);

public:
  I2D_FloatingObstacleOperator(ArgumentParser &parser, Grid<W, B> &grid,
                               const Real D, const Real eps, const Real Uinf[2],
                               I2D_PenalizationOperator &penalization,
                               const int _ID = 0,
                               RL::RL_TabularPolicy **_policy = NULL,
                               const int seed = 0)
      : grid(grid), eps(eps), D(D), Cd(0.0), dimT(0.0),
        penalization(penalization), ID(_ID), policy(_policy), policystore(NULL),
  ...
  // Hydrodynamics
  virtual void computeDragAndStuff(const Real time, const Real charLength = 1.0,
                                   const Real charVel = 1.0);
  virtual void
  update(const double dt, const double t, string filename = std::string(),
         map<string, vector<I2D_FloatingObstacleOperator *>> *_data = NULL) = 0;
  ...
  map<int, const VelocityBlock *> getDesiredVelocity() {
  ...
  virtual bool
  choose(const double t,
         map<string, vector<I2D_FloatingObstacleOperator *>> *_data = NULL);
  virtual void
  stopTest(const double t,
           map<string, vector<I2D_FloatingObstacleOperator *>> *_data = NULL);
  virtual void
  learn(const double t,
        map<string, vector<I2D_FloatingObstacleOperator *>> *_data = NULL,
        string name = string());
  virtual void
  reward(const double t,
         map<string, vector<I2D_FloatingObstacleOperator *>> *_data = NULL) {};
  virtual void mapAction(int action) {};
  virtual bool
  mapState(vector<int> &state,
           map<string, vector<I2D_FloatingObstacleOperator *>> *_data = NULL) {
    return true;
  };
