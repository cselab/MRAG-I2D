...
I2D_TestPoissonEquationPotential::I2D_TestPoissonEquationPotential(
...
Real I2D_TestPoissonEquationPotential::_mollified_heaviside(
  ...
  for (int i = 0; i < vInfo.size(); i++) {
    ...
    for (int iy = 0; iy < B::sizeY; iy++)
      for (int ix = 0; ix < B::sizeX; ix++) {
        ...
        const Real error =
            fabs(PoissonEquationPotIC::_fill(x, y) - b(ix, iy).tmp);
        if (error > Linf) {
          Linf = error;
          xerr = p[0];
          yerr = p[1];
