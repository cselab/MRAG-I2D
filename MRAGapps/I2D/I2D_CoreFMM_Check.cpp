...
void I2D_CoreFMM_Check::solve(const Real theta, const Real inv_scaling,
  ...
  /*gpu_solver.solve(theta, inv_scaling, dest, nblocks, srcparticles,
  nparticles);
  ...
  max_rel_error_gpu = l1_rel_error_gpu = max_rel_error_sse = l1_rel_error_sse =
      0.0;
    ...
    for (int y = 0; y < _BLOCKSIZE_; ++y) {
      for (int x = 0; x < _BLOCKSIZE_; ++x) {
        if (std::abs(b.u[0][y][x]) > std::numeric_limits<double>::epsilon()) {
          tmp = std::abs(gpu_result[i].u[0][y][x] - b.u[0][y][x]) /
                std::abs(b.u[0][y][x]);
        ...
        if (std::abs(b.u[1][y][x]) > std::numeric_limits<double>::epsilon()) {
          tmp = std::abs(gpu_result[i].u[1][y][x] - b.u[1][y][x]) /
                std::abs(b.u[1][y][x]);
          max_rel_error_gpu = std::max(max_rel_error_gpu, tmp);
          l1_rel_error_gpu += tmp;
          counter_gpu += 1;
        ...
        if (std::abs(b.u[0][y][x]) > std::numeric_limits<double>::epsilon()) {
          tmp = std::abs(sse_result[i].u[0][y][x] - b.u[0][y][x]) /
                std::abs(b.u[0][y][x]);
        ...
        if (std::abs(b.u[1][y][x]) > std::numeric_limits<double>::epsilon()) {
          tmp = std::abs(sse_result[i].u[1][y][x] - b.u[1][y][x]) /
                std::abs(b.u[1][y][x]);
          max_rel_error_sse = std::max(max_rel_error_sse, tmp);
          l1_rel_error_sse += tmp;
          counter_sse += 1;
  ...
  std::cout << "\n\n//////////////// Error Analysis //////////////////\n\n"
               "rel max error sse: "
            << setprecision(4) << scientific << max_rel_error_sse
            << "\trel l1 error sse: " << l1_rel_error_sse
            << "\n\n"
               "//////////////////////////////////////////////////\n\n";
