...
I2D_StefanFishSmartLattice::I2D_StefanFishSmartLattice(
    ...
    const Real D, const Real _T, const Real tau, const Real angle,
    const Real _dir[2], const Real eps, const Real Uinf[2],
    I2D_PenalizationOperator &penalization, const int LMAX, const int ID,
    RL::RL_TabularPolicy **_policy, const int seed)
    : T(_T), I2D_StefanFishSmart(parser, grid, _xm, _ym, D, _T, tau, angle, eps,
                                 Uinf, penalization, LMAX, ID, _policy, seed) {
  scale = shape->D;
  cutoff = scale;
...
bool I2D_StefanFishSmartLattice::mapState(
    vector<int> &state,
  ...
  for (vector<I2D_FloatingObstacleOperator *>::iterator it = agents.begin();
       it != agents.end(); ++it) {
    I2D_StefanFishSmartLattice *b =
        static_cast<I2D_StefanFishSmartLattice *>(*it);
    totVx += b->shape->vx;
    totVy += b->shape->vy;
  }
  ...
  bool valid = true;
  {
    const double dist = sqrt((myX[0] - center[0]) * (myX[0] - center[0]) +
                             (myX[1] - center[1]) * (myX[1] - center[1]));
...
void I2D_StefanFishSmartLattice::reward(
    const double t,
    map<string, vector<I2D_FloatingObstacleOperator *>> *_data) {
...
void I2D_StefanFishSmartLattice::update(
  ...
  fprintf(ppFile,
          ...
          shape->angular_velocity, target[0], target[1]);
  fflush(ppFile);
  fclose(ppFile);
