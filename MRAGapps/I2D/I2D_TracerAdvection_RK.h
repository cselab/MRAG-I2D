...
struct I2D_TracerAdvection_RK {
  ...
  // @param dt
  // @param Uinf
  I2D_TracerAdvection_RK(map<long int, vector<Real>> &particles,
                         map<I3, vector<long int>> &block2particles, Real dt,
                         Real Uinf[2])
      : t(0), particles(particles), block2particles(block2particles), dt(dt) {
  ...
  // Copy constructor
  // @param c
  I2D_TracerAdvection_RK(const I2D_TracerAdvection_RK &c)
      : t(0), particles(c.particles), block2particles(c.block2particles),
        dt(c.dt) {
    stencil_start[0] = stencil_start[1] = -4;
    stencil_end[0] = stencil_end[1] = +5;
    stencil_start[2] = 0;
  ...
  inline void operator()(Lab &lab, const BlockInfo &info,
    ...
    const Real startBlock[2] = {
        index[0] * dx,
        index[1] * dx}; /// actual location of the start of the block
    const Real endBlock[2] = {
        startBlock[0] + dx,
        startBlock[1] + dx}; /// actual location of the end limits of the block
    ...
    for (unsigned int j = 0; j < currentParticles.size(); j++) {
      ...
      assert(x >= startBlock[0] && x < endBlock[0] && y >= startBlock[1] &&
             y < endBlock[1]);
      ...
      for (int i = 0; i < nSubSteps; i++) {
        ...
        const Real xp[2] = {
            (xNext - ori[0]) / h,
            (yNext - ori[1]) /
                h}; /// x/h of the anchor point relative to the block
        ...
        const Real vx = Uinf[0] + lab.pcore.template _sample<1>(weights, start,
                                                                end, iap, lab);
        const Real vy = Uinf[1] + lab.pcore.template _sample<2>(weights, start,
                                                                end, iap, lab);
