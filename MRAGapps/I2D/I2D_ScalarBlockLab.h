...
template <typename Streamer> struct I2D_ScalarBlockLab {
public:
  template <typename BlockType> class Lab {
  protected:
  ...
  public:
    Lab()
        : m_state(Uninitialized), m_weightPool(NULL), m_weightPoolSize(0),
          m_valuePool(NULL), m_valuePoolSize(0), m_sourceData(NULL),
          row_size(0), base_offset(0), m_refCollection(NULL),
          m_refBoundaryInfo(NULL) {
    ...
    template <typename Processing> void inspect(Processing &p) {
    ...
    void load(const BlockInfo &info) {
      ...
      assert(boundaryInfo.boundaryInfoOfBlock.find(info.blockID) !=
             boundaryInfo.boundaryInfoOfBlock.end());
      ...
      // 3.
      {
        for (int icode2D = 0; icode2D < 9; icode2D++) {
          ...
          const int s[3] = {
              ...
              code[1] < 1 ? (code[1] < 0 ? m_stencilStart[1] : 0) : nY, 0};
          ...
          assert(bbinfo.boundary[icode2D + 9 * 1].nGhosts ==
            ...
            for (int iy = s[1]; iy < e[1]; iy++)
              for (int ix = s[0]; ix < e[0]; ix++, currentghost++) {
                ...
                for (vector<IndexWP>::const_iterator it = start; it != end;
                     it++)
                  w += (m_weightPool[it->weights_index[0]] *
                ...
                for (vector<IndexWP>::const_iterator it = start; it != end;
                     it++)
                  ghost += m_valuePool[it->point_index] *
                           (m_weightPool[it->weights_index[0]] *
                            m_weightPool[it->weights_index[1]]);
  ...
  private:
    // forbidden
    Lab(const Lab &)
        : m_state(Uninitialized), m_sourceData(NULL), m_weightPool(NULL),
          m_weightPoolSize(0), m_valuePool(NULL), m_valuePoolSize(0) {
      abort();
    }
