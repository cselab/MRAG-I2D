...
static const int maxParticleStencil[2][3] = {-3, -3, 0, +4, +4, +1};

I2D_FlowPastFixedObstacle::I2D_FlowPastFixedObstacle(const int argc,
                                                     const char **argv)
    : parser(argc, argv), t(0), step_id(0), velsolver(NULL), obstacle(NULL),
      penalization(NULL), advection(NULL), diffusion(NULL) {
  printf("////////////////////////////////////////////////////////////\n");
  printf("////////////            AVE MARIA         ///////////////\n");
  printf("////////////////////////////////////////////////////////////\n");
  ...
  // Note by Mattia:	All flow past cases MUST have a defined Uinf. Here the
  // case of Uinf=0 is allowed only in the constructor. 					The reason of this is
  //that the constructor of I2D_FlowPastFixedObstacle and
  //I2D_FlowPastFloatingObstacle is 					used also in the more general
  //I2D_FluidMediatedInteractions, which may have Uinf=0.
  //  					Therefore I put a check in the
  //  I2D_FlowPastFixedObstacle::run() and I2D_FlowPastFloatingObstacle::run()
  //  to make sure
  //					that at the moment of running Uinf!=0.
  //					As here Uinf could be zero, in order to
  //set a viscosity!=0 and avoid the assert in the diffusion operator, 					I just
  //define the characteristic velocity as below, therefore if Uinf!=0 ----> nu =
  //charVel*D/RE, otherwise nu = D/RE
  ...
  if (sOBSTACLE == "cyl") {
    ...
    obstacle =
        new I2D_CircularObstacleOperator(*grid, D / 2, pos, epsilon, isSharp);
  } else if (sOBSTACLE == "plate")
    obstacle =
        new I2D_RectangularObstacleOperator(*grid, 1 / 90., pos, D, epsilon);
    ...
    obstacle = new I2D_EllipticalObstacleOperator(*grid, D / 2.0, aspectRatio,
                                                  angle, pos, epsilon);
  }
...
set<int> I2D_FlowPastFixedObstacle::_getBoundaryBlockIDs() {
  ...
  if (sRIGID_INLET_TYPE == "rigid_frame") // RIGID AT EACH BOUNDARY
    for (vector<BlockInfo>::iterator it = vInfo.begin(); it != vInfo.end();
         it++) {
      const bool bX =
          it->index[0] == 0 || it->index[0] == pow(2.0, it->level) - 1;
      const bool bY =
          it->index[1] == 0 || it->index[1] == pow(2.0, it->level) - 1;

      if (bX || bY)
        result.insert(it->blockID);
    }
  ...
  else if (sRIGID_INLET_TYPE ==
           "free_outlet_only") // RIGID AT EACH BOUNDARY EXCEPT FOR THE OUTLET
    for (vector<BlockInfo>::iterator it = vInfo.begin(); it != vInfo.end();
         it++) {
      const bool bX = it->index[0] == 0;
      const bool bY =
    ...
    printf("I2D_FlowPastFixedObstacle::_getBoundaryBlockIDs: the chosen "
           "sRIGID_INLET_TYPE (=%s) is not supported. Aborting\n",
...
void I2D_FlowPastFixedObstacle::_tnext(double &tnext, double &tnext_dump,
                                       double &tend) {
  // 0. setup
  ...
  // 2.
  {
    ...
    printf("####################### t is %e, dt is %e, %s,\t{", t, tnext - t,
           dtBound.c_str());
    ...
    fprintf(f, "####################### t is %e, dt is %e and is bound by: %s",
            t, tnext - t, dtBound.c_str());
    fprintf(f, "stepid=%d\tT=%e\tDT=%e\t", (int)step_id, tnext * nondim_factor,
            (tnext - t) * nondim_factor);
    for (int i = 0; i < (int)tnext_candidates.size(); i++)
      fprintf(f, "%s: %2.2e,\t", tnext_names[i].c_str(),
              tnext_candidates[i] - t);
  ...
  while (true) {
    ...
    for (int i = 0; i < ADAPTFREQ; i++) {
      ...
      if (infoObstacle.size() != 0) {
        cor[0] = infoObstacle[0];
        cor[1] = infoObstacle[1];
      }
...

      {
        ...
        if (f != NULL) {
          if (step_id == 0)
            fprintf(f,
                    "stepid\twall-clock[s]\tT[-]\tblocks\tADV-rhs\tDIFF-rhs\n");

          fprintf(f, "%d\t%e\t%e\t%d\t%d\t%d\n", (int)step_id, wallclock, Tcurr,
                  nblocks, advection->get_nofrhs(), diffusion->get_nofrhs());
