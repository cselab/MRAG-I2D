...
I2D_ObjectFactory::I2D_ObjectFactory(Grid<W, B> &grid, const Real charLength,
                                     const Real XCM, const Real YCM,
                                     const Real eps, const Real Uinf[2],
                                     I2D_PenalizationOperator &penalization,
                                     const int _LMAX)
    : grid(grid), eps(eps), charLength(charLength), XCM(XCM), YCM(YCM),
      penalization(penalization), LMAX(_LMAX) {
  this->Uinf[0] = Uinf[0];
  this->Uinf[1] = Uinf[1];
}
...
void I2D_ObjectFactory::create(
    ArgumentParser &parser,
    map<string, vector<I2D_FloatingObstacleOperator *>> &shapesMap, bool smart,
  ...
  if (LR > 0.0 && LR <= 1.0 && GAMMA >= 0.0 && GAMMA <= 1.0 &&
      GREEDYEPS >= 0.0 && GREEDYEPS <= 1.0 && (LR + GAMMA) > 1.0) {
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (name == "I2D_FloatingCylinder") {
      ...
      if (d <= 0.0 || d > charLength) {
        printf("Wrong object dimension\n");
        abort();
      }
      ...
      I2D_FloatingCylinder *object = new I2D_FloatingCylinder(
          parser, grid, xm, ym, d, eps, Uinf, penalization);

      map<string, vector<I2D_FloatingObstacleOperator *>>::iterator it =
          shapesMap.find(name);
      ...
      I2D_PassiveTracer *object = new I2D_PassiveTracer(
          parser, grid, xm, ym, 0.0, eps, Uinf, penalization);
      ...
      I2D_FloatingEllipse *object = new I2D_FloatingEllipse(
          parser, grid, xm, ym, d, aspectRatio, eps, Uinf, penalization);
      ...
      I2D_ImposedCylinder *object = new I2D_ImposedCylinder(
          parser, grid, xm, ym, vx, vy, d, eps, Uinf, penalization);
    ...
    } else if (name == "I2D_ImposedCylinder_DiamArray") // diamond array
    {
      ...
      const Real ym = YCM + variable * charLength;
      fscanf(ppFile, " N=%f",
             &variable); // number of objects to create, must be square
      const Real Na = variable;
      if (sqrt(Na) - int(sqrt(Na)) != 0) {
      ...
      int s = int(sqrt(Na) - 1); // spacing number array center - outmost
                                 // cylinder, characteristic number
      if (xm - s * b < d / 2.0 || xm + s * b > 1.0 - d / 2.0) {
        printf("Wrong array dimension\n");
        abort();
      }
      if (ym - s * h < d / 2.0 || ym + s * b > 1.0 - d / 2.0) {
        printf("Wrong array dimension\n");
        abort();
      }
      ...
      for (int cx = -s; cx <= s; cx++)
        for (int cy = -(s - abs(cx)); cy <= s - abs(cx); cy += 2) {
          I2D_ImposedCylinder *object =
              new I2D_ImposedCylinder(parser, grid, xm + cx * b, ym + cy * h, 0,
                                      0, d, eps, Uinf, penalization);
          shapesMap[name].push_back(object);
        }
    } else if (name == "I2D_ImposedCylinder_RectArray") // rectangle array
    {
      ...
      // ensure correct array dimensions
      float s = (sqrt(Na) - 1.0) / 2.0; // spacing number array center - outmost
                                        // cylinder, characteristic number
      if (xm - s * b < d / 2.0 || xm + s * b > 1.0 - d / 2.0) {
        printf("Wrong array dimension\n");
        abort();
      ...
      fscanf(ppFile, " d1=%f",
             &variable); // maximum camber as percentage of the chord
      const int d1 = int(variable);
      if (d1 < 0 || d1 > 9) {
        printf("Wrong maximum camber\n");
        abort();
      }
      fscanf(ppFile, " d2=%f",
             &variable); // distance of maximum camber from leading edge in tens
                         // of percent's from the chord
      const int d2 = int(variable);
      if (d2 < 0 || d2 > 9) {
        printf("Wrong distance of maximum camber\n");
        abort();
      }
      fscanf(ppFile, " d3d4=%f", &variable); // maximum thickness of the airfoil
                                             // as percentage of the chord
      const int d3d4 = int(variable);
      if (d3d4 < 0 || d3d4 > 99) {
        printf("Wrong maximum thickness\n");
        abort();
      }
      ...
      const Real ym = YCM + variable * charLength;
      fscanf(ppFile, " angle=%f",
             &variable); // orientation of NACA profile, [ °]
      ...
      I2D_ImposedAirfoil *object =
          new I2D_ImposedAirfoil(grid, parser, xm, ym, d, angle, vx, vy, d1, d2,
                                 d3d4, eps, Uinf, penalization);
      ...
      fscanf(ppFile, " ym=%f", &variable); // physical y-coordinate leading edge
      const Real ym = YCM + variable * charLength;
      fscanf(ppFile, " l=%f", &variable); // distance in x between leading edge
                                          // airfoil and center of rotation
      const Real l = variable * charLength;
      if (l < 0.0 || l > d) {
        printf("Wrong center of rotation\n");
        abort();
      }
      ...
      fscanf(ppFile, " piAmp=%f",
             &variable); // pitch Amplitude, [rad], rotation: [-piAmp, piAmp]
      ...
      I2D_PitchingAirfoil *object = new I2D_PitchingAirfoil(
          grid, parser, xm, ym, d, l, d1, d2, d3d4, plAmp, plPer, plPhase,
          piAmp, piPer, piPhase, eps, Uinf, penalization);
    ...
    } else if (name == "I2D_ImposedAirfoil_DiamArray") // diamond array
    {
      ...
      const Real b = variable * charLength;
      if (b < d / 2.0) {
        printf("Wrong horizontal spacing\n");
        abort();
      }
      fscanf(ppFile, " h=%f", &variable); // vertical spacing between objects
      const Real h = variable * charLength;
      if (h < d3d4 / 100.0 * d) {
        printf("Wrong vertical spacing\n");
        abort();
      }

      // ensure correct array dimensions
      int s = int(sqrt(Na) - 1); // spacing number array center - outmost
                                 // airfoil, characteristic number
      if (xm - s * b < 0.0 || xm + s * b > 1.0 - d) {
        printf("Wrong array dimension\n");
        abort();
      }
      if (ym - s * h < d3d4 / 100.0 * d ||
          ym + s * b > 1.0 - d3d4 / 100.0 * d) {
        printf("Wrong array dimension\n");
        abort();
      }
      ...
      for (int cx = -s; cx <= s; cx++)
        for (int cy = -(s - abs(cx)); cy <= s - abs(cx); cy += 2) {
          I2D_ImposedAirfoil *object = new I2D_ImposedAirfoil(
              grid, parser, xm + cx * b, ym + cy * h, d, angle, 0.0, 0.0, d1,
              d2, d3d4, eps, Uinf, penalization);
          shapesMap[name].push_back(object);
        }
      ...
      I2D_ImposedEllipse *object =
          new I2D_ImposedEllipse(parser, grid, xm, ym, xm, ym, vx, vy, d,
                                 aspectRatio, eps, Uinf, penalization);
      ...
      I2D_ImposedEllipseRotation *object = new I2D_ImposedEllipseRotation(
          parser, grid, xm, ym, xcrot, ycrot, T, d, aspectRatio, eps, Uinf,
          penalization);
      ...
      const Real piPhase = variable;
      I2D_ImposedEllipseOscillation *object = new I2D_ImposedEllipseOscillation(
          parser, grid, xm, ym, xm, ym, aspectRatio, plAmp, plPer, piAmp, piPer,
          plPhase, piPhase, d, eps, Uinf, penalization);
      ...
      I2D_ImposedCylinderRotation *object =
          new I2D_ImposedCylinderRotation(parser, grid, xm, ym, radiusRot, T, d,
                                          align, eps, Uinf, penalization);
      ...
      I2D_ImposedCylinderOscillation *object =
          new I2D_ImposedCylinderOscillation(parser, grid, xm, ym, A, T,
                                             direction, d, eps, Uinf,
                                             penalization);
      ...
      assert(tstop > 0.0);
      I2D_ImposedCylinderStopAndGo *object = new I2D_ImposedCylinderStopAndGo(
          parser, grid, xm, ym, vx, vy, tstop, d, eps, Uinf, penalization);
      ...
      I2D_ThreeLinkFish *object = new I2D_ThreeLinkFish(
          parser, grid, xm, ym, d, aspectRatio, eps, Uinf, penalization);
      ...
      I2D_CarlingFish *object =
          new I2D_CarlingFish(parser, grid, xm, ym, d, T, phase, angle,
                              angleInSpace, eps, Uinf, penalization, LMAX, i);
      ...
      I2D_CarlingFishAirfoil *object = new I2D_CarlingFishAirfoil(
          parser, grid, naca, xm, ym, d, T, phase, angle, angleInSpace, eps,
          Uinf, penalization, LMAX, i);
      ...
      I2D_StefanFish *object = new I2D_StefanFish(
          parser, grid, xm, ym, d, T, phase, tau, angle, BASELINE, CURVATURE,
          angleInSpace, eps, Uinf, penalization, LMAX, i);
      ...
      I2D_CStartLarva *object = new I2D_CStartLarva(
          parser, grid, xm, ym, d, Tprep, Tprop, phase, tau, angle, BASELINE,
          CURVATURE, angleInSpace, eps, Uinf, penalization, LMAX, i);
      ...
      I2D_CarlingFishMorph *object = new I2D_CarlingFishMorph(
          parser, grid, xm, ym, d, T, phase, angle, WIDTH, angleInSpace, eps,
          Uinf, penalization, LMAX, i);
      ...
      I2D_StefanFishMorph *object = new I2D_StefanFishMorph(
          parser, grid, xm, ym, d, T, phase, tau, angle, WIDTH, BASELINE,
          CURVATURE, angleInSpace, eps, Uinf, penalization, LMAX, i);
      ...
      I2D_StefanFishSmart *object =
          new I2D_StefanFishSmart(parser, grid, xm, ym, d, T, tau, angle, eps,
                                  Uinf, penalization, LMAX, i, policy, rand());
      ...
      dir[1] = variable;
      I2D_StefanFishSmartInline *object = new I2D_StefanFishSmartInline(
          parser, grid, xm, ym, d, T, tau, angle, dir, eps, Uinf, penalization,
          LMAX, i, policy, rand());
      ...
      fscanf(ppFile, " diry=%f", &variable);
      dir[1] = variable;
      I2D_StefanFishSmartLattice *object = new I2D_StefanFishSmartLattice(
          parser, grid, xm, ym, d, T, tau, angle, dir, eps, Uinf, penalization,
          LMAX, i, policy, rand());
      ...
      fscanf(ppFile, " T=%f", &variable);
      const Real T = variable;
      I2D_StefanFishSmartSpiral *object = new I2D_StefanFishSmartSpiral(
          parser, grid, xm, ym, d, T, tau, angle, eps, Uinf, penalization, LMAX,
          i, policy, rand());
      ...
      I2D_FloatingRotatingCylinderPair *object =
          new I2D_FloatingRotatingCylinderPair(
              parser, grid, xm, ym, d, angle, width, g1, g2, eps, Uinf,
              penalization, LMAX, i, policy, rand());
      ...
      printf("Case not defined!! One of your shapes' name in the factory file "
             "is wrong!\n");
      abort();
    }
  }
