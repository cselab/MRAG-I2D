...
struct GetOmega {
  static inline Real stream(FluidElement2D &out) { return out.omega; }
};
...
void I2D_VelocitySolver_Mani::_count_sourceparticles() {
  ...
  ThresholdParticles<GetOmega, 0> countparticles(tolParticle, scaling_factor,
                                                 blockid2info);
  block_processing.process(vInfo, coll, countparticles);

  int curr = 0;
  for (map<int, SourceParticlesInfo>::iterator it = blockid2info.begin();
       it != blockid2info.end(); ++it) {
...
void I2D_VelocitySolver_Mani::_collect_sourceparticles() {
  ...
  ThresholdParticles<GetOmega, 1> collectparticles(tolParticle, scaling_factor,
                                                   blockid2info);
...
void I2D_VelocitySolver_Mani::_compute() {
  if (!bSKIPBLOCKS || nsource_particles == 0)
    ...
    printf("FMM-SKIP ACTIVE. : %zu instead of %zu (%f)\n", vDest.size(),
           vInfo.size(), vDest.size() / (double)vInfo.size());
  }
  ...
  coreFMM->solve(theta, 1.0 / scaling_factor, &vDest.front(), vDest.size(),
                 srcparticles, nsource_particles);
...
void I2D_VelocitySolver_Mani::_updateBlocks() {
  ...
  tbb::parallel_for(blocked_range<int>(0, vDest.size()), update,
                    auto_partitioner());
