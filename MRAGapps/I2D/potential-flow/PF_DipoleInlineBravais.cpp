...
DipoleInlineBravais::DipoleInlineBravais(double a1, double a2, double phi,
                                         double theta, int nDesired)
...
void PF::DipoleInlineBravais::computeRadius() {
  ...
  if (nActual < nDesired) {
    printf("** At this size, swimmers cannot be fit in the (0,1) x (0,1) "
           "domain.\n");
    abort();
  }
  ...
  bool isDone = false;
  while (~isDone) {
    if (nActual == nDesired || iter > maxIter)
      break;
...
double PF::DipoleInlineBravais::computeCoordinates(double rAssign,
                                                   bool isFillingVectors) {
  r = rAssign;
  ...
  if (isFillingVectors)
    printf("** Number in averaging window = %d\n", countTrue);
