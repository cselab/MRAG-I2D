 ...
 * @param alpha_
 */
SwarmCylinders::SwarmCylinders(int n_, double radius_, double rmin_, double A_,
                               double K1_, double K2_, double alpha_)
    : n(n_), radius(radius_), rmin(rmin_), A(A_), K1(K1_), K2(K2_),
      alpha(alpha_), spline(A_, K1_, K2_, alpha_), dt(1e-3), threshold(5e-6),
      factorCylinder(0.001), factorBoundary(0.05) {
 ...
 * @param alpha
 */
SwarmCylinders::Spline::Spline(double desiredArea, double k1, double k2,
                               double alpha) {
  ...
  double r =
      sqrt(0.35e2) *
      sqrt(0.1e1 /
           ((double)((9 * k1 - 13 * k2 * k2 - 9 * k1 * k2 + 13) * alpha) +
            0.13e2 * 0.3141592654e1 * (double)(k1 * k1) +
            0.13e2 * 0.3141592654e1 * (double)(k2 * k2) +
            0.9e1 * 0.3141592654e1 * (double)k1 * (double)k2) *
           desiredArea);
  double r1 = r * k1;
  double r2 = r * k2;
  ...
  m_a1 = 0.3141592654e1 * 0.3141592654e1 * (0.3141592654e1 - 0.3e1 * alpha) /
             (0.3141592654e1 - alpha) /
             (0.3141592654e1 * 0.3141592654e1 - 0.2e1 * 0.3141592654e1 * alpha +
              alpha * alpha) *
             r * k1 +
         alpha * alpha * (0.3e1 * 0.3141592654e1 - alpha) /
             (0.3141592654e1 - alpha) /
             (0.3141592654e1 * 0.3141592654e1 - 0.2e1 * 0.3141592654e1 * alpha +
              alpha * alpha) *
             r * k2;
  m_b1 = 0.6e1 * 0.3141592654e1 * alpha / (0.3141592654e1 - alpha) /
             (0.3141592654e1 * 0.3141592654e1 - 0.2e1 * 0.3141592654e1 * alpha +
              alpha * alpha) *
             r * k1 -
         0.6e1 * 0.3141592654e1 * alpha / (0.3141592654e1 - alpha) /
             (0.3141592654e1 * 0.3141592654e1 - 0.2e1 * 0.3141592654e1 * alpha +
              alpha * alpha) *
             r * k2;
  m_c1 = -0.3e1 * (0.3141592654e1 + alpha) / (0.3141592654e1 - alpha) /
             (0.3141592654e1 * 0.3141592654e1 - 0.2e1 * 0.3141592654e1 * alpha +
              alpha * alpha) *
             r * k1 +
         0.3e1 * (0.3141592654e1 + alpha) / (0.3141592654e1 - alpha) /
             (0.3141592654e1 * 0.3141592654e1 - 0.2e1 * 0.3141592654e1 * alpha +
              alpha * alpha) *
             r * k2;
  m_d1 = 0.2e1 / (0.3141592654e1 - alpha) /
             (0.3141592654e1 * 0.3141592654e1 - 0.2e1 * 0.3141592654e1 * alpha +
              alpha * alpha) *
             r * k1 -
         0.2e1 / (0.3141592654e1 - alpha) /
             (0.3141592654e1 * 0.3141592654e1 - 0.2e1 * 0.3141592654e1 * alpha +
              alpha * alpha) *
             r * k2;
...
void SwarmCylinders::findEquilibrium() {
  ...
  for (int i = 0; i < M; i++) {
    m.push_back(spline.evaluate(i * dalpha) *
                dalpha); /// assuming constant density charge on boundary
  }
  ...
  while (!steadyState) {
    ...
    for (int i = 0; i < xsize; i++) {
      ...
      {
        if (i != j) {
          double norm = sqrt((x[i] - x[j]) * (x[i] - x[j]) +
                             (y[i] - y[j]) * (y[i] - y[j]));
      ...
      {
        double norm = sqrt((x[i] - xs[j]) * (x[i] - xs[j]) +
                           (y[i] - ys[j]) * (y[i] - ys[j]));
    ...
    for (int i = 0; i < xsize; i++) {
      ...
      //			xNew[i] = min(spline.evaluate(yNew[i])-radius,
      //xNew[i]); /// make it sit on the boundary if it is not repelled enough
      //(not exactly correct)
    }
    ...
    for (int i = 0; i < xsize; i++) {
      double norm =
          sqrt((xNew[i] - x[i]) * (xNew[i] - x[i]) +
               (yNew[i] - y[i]) * (yNew[i] - y[i])); /// relative difference
    ...
    if (nIter > 50000) {
      printf("** Something is wrong, swarm shape is not converging quickly "
             "enough... aborting... \n");
      abort();
    }
  }
...
void SwarmCylinders::checkValidityOfSpline() const {
  ...
  const double ymin =
      0.10; // if 0, prevents you from creating splines that cross the y-axis
  ...
  for (int i = 0; i < nSpline; i++) {
    if (yTest[i] < ymin) {
      printf("** The spline crosses or is too close to the the mirror plane! "
             "(ymin = %0.4f, k1 = %0.4f k2 = %0.4f, alpha = %0.4f)\n",
             ymin, K1, K2, alpha);
      abort();
    }
  }
