...
protected:
  enum { learning, averaging } solverState;
  ...
  bool SMOOTH;
  Real LEARNINGTIME; /// amount of time given to all agents to initially learn a
                     /// 'good' policy
  Real FITNESSTIME;  /// amount of time given to average fitness function over
  Real FITNESSBUFFER; /// take (FINTESSBUFFER - 1)% longer to get rid of
                      /// transient after a refresh
  ...
  void _checkStatusPostAction(double time, double dt, bool valid,
  ...
  void _paintSquareCanvas(double edgeLength, double bottomLeftCorner[2],
                          double bgColor[3], bool isOutlinedBlack);
  void _paintBox(Real edgeLength, Real bottomLeftCorner[2], Real bgColor[3]);
  void _paintText(float x, float y, const char *text, float r, float g, float b,
                  float a);
