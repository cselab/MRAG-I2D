...
PF_DipoleEight::PF_DipoleEight(MRAG::ArgumentParser &parser, const Real _D,
                               const Real _V, const Real _radius,
                               const Real _acc,
                               const complex<Real> _locationCenter,
                               const Real _dir[2], const Real lr,
                               const Real greedyEps, bool isSmooth,
                               bool isControlled, const Real T, const int _ID,
                               RL::RL_TabularPolicy **_policy, const int seed)
    : PF_Agent(parser, lr, greedyEps, isSmooth, isControlled, "PF_DipoleEight",
               _ID, _policy, seed),
 ...
 * @param collection
 */
void PF_DipoleEight::update(const Real dt, const Real time,
 ...
 * @param collection
 */
void PF_DipoleEight::updatePostVelocityUpdate(
  ...
  boxLength = 0.35;
  {
    ...
    if (abs(targetPoint[0] - boxCenter) > 0.5 * boxLength ||
        abs(targetPoint[1] - boxCenter) > 0.5 * boxLength)
      isInBox = false;
    else
      isInBox = true;
  }
 ...
 * @param positionAgent
 */
void PF_DipoleEight::storePosition(
 ...
 * @param targetsAgent
 */
void PF_DipoleEight::getTargets(
...
bool PF_DipoleEight::mapState(vector<int> &state,
                              map<string, vector<PF_Agent *>> *collection) {
  ...
  if (ISCONTROLLED)
    if (_isOutsideDomain(myX) ||
        IdI > (20 * cutoff)) /// || far far away from the followed point
      valid = false;
 ...
 * @param collection
 */
void PF_DipoleEight::reward(const Real time,
                            map<string, vector<PF_Agent *>> *collection) {
...
void PF_DipoleEight::paint() {
  ...
  //	_paintCone(x, y, 5.0 * D, alpha, 0.0, 0.741, 0.965); // swimmer size is
...
void PF_DipoleEight::paint(Real scale, const Real center[2]) {
  ...
  //	_paintCone(x, y, 5.0 * scale * D, alpha, 0.0, 0.741, 0.965); // swimmer
  //size is actually approx 5 times the separating distince D, see constructor
}
...
void PF_DipoleEight::saveData(Real time) {
  ...
  if (out) {
    ...
    out << time << " " << real(locationCenter) << " " << imag(locationCenter)
        ...
        << " " << velDipole << " " << targetPoint[0] << " " << targetPoint[1]
        << endl;
    out.close();
  } else
...
void PF::PF_DipoleEight::fitness() {
  fitnessValue = (abs(gammaLeft + gammaRight) + abs(gammaLeft) +
                  abs(gammaRight) - 2 * gamma) /
                 gamma;
}
 ...
 * @param targetPoints
 */
void PF::PF_DipoleEight::getTargetPoints(
 ...
 */
void PF_DipoleEight::resetToInitialCondition() { setupInitialCondition(); }
