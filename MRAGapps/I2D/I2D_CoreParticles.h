...
public:
...
#endif
  void inline _computeWeights(const Real xp[2], const Real ap[2],
                              Real(weights[2])[4]) {
    for (int c = 0; c < 2; c++) {
      Real t[4] = {fabs(xp[c] - (ap[c] + -1)), fabs(xp[c] - (ap[c] + +0)),
                   fabs(xp[c] - (ap[c] + +1)), fabs(xp[c] - (ap[c] + +2))};
...
#ifndef NDEBUG
      assert(_is_close(
          weights[c][0] + weights[c][1] + weights[c][2] + weights[c][3], 1));
  ...
  Real inline _sample(const Real(w[2])[4], const int start[2], const int end[2],
                      const int iap[2], LabVel &lab) {
    static const int KS = -1;
    static const int KE = +3;
    ...
    for (int sy = start[1]; sy < end[1]; sy++) {
      ...
      for (int sx = start[0]; sx < end[0]; sx++)
        sample += wy * w[0][sx - KS] *
                  lab.template get<component>(iap[0] + sx, iap[1] + sy);
    }
  ...
  void inline _scatter(const Real(w[2])[4], const int start[2],
                       const int end[2], const int iap[2], Real value) {
    static const int KS = -1;
    static const int KE = +3;
  ...
  void push(const BlockInfo &info, LabVel &lab, double dt, const Real Uinf[3]) {
    ...
    for (int iy = PSY; iy < PEY; iy++)
      for (int ix = PSX; ix < PEX; ix++) {
        Real xp[2] = {ix + factor1 * (Uinf[0] + lab.template get<1>(ix, iy)),
                      iy + factor1 * (Uinf[1] + lab.template get<2>(ix, iy))};

        Real ap[2] = {floor(xp[0]), floor(xp[1])};

        int iap[2] = {(int)ap[0], (int)ap[1]};
        ...
        final_xp[0] = ix + factor2 * (Uinf[0] + _sample<1>(weights, start, end,
                                                           iap, lab));
        final_xp[1] = iy + factor2 * (Uinf[1] + _sample<2>(weights, start, end,
                                                           iap, lab));
      }
  }

  template <typename LabVel> void remesh(LabVel &lab) {
    static const int KS = -1;
    static const int KE = +3;
    ...
    for (int iy = PSY; iy < PEY; iy++)
      for (int ix = PSX; ix < PEX; ix++) {
        ...
        const int start[2] = {max(KS, 0 - iap[0]), max(KS, 0 - iap[1])};

        const int end[2] = {min(KE, B::sizeX - iap[0]),
