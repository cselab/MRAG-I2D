...
echo "Submission 0..."
bsub -J ${PROGNAME} -n ${NTHREADS} -R span[ptile=${NTHREADS}] -W ${WCLOCK} -o out time ./executable ${OPTIONS}
...
do
echo "Submission $c..."		
	bsub -J ${PROGNAME} -n ${NTHREADS} -R span[ptile=${NTHREADS}] -w "ended(${PROGNAME})" -W ${WCLOCK} -o out time ./executable ${OPTIONS}
done
fi
