...
Real I2D_CircularObstacleOperator::_get_smooth_radius(const Real R,
                                                      const Real eps) {
  const double D = 2 * R;
...
struct FillBlocks {
  ...
  FillBlocks(Real support_radius, Real smooth_radius, Real smoothing_length,
             Real sphere_position[3])
    ...
    if (_is_touching(info)) {
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
          ...
          b(ix, iy).tmp = I2D_CircularObstacleOperator::mollified_heaviside(
              r - smooth_radius, smoothing_length);
        }
    } else {
...
Real I2D_CircularObstacleOperator::mollified_heaviside(const Real x,
                                                       const Real eps) {
...
void I2D_CircularObstacleOperator::characteristic_function()
{
        ...
        FillBlocks fill(smooth_radius+smoothing_length, smooth_radius,
smoothing_length, position); block_processing.process(vInfo, coll, fill);
}
*/
...
double sample(const double x, const double y, const double h,
              const double position[2], const double radius) {
  const int stencil = 6;
  double temp[stencil];
  ...
  for (int idx = -1; idx < 2; idx++) {
    ...
    const Real dist =
        sqrt(pow(my_x - position[0], 2) + pow(my_y - position[1], 2)) - radius;
...
struct FillBlocksTowers {
  ...
  FillBlocksTowers(Real support_radius, Real smooth_radius,
                   Real smoothing_length, Real sphere_position[2])
      : support_radius(support_radius), smooth_radius(smooth_radius),
        smoothing_length(smoothing_length) {
    this->sphere_position[0] = sphere_position[0];
    this->sphere_position[1] = sphere_position[1];
    ...
    if (_is_touching(info)) {
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
          ...
          // b(ix, iy).tmp =
          // I2D_CircularObstacleOperator::mollified_heaviside(r-smooth_radius,
          // smoothing_length);

          b(ix, iy).tmp =
              sample(p[0], p[1], info.h[0], sphere_position, smooth_radius);
        }
...
void I2D_CircularObstacleOperator::characteristic_function() {
  ...
  if (!SHARP) {
    FillBlocks fill(smooth_radius + smoothing_length, smooth_radius,
                    smoothing_length, position);
    block_processing.process(vInfo, coll, fill);
  } else {
    FillBlocksTowers fill(smooth_radius + smoothing_length, smooth_radius,
                          smoothing_length, position);
    block_processing.process(vInfo, coll, fill);
  }
}
