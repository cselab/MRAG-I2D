...
protected:
  template <typename streamer, int stage> struct ThresholdParticles {
    const Real tolParticle;
    const Real scaling_factor;
    VelocitySourceParticle *destptr;
    map<int, SourceParticlesInfo> &blockid2info;

    ThresholdParticles(Real _tolParticle, Real _scaling_factor,
                       map<int, SourceParticlesInfo> &blockid2info)
        : tolParticle(_tolParticle), scaling_factor(_scaling_factor),
          blockid2info(blockid2info), destptr(NULL) {}
    ThresholdParticles(Real _tolParticle, Real _scaling_factor,
                       const ThresholdParticles &c)
        : tolParticle(_tolParticle), scaling_factor(_scaling_factor),
          blockid2info(c.blockid2info), destptr(c.destptr) {}

    inline void operator()(const BlockInfo &info, FluidBlock2D &b) const {

      map<int, SourceParticlesInfo>::iterator it =
          blockid2info.find(info.blockID);
  ...
  I2D_VelocitySolver_Mani(const int argc, const char **argv)
      : srcparticles(NULL), nsource_particles(0), my_velBlocks(NULL),
        grid_ptr(NULL) {
  ...
  void set_grid_ptr(Grid<W, B> *ptr) { grid_ptr = ptr; }
