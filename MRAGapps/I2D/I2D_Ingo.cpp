    ...
    if (_is_touching(info)) {
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
          ...
          b(ix, iy).tmp = max(I2D_Ingo::mollified_heaviside(-dist, m_epsilon),
                              b(ix, iy).tmp);
        }
    }
  }
    ...
    if (_is_touching(info)) {
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
          ...
          b(ix, iy).tmp =
              max(I2D_Ingo::mollified_heaviside(r - radius, epsilon),
                  b(ix, iy).tmp);
        }
    }
  }
...
Real I2D_Ingo::mollified_heaviside(const Real x, const Real eps) {
  const Real alpha = M_PI * min(1., max(0., (x + 0.5 * eps) / eps));

  return 0.5 + 0.5 * cos(alpha);
}

void I2D_Ingo::characteristic_function() {
  ...
  if ((cm[1] - CM[1]) > length) {
    bound = anglesPlusOne.size();
  } else {
    // addition of an extra plate (Ingo's constraint)
    bound = anglesPlusOne.size() + 1;
    lastAngle = asin((cm[1] - CM[1]) / length);
    anglesPlusOne.push_back(lastAngle);
  }
