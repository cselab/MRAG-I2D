...
I2D_FloatingEllipse::Ellipse::Ellipse()
    : angle(0), D(0.1), xm(0.5), ym(0.5), aspectRatio(2), angular_velocity(0),
...
I2D_FloatingEllipse::Ellipse::Ellipse(Real xm, Real ym, Real D, Real angle_rad,
                                      Real aspectRatio)
    : angle(angle_rad), D(D), xm(xm), ym(ym), aspectRatio(aspectRatio),
      angular_velocity(0), vx(0), vy(0), vdefx(0), vdefy(0), rho(1.0) {
  m = rho * M_PI * (D / 2.0) * (D / 2.0);
  J = 0.25 * M_PI * (D / 2.0) * (D / 2.0) * aspectRatio *
      ((D / 2.0) * (D / 2.0) * (1 + (aspectRatio * aspectRatio)));
}

Real I2D_FloatingEllipse::Ellipse::_mollified_heaviside(
...
/*
const I2D_FloatingEllipse::Ellipse::Ellipse&
I2D_FloatingEllipse::Ellipse::operator=(const Ellipse& c)
{
...
Real I2D_FloatingEllipse::Ellipse::sample(const Real x_, const Real y_,
...
void I2D_FloatingEllipse::Ellipse::bbox(const Real eps, Real xmin[2],
                                        Real xmax[2]) const {
...
struct FillBlocks {
  ...
  FillBlocks(Real eps, I2D_FloatingEllipse::Ellipse *_shape) : eps(eps) {
    shape = _shape;
  }

  FillBlocks(const FillBlocks &c) : eps(c.eps) { shape = c.shape; }

  static bool _is_touching(Real eps, const I2D_FloatingEllipse::Ellipse *wheel,
...
struct ComputeAll {
  ...
  ComputeAll(double eps, I2D_FloatingEllipse::Ellipse *_shape,
...
struct FillVelblocks {
  ...
  FillVelblocks(vector<pair<BlockInfo, VelocityBlock *>> &workitems, double eps,
                I2D_FloatingEllipse::Ellipse *_shape)
...
I2D_FloatingEllipse::I2D_FloatingEllipse(ArgumentParser &parser,
                                         Grid<W, B> &grid, const Real _xm,
                                         const Real _ym, const Real D,
                                         const Real aspectRatio, const Real eps,
                                         const Real Uinf[2],
...
void I2D_FloatingEllipse::update(
