...
I2D_ImposedCylinderOscillation::I2D_ImposedCylinderOscillation(
    ...
    const Real _A, const Real _T, const string _direction, const Real _D,
    ...
    : I2D_ImposedCylinder(parser, grid, _xm, _ym, 0.0, 0.0, _D, eps, Uinf,
                          penalization),
      XCM(_xm), YCM(_ym), A(_A), T(_T), direction(_direction) {}
...
void I2D_ImposedCylinderOscillation::update(
  ...
  if (filename == std::string()) {
    ...
    ppFile = fopen("update_I2D_ImposedCylinderOscillation.txt",
                   t == 0.0 ? "w" : "a");
    assert(ppFile != NULL);
  } else {
