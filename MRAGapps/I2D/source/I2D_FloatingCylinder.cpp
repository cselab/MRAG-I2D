...
I2D_FloatingCylinder::Cylinder::Cylinder()
    : angle(0), D(0.1), xm(0.5), ym(0.5), angular_velocity(0), vx(0), vy(0),
      vdefx(0), vdefy(0), rho(1.0) {
  m = rho * M_PI * (D / 2.0) * (D / 2.0);
  J = 0.5 * m * (D / 2.0) * (D / 2.0);
}

I2D_FloatingCylinder::Cylinder::Cylinder(Real xm, Real ym, Real D,
                                         Real angle_rad)
...
Real I2D_FloatingCylinder::Cylinder::_mollified_heaviside(
...
Real I2D_FloatingCylinder::Cylinder::sample(const Real x_, const Real y_,
                                            const Real eps) const {
  const double dist =
      sqrt((x_ - xm) * (x_ - xm) + (y_ - ym) * (y_ - ym)) - D / 2.0;
  return _mollified_heaviside(dist, eps);
}

void I2D_FloatingCylinder::Cylinder::bbox(const Real eps, Real xmin[2],
                                          Real xmax[2]) const {
  assert(eps >= 0);
...
struct FillBlocks {
  ...
  FillBlocks(Real eps, I2D_FloatingCylinder::Cylinder *_shape) : eps(eps) {
    shape = _shape;
  }
  ...
  static bool _is_touching(Real eps,
                           const I2D_FloatingCylinder::Cylinder *wheel,
...
struct ComputeAll {
  ...
  ComputeAll(double eps, I2D_FloatingCylinder::Cylinder *_shape,
...
struct FillVelblocks {
  ...
  FillVelblocks(vector<pair<BlockInfo, VelocityBlock *>> &workitems, double eps,
                I2D_FloatingCylinder::Cylinder *_shape)
...
I2D_FloatingCylinder::I2D_FloatingCylinder(
    ...
    const Real D, const Real eps, const Real Uinf[2],
...
void I2D_FloatingCylinder::computeDesiredVelocity(const double t) {
  ...
  // Compute all
  FloatingCylinder::ComputeAll computeAll(eps, shape, integrals, Uinf,
                                          nonempty);
...
void I2D_FloatingCylinder::update(
