...
I2D_CarlingFishMorph::CarlingFishMorph::CarlingFishMorph(double xm, double ym, double _D, double _T, double phase, double angle_rad, vector<double> WIDTH, double angleInSpace_rad, double eps, const int LMAX):
Fish(xm,ym,_D,_T,phase,angle_rad,angleInSpace_rad,eps,LMAX)
{
	...
	const double dx = (1.0/(double)(FluidBlock2D::sizeX*pow(2.0,LMAX)))*scalingdx;
...
double I2D_CarlingFishMorph::CarlingFishMorph::_coxDeBoorRecursion(const int j, const int d, const double * t, const double u) const
{
...
double I2D_CarlingFishMorph::CarlingFishMorph::_getWidth(const double & ss) const
{
...
I2D_CarlingFishMorph::I2D_CarlingFishMorph(ArgumentParser & parser, Grid<W,B>& grid, const Real _xm, const Real _ym, const Real D, const Real T, const Real phase, const Real angle, vector<double> WIDTH, const Real angleInSpace, const Real eps, const Real Uinf[2], I2D_PenalizationOperator& penalization, const int LMAX, const int ID):
										I2D_CarlingFish(parser, grid, _xm, _ym, D, T, phase, angle, angleInSpace, eps, Uinf, penalization, LMAX, ID)
{
	assert(shape!=NULL);
	if(shape!=NULL)
	{
	...
	shape = new CarlingFishMorph(_xm, _ym, D, T, phase, angle, WIDTH, angleInSpace, eps, LMAX);
