...
struct FloatVelocityBlock {
  ...
  void initialize(const float xstart, const float ystart,
                  const float h_gridpoint) {
    // clear u[0] and u[1]
    {
...
public:
  DirectInfo()
      : xs(NULL), ys(NULL), ws(NULL), num_sources(-1), is_overlapping(false) {}

  DirectInfo(const float *const _xs, const float *const _ys,
             const float *const _ws, int _num_sources, bool _is_overlapping)
      : xs(_xs), ys(_ys), ws(_ws), num_sources(_num_sources),
        is_overlapping(_is_overlapping) {}

  DirectInfo(const DirectInfo &other)
      : xs(other.xs), ys(other.ys), ws(other.ws),
        num_sources(other.num_sources), is_overlapping(other.is_overlapping) {}
...
public:
  ...
  IndirectInfo(float _xbox, float _ybox, const float *const _real_values,
               const float *const _imag_values)
      : xbox(_xbox), ybox(_ybox), real_values(_real_values),
        imag_values(_imag_values) {}

  IndirectInfo(const IndirectInfo &other)
      : xbox(other.xbox), ybox(other.ybox), real_values(other.real_values),
...
public:
  ...
  TargetInfo(float _xstart, float _ystart, float _h)
      : xstart(_xstart), ystart(_ystart), h(_h) {}

  float xstart, ystart, h;
};

typedef std::vector<DirectInfo, tbb::scalable_allocator<DirectInfo>>
    DirectInfoVector;
typedef std::vector<IndirectInfo, tbb::scalable_allocator<IndirectInfo>>
    IndirectInfoVector;
typedef std::vector<std::pair<int, int>,
                    tbb::scalable_allocator<std::pair<int, int>>>
    IntervalVector;
typedef std::vector<IntervalVector, tbb::scalable_allocator<IntervalVector>>
    IntervalVectors;
typedef std::vector<
    std::vector<bool, tbb::scalable_allocator<bool>>,
    tbb::scalable_allocator<std::vector<bool, tbb::scalable_allocator<bool>>>>
    OverlappingVectors;
typedef std::list<std::pair<int, int>,
                  tbb::scalable_allocator<std::pair<int, int>>>
    IntervalList;
...
template <typename T> class IntervalComparisonByStartPoint {
public:
  bool operator()(const std::pair<T, T> &interval_a,
                  const std::pair<T, T> &interval_b) const {
...
public:
  TimeInfo()
      : direct_reduction_time(0), indirect_reduction_time(0),
        direct_total_gflop(0), indirect_total_gflop(0), direct_avg_gflops(0),
        indirect_avg_gflops(0), direct_kernel_calls(0),
        indirect_kernel_calls(0) {}
  ...
  TimeInfo(double _direct_reduction_time, double _indirect_reduction_time,
           ...
           unsigned int _direct_kernel_calls,
           unsigned int _indirect_kernel_calls)
      : direct_reduction_time(_direct_reduction_time),
        indirect_reduction_time(_indirect_reduction_time),
        direct_total_gflop(_direct_total_gflop),
        indirect_total_gflop(_indirect_total_gflop),
        direct_avg_gflops(_direct_avg_gflops),
        indirect_avg_gflops(_indirect_avg_gflops),
        direct_kernel_calls(_direct_kernel_calls),
        indirect_kernel_calls(_indirect_kernel_calls) {}
...
public:
  SSE_Plan(int _num_target_blocks, BlockInfo *_target_blocks,
           int num_source_particles, tBox *root_node, bool _b_verbose)
      : m_num_target_blocks(_num_target_blocks),
        m_target_blocks(_target_blocks), m_plan_per_block(_num_target_blocks),
        m_num_source_particles(num_source_particles),
        m_direct_overlapping_intervals(_num_target_blocks),
        m_direct_non_overlapping_intervals(_num_target_blocks),
        m_root_source_particles(&root_node->vparticles[0]),
        m_indirect_interactions(_num_target_blocks), m_direct_buffer_size(0),
        m_indirect_buffer_size(0), b_verbose(_b_verbose), m_xs(NULL),
        m_ys(NULL), m_ws(NULL), m_real_values(NULL), m_imag_values(NULL) {}
  ...
  void merge_direct_intervals(int target_block) {
    ...
    for (int i = 0; i <= 1; ++i) {
      ...
      std::sort(intervals->begin(), intervals->end(),
                IntervalComparisonByStartPoint<int>());
      ...
      if (!intervals->empty()) {
        ...
        if (merged_intervals.size() != intervals->size()) {
          ...
          merge_iter = merged_intervals.begin();
          for (int i = 0; merge_iter != merged_intervals.end();
               ++merge_iter, ++i) {
  ...
  void extract_direct_data() {
    ...
    for (int i = 0; i < m_num_target_blocks; ++i)
      size += m_direct_overlapping_intervals[i].size() +
              m_direct_non_overlapping_intervals[i].size();
    ...
    // perform parallel sort of intervals by sorting according to their starting
    // value
    tbb::parallel_sort(all_intervals.begin(), all_intervals.end(),
                       IntervalComparisonByStartPoint<int>());
    ...
    // construct translation map
    {
      int offset = 0;
      int last_endpoint = 0;
      for (IntervalVector::iterator it = merged_intervals.begin();
           it != merged_intervals.end(); ++it) {
    ...
    for (map_iter = m_translation_map.begin();
         map_iter != m_translation_map.end(); ++map_iter) {
      ...
      assert(old_start <= old_end && old_start >= 0 &&
             old_end < m_num_source_particles);
  ...
  void make_direct_plan() {
    for (int i = 0; i < m_num_target_blocks; ++i) {
      ...
      for (unsigned int j = 0; j < m_direct_overlapping_intervals[i].size();
           ++j) {
        TranslationMap::const_iterator map_iter =
            m_translation_map.find(m_direct_overlapping_intervals[i][j].first);

        assert(map_iter != m_translation_map.end());
        assert(map_iter->first.min() <=
               m_direct_overlapping_intervals[i][j].first);
        assert(map_iter->first.max() >=
               m_direct_overlapping_intervals[i][j].first);
        assert(map_iter->first.min() <=
               m_direct_overlapping_intervals[i][j].second);
        assert(map_iter->first.max() >=
               m_direct_overlapping_intervals[i][j].second);

        const int index =
            m_direct_overlapping_intervals[i][j].first - map_iter->second;

        const int num_particles = m_direct_overlapping_intervals[i][j].second -
                                  m_direct_overlapping_intervals[i][j].first +
                                  1;
      ...
      for (unsigned int j = 0; j < m_direct_non_overlapping_intervals[i].size();
           ++j) {
        TranslationMap::const_iterator map_iter = m_translation_map.find(
            m_direct_non_overlapping_intervals[i][j].first);
        ...
        assert(map_iter->first.max() >=
               m_direct_non_overlapping_intervals[i][j].first);
        assert(map_iter->first.min() <=
               m_direct_non_overlapping_intervals[i][j].second);
        assert(map_iter->first.max() >=
               m_direct_non_overlapping_intervals[i][j].second);

        const int index =
            m_direct_non_overlapping_intervals[i][j].first - map_iter->second;

        const int num_particles =
            m_direct_non_overlapping_intervals[i][j].second -
            m_direct_non_overlapping_intervals[i][j].first + 1;
  ...
  void addDirectInteraction(const tBox *const source_box, int target_id,
                            bool is_overlapping) {
    assert(source_box->vparticles[0].x[0] ==
           m_root_source_particles[&source_box->vparticles[0] -
                                   m_root_source_particles]
               .x[0]);

    int left_index = (&source_box->vparticles[0] - m_root_source_particles);
    int right_index = (&source_box->vparticles[source_box->nParticles - 1] -
                       m_root_source_particles);
    if (is_overlapping) {
      assert(target_id < m_direct_overlapping_intervals.size());
      m_direct_overlapping_intervals[target_id].push_back(
          std::pair<int, int>(left_index, right_index));
    } else {
      assert(target_id < m_direct_non_overlapping_intervals.size());
      m_direct_non_overlapping_intervals[target_id].push_back(
          std::pair<int, int>(left_index, right_index));
    }
  }
  ...
  struct CreateExpansionPlan {
    ...
    CreateExpansionPlan(const CreateExpansionPlan &other)
        : expansion_reals(other.expansion_reals),
          expansion_imags(other.expansion_imags),
          indirectinfo(other.indirectinfo), id2index(other.id2index),
          m_plan_per_block(other.m_plan_per_block) {}

    CreateExpansionPlan(
        const float *const expansion_reals_,
        const float *const expansion_imags_,
        const std::vector<std::vector<const tBox *>> &indirectinfo_,
        const std::map<int, std::pair<const tBox *, int>> &id2index_,
    ...
    void operator()(blocked_range<int> range) const {
      for (int i = range.begin(); i < range.end(); ++i) {
        ...
        for (std::vector<const tBox *>::const_iterator it = myinfo.begin();
             it != myinfo.end(); ++it) {
          assert(*it != NULL);
          const tBox &box = **it;

          const int boxID = box.getId();
          std::map<int, std::pair<const tBox *, int>>::const_iterator itIndex =
              id2index.find(boxID);
          ...
          IndirectInfo indirect_info(
              box.expansions.Center[0], box.expansions.Center[1],
  ...
  void make_indirect_plan() {
    for (int i = 0; i < m_num_target_blocks; ++i) {
      m_plan_per_block[i].m_velocity_block =
          (VelocityBlock *)m_target_blocks[i].ptrBlock;
...

    {
      int ctr = 0;
      for (int i = 0; i < m_num_target_blocks; ++i) {
        const std::vector<const tBox *>::const_iterator START =
            m_indirect_interactions[i].begin();
        const std::vector<const tBox *>::const_iterator END =
            m_indirect_interactions[i].end();

        for (std::vector<const tBox *>::const_iterator it = START; it != END;
             ++it) {
          const tBox *source_box = *it;

          std::map<int, std::pair<const tBox *, int>>::iterator map_it =
              m_indirect_source_id2index.find(source_box->getId());

          if (map_it == m_indirect_source_id2index.end()) {
            m_indirect_source_id2index.insert(
                std::pair<int, std::pair<const tBox *, int>>(
                    source_box->id,
                    std::pair<const tBox *, int>(source_box, ctr * _ORDER_)));
            ctr++;
          } else {
    ...
    m_real_values =
        (float *)_mm_malloc(sizeof(float) * m_indirect_buffer_size, 16);
    m_imag_values =
        (float *)_mm_malloc(sizeof(float) * m_indirect_buffer_size, 16);

    // loop over all elements in m_indirect_interactions
    CreateExpansionPlan create_indirect_plan(
        m_real_values, m_imag_values, m_indirect_interactions,
        m_indirect_source_id2index, m_plan_per_block);
    tbb::parallel_for(tbb::blocked_range<int>(0, m_num_target_blocks),
                      create_indirect_plan, auto_partitioner());

    // loop over m_indirect_source_id2index
    for (std::map<int, std::pair<const tBox *, int>>::const_iterator
             it_id2index = m_indirect_source_id2index.begin();
         it_id2index != m_indirect_source_id2index.end(); ++it_id2index) {
      const tBox *source_box = it_id2index->second.first;
      const int start_index = it_id2index->second.second;

      assert(start_index >= 0);
      assert(start_index + _ORDER_ <=
             m_indirect_source_id2index.size() * _ORDER_);

      for (int j = 0; j < _ORDER_; j++) {
        m_real_values[j + start_index] =
            source_box->expansions.values[0][j].real();
        m_imag_values[j + start_index] =
            source_box->expansions.values[0][j].imag();
      }
    }
  }
  ...
  std::vector<PlanPerBlock> m_plan_per_block;
  IntervalVectors m_direct_overlapping_intervals,
      m_direct_non_overlapping_intervals;
...
public:
  void operator()(tbb::blocked_range<int> range) {
    ...
    for (int i = range.begin(); i != range.end(); ++i) {
      ...
      if (source_infos[i].num_sources < 4) {
        AggressiveDiego::direct_interactions_cpp(
            target_info.xstart, target_info.ystart, target_info.h,
            source_infos[i].xs, source_infos[i].ys, source_infos[i].ws,
            source_infos[i].num_sources, velocity_block->u[0],
            velocity_block->u[1], source_infos[i].is_overlapping);

      } else {
        AggressiveDiego::direct_interactions_sse(
            velocity_block->xpos, velocity_block->ypos, source_infos[i].xs,
            source_infos[i].ys, source_infos[i].ws, source_infos[i].num_sources,
            velocity_block->u[0], velocity_block->u[1],
            source_infos[i].is_overlapping);
      }

      gflop +=
          source_infos[i].num_sources * _BLOCKSIZE_ * _BLOCKSIZE_ * 14. * 1e-9;
    }
  ...
  void _setup() {
    ...
    velocity_block->initialize(target_info.xstart, target_info.ystart,
                               target_info.h);
  }

  ReduceDirectSourceContributions(const DirectInfoVector &_source_infos,
                                  const TargetInfo &_target_info)
      : velocity_block(NULL), source_infos(_source_infos),
        target_info(_target_info), elapsed_time(0), total_gflop(0),
        kernel_calls(0), avg_gflops(0) {
    _setup();
  }
  ...
  ReduceDirectSourceContributions(ReduceDirectSourceContributions &other,
                                  tbb::split)
      : velocity_block(NULL), source_infos(other.source_infos),
        target_info(other.target_info), elapsed_time(0), total_gflop(0),
        kernel_calls(0), avg_gflops(0) {
    _setup();
  }
...
public:
  void operator()(tbb::blocked_range<int> range) {
    ...
    for (int i = range.begin(); i != range.end(); ++i) {
      ...
      AggressiveDiego::indirect_interactions_sse(
          velocity_block->xpos, velocity_block->ypos, source_infos[i].xbox,
          source_infos[i].ybox, source_infos[i].real_values,
          source_infos[i].imag_values, velocity_block->u[0],
          velocity_block->u[1]);
#else
      AggressiveDiego::indirect_interactions_ssefloat(
          velocity_block->xpos, velocity_block->ypos, source_infos[i].xbox,
          source_infos[i].ybox, source_infos[i].real_values,
          source_infos[i].imag_values, velocity_block->u[0],
          velocity_block->u[1]);

      // AggressiveDiego::indirect_interactions_sse (target_info.xstart,
      // target_info.ystart, target_info.h, 											source_infos[i].xbox,
      //source_infos[i].ybox, source_infos[i].real_values,
      //source_infos[i].imag_values, velocity_block->u[0],
      //velocity_block->u[1]);
#endif
      //	AggressiveDiego::indirect_interactions_cpp (target_info.xstart,
      //target_info.ystart, target_info.h, 			source_infos[i].xbox,
      //source_infos[i].ybox, source_infos[i].real_values,
      //source_infos[i].imag_values, velocity_block->u[0],
      //velocity_block->u[1]);
    ...
    const double gflop =
        1e-9 * (double)(_BLOCKSIZE_ * _BLOCKSIZE_ * (23 + 14 * _ORDER_)) *
        range.size();
    total_gflop += gflop;
    kernel_calls += range.size();
    elapsed_time += (end - start).seconds();
  ...
  ReduceIndirectSourceContributions(const IndirectInfoVector &_source_infos,
                                    const TargetInfo &_target_info)
      : velocity_block(NULL), source_infos(_source_infos),
        target_info(_target_info), elapsed_time(0), total_gflop(0),
        kernel_calls(0), avg_gflops(0) {
  ...
  ReduceIndirectSourceContributions(
      const ReduceIndirectSourceContributions &other, tbb::split)
      : velocity_block(NULL), source_infos(other.source_infos),
        target_info(other.target_info), elapsed_time(0), total_gflop(0),
        kernel_calls(0), avg_gflops(0) {
    _setup();
  ...
  void join(ReduceIndirectSourceContributions &other) {
    for (int d = 0; d < 2; ++d) {
      for (int y = 0; y < _BLOCKSIZE_; ++y) {
        for (int x = 0; x < _BLOCKSIZE_; ++x) {
          velocity_block->u[d][y][x] +=
              (double)other.velocity_block->u[d][y][x];
        }
      }
    }
...
template <int stage> class ComputeTargetBlocks {
public:
  void operator()(tbb::blocked_range<int> &range) const {
    ...
    for (int i = range.begin(); i != range.end(); ++i) {
      ...
      VelocityBlock *const velocity_block =
          m_plan.m_plan_per_block[i].m_velocity_block;
      assert(velocity_block != NULL);

      switch (stage) {
      case 0: {
        ReduceDirectSourceContributions reduce_direct(
            m_plan.m_plan_per_block[i].m_direct_infos, target_info);
        const int ndirect = m_plan.m_plan_per_block[i].m_direct_infos.size();
        parallel_reduce(tbb::blocked_range<int>(0, ndirect), reduce_direct,
                        auto_partitioner());
        ...
        for (int d = 0; d < 2; ++d)
          for (int y = 0; y < _BLOCKSIZE_; ++y)
            for (int x = 0; x < _BLOCKSIZE_; ++x)
              velocity_block->u[d][y][x] =
                  m_plan.m_scale_factor *
                  reduce_direct.velocity_block->u[d][y][x];
      ...
      case 1: {
        ReduceIndirectSourceContributions reduce_indirect(
            m_plan.m_plan_per_block[i].m_indirect_infos, target_info);
        const int nindirect =
            m_plan.m_plan_per_block[i].m_indirect_infos.size();
        parallel_reduce(tbb::blocked_range<int>(0, nindirect), reduce_indirect,
                        auto_partitioner());
        ...
        for (int d = 0; d < 2; ++d)
          for (int y = 0; y < _BLOCKSIZE_; ++y)
            for (int x = 0; x < _BLOCKSIZE_; ++x)
              velocity_block->u[d][y][x] +=
                  m_plan.m_scale_factor *
                  reduce_indirect.velocity_block->u[d][y][x];
  ...
  ComputeTargetBlocks(ComputeTargetBlocks &other, tbb::split)
      : m_plan(other.m_plan), b_verbose(other.b_verbose),
        time_infos(other.time_infos) {}
  ...
  ComputeTargetBlocks(const SSE_Plan &_plan, bool _b_verbose,
                      TimeInfos *const _time_infos)
...
public:
  typedef HCFMM::Box<_VortexExpansions<VelocitySourceParticle, _ORDER_>,
                     _FMM_MAX_LEVEL_>
      tBox;
  typedef HCFMM::boxBuilder_serial<
  ...
  VelocityEvaluatorSSE(BlockInfo *target_blocks, int num_target_blocks,
                       tBox *root_node, const Real inv_scaling,
                       int num_particles, bool _b_verbose)
      : m_target_blocks(target_blocks), m_num_target_blocks(num_target_blocks),
        m_plan(num_target_blocks, target_blocks, num_particles, root_node,
               _b_verbose),
        m_root_node(root_node), inv_scaling(inv_scaling),
        m_num_source_particles(num_particles), b_verbose(_b_verbose),
        m_time_infos(num_target_blocks) {
  ...
  void extract_interaction_data() {
    //		PlanBuilder plan_builder (&m_plan, m_root_node, m_target_blocks,
    //m_num_target_blocks);
    PlanBuilderWim plan_builder(&m_plan, m_root_node, m_target_blocks,
                                m_num_target_blocks);
    plan_builder.run();
  }

  void extract_direct_data() { m_plan.extract_direct_data(); }
  void make_direct_plan() { m_plan.make_direct_plan(); }
  void make_indirect_plan() { m_plan.make_indirect_plan(); }

  void run() {
    tbb::parallel_for(tbb::blocked_range<int>(0, m_num_target_blocks),
                      ComputeTargetBlocks<0>(m_plan, b_verbose, &m_time_infos),
                      auto_partitioner());
    tbb::parallel_for(tbb::blocked_range<int>(0, m_num_target_blocks),
                      ComputeTargetBlocks<1>(m_plan, b_verbose, &m_time_infos),
                      auto_partitioner());
  }
};

void I2D_CoreFMM_SSE::solve(const Real theta, const Real inv_scaling,
  ...
  if ((b_verbose && timestamp % 5 == 0) || timestamp % 10 == 0) {
    ...
    unsigned int measured_direct_kernel_calls = 0,
                 measured_indirect_kernel_calls = 0;
    ...
    for (int i = 0; i < evaluator.m_num_target_blocks; ++i) {
      ...
      total_indirect_boxes +=
          evaluator.m_plan.m_plan_per_block[i].m_indirect_infos.size();

      for (unsigned int j = 0;
           j < evaluator.m_plan.m_plan_per_block[i].m_direct_infos.size();
           ++j) {
        total_direct_particles +=
            evaluator.m_plan.m_plan_per_block[i].m_direct_infos[j].num_sources;

        if (evaluator.m_plan.m_plan_per_block[i].m_direct_infos[j].num_sources <
            4)
          num_direct_cpp_evals += 1;
      }

      num_direct_kernel_calls +=
          evaluator.m_plan.m_plan_per_block[i].m_direct_infos.size();
      num_indirect_kernel_calls +=
          evaluator.m_plan.m_plan_per_block[i].m_indirect_infos.size();
      ...
      measured_direct_kernel_calls +=
          evaluator.m_time_infos[i].direct_kernel_calls;
      measured_indirect_kernel_calls +=
          evaluator.m_time_infos[i].indirect_kernel_calls;
    }
    ...
    const double average_num_particles_per_box =
        total_direct_particles / num_direct_kernel_calls;

    const double direct_average_gflops = direct_total_gflop / total_direct_time;
    const double indirect_average_gflops =
        indirect_total_gflop / total_indirect_time;

    double num_effective_interactions =
        (double)evaluator.m_plan.m_direct_buffer_size *
        (double)evaluator.m_num_target_blocks * (double)_BLOCKSIZE_ *
        (double)_BLOCKSIZE_;
    ...
    const double plan_wallclock_time =
        (start_before_run - start_before_plan).seconds();
    const double expansions_wallclock_time =
        (start_before_plan - start_before_expansions).seconds();
    // const double fmm_wallclock_time = (end-start_before_tree).seconds ();
    const double tree_wallclock_time =
        (start_before_expansions - start_before_tree).seconds();
    ...
    std::cout << "\nGeneral Information\n";
    std::cout << "\n\tNumber of direct interacting source particles: "
              << evaluator.m_plan.m_direct_buffer_size << "\n";
    std::cout << "\tNumber of target blocks: " << evaluator.m_num_target_blocks
              << "\n";
    std::cout << "\tEffective number of interactions: " << setprecision(6)
              << scientific << num_effective_interactions << "\n";
    std::cout << "\tActual number of interactions: "
              << (total_direct_particles + total_indirect_boxes) * _BLOCKSIZE_ *
                     _BLOCKSIZE_
              << ", (direct: " << fixed << setprecision(2)
              << 100 * total_direct_particles /
                     (total_direct_particles + total_indirect_boxes)
              << "%, indirect: "
              << 100 * total_indirect_boxes /
                     (total_direct_particles + total_indirect_boxes)
              << "%)\n";
    std::cout << "\tNumber of direct c++ kernel calls: " << num_direct_cpp_evals
              << scientific << " ("
              << (double)num_direct_cpp_evals / num_direct_kernel_calls
              << " % of total direct kernel calls ( " << num_direct_kernel_calls
              << ") )\n";
    std::cout << "\tAverage number of source particles per box: "
              << average_num_particles_per_box << "\n";

    std::cout.precision(2);
    std::cout << "\nMemory\n";
    std::cout << "\n\tTotal memory for direct interactions: " << scientific
              << (double)3 * evaluator.m_plan.m_direct_buffer_size *
                     sizeof(float) / (1024 * 1024)
              << " MB\n";
    std::cout << "\tTotal memory for indirect interactions: "
              << (double)2 * evaluator.m_plan.m_indirect_buffer_size *
                     sizeof(float) / (1024 * 1024)
              << " MB\n";
    std::cout << "\nElapsed time\n";
    std::cout << "\n\tTotal elapsed wall-clock time: " << total_wallclock_time
              << "\n";
    std::cout << "\tTotal CPU time: " << total_time
              << " [s],\t(direct: " << fixed << setprecision(1)
              << 100 * total_direct_time / total_time
              << "%, indirect: " << 100 * total_indirect_time / total_time
              << "%)\n";
    std::cout.precision(2);
    std::cout << "\tAverage CPU time per target block: " << scientific
              << total_time / evaluator.m_num_target_blocks << " [s]\n";
    std::cout << "\tAverage direct SSE kernel time per source particle: "
              << total_direct_time / total_direct_particles << " [s]\n";
    std::cout << "\tAverage indirect SSE kernel time per source box: "
              << total_indirect_time / total_indirect_boxes << " [s]\n";
    std::cout << "\nFloating point operations\n";
    std::cout << "\n\tTotal number of operations: " << total_gflop
              << " [GFLOP], (direct: " << fixed << setprecision(1)
              << 100 * direct_total_gflop / total_gflop
              << "%, indirect: " << 100 * indirect_total_gflop / total_gflop
              << "%)\n";
    std::cout.precision(2);
    std::cout << "\tAverage number of operations per target block: "
              << scientific << total_gflop / evaluator.m_num_target_blocks
              << " [GFLOP]\n";
    std::cout << "\nPerformance\n";
    std::cout << "\tDirect SSE kernel (per call): " << fixed << setprecision(3)
              << direct_avg_gflops << " [GFLOP/s]\n";
    std::cout << "\tIndirect SSE kernel (per call): " << indirect_avg_gflops
              << " [GFLOP/s]\n";
    std::cout << "\n\tDirect SSE kernel (average): " << fixed << setprecision(3)
              << direct_average_gflops << " [GFLOP/s]\n";
    std::cout << "\tIndirect SSE kernel (average): " << indirect_average_gflops
              << " [GFLOP/s]\n";

    std::cout << "\nFloating point operations per second wall-clock time\n";
    std::cout << "\n\tPlan + Evaluations: "
              << total_gflop / total_wallclock_time << " [GFLOP/s]\n";
    std::cout << "\n\tEvaluations only: "
              << total_gflop / evaluation_wallclock_time << " [GFLOP/s]\n";
    ...
    file << setprecision(6) << scientific << num_effective_interactions << "\t"
         << total_gflop << "\t" << evaluation_wallclock_time << "\t"
         << plan_wallclock_time << "\t" << expansions_wallclock_time << "\t"
         << tree_wallclock_time << "\n";
    file.close();
    if (file.fail()) {
