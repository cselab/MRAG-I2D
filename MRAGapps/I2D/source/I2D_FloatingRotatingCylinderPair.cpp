...
I2D_FloatingRotatingCylinderPair::RotatingCylinderPair::RotatingCylinderPair(
    Real xm, Real ym, Real D, Real width, Real angle_rad, Real GammaLeft,
    Real GammaRight, Real burgersTime)
    : xm(xm), ym(ym), D(D), width(width), angle(angle_rad),
      GammaLeft(GammaLeft), GammaRight(GammaRight), burgersTime(burgersTime),
      vx(0), vy(0), vdefx(0), vdefy(0), rho(1.0) {
  ...
  m = 2 * mass; // total mass
  J = 2.0 * (momInertDisk +
             mass * std::pow(0.5 * width, 2.)); // parallel axis theorem

  std::cout << "MASS = " << m << " INERTIA = " << J << std::endl;
  _setTemporalPreFactor(0.0);
}

Real I2D_FloatingRotatingCylinderPair::RotatingCylinderPair::
    _mollified_heaviside(const double dist, const double eps) const {
  // Positive outside/negative inside
  const double alpha = M_PI * min(1., max(0., (dist + 0.5 * eps) / eps));
  return 0.5 + 0.5 * cos(alpha);
}

void I2D_FloatingRotatingCylinderPair::RotatingCylinderPair::update_all(
    double dt, double t) {
  xm += vx * dt;
  ym += vy * dt;
...
void I2D_FloatingRotatingCylinderPair::RotatingCylinderPair::save(
    FILE *f) const {
  fprintf(f, "xm: %20.20e\n", xm);
  fprintf(f, "ym: %20.20e\n", ym);
...
Real I2D_FloatingRotatingCylinderPair::RotatingCylinderPair::_getBurgersFactor(
    double tRel) {
...
void I2D_FloatingRotatingCylinderPair::RotatingCylinderPair::
    _setTemporalPreFactor(double t) {
  const double g =
      GammaLeft == 0
          ? std::abs(GammaRight)
          : (GammaRight == 0
                 ...
                 : std::min(
                       std::abs(GammaLeft),
                       std::abs(
                           GammaRight))); // want to take the minimum but should
                                          // still work if one of them is zero
  assert(g > 0);
  const double Omega = g / (0.5 * M_PI * D * D); // angular velocity in rad/s
  const double tPerRevolution =
      2.0 * M_PI / Omega; // time for one revolution (the slowest one)
  const double nRevs = 20.0;

  // timePrefac = 1.0; // impulsively started
  // timePrefac = t < nRevs*tPerRevolution ?
  // std::sin(0.5*M_PI*t/(nRevs*tPerRevolution)) : 1.0; // ramp it up over nRevs
  // revolutions
  //     timePrefac = std::sin(2.0*M_PI*t/(nRevs*tPerRevolution)); // oscillate
  //     it with period of nRevs revolutions
  timePrefac = _getBurgersFactor(
      t / (nRevs * tPerRevolution)); // oscillate it with burgers wave, period
                                     // is nRevs revolutions
}

Real I2D_FloatingRotatingCylinderPair::RotatingCylinderPair::sample(
    const Real x_, const Real y_, const Real eps) const {
  const double CM_left[2] = {xm - 0.5 * width * std::cos(angle),
                             ym - 0.5 * width * std::sin(angle)};

  const double CM_right[2] = {xm + 0.5 * width * std::cos(angle),
                              ym + 0.5 * width * std::sin(angle)};

  const double dist_left = std::sqrt((x_ - CM_left[0]) * (x_ - CM_left[0]) +
                                     (y_ - CM_left[1]) * (y_ - CM_left[1])) -
                           0.5 * D;

  const double dist_right = std::sqrt((x_ - CM_right[0]) * (x_ - CM_right[0]) +
                                      (y_ - CM_right[1]) * (y_ - CM_right[1])) -
                            0.5 * D;

  return _mollified_heaviside(std::min(dist_left, dist_right), eps);
}

void I2D_FloatingRotatingCylinderPair::RotatingCylinderPair::bbox(
    const Real eps, Real xmin[2], Real xmax[2]) const {
  assert(eps >= 0);

  const Real rad = 0.5 * D;

  const Real xdist[4] = {xm + 0.5 * width * std::cos(angle) + rad,
                         xm + 0.5 * width * std::cos(angle) - rad,
                         xm - 0.5 * width * std::cos(angle) + rad,
                         xm - 0.5 * width * std::cos(angle) - rad};

  const Real ydist[4] = {ym + 0.5 * width * std::cos(angle) + rad,
...
struct FillBlocks {
  ...
  FillBlocks(Real eps,
             I2D_FloatingRotatingCylinderPair::RotatingCylinderPair *_shape)
      : eps(eps) {
    shape = _shape;
  }
  ...
  static bool _is_touching(
      Real eps,
      const I2D_FloatingRotatingCylinderPair::RotatingCylinderPair *shape,
...
struct FillVelblocks {
  ...
  inline void operator()(blocked_range<int> range) const {
    for (int iblock = range.begin(); iblock < range.end(); iblock++) {
      ...
      // Gamma = int omega dx dy // Gamma is circulation (imposed here), omega
      // is the vorticity = 2 * Omega, so: Gamma = 2*Omega*pi*R^2 =
      // 0.5*pi*Omega*D^2 Omega = Gamma / (0.5 * pi * D^2)
          ...
          for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
            ...
            if (Xs > 0) {
              ...
              const double dist_left =
                  std::sqrt((p[0] - CM_left[0]) * (p[0] - CM_left[0]) +
                            (p[1] - CM_left[1]) * (p[1] - CM_left[1]));

              const double dist_right =
                  std::sqrt((p[0] - CM_right[0]) * (p[0] - CM_right[0]) +
                            (p[1] - CM_right[1]) * (p[1] - CM_right[1]));

              const bool isLeft = dist_left < dist_right;
              const double av_here = isLeft ? av_left : av_right;
              const double CM_here[2] = {isLeft ? CM_left[0] : CM_right[0],
...
I2D_FloatingRotatingCylinderPair::I2D_FloatingRotatingCylinderPair(
    ...
    const Real D, const Real angle, const Real width, const Real GammaLeft,
    const Real GammaRight, const Real eps, const Real Uinf[2],
  ...
  // default of argumentparser is to return 0 so if we argument is not passed
  // the circulation will vary like a sine then whatever we pass to the burgers
  // equation should be between 0 and 1.0/(2*pi) because that's the break time
  // of the wave so to make it easier we normalize by 1/(2pi) here so that we
  // can pass an argument between 0 (sine) and <1 (1 = broken wave)
  const Real burgersTime = parser("-burgersfactor").asDouble() / (2.0 * M_PI);
  assert(burgersTime >= 0 && burgersTime < 1.0);
  shape = new RotatingCylinderPair(_xm, _ym, D, width, angle, GammaLeft,
                                   GammaRight, burgersTime);
...
void I2D_FloatingRotatingCylinderPair::restart(const double t,
                                               string filename) {
  FILE *ppFile = NULL;
...
void I2D_FloatingRotatingCylinderPair::refresh(const double t,
                                               string filename) {
  // wim: I will not implemented this unless i figure out i need it for
  // something
...
void I2D_FloatingRotatingCylinderPair::computeDesiredVelocity(const double t) {
  ...
  // Compute all
  FloatingRotatingCylinderPair::ComputeAll computeAll(eps, shape, integrals,
                                                      Uinf, nonempty);
  ...
  FloatingRotatingCylinderPair::FillVelblocks fillvelblocks(velblocks, eps,
                                                            shape);
  tbb::parallel_for(blocked_range<int>(0, velblocks.size()), fillvelblocks,
                    auto_partitioner());
}

void I2D_FloatingRotatingCylinderPair::update(
  ...
  if (filename == std::string()) {
    // If the string is not set I write my own file
    ppFile =
        fopen("I2D_FloatingRotatingCylinderPair.txt", t == 0.0 ? "w" : "a");
    assert(ppFile != NULL);
  } else {
    // If string is set I open the corresponding file
  ...
  fprintf(ppFile, "%e %e %e %e %e %e %e %e %e %e %e %e\n", t, shape->xm,
          shape->ym, shape->vx, shape->vy, shape->angle,
          shape->angular_velocity, shape->J, shape->m, shape->rho,
          shape->timePrefac * shape->GammaLeft,
          shape->timePrefac * shape->GammaRight);
