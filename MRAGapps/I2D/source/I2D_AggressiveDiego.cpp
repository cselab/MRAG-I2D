...
inline __m128 worse_division(const __m128 a, const __m128 b) {
  // from the "Software Optimization Guide for AMD Family 10h and 12h
  // Processors"
...
template <int c> inline __m128 _select(__m128 d) {
  abort();
  return d;
}

template <> inline __m128 _select<0>(__m128 d) {
  return _mm_shuffle_ps(d, d, _MM_SHUFFLE(0, 0, 0, 0));
}
template <> inline __m128 _select<1>(__m128 d) {
  return _mm_shuffle_ps(d, d, _MM_SHUFFLE(1, 1, 1, 1));
}
template <> inline __m128 _select<2>(__m128 d) {
  return _mm_shuffle_ps(d, d, _MM_SHUFFLE(2, 2, 2, 2));
}
template <> inline __m128 _select<3>(__m128 d) {
  return _mm_shuffle_ps(d, d, _MM_SHUFFLE(3, 3, 3, 3));
}
...
struct DirectInteractions {
  template <bool safe>
  inline void kernel(const __m128 xd, const __m128 yd, const __m128 xs,
                     const __m128 ys, const __m128 ws, float *const u_dest,
                     float *const v_dest);

  inline __m128 get(const float *const ptr, const int s,
                    const float val = -1e7) {
    switch (s) {
    case 1:
  ...
  inline __m128 get(const float *const ptr) { return _mm_load_ps(ptr); }

  template <bool safe>
  void evaluate_tile(const __m128 x0, const __m128 y0, const __m128 h,
                     const __m128 d, const __m128 xs, const __m128 ys,
                     const __m128 ws, float *const u, float *const v) {
  ...
  void evaluate_tile(const float *const xd, const float *const yd,
                     const __m128 xs, const __m128 ys, const __m128 ws,
...
#pragma ivdep
      for (int dx = 0; dx < _BLOCKSIZE_; dx += 4) {
        ...
        kernel<safe>(_mm_load_ps(xd + dx), y, xs, ys, ws, u + offset1,
                     v + offset1);
      }
    }
  }
  ...
  void evaluate(const float x_start, const float y_start, const float h,
                ...
                const float *const ws, const int nsources, float *const u,
                float *const v) {
    ...
    for (int s = start_tile; s < end_tile; s += 4)
      evaluate_tile<safe>(x0, y0, dx, d, get(xs + s), get(ys + s), get(ws + s),
                          u, v);
...
template <>
void DirectInteractions::kernel<false>(const __m128 xd, const __m128 yd,
                                       const __m128 xs, const __m128 ys,
                                       const __m128 ws, float *const u_dest,
                                       float *const v_dest) {
...
template <>
void DirectInteractions::kernel<true>(const __m128 xd, const __m128 yd,
                                      const __m128 xs, const __m128 ys,
                                      const __m128 ws, float *const u_dest,
                                      float *const v_dest) {
  const __m128 rx1 = xd - _select<0>(xs);
...
void direct_interactions_sse(float xd[_BLOCKSIZE_], float (*const yd)[4],
                             const float *const xs, const float *const ys,
                             const float *const ws, const int nsources,
                             float (*const u)[_BLOCKSIZE_],
                             float (*const v)[_BLOCKSIZE_], const bool safe) {
#if (_BLOCKSIZE_ < 8)
#pragma warning _BLOCKSIZE_ is not big enough.
  ...
  if (safe)
    direct_interactions.evaluate<true>((float *)xd, (float *)yd, xs, ys, ws,
                                       nsources, (float *)u, (float *)v);
  else
    direct_interactions.evaluate<false>((float *)xd, (float *)yd, xs, ys, ws,
                                        nsources, (float *)u, (float *)v);
}

void direct_interactions_sse(const float x_start, const float y_start,
                             const float h, const float *const xs,
                             const float *const ys, const float *const ws,
                             const int nsources, float (*const u)[_BLOCKSIZE_],
                             float (*const v)[_BLOCKSIZE_], const bool safe) {
#if (_BLOCKSIZE_ < 8)
#pragma warning _BLOCKSIZE_ is not big enough.
  printf("aborting, _BLOCKSIZE_ is not greater or equal than 8.");
  ...
  if (safe)
    direct_interactions.evaluate<true>(x_start, y_start, h, xs, ys, ws,
                                       nsources, (float *)u, (float *)v);
  // direct_interactions_cpp(x_start, y_start, h, xs, ys, ws, nsources, u, v,
  // safe);
  else
    direct_interactions.evaluate<false>(x_start, y_start, h, xs, ys, ws,
                                        nsources, (float *)u, (float *)v);
}

void direct_interactions_cpp(const float x_start, const float y_start,
                             const float h, const float *const xs,
                             const float *const ys, const float *const ws,
                             const int nsources, float (*const u)[_BLOCKSIZE_],
                             float (*const v)[_BLOCKSIZE_], const bool safe) {
  ...
  for (int dy = 0; dy < _BLOCKSIZE_; dy++)
    for (int dx = 0; dx < _BLOCKSIZE_; dx++) {
      ...
      for (int s = 0; s < nsources; s++) {
        Real r[2] = {x_start + dx * h - xs[s], y_start + dy * h - ys[s]};
...
void check_quality(const float x_start, const float y_start, const float h,
                   const float *const xs, const float *const ys,
                   const float *const ws, const int nsources,
                   float (*const ures)[_BLOCKSIZE_],
                   float (*const vres)[_BLOCKSIZE_], const double tol) {
  assert(nsources > 0);
  ...
  for (int dy = 0; dy < _BLOCKSIZE_; dy++)
    for (int dx = 0; dx < _BLOCKSIZE_; dx++) {
      ...
      for (int s = 0; s < nsources; s++) {
        ...
        const double factor =
            safety_value / (r2 * safety_value + (1 - safety_value));
  ...
  printf("average discrepancies: u: %e v: %e\n",
         L1[0] / (double)(_BLOCKSIZE_ * _BLOCKSIZE_),
         L1[1] / (double)(_BLOCKSIZE_ * _BLOCKSIZE_));
  printf("average interaction discrepancy: u: %e v: %e\n",
         L1[0] / (double)(nsources * _BLOCKSIZE_ * _BLOCKSIZE_),
         L1[1] / (double)(nsources * _BLOCKSIZE_ * _BLOCKSIZE_));
}
...
struct IndirectInteractions {
  template <int s>
  inline __m128d sum_step(const __m128 rexp, const __m128 iexp,
                          const __m128d prod) {
    return _mm_addsub_pd(
        _mm_cvtps_pd(_mm_shuffle_ps(rexp, rexp, _MM_SHUFFLE(s, s, s, s))) *
            prod,
        _mm_cvtps_pd(_mm_shuffle_ps(iexp, iexp, _MM_SHUFFLE(s, s, s, s))) *
            _mm_shuffle_pd(prod, prod, _MM_SHUFFLE2(0, 1)));
  }

  template <int d>
  inline __m128d prod_step(const __m128 realrp, const __m128 imagrp,
                           const __m128d oldprod) {
    return _mm_addsub_pd(
        _mm_cvtps_pd(_mm_shuffle_ps(realrp, realrp, _MM_SHUFFLE(d, d, d, d))) *
            oldprod,
        _mm_cvtps_pd(_mm_setzero_ps() -
                     _mm_shuffle_ps(imagrp, imagrp, _MM_SHUFFLE(d, d, d, d))) *
            _mm_shuffle_pd(oldprod, oldprod, _MM_SHUFFLE2(0, 1)));
  }

  inline __m128 rearrange_real(const __m128d s0, const __m128d s1,
                               const __m128d s2, const __m128d s3) {
    return _mm_shuffle_ps(
        _mm_cvtpd_ps(_mm_shuffle_pd(s0, s1, _MM_SHUFFLE2(0, 0))),
        _mm_cvtpd_ps(_mm_shuffle_pd(s2, s3, _MM_SHUFFLE2(0, 0))),
        _MM_SHUFFLE(1, 0, 1, 0));
  }

  inline __m128 rearrange_imag(const __m128d s0, const __m128d s1,
                               const __m128d s2, const __m128d s3) {
    return _mm_shuffle_ps(
        _mm_cvtpd_ps(_mm_shuffle_pd(s0, s1, _MM_SHUFFLE2(1, 1))),
...
#pragma warning _ORDER_ is not multiple of 4
    ...
    for (int dy = 0; dy < _BLOCKSIZE_; dy++) {
      ...
      for (int dx = 0; dx < _BLOCKSIZE_; dx += 4) {
        ...
        _mm_store_ps(&u[dy][dx],
                     _mm_add_ps(_mm_load_ps(&u[dy][dx]),
                                imagsum * realrp - realsum * imagrp));
        _mm_store_ps(&v[dy][dx],
                     _mm_add_ps(_mm_load_ps(&v[dy][dx]),
                                imagsum * imagrp + realsum * realrp));
      }
    }
  }
...
void indirect_interactions_sse(const float xstart, const float ystart,
                               const float h_, const float xcenter,
                               const float ycenter,
                               const float *const rexpansions,
                               const float *const iexpansions,
                               float (*const u)[_BLOCKSIZE_],
                               float (*const v)[_BLOCKSIZE_]) {
  if ((((unsigned long int)u) & 0xf) != 0) {
    printf("float * const u is not 16-byte aligned!\n");
    abort();
  }
  ...
  indirect.compute(xstart, ystart, h_, xcenter, ycenter, rexpansions,
                   iexpansions, u, v);
}
...
void indirect_interactions_sse(float xd[_BLOCKSIZE_], float (*const yd)[4],
  ...
  indirect.compute((float *)xd, (float *)yd, xcenter, ycenter, rexpansions,
                   iexpansions, u, v);
}

void indirect_interactions_ssefloat(const float xstart, const float ystart,
                                    const float h_, const float xcenter,
                                    const float ycenter,
                                    const float *const rexpansions,
                                    const float *const iexpansions,
  ...
  for (int dy = 0; dy < _BLOCKSIZE_; dy++) {
    ...
    for (int dx = 0; dx < _BLOCKSIZE_; dx += 4) {
      ...
      for (int e = 0; e < _ORDER_; e += 4) {
        ...
        realsum = realsum + realprod * _select<0>(rexps) -
                  imagprod * _select<0>(iexps);
        imagsum = imagsum + imagprod * _select<0>(rexps) +
                  realprod * _select<0>(iexps);
        ...
        realsum = realsum + realprod * _select<1>(rexps) -
                  imagprod * _select<1>(iexps);
        imagsum = imagsum + imagprod * _select<1>(rexps) +
                  realprod * _select<1>(iexps);
        ...
        realsum = realsum + realprod * _select<2>(rexps) -
                  imagprod * _select<2>(iexps);
        imagsum = imagsum + imagprod * _select<2>(rexps) +
                  realprod * _select<2>(iexps);
        ...
        realsum = realsum + realprod * _select<3>(rexps) -
                  imagprod * _select<3>(iexps);
        imagsum = imagsum + imagprod * _select<3>(rexps) +
                  realprod * _select<3>(iexps);
      ...
      _mm_store_ps(&u[dy][dx], _mm_add_ps(_mm_load_ps(&u[dy][dx]),
                                          imagsum * realrp - realsum * imagrp));
      _mm_store_ps(&v[dy][dx], _mm_add_ps(_mm_load_ps(&v[dy][dx]),
                                          imagsum * imagrp + realsum * realrp));
    }
  }
}

void indirect_interactions_ssefloat(float xd[_BLOCKSIZE_],
                                    float (*const yd_)[4], const float xcenter_,
                                    const float ycenter_,
                                    const float *const rexpansions,
                                    const float *const iexpansions,
                                    float (*const u)[_BLOCKSIZE_],
                                    float (*const v)[_BLOCKSIZE_]) {
...
void indirect_interactions_cpp(const float xstart, const float ystart,
                               const float h, const float xcenter,
                               const float ycenter,
                               const float *const rexpansions,
                               const float *const iexpansions,
                               float (*const u)[_BLOCKSIZE_],
  ...
  for (int dy = 0; dy < _BLOCKSIZE_; dy++)
    for (int dx = 0; dx < _BLOCKSIZE_; dx++) {
      const double location[2] = {xstart + dx * h, ystart + dy * h};

      const ExpansionsValueType rp =
          ExpansionsValueType(location[0], location[1]) -
          ExpansionsValueType(xcenter, ycenter);
...
void check_quality(const float xstart, const float ystart, const float h,
                   ...
                   const float *const rexpansions,
                   const float *const iexpansions, const int nexpansions,
                   float (*const u)[_BLOCKSIZE_], float (*const v)[_BLOCKSIZE_],
                   const double tol) {
  double L1[2] = {0, 0};
  double LINF[2] = {0, 0};
  ...
  for (int dy = 0; dy < _BLOCKSIZE_; dy++)
    for (int dx = 0; dx < _BLOCKSIZE_; dx++) {
      ...
      for (int e = 0; e < nexpansions; e++) {
        ...
        const ExpansionsValueType rp =
            ExpansionsValueType(location[0], location[1]) -
            ExpansionsValueType(xcenter[e], ycenter[e]);

        std::complex<double> csum = std::complex<double>(0, 0);
        std::complex<double> prod = std::complex<double>(1, 0);

        for (int n = 0; n < _ORDER_; ++n) {
          csum += (prod * std::complex<double>(rexpansions[e * _ORDER_ + n],
                                               iexpansions[e * _ORDER_ + n]));
          prod /= (std::complex<double>)rp;
        }
      ...
      const double diffu =
          (ucheck - uactual) / (max(1e-3, max(fabs(uactual), fabs(ucheck))));
      const double diffv =
          (vcheck - vactual) / (max(1e-3, max(fabs(vactual), fabs(vcheck))));
  ...
  printf("average relative expansion discrepancy: u: %e v: %e\n",
         L1[0] / (double)(_ORDER_ * _BLOCKSIZE_ * _BLOCKSIZE_ * nexpansions),
         L1[1] / (double)(_ORDER_ * _BLOCKSIZE_ * _BLOCKSIZE_ * nexpansions));
}
} // namespace AggressiveDiego
