...
RL_SmartyLattice::RL_SmartyLattice(MRAG::ArgumentParser &parser, const Real _x,
                                   const Real _y, const Real _D,
                                   const Real _dir[2], const int _ID,
                                   RL_TabularPolicy **_policy, const int seed)
    : x(_x), y(_y), D(_D), T(1.0), maxDomainRadius(0.45), rng(seed),
      RL_Agent(parser, "RL_SmartyLattice", _ID, _policy, seed) {
  modv = D / 5.0;
...
bool RL_SmartyLattice::mapState(vector<int> &state,
  ...
  bool valid = true;
  {
    const Real dist = sqrt((myX[0] - center[0]) * (myX[0] - center[0]) +
...
void RL_SmartyLattice::reward(const Real t,
                              map<string, vector<RL_Agent *>> *_data) {
...
void RL_SmartyLattice::update(const Real dt, const Real t,
                              map<string, vector<RL_Agent *>> *_data,
                              string filename) {
  x += vx * dt;
  y += vy * dt;
  ...
  // out << t << "  " << x << "  " << y << "  " << vx << "  " << vy << "  " <<
  // target[0] << "  " << target[1] << "  " << modvLattice << endl; out.flush();
  // out.close();
}
