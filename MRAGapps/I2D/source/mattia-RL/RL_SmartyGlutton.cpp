...
RL_SmartyGlutton::RL_SmartyGlutton(MRAG::ArgumentParser &parser, const Real _x,
                                   const Real _y, const Real _D, const Real _T,
                                   const int _nactions,
                                   const Real _cutoffGluttons,
                                   const Real _cutoffFood, const Real _steer,
                                   const Real _noise, const Real _selfAvoid,
                                   const int NN, const int _ID,
                                   RL_TabularPolicy **_policy, const int seed)
    : x(_x), y(_y), D(_D), T(_T), nactions(_nactions),
      cutoffGluttons(_cutoffGluttons), cutoffFood(_cutoffFood), steer(_steer),
      noise(_noise), selfAvoid(_selfAvoid), rng(seed), myfoodcounter(0), NN(NN),
      STARVING(true), RL_Agent(parser, "RL_SmartyGlutton", _ID, _policy, seed) {
...
void RL_SmartyGlutton::_mapForFoods(const Real cutoff, const int idxDistLevel,
                                    const int idxAngleLevel, int &idxDist,
                                    int &idxAngle,
  ...
  for (vector<RL_Agent *>::iterator it = foods.begin(); it != foods.end();
       ++it) {
...
void RL_SmartyGlutton::_mapForGluttons(
    const Real cutoff, const int idxDistLevel, const int idxAngleLevel,
    const int idxAngleNeighLevel, RL_SmartyGlutton *neigh, Real vneigh[2],
    int &idxDist, int &idxAngle, int &idxAngleNeigh,
    map<string, vector<RL_Agent *>> *_data) {
  // Gather my info (always double check with getInfo!!)
  const Real myX[2] = {x, y};
  const Real myV[2] = {vx, vy};
  ...
  idxAngleNeigh =
      _discretize1SS(angleNeigh, 0, 360, idxAngleNeighLevel, deadEnd);
}

bool RL_SmartyGlutton::mapState(vector<int> &state,
                                map<string, vector<RL_Agent *>> *_data) {
  assert(_data != NULL);
  assert((*_data).size() != 0);
  ...
  //	Neighborhood velocity and velocity of the closest neighbor (if any)
  //within cutoff
  {
    const Real myX[2] = {x, y};
    Real myVm[2] = {0.0, 0.0};
    ...
    for (vector<RL_Agent *>::iterator it = gluttons.begin();
         it != gluttons.end(); ++it) {
    ...
    if (nactions == 4) {
      ...
      const int maxNN =
          (distances.size() < nnInteraction) ? distances.size() : nnInteraction;
      ...
      // Vm[0] = ( counter1 != 0 && ImyVmI1 != 0.0 &&
      // maxNN!=0)?(myVm[0]/ImyVmI1)*modv:vx; Vm[1] = ( counter1 != 0 && ImyVmI1
      // != 0.0 && maxNN!=0)?(myVm[1]/ImyVmI1)*modv:vy;
    }
  }
  ...
  // Map foods
  {
    ...
    _mapForFoods(cutFood, signature[0], signature[1], idxDistMaxFoods,
                 idxAngleMaxFoods, _data);
  ...
  // Map gluttons
  {
    ...
    int idxAngleNeigh = 0;
    _mapForGluttons(cutGluttons, signature[2], signature[3], signature[4],
                    neigh, vneigh, idxDistMaxGluttons, idxAngleMaxGluttons,
                    idxAngleNeigh, _data);
...
void RL_SmartyGlutton::reward(const double t,
                              map<string, vector<RL_Agent *>> *_data) {
  ...
  if ((t > (learningTimer + learningInterval)) && status == Waiting) {
    ...
    // Gluttons
    {
      ...
      for (vector<RL_Agent *>::iterator it = gluttons.begin();
        ...
        const Real localReward =
            (IdI <= 0.0 && otherID != this->ID) ? rewardGluttons : 0;
        reward += localReward;
...
void RL_SmartyGlutton::update(const double dt, const double t,
                              map<string, vector<RL_Agent *>> *_data,
                              string filename) {
  // Update positions
  x += vx * dt;
...
void RL_SmartyGlutton::dump(const double t, ofstream &out) {
  out.precision(6);
  out << scientific << t << "\t" << this->x << "\t" << this->y << "\t"
      << this->vx << "\t" << this->vy << "\t";
