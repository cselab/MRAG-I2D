...
RL_TestTabular::RL_TestTabular(const int argc, const char **argv)
    : parser(argc, argv), policy(NULL) {
  printf("/////////////////////////////////////////////////////////////////////"
         "/\n");
...
RL_TestTabular::~RL_TestTabular() { _dispose(); }
...
void RL_TestTabular::run() {
  /*
          ...
          //void RL_Agent::_rotate(const Real alpha, const Real v1[2], Real
     v2[2]) const v2[0] = v1[0]*cos(alpha) - v1[1]*sin(alpha); v2[1] =
     v1[0]*sin(alpha) + v1[1]*cos(alpha); cout << "v2[0]=" << v2[0] << " v2[1]="
     << v2[1] << std::endl;
  ...
  map<string, vector<RL_Agent *>>::iterator itmap =
      b->data.find("RL_SmartyGlutton");
  const long unsigned int ngluttons =
      (itmap != b->data.end()) ? itmap->second.size() : 0;
  const long unsigned int totNumPolicyEval = 1e15;
  const long unsigned int nplays =
      (ngluttons == 0) ? totNumPolicyEval
                       : ((int)ceil((Real)totNumPolicyEval / (Real)ngluttons));
