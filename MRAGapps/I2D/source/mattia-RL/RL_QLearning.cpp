...
RL_QLearning::RL_QLearning(double LR, double gamma, double epsilon, int seed,
                           int freq)
    : rng(seed), LR(LR), gamma(gamma), epsilon(epsilon), plays(0), freq(freq),
  ...
  if (LR > 0.0 && LR <= 1.0 && gamma >= 0.0 && gamma <= 1.0 && epsilon >= 0.0 &&
      epsilon <= 1.0 && (LR + gamma) > 1.0) {
...
double RL_QLearning::_getValue(const vector<int> &idx) { return Q.read(idx); }
...
void RL_QLearning::setReward(double reward) { rewards.push_back(reward); }
...
bool RL_QLearning::_jumpedToDifferentState(const vector<int> &stateActionStart,
                                           const vector<int> &stateEnd) {
...
void RL_QLearning::update(string name) {
  ...
  if (histPlays.size() % freq == 0) {
    ...
    for (unsigned int i = 0; i < histRewards.size(); ++i)
      out << histPlays[i] << "  " << histRewards[i] << "  " << histUsage[i]
          << endl;
