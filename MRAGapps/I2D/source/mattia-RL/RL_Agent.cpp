...
void RL_Agent::_distPeriodic(const Real x2[2], const Real x1[2],
                             Real d[2]) const {
...
int RL_Agent::_discretize(const Real signal, const Real minsignal,
                          const Real maxsignal, const int nlevels) const {
...
int RL_Agent::_discretize2SS(const Real signal, const Real minsignal,
                             const Real maxsignal, const int nlevels,
                             const bool deadEnd, const bool inside) const {
  assert(maxsignal >= minsignal);
...
int RL_Agent::_discretize1SS(const Real signal, const Real minsignal,
                             const Real maxsignal, const int nlevels,
...
bool RL_Agent::_isRight(const Real x1[2], const Real dir[2],
                        const Real x2[2]) const {
  const Real alpha = -M_PI / 2.0;
  const Real n[2] = {dir[0] * cos(alpha) - dir[1] * sin(alpha),
...
bool RL_Agent::startChoose(const double t,
...
void RL_Agent::setReward(const double t) {
  if (policy != NULL && (t > (learningTimer + learningInterval)) &&
      status == Waiting) {
...
void RL_Agent::stopTest(const double t,
                        map<string, vector<RL_Agent *>> *_data) {
  if (policy != NULL && (t > (learningTimer + learningInterval)) &&
      status == Waiting) {
    assert(status == Waiting);
...
void RL_Agent::learn(const double t, map<string, vector<RL_Agent *>> *_data,
                     string name) {
  if (policy != NULL && (t > (learningTimer + learningInterval)) &&
      status == Ready) {
    assert((*policy) != NULL);
...
#ifdef _RL_VIZ
void RL_Agent::_paintSphere(Real x, Real y, Real radius, Real r, Real g,
                            Real b) const {
...
void RL_Agent::_drawCircle(Real radius, Real xc, Real yc, Real r, Real g,
                           Real b) const {
...
void RL_Agent::_drawFullCircle(Real radius, Real xc, Real yc, Real r, Real g,
                               Real b) const {
  const Real deg2rad = M_PI / 90;
