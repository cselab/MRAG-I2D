 ...
 * Frame 3 -> attached to center of mass, moving w/ COM, rotates to conserve
 * angular momentum Frame 4 -> attached to center of mass, moving w/ COM,
 * rotates to conserve angular momentum, rotated by thetav instantaneously
 */
...
public:
  ...
  public:
    ...
    void computeAngVelWrtCurrent(double &u, double &v, double omega, double x,
                                 double y) const;

    void shiftFromFrame1To2(const double x, const double y, const double u,
                            const double v);
    ...
    void sampleDeformationVel(const Real x_, const Real y_, Real &udef,
                              Real &vdef) const;
  ...
  I2D_ThreeLinkFish(ArgumentParser &parser, Grid<W, B> &grid, const Real _xm,
                    const Real _ym, const Real D, const Real aspectRatio,
  ...
  void printGeneralStuff(const double t, const string filename,
                         vector<double> stuffToPrint);

  Real getModulusMaxVel() {
    return max(shape->D / shape->period1, shape->D / shape->period2);
  }
};
