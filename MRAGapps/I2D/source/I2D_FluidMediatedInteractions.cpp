...
I2D_FluidMediatedInteractions::I2D_FluidMediatedInteractions(const int argc,
                                                             const char **argv)
  ...
  // Set overall system's characteristic length (Every shape must have its own
  ...
  while (true) {
    ...
    for (int i = 0; i < ADAPTFREQ; i++) {
      ...
      if (bUSEOPTIMIZER) {
        ...
        // TODO put next line into if statement as soon as cD is not written to
        // file any more
      ...
      //                        fitness_accel.fitness(floatingObstacle, t, 0.0,
      //                        TEND);
