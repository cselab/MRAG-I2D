 ...
 *
 *	perform_timestep() integrates the penalization term with an implicit
 * Euler, updates u[0-2] and omega[0-2]. compute_dragandstuff() computes the
 * forces using tmp and u[0-2]. Tested!
 *
 *	perform_timestep():
...
public:
  I2D_PenalizationOperator(Grid<W, B> &grid, Real lambda, const Real Uinf[2],
                           bool bAppendToFile = false)
      : grid(grid), lambda(lambda), bAppendToFile(bAppendToFile),
        desired_velocities(NULL) {
    this->Uinf[0] = Uinf[0];
    this->Uinf[1] = Uinf[1];
  }

  void set_desired_velocity(
      const map<int, const VelocityBlock *> *desired_velocities_) {
    this->desired_velocities = desired_velocities_;
  }

  void perform_timestep(Real dt);

  void compute_dragandstuff(Real time, const Real D, const Real cor[2],
                            string filename);
