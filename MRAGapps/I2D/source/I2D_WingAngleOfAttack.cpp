			...
			if(_is_touching(info))
			{
				for(int iy=0; iy<FluidBlock2D::sizeY; iy++)
					for(int ix=0; ix<FluidBlock2D::sizeX; ix++)
					{
						...
						b(ix,iy).tmp = max(I2D_WingAngleOfAttack::mollified_heaviside(-dist,m_epsilon),b(ix,iy).tmp);
					}
			}
		}
	};	
			...
			if(_is_touching(info))
			{
				for(int iy=0; iy<FluidBlock2D::sizeY; iy++)
					for(int ix=0; ix<FluidBlock2D::sizeX; ix++)
					{
						...
						b(ix, iy).tmp = max(I2D_WingAngleOfAttack::mollified_heaviside(r-radius, epsilon), b(ix, iy).tmp);
					}
			}		
		}
	};
...
void I2D_WingAngleOfAttack::characteristic_function()
{
	...
	{
		LinksStuff::FillBlocksRectangle fillRectangle(length, width, epsilon, cm, anglesPlusOne[k]-(angleAttack*M_PI/180.0));
	...
	cm[0] += X;
	cm[1] += Y;
	LinksStuff::FillBlocksRectangle fillRectangle(length, width, epsilon, cm, anglesPlusOne[0]-(angleAttack*M_PI/180.0));
	block_processing.process(vInfo, coll, fillRectangle);
