...
void I2D_VelocitySolverMPI_Mani::_bcast_header() {
  ...
  if (comm_rank == 0) {
    ...
    *ptr = nsource_particles;
    ptr++;
    *ptr = ntotaldest_blocks;
    ptr++;
  ...
  if (comm_rank != 0) {
    ...
    nsource_particles = *ptr;
    ptr++;
    ntotaldest_blocks = *ptr;
    ptr++;
...
I2D_VelocitySolverMPI_Mani::I2D_VelocitySolverMPI_Mani(const int argc,
                                                       const char **argv)
...
void I2D_VelocitySolverMPI_Mani::_compute() {
  assert(coreFMM != NULL);
  coreFMM->solve(theta, 1. / scaling_factor, &mydestinfo.front(),
                 work2node[comm_rank], srcparticles, nsource_particles);
}
...
void I2D_VelocitySolverMPI_Mani::_collect_results() {
  if (comm_rank != 0) {
    ...
    MPI_Isend(mydestblocks, mydestinfo.size() * sizeof(VelocityBlock), MPI_BYTE,
              0, stepid, MPI_COMM_WORLD, &Velocity_MPI::requests.front());
    // printf("slave sent!\n");
    ...
    for (int slave = 1; slave < comm_size; slave++) {
      ...
      MPI_Irecv(alldestblocks[slave], work2node[slave] * sizeof(VelocityBlock),
                MPI_BYTE, slave, stepid, MPI_COMM_WORLD,
                &Velocity_MPI::requests[slave]);
    ...
    for (int c = 1; c < comm_size; c++) {
      ...
      while (!bReceived)
        for (set<int>::iterator it = pending_slaves.begin();
             it != pending_slaves.end(); it++) {
          MPI_Status status;
          int flag = 0;
...
void I2D_VelocitySolverMPI_Mani::_bcast_sourcedata() {
  ...
  const long int particle_bytes =
      nsource_particles * sizeof(VelocitySourceParticle);
  const long int message_bytes =
      particle_bytes + ntotaldest_blocks * blockinfo_bytes;
  ...
  if (comm_rank != 0) {
    ...
    mydestblocks =
        VelocityBlock::allocate(end - start); // new VelocityBlock[end-start];
    ...
    for (int iblock = start; iblock < end; iblock++) {
      ...
      const Real h[3] = {dilate / B::sizeX, dilate / B::sizeY,
                         dilate / B::sizeZ};
    ...
    for (int i = 1; i < comm_size; i++)
      alldestblocks[i] = VelocityBlock::allocate(
          work2node[i]); // new VelocityBlock[work2node[i]];
  }
...
void I2D_VelocitySolverMPI_Mani::_master_update_blocks(int rank) {
  ...
  tbb::parallel_for(blocked_range<int>(0, work2node[rank]), update,
                    auto_partitioner());
}
...
void I2D_VelocitySolverMPI_Mani::compute_velocity() {
  ...
  if (slaves > 0) {
    ...
    slave_remainder_work =
        vInfo.size() - master_work - slaves * slave_homogenous_work;
  } else {
