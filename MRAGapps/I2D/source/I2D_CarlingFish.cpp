...
I2D_CarlingFish::Fish::Fish(double xm, double ym, double _D, double _T,
                            double phase, double angle_rad,
                            double angleInSpace_rad, double eps, const int LMAX,
                            const bool isSharp)
    : angle(angle_rad), angleInSpace(angleInSpace_rad), D(_D), xm(xm), ym(ym),
      phase(phase), angular_velocity(0), vx(0), vy(0), vdefx(0), vdefy(0), N(0),
      SIZEX(0), SIZEY(0), MAPSIZEX(0), MAPSIZEY(0), m(1.0), J(1.0), WH(0),
      SB(0), WT(0), ST(0), T(_T), EXTENSION(4.0), EPS(0), DS(0), traslX(0.0),
      traslY(0.0), angular_velocityInSpace(0.0), TOrig(_T), LMAX(LMAX),
      SHARP(isSharp) {
  ...
  if (MAPSIZEX % 2 != 0) {
    printf("MAPSIZEX not even!!\n");
    abort();
  }
  if (MAPSIZEY % 2 != 0) {
    printf("MAPSIZEY not even!!\n");
    abort();
  }
...
double I2D_CarlingFish::Fish::mollified_heaviside(const double dist,
                                                  const double eps) {
...
void I2D_CarlingFish::Fish::_rotateAboutTheOriginSingle(const double &theta,
                                                        double &x,
                                                        double &y) const {
...
double I2D_CarlingFish::Fish::_sample(const double x, const double y) const {
  ...
  const double wx[4] = {_bsp4(xr + 1), _bsp4(xr + 0), _bsp4(xr - 1),
                        _bsp4(xr - 2)};
  const double wy[4] = {_bsp4(yr + 1), _bsp4(yr + 0), _bsp4(yr - 1),
                        _bsp4(yr - 2)};
...
void I2D_CarlingFish::Fish::_sample(const double x, const double y, double &Xs,
                                    double &defvelx, double &defvely) const {
  Xs = 0.0;
  defvelx = 0.0;
  defvely = 0.0;
...
double I2D_CarlingFish::Fish::_sample(const double x, const double y,
  ...
  for (int idx = -1; idx < 2; idx++) {
    ...
    const double wx_chi[2] = {_bsp2(my_x - floor(my_x)),
                              _bsp2(my_x - floor(my_x) - 1)};
    const double wy_chi[2] = {_bsp2(my_y - floor(my_y)),
                              _bsp2(my_y - floor(my_y) - 1)};
...
double I2D_CarlingFish::Fish::sample(const double x_, const double y_,
                                     const double h_) const {
  ...
  if (xobject[0] < myh || xobject[0] > 1.0)
    return 0;
  if (xobject[1] < myh || xobject[1] > 1.0)
    return 0;
  return _sample(xobject[0] * (MAPSIZEX - 1), xobject[1] * (MAPSIZEY - 1), h_);
}

void I2D_CarlingFish::Fish::sample(const double x_, const double y_, double &Xs,
                                   double &defvelx, double &defvely,
                                   const double h_) const {
  Xs = 0.0;
  defvelx = 0.0;
  ...
  if (xobject[1] < myh || xobject[1] > 1.0)
    return;
  _sample(xobject[0] * (MAPSIZEX - 1), xobject[1] * (MAPSIZEY - 1), Xs, defvelx,
          defvely, h_);
...
void I2D_CarlingFish::Fish::_cubicInterpolation(double t0, double t1, double t,
                                                double k0, double k1, double &k,
                                                double &dkdt) {
  k = 0.0;
  dkdt = 0.0;
...
void I2D_CarlingFish::Fish::updateInSpace(const double t) {
  ...
  _calculateNormalsAndVelocities(t, T, DS, S, X, Y, VX, VY, NORX, NORY, VNORX,
                                 VNORY, N);
  ...
  if ((fabs(hL) > 1e-10) || (fabs(hCMX) > 1e-10) || (fabs(hCMY) > 1e-10) ||
      (fabs(hVCMX) > 1e-10) || (fabs(hVCMY) > 1e-10)) {
...
void I2D_CarlingFish::Fish::_calculateCenterlineHead(
    const double t, const double factor, const double T, const double ds,
    const double *ss, double *rX, double *rY, const int nn) {
  double dxx[nn - 1];
  double dyy[nn - 1];
  ...
  const double ll = 1.0;
  for (int i = 0; i < nn; i++) {
    rY[i] = factor * 0.125 * ll * ((ss[i] + offset) / ll + 0.03125) / 1.03125 *
            sin(2.0 * M_PI * ((ss[i] + offset) / ll - t / T) + phase);
  }
  ...
  for (int i = 0; i < nn - 1; i++) {
#ifndef NDEBUG
    if ((ds * ds - dyy[i] * dyy[i]) <= 0.0) {
      std::cout << ds << "  " << dyy[i] << " orca troia lo sapevo" << std::endl;
    }
#endif
  ...
  double length = 0.0;
  for (int i = 1; i < nn; i++) {
    length += sqrt((rX[i] - rX[i - 1]) * (rX[i] - rX[i - 1]) +
                   (rY[i] - rY[i - 1]) * (rY[i] - rY[i - 1]));
  }
  printf("length: %e, %e", length, ll);
#endif
}

void I2D_CarlingFish::Fish::_calculateNormalsAndVelocities(
    const double t, const double TT, const double ds, const double *ss,
    const double *rX, const double *rY, double *vX, double *vY, double *norX,
    double *norY, double *vNorX, double *vNorY, const int nn) {
...
void I2D_CarlingFish::Fish::_createShapeBoundary(
    const double *rX, const double *rY, const double *width, const double *norX,
    const double *norY, double *shapeX, double *shapeY, const int n) {
  for (int i = 0; i < n; i++) {
...
void I2D_CarlingFish::Fish::_getDefGridCoord(const int &ix, const int &iy,
                                             const double &dg, const int &coeff,
                                             const double *rX, const double *rY,
                                             const double *norX,
                                             const double *norY, double &xCoord,
                                             double &yCoord) const {
  xCoord = rX[coeff * ix] + norX[coeff * ix] * iy * dg;
  yCoord = rY[coeff * ix] + norY[coeff * ix] * iy * dg;
}

void I2D_CarlingFish::Fish::_getDefGridVel(const int &ix, const int &iy,
                                           const double &dg, const int &coeff,
                                           const double *vX, const double *vY,
                                           const double *vNorX,
                                           const double *vNorY, double &vxCoord,
                                           double &vyCoord) const {
  vxCoord = vX[coeff * ix] + vNorX[coeff * ix] * iy * dg;
  vyCoord = vY[coeff * ix] + vNorY[coeff * ix] * iy * dg;
}

void I2D_CarlingFish::Fish::_computeMinStartMaxEnd(const double *width,
                                                   int &_minStart,
                                                   int &_maxEnd) {
  _minStart = 0;
  for (int i = 0; i < N; i++) {
...
#pragma omp parallel for reduction(+ : M, meanVx, meanVy, meanX, meanY)        \
    schedule(static, 1)
...
#pragma omp parallel for
  for (int ix = 0; ix < MAPSIZEX; ix++)
    for (int iy = 0; iy < MAPSIZEY; iy++) {
      ...
      double vRotX = 0.0;
      double vRotY = 0.0;
      _getRotationalVelocityAboutTheOrigin(omega, (xx - xxcm), (yy - yycm),
                                           vRotX, vRotY);
      VDEFX[idx] += vRotX;
      VDEFY[idx] += vRotY;
    }
...
#ifndef NDEBUG
  lcum = IIcum = M = meanVx = meanVy = meanX = meanY = 0.0;
#pragma omp parallel for reduction(+ : M, meanVx, meanVy, lcum, IIcum)         \
    schedule(static, 1)
  for (int ix = 0; ix < MAPSIZEX; ix++)
    for (int iy = 0; iy < MAPSIZEY; iy++) {
      const double xx = (double)ix * H;
  ...
  if ((fabs(meanVxAfter) > 1e-10) || (fabs(meanVyAfter) > 1e-10) ||
      (fabs(omegaAfter) > 1e-10)) {
    printf("translational or rotational velocities fucked up in uniform shape "
           "representation!\n");
    abort();
  }
#endif
...
void I2D_CarlingFish::Fish::_getDefGridCharFunc(
    const int &ix, const int &iy, const int &spany, const double &dg,
    const int &coeff, const double *rX, const double *rY, const double *width,
    double &sdf) const {
  const double x = dataX[_dd2l(ix, iy)];
  const double y = dataY[_dd2l(ix, iy)];
  const double signInOne = 1.0;
  const double signInRight =
      -(width[coeff * ix] - fabs((double)(iy - spany) * dg)) /
      fabs(width[coeff * ix] - fabs((double)(iy - spany) * dg));
  ...
  if ((iy - spany) >= 0) {
    for (int i = start; i <= end; i++) {
      const double d = sqrt((x - SHAPEX[i]) * (x - SHAPEX[i]) +
                            (y - SHAPEY[i]) * (y - SHAPEY[i]));
      minDist = min(d, minDist);
    }
  } else {
    for (int i = start; i <= end; i++) {
      const double d =
          sqrt((x - SHAPEX[2 * N - 1 - i]) * (x - SHAPEX[2 * N - 1 - i]) +
               (y - SHAPEY[2 * N - 1 - i]) * (y - SHAPEY[2 * N - 1 - i]));
      minDist = min(d, minDist);
    }
  }
...
#pragma omp parallel for
  for (int ix = 0; ix < SIZEX; ix++)
    for (int iy = 0; iy < SIZEY; iy++) {
      dataDist[_dd2l(ix, iy)] =
          mollified_heaviside(dataDist[_dd2l(ix, iy)], EPS);
    }
}
...
void I2D_CarlingFish::Fish::_cross(const double *v1, const double *v2,
                                   double *v3) const {
  v3[0] = v1[0] * v2[1];
  v3[1] = -v1[1] * v2[0];
}
...
void I2D_CarlingFish::Fish::_boundingBoxTriangle(
    const double *a, const double *b, const double *c, const double H,
    int &ixMax, int &ixMin, int &iyMax, int &iyMin) const {
...
bool I2D_CarlingFish::Fish::_pointInTriangle(const double *p, const double *a,
                                             const double *b,
...
bool I2D_CarlingFish::Fish::_bilinearTriangle(const double *p, const double *a,
...
void I2D_CarlingFish::Fish::_getCenterOfMassFull(double &xCoord,
                                                 double &yCoord) const {
  double M = 0.0;
  double xCum = 0.0;
  double yCum = 0.0;
...
void I2D_CarlingFish::Fish::_getVelCenterOfMassFull(double &vxCoord,
                                                    double &vyCoord) const {
  double M = 0.0;
  double vxCum = 0.0;
  double vyCum = 0.0;
...
void I2D_CarlingFish::Fish::_centerlineCenterOfMassFrameTransform(
    const double &cmX, const double &cmY, const double &vcmX,
    const double &vcmY) {
  for (int i = 0; i < N; i++) {
    X[i] -= cmX;
    Y[i] -= cmY;
...
void I2D_CarlingFish::Fish::_defGridCenterOfMassFrameTransform(
    const double &cmX, const double &cmY, const double &vcmX,
    const double &vcmY) {
#pragma omp parallel for
  for (int ix = 0; ix < SIZEX; ix++)
...
void I2D_CarlingFish::Fish::_getRotationalVelocityAboutTheOrigin(
    const double &omega, const double &rX, const double &rY, double &vRotX,
    double &vRotY) const {
  vRotX = -omega * rY;
  vRotY = omega * rX;
}

void I2D_CarlingFish::Fish::_correctCenterlineForRotationalImpulse(
    const double &omega) {
  double vRotX = 0.0;
  double vRotY = 0.0;
  for (int i = 0; i < N; i++) {
...
void I2D_CarlingFish::Fish::_correctDefGridForRotationalImpulse(
    const double &omega) {
#pragma omp parallel for
  for (int ix = 0; ix < SIZEX; ix++)
    for (int iy = 0; iy < SIZEY; iy++) {
      ...
      double vRotX = 0.0;
      double vRotY = 0.0;
      _getRotationalVelocityAboutTheOrigin(omega, dataX[idx], dataY[idx], vRotX,
                                           vRotY);
      dataVX[idx] += vRotX;
      dataVY[idx] += vRotY;
    }
...
void I2D_CarlingFish::Fish::_rotateAboutTheOrigin(const double &theta,
                                                  double *x, double *y,
                                                  const int n) {
  const double a00 = cos(theta);
  const double a01 = -sin(theta);
...
#pragma omp parallel for
  for (int ixD = sX; ixD < eX; ixD++)
    for (int iyD = sY; iyD < eY; iyD++) {
      ...
      const double va[3] = {dataVX[linIdx1], dataVY[linIdx1],
                            dataDist[linIdx1]};
      const double va2[3] = {dataVX[linIdx1], dataVY[linIdx1],
                             dataSDF[linIdx1]};
      ...
      const double vb[3] = {dataVX[linIdx2], dataVY[linIdx2],
                            dataDist[linIdx2]};
      const double vb2[3] = {dataVX[linIdx2], dataVY[linIdx2],
                             dataSDF[linIdx2]};
      ...
      const double vc[3] = {dataVX[linIdx3], dataVY[linIdx3],
                            dataDist[linIdx3]};
      const double vc2[3] = {dataVX[linIdx3], dataVY[linIdx3],
                             dataSDF[linIdx3]};
      ...
      for (int iy = iyMin; iy < iyMax; iy++)
        for (int ix = ixMin; ix < ixMax; ix++) {
          ...
          const bool isIn2 =
              _bilinearTriangle(P, A, B, C, va2, vb2, vc2, interp2);
          if (isIn) {
      ...
      const double vaa[3] = {dataVX[linIdx21], dataVY[linIdx21],
                             dataDist[linIdx21]};
      const double vaa2[3] = {dataVX[linIdx21], dataVY[linIdx21],
                              dataSDF[linIdx21]};
      ...
      const double vbb[3] = {dataVX[linIdx22], dataVY[linIdx22],
                             dataDist[linIdx22]};
      const double vbb2[3] = {dataVX[linIdx22], dataVY[linIdx22],
                              dataSDF[linIdx22]};
      ...
      const double vcc[3] = {dataVX[linIdx23], dataVY[linIdx23],
                             dataDist[linIdx23]};
      const double vcc2[3] = {dataVX[linIdx23], dataVY[linIdx23],
                              dataSDF[linIdx23]};
      ...
      for (int iy = iyMin2; iy < iyMax2; iy++)
        for (int ix = ixMin2; ix < ixMax2; ix++) {
          ...
          double interp[3] = {0.0, 0.0, 0.0};
          const bool isIn =
              _bilinearTriangle(P, AA, BB, CC, vaa, vbb, vcc, interp);
          double interp2[3] = {0.0, 0.0, 0.0};
          const bool isIn2 =
              _bilinearTriangle(P, AA, BB, CC, vaa2, vbb2, vcc2, interp2);
          if (isIn) {
            const int lIdx = _ud2l(ix, iy);
            VDEFX[lIdx] = interp[0];
            VDEFY[lIdx] = interp[1];
...
void I2D_CarlingFish::Fish::_bbox(void) {
  ...
  fish_xmin[0] =
      min((Real)min((Real)min((Real)xw1[0], (Real)xw2[0]), (Real)xw3[0]),
          (Real)xw4[0]);
  fish_xmin[1] =
      min((Real)min((Real)min((Real)xw1[1], (Real)xw2[1]), (Real)xw3[1]),
          (Real)xw4[1]);
  fish_xmax[0] =
      max((Real)max((Real)max((Real)xw1[0], (Real)xw2[0]), (Real)xw3[0]),
          (Real)xw4[0]);
  fish_xmax[1] =
      max((Real)max((Real)max((Real)xw1[1], (Real)xw2[1]), (Real)xw3[1]),
          (Real)xw4[1]);
...
void I2D_CarlingFish::Fish::_bboxShapeBoundary(const double *shapeX,
                                               const double *shapeY,
                                               const int n, double &width,
                                               double &height) {
  double minX = std::numeric_limits<double>::max();
  double maxX = -std::numeric_limits<double>::max();
  double minY = std::numeric_limits<double>::max();
...
void I2D_CarlingFish::Fish::bbox(const double eps, double xmin[2],
                                 double xmax[2]) const {
  xmin[0] = fish_xmin[0];
  xmin[1] = fish_xmin[1];
  xmax[0] = fish_xmax[0];
...
struct FillBlocks {
  ...
  FillBlocks(double eps, I2D_CarlingFish::Fish *_shape) : eps(eps) {
    shape = _shape;
  }

  FillBlocks(const FillBlocks &c) : eps(c.eps) { shape = c.shape; }

  static bool _is_touching(double eps, const I2D_CarlingFish::Fish *fish,
    ...
    double intersection[2][2] = {
        max(min_pos[0], bbox[0][0]),
        min(max_pos[0], bbox[1][0]),
        max(min_pos[1], bbox[0][1]),
        min(max_pos[1], bbox[1][1]),
    };

    return intersection[0][1] - intersection[0][0] > 0 &&
...
struct FillBlocksTowers {
  ...
  FillBlocksTowers(double eps, I2D_CarlingFish::Fish *_shape) : eps(eps) {
    shape = _shape;
  }
    ...
    if (FillBlocks::_is_touching(eps, shape, info)) {
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
          ...
          info.pos(p, ix, iy);
          b(ix, iy).tmp =
              max((Real)shape->sample(p[0], p[1], info.h[0]), b(ix, iy).tmp);
        }
...
struct DivergenceOp : I2D_GradOfVector_4thOrder {
  ...
  struct TmpAdd {
    static inline void stream(FluidElement2D &out, Real in) { out.tmp += in; }
  };

  template <typename Lab>
  inline void operator()(Lab &lab, const BlockInfo &info,
                         FluidBlock2D &out) const {
    if (FillBlocks::_is_touching(eps, shape, info)) {
      // Make my little copy :)
      ...
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
          ...
          double p[2];
          info.pos(p, ix, iy);
          const double Xs = shape->SHARP ? shape->sample(p[0], p[1], info.h[0])
                                         : shape->sample(p[0], p[1]);
...
struct ComputeDivergenceDefVel {
  ...
  ComputeDivergenceDefVel(double xcm, double ycm, double vxcorr, double vycorr,
                          double omegacorr, double eps,
                          I2D_CarlingFish::Fish *_shape)
      : xcm(xcm), ycm(ycm), vxcorr(vxcorr), vycorr(vycorr),
        omegacorr(omegacorr), eps(eps) {
    shape = _shape;
  }

  ComputeDivergenceDefVel(const ComputeDivergenceDefVel &c)
      : xcm(c.xcm), ycm(c.ycm), vxcorr(c.vxcorr), vycorr(c.vycorr),
...
struct ComputeCenterOfMass {
  ...
  ComputeCenterOfMass(double eps, I2D_CarlingFish::Fish *_shape,
                      map<int, vector<double>> &b2sum, Real Uinf[2],
                      map<int, bool> &b2nonempty)
      : eps(eps), b2sum(b2sum), b2nonempty(b2nonempty) {
    this->Uinf[0] = Uinf[0];
    this->Uinf[1] = Uinf[1];
  ...
  ComputeCenterOfMass(const ComputeCenterOfMass &c)
      : eps(c.eps), b2sum(c.b2sum), b2nonempty(c.b2nonempty) {
    Uinf[0] = c.Uinf[0];
    Uinf[1] = c.Uinf[1];
...
struct ComputeAllCorrections {
  ...
  ComputeAllCorrections(double xcm, double ycm, double eps,
                        I2D_CarlingFish::Fish *_shape,
                        map<int, vector<double>> &b2sum, Real Uinf[2],
                        map<int, bool> &b2nonempty)
      : xcm(xcm), ycm(ycm), eps(eps), b2sum(b2sum), b2nonempty(b2nonempty) {
    this->Uinf[0] = Uinf[0];
    this->Uinf[1] = Uinf[1];
  ...
  ComputeAllCorrections(const ComputeAllCorrections &c)
      : xcm(c.xcm), ycm(c.ycm), eps(c.eps), b2sum(c.b2sum),
        b2nonempty(c.b2nonempty) {
    Uinf[0] = c.Uinf[0];
    Uinf[1] = c.Uinf[1];
...
struct ComputeAll {
  ...
  ComputeAll(double xcm, double ycm, double eps, I2D_CarlingFish::Fish *_shape,
             map<int, vector<double>> &b2sum, Real Uinf[2],
             map<int, bool> &b2nonempty)
      : xcm(xcm), ycm(ycm), eps(eps), b2sum(b2sum), b2nonempty(b2nonempty) {
    this->Uinf[0] = Uinf[0];
    ...
    if (FillBlocks::_is_touching(eps, shape, info)) {
      ...
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
          ...
          omegabar += Xs * ((b(ix, iy).u[1] + Uinf[1]) * (p[0] - xcm) -
                            (b(ix, iy).u[0] + Uinf[0]) * (p[1] - ycm));
        }
...
struct FillVelblocks {
  ...
  FillVelblocks(vector<pair<BlockInfo, VelocityBlock *>> &workitems, double xcm,
                double ycm, double vxcorr, double vycorr, double omegacorr,
                double eps, I2D_CarlingFish::Fish *_shape)
      : workitems(workitems), xcm(xcm), ycm(ycm), vxcorr(vxcorr),
        vycorr(vycorr), omegacorr(omegacorr), eps(eps) {
    shape = _shape;
  }

  FillVelblocks(const FillVelblocks &c)
      : workitems(c.workitems), xcm(c.xcm), ycm(c.ycm), vxcorr(c.vxcorr),
        vycorr(c.vycorr), omegacorr(c.omegacorr), eps(c.eps) {
    shape = c.shape;
  }
...
I2D_CarlingFish::I2D_CarlingFish(ArgumentParser &parser, Grid<W, B> &grid,
                                 const Real _xm, const Real _ym, const Real D,
                                 const Real T, const Real phase,
                                 const Real angle, const Real angleInSpace,
                                 const Real eps, const Real Uinf[2],
                                 I2D_PenalizationOperator &penalization,
                                 const int LMAX, const int ID,
                                 RL::RL_TabularPolicy **policy, const int seed)
    : I2D_FloatingObstacleOperator(parser, grid, D, eps, Uinf, penalization, ID,
                                   policy, seed),
  ...
  shape =
      new Fish(_xm, _ym, D, T, phase, angle, angleInSpace, eps, LMAX, isSharp);
...
void I2D_CarlingFish::refresh(const double t, string filename) {
  ...
  for (int i = 0; i < N; i++) {
    fprintf(f, "%e %e %e %e %e %e %e %e %e\n", dataVect[i][0], dataVect[i][1],
            dataVect[i][2], dataVect[i][3], dataVect[i][4], dataVect[i][5],
            dataVect[i][6], dataVect[i][7], dataVect[i][8]);
  }
  fclose(f);
}
...
void I2D_CarlingFish::create(const double t) {
  ...
  for (vector<BlockInfo>::const_iterator it = vInfo.begin(); it != vInfo.end();
       it++)
  ...
  FloatingFish::ComputeCenterOfMass computeCenterOfMass(eps, shape, integrals,
                                                        Uinf, nonempty);
  block_processing.process(vInfo, coll, computeCenterOfMass);
  for (map<int, vector<double>>::const_iterator it = integrals.begin();
       it != integrals.end(); ++it) {
    mass += (it->second)[0];
    xbar += (it->second)[1];
    ybar += (it->second)[2];
  ...
  mass = J = xbar = ybar = vxbar = vybar = omegabar = 0.0;
  FloatingFish::ComputeAllCorrections computeAllCorrections(
      xm_corr, ym_corr, eps, shape, integrals, Uinf, nonempty);
  ...
  // Interpolate corrected deformation velocities on computational grid (no
  // masking with Xs)
  FloatingFish::ComputeDivergenceDefVel computeDivergenceDefVel(
      xm_corr, ym_corr, vx_corr, vy_corr, omega_corr, eps, shape);
  ...
  block_processing.process<I2D_VectorBlockLab<Streamer_Velocity, 2>::Lab>(
      vInfo, coll, binfo, div);
}
...
void I2D_CarlingFish::computeDesiredVelocity(const double t) {
  ...
  mass = J = xbar = ybar = vxbar = vybar = omegabar = 0.0;
  FloatingFish::ComputeAll computeAll(xm_corr, ym_corr, eps, shape, integrals,
                                      Uinf, nonempty);
  ...
  for (map<int, const VelocityBlock *>::iterator it = desired_velocity.begin();
       it != desired_velocity.end(); it++) {
  ...
  FloatingFish::FillVelblocks fillvelblocks(
      velblocks, xm_corr, ym_corr, vx_corr, vy_corr, omega_corr, eps, shape);
  tbb::parallel_for(blocked_range<int>(0, velblocks.size()), fillvelblocks,
                    auto_partitioner());
}

void I2D_CarlingFish::update(
    const double dt, const double t, string filename,
    map<string, vector<I2D_FloatingObstacleOperator *>> *_data) {
  // If string is set I open the corresponding file
  assert(filename != std::string());
  FILE *ppFile = NULL;
  ...
  assert(ppFile != NULL);
  fprintf(ppFile,
          "%20.10e %20.10e %20.10e %20.10e %20.10e %20.10e %20.10e %20.10e "
          "%20.10e\n",
          t, shape->xm, shape->ym, shape->vx, shape->vy, shape->angle,
          shape->angular_velocity, shape->angleInSpace, shape->m);
  fflush(ppFile);
  fclose(ppFile);
