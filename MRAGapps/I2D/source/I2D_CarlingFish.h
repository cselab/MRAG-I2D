...
public:
  ...
  protected:
    ...
    void _sample(const double x, const double y, double &Xs, double &defvelx,
                 double &defvely, const double h) const;
    double _sample(const double x, const double y) const;
    void _sample(const double x, const double y, double &Xs, double &defvelx,
                 double &defvely) const;
    virtual double _getWidth(const double &ss) const;
    void _calculateCenterlineHead(const double t, const double factor,
                                  const double T, const double ds,
                                  const double *ss, double *rX, double *rY,
                                  const int nn);
    void _calculateNormalsAndVelocities(const double t, const double T,
                                        const double ds, const double *ss,
                                        const double *rX, const double *rY,
                                        double *vX, double *vY, double *norX,
                                        double *norY, double *vNorX,
                                        double *vNorY, const int nn);
    void _createShapeBoundary(const double *rX, const double *rY,
                              const double *width, const double *norX,
                              const double *norY, double *shapeX,
                              double *shapeY, const int n);
    void _getDefGridCoord(const int &ix, const int &iy, const double &dg,
                          const int &coeff, const double *rX, const double *rY,
                          const double *norX, const double *norY,
                          double &xCoord, double &yCoord) const;
    void _getDefGridVel(const int &ix, const int &iy, const double &dg,
                        const int &coeff, const double *vX, const double *vY,
                        const double *vNorX, const double *vNorY,
                        double &vxCoord, double &vyCoord) const;
    void _getDefGridCharFunc(const int &ix, const int &iy, const int &spany,
                             const double &dg, const int &coeff,
                             const double *rX, const double *rY,
                             const double *width, double &sdf) const;
    void _fillDefGrid();
    ...
    void _centerlineCenterOfMassFrameTransform(const double &cmX,
                                               const double &cmY,
                                               const double &vcmX,
                                               const double &vcmY);
    void _defGridCenterOfMassFrameTransform(const double &cmX,
                                            const double &cmY,
                                            const double &vcmX,
                                            const double &vcmY);
    void _getRotationalVelocityAboutTheOrigin(const double &omega,
                                              const double &rX,
                                              const double &rY, double &vRotX,
                                              double &vRotY) const;
    ...
    void _rotateAboutTheOrigin(const double &theta, double *x, double *y,
                               const int n);
    void _rotateDefGridAboutTheOrigin(const double &theta);
    void _bbox();
    void _bboxShapeBoundary(const double *shapeX, const double *shapeY,
                            const int n, double &width, double &height);
    void _computeMinStartMaxEnd(const double *width, int &_minStart,
                                int &_maxEnd);
    void _getUniformGridProperties();
    void _rotateAboutTheOriginSingle(const double &theta, double &x,
                                     double &y) const;
    void _cubicInterpolation(double t0, double t1, double t, double k0,
                             double k1, double &k, double &dkdt);

    void _cross(const double *v1, const double *v2, double *v3) const;
    double _dot(const double *v1, const double *v2) const;
    void _boundingBoxTriangle(const double *a, const double *b, const double *c,
                              const double H, int &ixMax, int &ixMin,
                              int &iyMax, int &iyMin) const;
    bool _pointInTriangle(const double *p, const double *a, const double *b,
                          const double *c) const;
    bool _bilinearTriangle(const double *p, const double *a, const double *b,
                           const double *c, const double *va, const double *vb,
                           const double *vc, double *interp) const;

  public:
    const double TOrig;
    int SIZEX, SIZEY, MAPSIZEX, MAPSIZEY, N;
    double WH, SB, WT, ST, T, DS, EXTENSION, EPS;
    double angle, angleInSpace, xm, ym, D, angular_velocity,
        angular_velocityInSpace, vx, vy, m, J, vdefx, vdefy, phase, tau, traslX,
        traslY;
    int LMAX;
    bool SHARP;

    Fish(double xm, double ym, double _D, double _T, double phase,
         double angle_rad, double angleInSpace_rad, double eps, const int LMAX,
         const bool isSharp = false);
    virtual ~Fish();
    ...
    void sample(const double x_, const double y_, double &Xs, double &defvelx,
                double &defvely, const double h_) const;
  ...
  I2D_CarlingFish(ArgumentParser &parser, Grid<W, B> &grid, const Real _xm,
                  const Real _ym, const Real D, const Real T, const Real phase,
                  const Real angle, const Real angleInSpace, const Real eps,
                  const Real Uinf[2], I2D_PenalizationOperator &penalization,
                  const int LMAX, const int ID = 0,
                  RL::RL_TabularPolicy **policy = NULL, const int seed = 0);
