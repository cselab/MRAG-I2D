...
I2D_RotatingCylinderPair::I2D_RotatingCylinderPair(const int argc,
                                                   const char **argv)
    : I2D_FlowPastFloatingObstacle(argc, argv) {
  ...
  // ok fuck it we fake one
  {
    ...
    I2D_FloatingRotatingCylinderPair *object =
        new I2D_FloatingRotatingCylinderPair(parser, *grid, XPOS, YPOS, D,
                                             angle, width, g1, g2, epsilon,
                                             Uinf, *penalization, LMAX);

    shapesArray["I2D_FloatingRotatingCylinderPair"].push_back(object);
    floatingObstacle = new I2D_FloatingObstacleVector(
        parser, *grid, epsilon, Uinf, *penalization, shapesArray, charLength,
        charVel);
  }
...
void I2D_RotatingCylinderPair::_tnext(double &tnext, double &tnext_dump,
                                      double &tend) {
  // 0. setup
...

  {
    ...
    const Real dt_max =
        LCFL / (2.0 * OMEGA); // final dt = LCFL / max_omega, where max_omega
                              // = 2.0*OMEGA (rad/s)
    ...
    const double dtramp =
        (step_id >= nsteps)
            ? 1000.0
            : dt_min + (Real)(step_id) / (Real)(nsteps) * (dt_max - dt_min);
  ...
  // 2.
  {
    ...
    fprintf(f, "stepid=%d\tT=%e\tDT=%e\t", (int)step_id, tnext / TIMESCALE,
            (tnext - t) / TIMESCALE);
    for (int i = 0; i < (int)tnext_candidates.size(); i++)
