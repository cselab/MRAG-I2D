...
struct Penalization {
  ...
  Penalization(Real dt, Real lambda, const Real Uinf[2])
      : lambda(lambda), dt(dt) {
    this->Uinf[0] = Uinf[0];
    this->Uinf[1] = Uinf[1];
  }
  ...
  inline void operator()(const BlockInfo &info, FluidBlock2D &b) const {
    ...
    for (int i = 0; i < n; i++) {
      ...
      e[i].u[0] =
          (u_current[0] - Uinf[0] * lamdtX) / (1 + lamdtX) - u_current[0];
      e[i].u[1] =
          (u_current[1] - Uinf[1] * lamdtX) / (1 + lamdtX) - u_current[1];
    }
  }
};
...
struct PenalizationCustomizedVelocity {
  ...
  PenalizationCustomizedVelocity(
      Real dt, Real lambda,
      const map<int, const VelocityBlock *> &customized_velocity,
      const Real Uinf[2])
      : lambda(lambda), dt(dt), customized_velocity(customized_velocity) {
    this->Uinf[0] = Uinf[0];
    this->Uinf[1] = Uinf[1];
  }

  PenalizationCustomizedVelocity(const PenalizationCustomizedVelocity &c)
      : lambda(c.lambda), dt(c.dt), customized_velocity(c.customized_velocity) {
  ...
  inline void operator()(const BlockInfo &info, FluidBlock2D &b) const {
    ...
    const map<int, const VelocityBlock *>::const_iterator it =
        customized_velocity.find(info.blockID);
    ...
    if (it != customized_velocity.end()) {
      ...
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
          ...
          e[i].u[0] =
              (u_current[0] + (u_desired->u[0][iy][ix] - Uinf[0]) * lamdtX) /
                  (1 + lamdtX) -
              u_current[0];
          e[i].u[1] =
              (u_current[1] + (u_desired->u[1][iy][ix] - Uinf[1]) * lamdtX) /
                  (1 + lamdtX) -
              u_current[1];
        }
    } else {
...
void I2D_PenalizationOperator::perform_timestep(Real dt) {
  ...
  if (desired_velocities == NULL) {
    ...
    PenalizationCustomizedVelocity penalization(dt, lambda, *desired_velocities,
                                                Uinf);
    block_processing.process(vInfo, coll, penalization);
...
struct ComputeDiagnostics {
  ...
  ComputeDiagnostics(map<int, Diagnostic> &diagnostics, Real lambda,
                     const Real Uinf[2], const Real cor[2])
      : diagnostics(diagnostics), lambda(lambda) {
    this->Uinf[0] = Uinf[0];
    this->Uinf[1] = Uinf[1];
    this->cor[0] = cor[0];
    this->cor[1] = cor[1];
  ...
  ComputeDiagnostics(const ComputeDiagnostics &c)
      : diagnostics(c.diagnostics), lambda(c.lambda) {
    Uinf[0] = c.Uinf[0];
    Uinf[1] = c.Uinf[1];
    cor[0] = c.cor[0];
    cor[1] = c.cor[1];
...
void I2D_PenalizationOperator::compute_dragandstuff(Real time, const Real D,
                                                    const Real cor[2],
                                                    string filename) {
  ...
  Diagnostic global;
  for (map<int, Diagnostic>::iterator it = diagnostics.begin();
  ...
  assert(f != NULL);
  if (!bAppendToFile)
    fprintf(f, "T\t\tcD\t\tcL\t\tCm\t\tcirculation\t\tArea\t\tpoints\t\tN-"
               "obstcl-1D\n");

  fprintf(f, "%e\t%e\t%e\t%e\t%e\t%e\t%d\t%f\n", T, cD, cL, Cm, Circulation,
          Area, totalNumPoints, pow((float)GridPoints, (float)(1. / 2.)));

  fclose(f);

  printf("%e\t%e\t%e\t%e\t%e\t%e\t%d\t%f\n", T, cD, cL, Cm, Circulation, Area,
         totalNumPoints, pow((float)GridPoints, (float)(1. / 2.)));
