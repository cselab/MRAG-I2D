...
struct Solver_DiffusionRHS_4thOrder : I2D_DivGradOfScalar_4thOrder {
  ...
  Solver_DiffusionRHS_4thOrder(const Solver_DiffusionRHS_4thOrder &copy)
      : viscosity(copy.viscosity), t(0) {
    stencil_start[0] = stencil_start[1] = -2;
    stencil_end[0] = stencil_end[1] = +3;
    stencil_start[2] = 0;
    stencil_end[2] = 1;
  ...
  inline void operator()(Lab &lab, const BlockInfo &info,
    ...
    for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
      for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
        dwdx[0] = -2.0 * lab(ix - 3, iy) + 15.0 * lab(ix - 2, iy) -
                  60 * lab(ix - 1, iy) + 20.0 * lab(ix, iy) +
                  30.0 * lab(ix + 1, iy) - 3.0 * lab(ix + 2, iy);
        dwdx[1] = 2.0 * lab(ix + 3, iy) - 15.0 * lab(ix + 2, iy) +
                  60 * lab(ix + 1, iy) - 20.0 * lab(ix, iy) -
                  30.0 * lab(ix - 1, iy) + 3.0 * lab(ix - 2, iy);

        dwdy[0] = -2.0 * lab(ix, iy - 3) + 15.0 * lab(ix, iy - 2) -
                  60 * lab(ix, iy - 1) + 20.0 * lab(ix, iy) +
                  30.0 * lab(ix, iy + 1) - 3.0 * lab(ix, iy + 2);
        dwdy[1] = 2.0 * lab(ix, iy + 3) - 15.0 * lab(ix, iy + 2) +
                  60 * lab(ix, iy + 1) - 20.0 * lab(ix, iy) -
                  30.0 * lab(ix, iy - 1) + 3.0 * lab(ix, iy - 2);
    ...
    if (info.index[0] == 0) {
      ...
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < 3; ix++) {
          ...
          dwdx[0] = 20.0 * lab(ix, iy) + 30.0 * lab(ix + 1, iy) -
                    3.0 * lab(ix + 2, iy);
          dwdx[1] = 2.0 * lab(ix + 3, iy) - 15.0 * lab(ix + 2, iy) +
                    60 * lab(ix + 1, iy) - 20.0 * lab(ix, iy);
...
void I2D_SolverOperator_4thOrder::perform_timestep(double dt) {
  ...
  block_processing.process<I2D_ScalarBlockLab<Streamer_Omega>::Lab>(
      vInfo, coll, binfo, diff);
  block_processing.process<I2D_ScalarBlockLab<Streamer_Omega>::Lab>(
      vInfo, coll, binfo, advection);
