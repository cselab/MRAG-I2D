...
namespace PitchingStuff {
	...
	/**
	 * creates a non-rotated NACA profile at (-l,0) such that the center of rotation lies at (0,0)
	 * geometry can be specified by the chord length, the x-offset of the center of rotation and the NACA digits d1, d2, d3d4
	 *
	 */
	class NACA4gen
		{
		private:
			...
			// maxBoxX: stores physical coordinate of maximal base point in x; the others: analogous
		...
		public:
			...
			{
				assert((xx[0]+l)>=-2.0*epsilon && (xx[0]+l)<0.05*leadingEdgeRadius);
			...
			Real getYUpperProfile(Real xx[2], Real & dist) const
			{
				...
				const int midIdx = floor((xx[0]+l)/dx); // get base point before sought x-coordinate x[0]
			...
			Real sdf(Real xo[2]) const
			{
				...
				if ((xo[0]+l)<-2.0*epsilon){ return sqrt(pow((xo[0]-l),2) + pow(xo[1], 2)); } // return distance to origin

				if ((xo[0]+l)>chord){ return sqrt(pow((xo[0]-chord+l),2) + pow(xo[1], 2)); } // return distance to trailing edge

				if ((xo[0]+l)>=-2.0*epsilon && (xo[0]+l)<0.05*leadingEdgeRadius){ return getNoseDistance(xo); } // return distance to leading edge
			...
			NACA4gen(Real chord_in,unsigned int nPanels_in,bool isFiniteTE,Real epsilon_in, const int _d1, const int _d2, const int _d3d4, const Real _l)
			:d1(_d1), d2(_d2), d3d4(_d3d4), l(_l)
			{
				...
				// 4. shift basepoints [0,chord] -> [-l, chord-l], such that center of rotation lies in [0,0]
				for(unsigned int i=0; i<nPanels; i++)
				{
					x[i] -= l;
					xu[i] -= l;
				...
				// assure assignment of one of the boxes (values are > -l but smaller than max of Real for sure)
				maxBoxX = -l;
	...
	/**
	 * Provides transformation matrices between domain [unrotated airfoil at (0,0)] and codomain [rotated airfoil at (tx-l, ty)]
	...
	public:
		Real tx, ty, angle, vx, vy, angular_velocity, rho, m, J; // (tx-l,ty): coordinates of airfoil's leading edge

		DiscretizedWing(Real xm, Real ym, Real width, Real angle_rad, Real epsilon, const int d1, const int d2, const int d3d4, const Real l):
		naca(width,5000,false,epsilon,d1,d2,d3d4,l),
		scaling(1.0),
		epsilon(epsilon),
		width(width),
		tx(xm), ty(ym), angle(angle_rad), s(1.0), vx(0.0), vy(0.0), angular_velocity(0.0), rho(1.0), m(0.0), J(0.0)
		{
			ca = cos(angle);
			sa = sin(angle);
		 ...
		 * tx, ty and angle are updated explicitely in I2D_PitchingAirfoil::update, before this function is called
		 */
	...
	struct FillBlocks
	{
		...
		FillBlocks(Real eps, PitchingStuff::DiscretizedWing *_shape): eps(eps) { shape = _shape; }

		FillBlocks(const FillBlocks& c): eps(c.eps) { shape = c.shape; }

		static bool _is_touching(Real eps, const PitchingStuff::DiscretizedWing * wing, const BlockInfo& info)
		{
	...
	struct GetNonEmpty
	{
		...
		GetNonEmpty(Real eps, PitchingStuff::DiscretizedWing *_shape, map<int, bool>& b2nonempty): eps(eps), b2nonempty(b2nonempty)
		{
			shape = _shape;
		}
	...
	struct FillVelblocks
	{
		...
		FillVelblocks(vector<pair< BlockInfo, VelocityBlock *> >& workitems, double eps, PitchingStuff::DiscretizedWing *_shape):
...
I2D_PitchingAirfoil::I2D_PitchingAirfoil(Grid<W,B>& grid, ArgumentParser& parser,
...
:I2D_FloatingObstacleOperator(parser, grid, D, _eps, Uinf, penalization), plungePeriod(_plungePeriod),
 plungeAmplitude(_plungeAmplitude), plungePhase(_plungePhase), pitchPeriod(_pitchPeriod),
 pitchAmplitude(_pitchAmplitude), pitchPhase(_pitchPhase), D_char(_D), l(_l), xcrot0(_xm),
 ycrot0(_ym)
{
	this->Uinf[0] = Uinf[0];
	this->Uinf[1] = Uinf[1];
	...
	wing = new PitchingStuff::DiscretizedWing(xcrot0,y_plunge0,_D,angle0,eps,d1,d2,d3d4,l); // get rotated NACA profile
}
...
void I2D_PitchingAirfoil::update(const double dt, const double t, string filename, map< string, vector<I2D_FloatingObstacleOperator *> > * _data)
{
	// update location of the body explicitely
	wing->tx = xcrot0;
	wing->ty = ycrot0 + plungeAmplitude*sin(2.0*M_PI/plungePeriod*t+plungePhase);
...
void I2D_PitchingAirfoil::_setMotionPattern(const Real t)
{
	...
	wing->angular_velocity = pitchAmplitude*factor2*cos(factor2*t+pitchPhase);
}
