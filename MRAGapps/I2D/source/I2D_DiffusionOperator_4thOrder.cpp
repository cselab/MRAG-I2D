...
struct DiffusionRHS_4thOrder : I2D_DivGradOfScalar_4thOrder {
  ...
  DiffusionRHS_4thOrder(const DiffusionRHS_4thOrder &copy)
      : viscosity(copy.viscosity), t(copy.t) {
    stencil_start[0] = stencil_start[1] = -2;
    stencil_end[0] = stencil_end[1] = +3;
    stencil_start[2] = 0;
    stencil_end[2] = 1;
...
struct DiffusionSimple_4thOrder {
  ...
  DiffusionSimple_4thOrder(Grid<W, B> &grid, BlockProcessing &block_processing,
                           int &rhscounter)
...
struct DiffusionLTS_4thOrder {
  ...
  DiffusionLTS_4thOrder(Grid<W, B> &grid, BlockProcessing &block_processing,
                        bool bSmartTrick, int &rhscounter, double FC)
      : grid(grid), block_processing(block_processing),
        bSmartTrick(bSmartTrick), rhscounter(rhscounter), FC(FC), DIM(2.0) {
    stsorter.connect(grid);
  }

  ~DiffusionLTS_4thOrder() { stsorter.disconnect(); }
