...
PF_ObjectFactory::PF_ObjectFactory(const Real charLength, const Real XCM,
                                   const Real YCM)
...
void PF_ObjectFactory::create(MRAG::ArgumentParser &parser,
                              map<string, vector<PF_Agent *>> &shapesMap,
                              const Real lr, const Real greedyEps,
  ...
  if (SHARED)
    if (LR >= 0.0 && LR <= 1.0 && GAMMA >= 0.0 && GAMMA <= 1.0 &&
      ...
      (*policy) = new RL::RL_QLearning((double)LR, (double)GAMMA,
                                       (double)GREEDYEPS, rand(), learnDump);
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (name == "PF_Dipole") {
      ...
      PF_Dipole *object =
          new PF_Dipole(parser, d, v, location, angle, lr, greedyEps, SMOOTH,
                        ISCONTROLLED, T, counterID, policy);
      ...
      PF_DipoleInline *object = new PF_DipoleInline(
          parser, d, v, radius, acc, location, dir, lr, greedyEps, SMOOTH,
          ISCONTROLLED, FITSELECT, T, counterID, policy, rand());
     ...
     */
      ...
      PF_DipoleEight *object = new PF_DipoleEight(
          parser, d, v, radius, acc, location, dir, lr, greedyEps, SMOOTH,
          ISCONTROLLED, T, counterID, policy, rand());
     ...
     */
      ...
      const Real avgDistToDipole =
          variable * charLength; /// area enclosed by spline
      ...
      const Real xcorrect =
          0.5 * (side - 1) *
          deltax; // to center the school according to the axe x=0.5
        ...
        for (int tr = 0; tr < length; tr++) {
          ...
          for (int hexa = 0; hexa < 6; hexa++) {
            ...
            for (int k = 0; k < (int)locations.size(); k++) {
              ...
              if (realLocK > realLoc - error && realLocK < realLoc + error &&
                  imagLocK > imagLoc - error && imagLocK < imagLoc + error) {
                same = true;
                break;
              }
            }
        ...
        for (int gs = 0; gs < length; gs++)
          for (int h = 0; h < ccl * 6; h++) {
            ...
            for (int k = 0; k < (int)allCenters.size(); k++)
              if (allCenters[k].first > center[0] - error &&
                  allCenters[k].first < center[0] + error &&
                  allCenters[k].second > center[1] - error &&
                  allCenters[k].second < center[1] + error) {
                same = true;
                break;
              }
