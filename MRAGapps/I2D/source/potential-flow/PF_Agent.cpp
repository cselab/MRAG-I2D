...
PF_Agent::PF_Agent(MRAG::ArgumentParser &parser, const Real lr,
                   const Real greedyEps, bool isSmooth, bool isControlled,
                   string _name, const int _ID, RL::RL_TabularPolicy **_policy,
                   const int seed)
    : name(_name), ID(_ID), policy(_policy), policystore(NULL),
      integralReward(0.0), status(Ready), learningInterval(0.0),
      learningTimer(0.0), fitnessValue(0.0), errorValue(0.0), SMOOTH(isSmooth),
      ISCONTROLLED(isControlled), myAction(0), myPreviousAction(0),
      isAveraged(true) {
  isDead = false;
...
bool PF_Agent::mapStart(const Real time,
...
bool PF_Agent::choose(const Real time,
                      map<string, vector<PF_Agent *>> *collection) {
  bool valid = true;
  ...
  if (policy != NULL) {
    ...
    if (learningTimer == 0.0 && status == Ready) {
      int action = 0;
      if (ISCONTROLLED)
        action = (*policy)->selectAction(myState);
...
void PF_Agent::setReward(const Real time) {
  if (policy != NULL && (time > (learningTimer + learningInterval)) &&
      status == Waiting) {
    assert((*policy) != NULL);
...
void PF_Agent::mapTest(const Real time,
                       map<string, vector<PF_Agent *>> *collection) {
  if (policy != NULL && (time > (learningTimer + learningInterval)) &&
      status == Waiting) {
    assert((*policy) != NULL);
...
void PF_Agent::stopTest(const Real time,
                        map<string, vector<PF_Agent *>> *collection) {
  if (policy != NULL && (time > (learningTimer + learningInterval)) &&
      status == Waiting) {
    assert((*policy) != NULL);
...
void PF_Agent::learn(const Real time,
                     map<string, vector<PF_Agent *>> *collection,
                     string filename) {
  if (policy != NULL && (time > (learningTimer + learningInterval)) &&
      status == Ready) {
...
int PF_Agent::_discretize(const Real signal, const Real minsignal,
                          const Real maxsignal, const int nlevels,
                          const bool deadEnd) const {
  ...
  return max(0,
             min(numberOfLevels - 1, (int)floor((signal - minsignal) / delta)));
}

int PF_Agent::_discretizeRange(const Real value, const Real minvalue,
                               const Real maxvalue, const int levels) {
  assert(maxvalue >= minvalue);
  const Real h = (maxvalue - minvalue) / levels;
  return max(0, min(levels - 1, (int)floor((value - minvalue) / h)));
}

int PF_Agent::_discretizeAngle(Real angle, const Real sightangle,
                               const int levels) {
  ...
  if (angle < 180.0) {
    ...
    return max(
        levels / 2,
        min(levels - 1, levels - 1 - (int)floor((360.0 - angle) / dangle)));
  }
}

#ifdef _RL_VIZ
void PF_Agent::_paintSphere(Real x, Real y, Real radius, Real r, Real g,
                            Real b) const {
 ...
 */
void PF_Agent::_paintX(Real x, Real y, Real radius, Real r, Real g,
                       Real b) const {
  const int nx = 13;
  const Real t = 0.4;
  const Real xx[nx] = {t,  1,     1 - t, 0,     t - 1, -1, -t,
                       -1, t - 1, 0,     1 - t, 1,     t};
  const Real yx[nx] = {0,     1 - t, 1,  t,  1,     1 - t, 0,
                       t - 1, -1,    -t, -1, t - 1, 0};
...
void PF_Agent::_drawCircle(Real radius, Real xc, Real yc, Real r, Real g,
                           Real b) const {
...
void PF_Agent::_paintTriangle(Real xc, Real yc, Real radius, Real direction,
                              Real r, Real g, Real b) const {
  GLfloat red = 1.3 * r;
  GLfloat green = 1.3 * g;
  GLfloat blue = 1.3 * b;
  GLfloat lightColorExit[] = {red, green, blue, 1};
...
void PF_Agent::_paintCone(Real xc, Real yc, Real radius, Real direction, Real r,
                          Real g, Real b) const {
  glShadeModel(GL_SMOOTH);
  glEnable(GL_LIGHTING);
  ...
  glTranslated(-radius * cos(direction), -radius * sin(direction),
               0); // move back a little so that the COM coincides with (xc, yc)
...
void PF_Agent::_drawFullCircle(Real radius, Real xc, Real yc, Real r, Real g,
                               Real b) const {
  const Real deg2rad = M_PI / 180;
