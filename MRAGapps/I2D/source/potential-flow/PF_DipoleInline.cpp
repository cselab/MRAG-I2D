...
PF_DipoleInline::PF_DipoleInline(
    MRAG::ArgumentParser &parser, const Real _D, const Real _V,
    const Real _radius, const Real _acc, const complex<Real> _locationCenter,
    const Real _dir[2], const Real lr, const Real greedyEps, bool isSmooth,
    bool isControlled, int fitselect, const Real T, const int _ID,
    RL::RL_TabularPolicy **_policy, const int seed)
    : PF_Agent(parser, lr, greedyEps, isSmooth, isControlled, "PF_DipoleInline",
               _ID, _policy, seed),
      D(_D / (2 * sqrt(2 * M_PI))), V(_V), locationCenter(_locationCenter),
      locationCenter0(_locationCenter), maxDomainRadius(0.5),
      fitselect(fitselect) {
  cutoff = 10 * D; /// note that D is actually \ell_n in the paper, see above,
                   /// in the factory, D = \ell 2*sqrt(2*\pi)
  gamma = 2 * M_PI * V * D;
  ...
  if (policy != NULL)
    if ((*policy) != NULL) {
      signature.push_back(
          30); // n levels to discretize dipole distance to target point
      signature.push_back(30); // n levels to discretize dipole orientation with
                               // respect to target point
                               //			signature.push_back(5); // n levels to
      //discretize speed between dipole and the group 			signature.push_back(5); //
      //n levels to discretize distance from follow to target point
 ...
 * @param collection
 */
void PF_DipoleInline::update(const Real dt, const Real time,
                             map<string, vector<PF_Agent *>> *collection) {
 ...
 * @param collection
 */
void PF_DipoleInline::updatePostVelocityUpdate(
  ...
  //
  //	for (vector<PF_Agent *>::iterator it = agents.begin(); it !=
  //agents.end(); ++it)
  //	{
  ...
  followedPoint[0] =
      targetPoint[0] -
      followFactor * dir[0] * D; // follow one \ell behind at all times
 ...
 * @param positionAgent
 */
void PF_DipoleInline::storePosition(
 ...
 * @param targetsAgent
 */
void PF_DipoleInline::getTargets(
...
bool PF_DipoleInline::mapState(vector<int> &state,
                               map<string, vector<PF_Agent *>> *collection) {
  ...
  const int idxangle =
      std::max(0, std::min(levelAngle - 1, (int)floor(angle / dangle)));
  ...
  bool valid = true;
  if (ISCONTROLLED)
    if (_isOutsideDomain(myX) ||
        IdI > (8 * cutoff)) /// || far far away from the followed point
      valid = false;
...
void PF_DipoleInline::perform(const Real time) {
  Real diffTime =
      min(1.0, 0.1 + (time - learningTimer) / (0.5 * learningInterval));
 ...
 * @param collection
 */
void PF_DipoleInline::reward(const Real time,
                             map<string, vector<PF_Agent *>> *collection) {
  ...
  if (time > (learningTimer + learningInterval) && status == Waiting) {
    ...
    //		Real rewardVel = 0;
    //		{ // Reward based on having the same speed as your neighbors
    //(speeds are discontinuous!!) 			Real totVx = 0.0; 			Real totVy = 0.0;
    //
    //			// Compute average velocity among all lattice agents
    //			vector<PF_Agent *> &agents =
    //(*collection)["PF_DipoleInline"];
    //
    //			for (vector<PF_Agent *>::iterator it = agents.begin();
    //it != agents.end(); ++it)
    //			{
    //				PF_DipoleInline * b =
    //static_cast<PF_DipoleInline*>(*it); 				totVx += b->vx; 				totVy += b->vy;
    //			}
    //
    //			totVx /= (Real) agents.size();
    //			totVy /= (Real) agents.size();
    //
    //			const Real modvLattice = totVx * dir[0] + totVy *
    //dir[1]; 			const Real IvI = vx * dir[0] + vy * dir[1]; 			const Real
    //diffVelPercent = (fabs(IvI - modvLattice) / modvLattice) * (fabs(IvI -
    //modvLattice) / modvLattice);
    //
    ...
    { /// Reward based on taking good actions
      //			rewardForGoodAction = isTakingBadAction ? 0.0
      //: 1.0; // before testing
    ...
    //		{ /// Reward based on orientation with respect to where it wants
    //to go 			const Real angleDifference = alpha - alpha0; // or use actual
    //bearing atan2(vy,vx); 			const Real angleFactor = 10.0;
    //			rewardInRightDirection = -angleFactor *
    //abs(angleDifference) + 1.0;
    //		}

    integralReward +=
        wForBeingOnTarget * rewardDist + wForGoodAction * rewardForGoodAction;

    //		// Sum up integral reward as a combination of the ones
    //calculated above 		const Real w[5] = { 1.0, 0, 0, 0.1, 0 }; // weights are
    //hard coded right now 		integralReward += w[0] * rewardDist + w[1] *
    //rewardVel + w[2] * rewardTarget + w[3] * rewardForBadAction + w[4] *
    //rewardInRightDirection; 		printf("Rewards (dist, vel, target, action):
    //(%7.4f, %7.4f, %7.4f, %7.4f, %7.4f) \n", rewardDist, rewardVel,
    //rewardTarget, rewardNoAction, rewardInRightDirection);
  }
}
...
void PF_DipoleInline::paint() {
  ...
  if (isDead) {
    //		_paintSphere(x, y, 2.5 * D, 1.0, 0.2, 0.1); // swimmer size is
    //actually approx 5 times the separating distince D, see constructor
    _paintX(x, y, 5.0 * D, 1.0, 0.2, 0.1);
  } else {
    if (isAveraged == false) {
      _paintCone(x, y, 5.0 * D, alpha, 0.7, 0.7,
                 0.7); // swimmer size is actually approx 5 times the separating
                       // distince D, see constructor
                       //			_paintTriangle(x, y, 5.0 * D, alpha, 0.7, 0.7,
      //0.7); // swimmer size is actually approx 5 times the separating distince
      //D, see constructor
    } else if (isAveraged == true) {
      //			_paintCone(x, y, 5. * D, alpha, 0.0, 1.0, 0.0);
      //// swimmer size is actually approx 5 times the separating distince D,
      //see constructor
      _drawFullCircle(1.5 * D, targetPoint[0], targetPoint[1], 1.0, 0.0, 0.0);
      _drawCircle(1.5 * D, targetPoint[0], targetPoint[1], 0.0, 0.0,
                  0.0); // swimmer size is actually approx 5 times the
                        // separating distince D, see constructor
      _paintTriangle(x, y, 5.0 * D, alpha, 0.0, 1.0,
                     0.0); // swimmer size is actually approx 5 times the
                           // separating distince D, see constructor
    }
  }
...
void PF_DipoleInline::paint(Real scale, const Real center[2]) {
  ...
  if (isDead) {
    _paintSphere(x, y, 2.5 * scale * D, 1.0, 0.2,
                 0.1); // swimmer size is actually approx 5 times the separating
                       // distince D, see constructor
                       //		_paintX(x, y, 5.0 * scale * D, 1.0, 0.2, 0.1);
  } else if (isAveraged == false) {
    _paintCone(x, y, 5.0 * scale * D, alpha, 0.7, 0.7,
               0.7); // swimmer size is actually approx 5 times the separating
                     // distince D, see constructor
    //			_paintTriangle(x, y, 5.0 * D, alpha, 0.7, 0.7, 0.7); //
    //swimmer size is actually approx 5 times the separating distince D, see
    //constructor
  } else if (isAveraged == true) {
    _paintCone(x, y, 5. * scale * D, alpha, 0.0, 0.741,
               0.965); // swimmer size is actually approx 5 times the separating
                       // distince D, see constructor
                       //					_paintTriangle(x, y, 5.0 * D, alpha,
    //0.0, 0.741, 0.965); // swimmer size is actually approx 5 times the
    //separating distince D, see constructor
  }
}
...
void PF_DipoleInline::saveData(Real time) {
  ...
  if (out) {
    ...
    out << time << " " << real(locationCenter) << " " << imag(locationCenter)
        << " " << real(locationRightVortex) << " " << imag(locationRightVortex)
        << " " << real(locationLeftVortex) << " " << imag(locationLeftVortex)
        << " " << alpha << " " << gammaRight << " " << gammaLeft << " " << D
        << " " << velDipole << endl;
    out.close();
  } else
...
void PF::PF_DipoleInline::fitness() {
  ...
  if (fitselect == 1)
    fitnessValue = fitnessSpeed + fitnessTurn;
  else if (fitselect == 2)
    fitnessValue = fitnessSpeed;
  else if (fitselect == 3)
    fitnessValue = fitnessTurn;
}
 ...
 * @param targetPoints
 */
void PF::PF_DipoleInline::getTargetPoints(
 ...
 */
void PF_DipoleInline::resetToInitialCondition() { setupInitialCondition(); }
...
bool PF_DipoleInline::_isOutsideDomain(const Real myX[2]) {
  if ((myX[0] > 1 || myX[0] < 0) || (myX[1] > 1 || myX[1] < 0))
    return (true);
  else
    return (false);
}
