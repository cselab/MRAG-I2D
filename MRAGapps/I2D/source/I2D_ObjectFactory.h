...
public:
  I2D_ObjectFactory(Grid<W, B> &grid, const Real charLength, const Real XCM,
                    const Real YCM, const Real eps, const Real Uinf[2],
                    I2D_PenalizationOperator &penalization, const int _LMAX);
  ~I2D_ObjectFactory();

  void create(ArgumentParser &parser,
              map<string, vector<I2D_FloatingObstacleOperator *>> &shapesMap,
              bool smart = false, RL::RL_TabularPolicy **policy = NULL);
