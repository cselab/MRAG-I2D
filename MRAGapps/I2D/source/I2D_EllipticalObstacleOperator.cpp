...
struct FillBlocksElly {
  ...
  FillBlocksElly(Real semiMajorAxis, Real aspectRatio, Real angle, Real epsilon,
                 Real position[2])
      : semiMajorAxis(semiMajorAxis), aspectRatio(aspectRatio), angle(angle),
        epsilon(epsilon) {
  ...
  FillBlocksElly(const FillBlocksElly &c)
      : semiMajorAxis(c.semiMajorAxis), aspectRatio(c.aspectRatio),
        angle(c.angle), epsilon(c.epsilon) {
  ...
  bool _is_touching(const BlockInfo &info) const {
    ...
    Real intersection[2][2] = {
        max(min_pos[0], box[0][0]),
        min(max_pos[0], box[0][1]),
        max(min_pos[1], box[1][0]),
        min(max_pos[1], box[1][1]),
    };
    ...
    if (_is_touching(info)) {
      ...
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
          ...
          radius = a * b /
                   sqrt(b * b * cos(alpha) * cos(alpha) +
                        a * a * sin(alpha) * sin(alpha));
          dist = sqrt(dx * dx + dy * dy) - radius;

          o(ix, iy).tmp = I2D_EllipticalObstacleOperator::mollified_heaviside(
              dist, epsilon);
        }
    } else {
...
Real I2D_EllipticalObstacleOperator::mollified_heaviside(const Real x,
                                                         const Real eps) {
...
void I2D_EllipticalObstacleOperator::characteristic_function() {
  ...
  EllyStuff::FillBlocksElly fill(semiMajorAxis, aspectRatio, angle, epsilon,
                                 position);
