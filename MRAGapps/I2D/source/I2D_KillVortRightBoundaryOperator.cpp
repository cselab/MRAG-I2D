...
struct FillBlocks2 {
  ...
  bool _is_touching(
      const BlockInfo &info) const // true: block within killing zone, false:
                                   // block not within killing zone
  {
    Real max_pos[2];
    info.pos(max_pos, FluidBlock2D::sizeX - 1,
             FluidBlock2D::sizeY - 1); // max position in physical coordinates
    ...
    if (_is_touching(info)) {
      ...
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
          Real p[2];
          info.pos(p, ix, iy);
          factor = cos((M_PI / (2 * killed_width * max_dx)) *
                       max(0.0, (p[0] - 1.0 + (5 + killed_width) * max_dx)));
          factor = max(Real(0.0),
                       factor); // smooth within killing zone (factor <= 1) and
                                // kill at very boundaries (factor < 0)
