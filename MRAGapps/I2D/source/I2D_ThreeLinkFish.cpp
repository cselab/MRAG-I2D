 ...
 *
 * @todo Fix mass for all other ObjectFactory objects that include
 * ThreeEllipses.
 */
 ...
 */
I2D_ThreeLinkFish::ThreeEllipse::ThreeEllipse()
    : thetam(0), D(0.1), xm(0.5), ym(0.5), aspectRatio(2), omegam(0), um(0),
      vm(0), rho(1.0) {
  ...
  J = 3 * J0 +
      m0 * (dist1Sqr + dist2Sqr); /// Use parallel axis theorem, will be
                                  /// updated, don't worry about details
}

/**
 * Overloaded constructor for ThreeEllipse structure
 */
I2D_ThreeLinkFish::ThreeEllipse::ThreeEllipse(Real xm_, Real ym_, Real D,
                                              Real angle_rad, Real aspectRatio)
    : thetam(angle_rad), D(D), xm(xm_), ym(ym_), aspectRatio(aspectRatio),
      omegam(0), um(0), vm(0), rho(1.0) {
  /// @todo Remove hard coded stuff
  thetam = 0.5 * M_PI;
  omegam = 0.0;
 ...
 * @return
 */
Real I2D_ThreeLinkFish::ThreeEllipse::_mollified_heaviside(
    const double dist, const double eps) const {
 ...
 * @param dkdt
 */
void I2D_ThreeLinkFish::ThreeEllipse::_cubicInterpolation(double t0, double t1,
                                                          double t, double k0,
                                                          double k1, double &k,
                                                          double &dkdt) {
  k = 0.0;
 ...
 */
Real I2D_ThreeLinkFish::ThreeEllipse::sample(const Real x_, const Real y_,
                                             const Real eps) const {
  /// ThreeEllipse0
  const double alpha0 = atan2(y_ - y0, x_ - x0) - theta0;
  const double radius0 = a * b /
                         sqrt(b * b * cos(alpha0) * cos(alpha0) +
                              a * a * sin(alpha0) * sin(alpha0));
  const double dist0 =
      sqrt((x_ - x0) * (x_ - x0) + (y_ - y0) * (y_ - y0)) - radius0;
  ...
  const double radius1 = a * b /
                         sqrt(b * b * cos(alpha1) * cos(alpha1) +
                              a * a * sin(alpha1) * sin(alpha1));
  const double dist1 =
      sqrt((x_ - x1) * (x_ - x1) + (y_ - y1) * (y_ - y1)) - radius1;
  ...
  const double radius2 = a * b /
                         sqrt(b * b * cos(alpha2) * cos(alpha2) +
                              a * a * sin(alpha2) * sin(alpha2));
  const double dist2 =
      sqrt((x_ - x2) * (x_ - x2) + (y_ - y2) * (y_ - y2)) - radius2;
...
void I2D_ThreeLinkFish::ThreeEllipse::sampleDeformationVel(const Real x_,
                                                           const Real y_,
                                                           Real &udef,
                                                           Real &vdef) const {
...
void I2D_ThreeLinkFish::ThreeEllipse::computeAndSetShapeChangeWrtFrame1(
    const double t) {
...
void I2D_ThreeLinkFish::ThreeEllipse::computeComCurrent(double &xcom,
                                                        double &ycom) const {
  assert(m0 != 0.0);
  assert(m != 0.0);
  xcom = m0 * (x0 + x1 + x2) / m;
...
void I2D_ThreeLinkFish::ThreeEllipse::computeComCurrentNumerical(
    double &xcom, double &ycom) const {
  assert(m0 != 0.0);
  assert(m != 0.0);
  xcom = m0 * (x0 + x1 + x2) / m;
  ycom = m0 * (y0 + y1 + y2) / m;
...
void I2D_ThreeLinkFish::ThreeEllipse::computeVelComCurrent(double &ucom,
                                                           double &vcom) const {
  assert(m0 != 0.0);
  assert(m != 0.0);
  ucom = m0 * (u0 + u1 + u2) / m;
...
void I2D_ThreeLinkFish::ThreeEllipse::computeVelComCurrentNumerical(
    double &ucom, double &vcom) const {
  assert(m0 != 0.0);
  assert(m != 0.0);
  ucom = m0 * (u0 + u1 + u2) / m;
  vcom = m0 * (v0 + v1 + v2) / m;
...
void I2D_ThreeLinkFish::ThreeEllipse::shiftFromFrame1To2(const double x,
                                                         const double y,
                                                         const double u,
                                                         const double v) {
  x0 -= x;
...
void I2D_ThreeLinkFish::ThreeEllipse::updateShapeAndMotionInVacuum(
    const double t) {
...
void I2D_ThreeLinkFish::ThreeEllipse::updateShapeAndMotionInVacuumNumerical(
    const double t) {
  double xcom, ycom, ucom, vcom;
  double extraAngMom; /// amount of extra angular momentum
  ...
  computeAndSetShapeChangeWrtFrame1(t);
  printShapeLinks(t, "ThreeLinkFrame1.txt");
  computeComCurrent(xcom, ycom); /// move it to where you expect using exact
                                 /// results, for an estimate
...
void I2D_ThreeLinkFish::ThreeEllipse::updateIntertialQuantities(
    const double dt) {
  xm += um * dt;
  ym += vm * dt;
  thetam += omegam * dt;
...
void I2D_ThreeLinkFish::ThreeEllipse::computeAngVelWrtCurrent(
    double &u, double &v, double omega, double x, double y) const {
  u = -omega * y;
  v = omega * x;
}

void I2D_ThreeLinkFish::ThreeEllipse::rotateVecWrtCurrent(double &x, double &y,
                                                          double beta) {
...
void I2D_ThreeLinkFish::ThreeEllipse::printExactMomentumDiagnostics(
...
void I2D_ThreeLinkFish::ThreeEllipse::printNumericalMomentumDiagnostics(
    const double t) const {
  double L;
  double xcom, ycom;
  double ucom, vcom;
...
void I2D_ThreeLinkFish::ThreeEllipse::printShapeLinks(
    const double t, const string filename) const {
  FILE *outfile = NULL;
...
void I2D_ThreeLinkFish::ThreeEllipse::printShape(const double t,
 ...
 */
void I2D_ThreeLinkFish::ThreeEllipse::bbox(const Real eps, Real xmin[2],
                                           Real xmax[2]) const {
  xmin[0] = x0 - D / 2.0; /// length of entire fish is D
  xmax[0] = x0 + D / 2.0;
  xmin[1] =
      y0 - (separation + ellipseLength); /// maximum of one ThreeEllipse length
...
struct FillBlocks {
   ...
   * @param _shape
   */
  FillBlocks(Real eps, I2D_ThreeLinkFish::ThreeEllipse *_shape) : eps(eps) {
    shape = _shape;
  }
  ...
  static bool _is_touching(Real eps,
                           const I2D_ThreeLinkFish::ThreeEllipse *wheel,
...
struct ComputeCenterOfMass {
  ...
  ComputeCenterOfMass(double eps, I2D_ThreeLinkFish::ThreeEllipse *_shape,
...
struct ComputeAllCorrections {
  ...
  ComputeAllCorrections(double xcm, double ycm, double eps,
                        I2D_ThreeLinkFish::ThreeEllipse *_shape,
                        map<int, vector<double>> &b2sum, Real Uinf[2],
                        map<int, bool> &b2nonempty)
...
struct ComputeAll {
   ...
   * @param b2nonempty
   */
  ComputeAll(double eps, I2D_ThreeLinkFish::ThreeEllipse *_shape,
             map<int, vector<double>> &b2sum, Real Uinf[2],
             map<int, bool> &b2nonempty)
      : eps(eps), b2sum(b2sum), b2nonempty(b2nonempty) {
    this->Uinf[0] = Uinf[0];
    ...
    if (FillBlocks::_is_touching(eps, shape, info)) {
      ...
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
          ...
          mass += Xs;
          J += Xs * ((p[0] - shape->xm) * (p[0] - shape->xm) +
                     (p[1] - shape->ym) * (p[1] - shape->ym));
          vxbar += Xs * (b(ix, iy).u[0] + Uinf[0]);
          vybar += Xs * (b(ix, iy).u[1] + Uinf[1]);
          omegabar += Xs * ((b(ix, iy).u[1] + Uinf[1]) * (p[0] - shape->xm) -
                            (b(ix, iy).u[0] + Uinf[0]) * (p[1] - shape->ym));
        }
...
struct FillVelblocks {
  ...
  FillVelblocks(vector<pair<BlockInfo, VelocityBlock *>> &workitems, double xcm,
                ...
                double eps, I2D_ThreeLinkFish::ThreeEllipse *_shape)
...
I2D_ThreeLinkFish::I2D_ThreeLinkFish(ArgumentParser &parser, Grid<W, B> &grid,
                                     const Real _x0, const Real _y0,
                                     const Real D, const Real aspectRatio,
...
void I2D_ThreeLinkFish::computeDesiredVelocity(const double t) {
  ...
  ThreeLinkFish::FillVelblocks fillvelblocks(
...
void I2D_ThreeLinkFish::create(const double t) {
  ...
  ThreeLinkFish::ComputeCenterOfMass computeCenterOfMass(eps, shape, integrals,
                                                         Uinf, nonempty);
  ...
  mass = J = xbar = ybar = ubar = vbar = omegabar = 0.0;
  ThreeLinkFish::ComputeAllCorrections computeAllCorrections(
...
void I2D_ThreeLinkFish::printGeneralStuff(const double t, const string filename,
                                          vector<double> stuffToPrint) {
