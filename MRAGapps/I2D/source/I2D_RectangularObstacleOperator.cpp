...
Real I2D_RectangularObstacleOperator::mollified_heaviside(const Real x,
                                                          const Real eps) {
...
inline double area(double LX, double LY, double eps0, double eps1) {
  ...
  const double Asmooth2 = (LX + LY) * (eps1 - eps0) +
                          0.5 * M_PI * (eps1 - eps0) * (eps1 + eps0) -
                          2. / M_PI * pow(eps0 - eps1, 2);
...
Real I2D_RectangularObstacleOperator::_get_eps1() {
  ...
  while (true) {
    const double rel_diff =
        fabs((area(LX, LY, eps0, l) - desired_area) / desired_area);
...
struct FillBlocksPlate {
  ...
  bool _is_touching(const BlockInfo &info) const {
    ...
    Real intersection[2][2] = {
        ...
        min(max_pos[0], bbox[0][1]),
        max(min_pos[1], bbox[1][0]),
    ...
    if (_is_touching(info)) {
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
          ...
          const Real r =
              sqrt(pow(p[0] - pclst[0], 2) + pow(p[1] - pclst[1], 2));

          b(ix, iy).tmp = I2D_RectangularObstacleOperator::mollified_heaviside(
              r - (eps0 + eps1) * 0.5, eps1 - eps0);
        }
    } else {
