...
I2D_StefanFishSmart::StefanFishSmart::StefanFishSmart(
    double xm, double ym, double _D, double T, double tau, double angle_rad,
    double eps, const int LMAX)
    : StefanFish(xm, ym, _D, T, 0.0, tau, angle_rad, vector<double>(6),
                 vector<double>(6), 0.0, eps, LMAX) {}

I2D_StefanFishSmart::I2D_StefanFishSmart(
    ArgumentParser &parser, Grid<W, B> &grid, const Real _xm, const Real _ym,
    const Real D, const Real T, const Real tau, const Real angle,
    const Real eps, const Real Uinf[2], I2D_PenalizationOperator &penalization,
    const int LMAX, const int ID, RL::RL_TabularPolicy **_policy,
    const int seed)
    : I2D_StefanFish(parser, grid, _xm, _ym, D, T, 0.0, tau, angle,
                     vector<double>(6), vector<double>(6), 0.0, eps, Uinf,
                     penalization, LMAX, ID, _policy, seed),
      maxDomainRadius(0.45) {
  assert(shape != NULL);
  if (shape != NULL) {
    delete shape;
    shape = NULL;
...
bool I2D_StefanFishSmart::_mapStateDistance(
    vector<int> &state,
  ...
  double dist = sqrt(myX[0] * myX[0] + myX[1] * myX[1]);
...
bool I2D_StefanFishSmart::mapState(
    vector<int> &state,
    map<string, vector<I2D_FloatingObstacleOperator *>> *_data) {
...
void I2D_StefanFishSmart::reward(
    const double t,
    map<string, vector<I2D_FloatingObstacleOperator *>> *_data) {
  ...
  if (t > (learningTimer + learningInterval) && status == Waiting) {
    ...
    const double dist =
        sqrt((x[0] - 0.5) * (x[0] - 0.5) + (x[1] - 0.5) * (x[1] - 0.5));
    const bool inside = (dist > innerR) && (dist < outerR);

    if (inside)
      reward +=
          rewardRing - (rewardRing / fabs(innerR - midR)) * fabs(dist - midR);
    else {
      if (dist <= innerR)
        reward += -rewardRing + (rewardRing / innerR) * dist;
      else
        reward += -(rewardRing / fabs(maxDomainRadius - outerR)) *
                  fabs(dist - outerR);
    }
