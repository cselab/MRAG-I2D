...
I2D_ImposedCylinderStopAndGo::I2D_ImposedCylinderStopAndGo(
    ...
    const Real _vx, const Real _vy, const Real _tstop, const Real _D,
    const Real eps, const Real Uinf[2], I2D_PenalizationOperator &penalization)
    : I2D_ImposedCylinder(parser, grid, _xm, _ym, _vx, _vy, _D, eps, Uinf,
                          penalization),
      tstop(_tstop) {}
...
void I2D_ImposedCylinderStopAndGo::_setMotionPattern(const Real t) {
  ...
  if (t < tfinal) {
    ...
    fprintf(ppFile, "tstop=%e, tfinal=%e, diff=%e, dt=%e\n", t, tfinal,
            tfinal - tstop, eps / fabs(vx_imposed));
    fclose(ppFile);
...
void I2D_ImposedCylinderStopAndGo::update(
  ...
  if (filename == std::string()) {
    ...
    ppFile =
        fopen("update_I2D_ImposedCylinderStopAndGo.txt", t == 0.0 ? "w" : "a");
    assert(ppFile != NULL);
  } else {
