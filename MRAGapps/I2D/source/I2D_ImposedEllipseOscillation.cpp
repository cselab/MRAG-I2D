...
I2D_ImposedEllipseOscillation::I2D_ImposedEllipseOscillation(
    ...
    const Real _xcrot, const Real _ycrot, const Real _aspectRatio,
    const Real _plungeAmplitude, const Real _plungePeriod,
    const Real _pitchAmplitude, const Real _pitchPeriod,
    const Real _plungePhase, const Real _pitchPhase, const Real _D,
    const Real eps, const Real Uinf[2], I2D_PenalizationOperator &penalization)
    : I2D_ImposedEllipse(parser, grid, _xm, _ym, _xcrot, _ycrot, 0.0, 0.0, _D,
                         _aspectRatio, eps, Uinf,
                         penalization), // Inherited constructor
      x0(_xm),                          // define constants
      y0(_ym), plungeAmplitude(_plungeAmplitude), plungePeriod(_plungePeriod),
      pitchAmplitude(_pitchAmplitude), pitchPeriod(_pitchPeriod),
      plungePhase(_plungePhase), pitchPhase(_pitchPhase), D(_D) {
  // Initialize
  shape->xm = x0;
 ...
 * @param filename
 */
void I2D_ImposedEllipseOscillation::update(
    const double dt, const double t, string filename,
    map<string, vector<I2D_FloatingObstacleOperator *>> *_data) {
  shape->xm = x0;
  shape->ym =
      y0 + plungeAmplitude * sin(2.0 * M_PI / plungePeriod * t + plungePhase);
  shape->xcrot = shape->xm;
  shape->ycrot = shape->ym;
  shape->angle =
      pitchAmplitude * sin(2.0 * M_PI / pitchPeriod * t + pitchPhase);
  ...
  {
    ppFile =
        fopen("update_I2D_ImposedEllipseOscillation.txt", t == 0.0 ? "w" : "a");
    assert(ppFile != NULL);
