...
bool PlanBuilderWim::isCloseBox(Real srcBoxCtr[2], Real srcBoxRad,
...
bool PlanBuilderWim::isIntersectingBox(Real srcBoxCtr[2], Real srcBoxRad,
                                       const Real trgBoxCtr[2],
                                       const Real halfTrgBoxWidth) const {
  const Real intersection[3] = {
      std::min(trgBoxCtr[0] + halfTrgBoxWidth, srcBoxCtr[0] + srcBoxRad) -
          std::max(trgBoxCtr[0] - halfTrgBoxWidth, srcBoxCtr[0] - srcBoxRad),
      std::min(trgBoxCtr[1] + halfTrgBoxWidth, srcBoxCtr[1] + srcBoxRad) -
          std::max(trgBoxCtr[1] - halfTrgBoxWidth, srcBoxCtr[1] - srcBoxRad),
  };
...
void PlanBuilderWim::run() const {
  tbb::parallel_for(blocked_range<int>(0, m_num_target_blocks), *this,
                    auto_partitioner());
}
...
void PlanBuilderWim::operator()(const blocked_range<int> &range) const {
  ...
  for (int iblock = range.begin(); iblock != range.end(); ++iblock) {
    ...
    const Real halfTargetWidth =
        0.5 * (_BLOCKSIZE_ - 1) *
        info.h[0]; // our destination blocks have size _BLOCKSIZE_
    const Real targetCenter[2] = {block_org[0] + halfTargetWidth,
                                  block_org[1] + halfTargetWidth};

    while (srcBox != NULL && (srcBox->nParticles > 0)) {
      bool canRemove = true;

      const bool isClose =
          isCloseBox(srcBox->expansions.Center, srcBox->expansions.Radius,
                     targetCenter, halfTargetWidth);
      const bool isIntersecting = isIntersectingBox(
          srcBox->expansions.Center, srcBox->expansions.Radius, targetCenter,
          halfTargetWidth);
