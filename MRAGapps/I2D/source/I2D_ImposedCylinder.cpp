...
I2D_ImposedCylinder::Cylinder::Cylinder()
    : angle(0), D(0.1), xm(0.5), ym(0.5), angular_velocity(0), vx(0), vy(0),
      rho(1.0) {
  m = rho * M_PI * (D / 2.0) * (D / 2.0);
  J = 0.5 * m * (D / 2.0) * (D / 2.0);
}

I2D_ImposedCylinder::Cylinder::Cylinder(Real xm, Real ym, Real D,
                                        Real angle_rad)
    : angle(angle_rad), D(D), xm(xm), ym(ym), angular_velocity(0), vx(0), vy(0),
      rho(1.0) {
  m = rho * M_PI * (D / 2.0) * (D / 2.0);
  J = 0.5 * m * (D / 2.0) * (D / 2.0);
}

Real I2D_ImposedCylinder::Cylinder::_mollified_heaviside(
    const double dist, const double eps) const {
  // Positive outside/negative inside
  const double alpha = M_PI * min(1., max(0., (dist + 0.5 * eps) / eps));
  return 0.5 +
         0.5 *
             cos(alpha); // outside: 0, inside: 1, between: mollified heaviside
}

/*
const I2D_ImposedCylinder::Cylinder::Cylinder&
I2D_ImposedCylinder::Cylinder::operator=(const Cylinder& c)
{
...
Real I2D_ImposedCylinder::Cylinder::sample(const Real x_, const Real y_,
                                           const Real eps) const {
  const double dist =
      sqrt((x_ - xm) * (x_ - xm) + (y_ - ym) * (y_ - ym)) -
      D / 2.0; // negative = inside cylinder, positive = outside cylinder
  return _mollified_heaviside(dist, eps);
}

void I2D_ImposedCylinder::Cylinder::bbox(
    const Real eps, Real xmin[2],
    Real xmax[2]) const // calculate coordinates of a box around the cylinder
{
  assert(eps >= 0);
...
struct FillBlocks {
  ...
  FillBlocks(Real eps, I2D_ImposedCylinder::Cylinder *_shape) : eps(eps) {
    shape = _shape;
  }
  ...
  static bool
  _is_touching(Real eps, const I2D_ImposedCylinder::Cylinder *wheel,
               const BlockInfo &info) // true if cylinder coincides with block
  {
    ...
    if (_is_touching(eps, shape, info)) {
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
          ...
          info.pos(
              p, ix,
              iy); // returns physical coordinates of each point within the box
...
struct GetNonEmpty {
  ...
  GetNonEmpty(Real eps, I2D_ImposedCylinder::Cylinder *_shape,
              map<int, bool> &b2nonempty)
...
struct FillVelblocks {
  ...
  FillVelblocks(vector<pair<BlockInfo, VelocityBlock *>> &workitems, double eps,
                I2D_ImposedCylinder::Cylinder *_shape)
...
I2D_ImposedCylinder::I2D_ImposedCylinder(ArgumentParser &parser,
                                         Grid<W, B> &grid, const Real _xm,
                                         const Real _ym, const Real _vx,
                                         const Real _vy, const Real _D,
...
void I2D_ImposedCylinder::update(
  ...
  fprintf(ppFile, "%e %e %e %e %e %e %e %e %e %e %e %e\n", this->dimT, t,
          shape->xm, shape->ym, shape->vx, shape->vy, shape->angle,
          shape->angular_velocity, shape->J, shape->m, shape->rho, this->Cd);
  fflush(ppFile);
