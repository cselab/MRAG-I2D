...
I2D_TestAdvection::I2D_TestAdvection(const int argc, const char **argv)
...
void I2D_TestAdvection::run() {
  ...
  if (dumpfreq > 0 && step_id % dumpfreq == 0)
    _dump(buf);
...
void I2D_TestAdvection::_dump(string filename) {
  ...
  dumpframes.push_back(
      dumper.Write(*grid, filename, (double)step_id, step_id, names));
