...
struct Mass {
  ...
  Mass(Real density, map<int, Real> &local_mass)
      : density(density), local_mass(local_mass) {}
...
struct Diagnostic {
  ...
  Diagnostic() : area(0), gridpoints(0) { force[0] = force[1] = 0; }
...
struct ComputeDiagnostics {
  ...
  ComputeDiagnostics(map<int, Diagnostic> &diagnostics,
                     map<int, const VelocityBlock *> &desiredVels, Real lambda,
                     const Real _Uinf[2])
  ...
  ComputeDiagnostics(const ComputeDiagnostics &c)
      : diagnostics(c.diagnostics), desiredVels(c.desiredVels),
        lambda(c.lambda) {
  ...
  inline void operator()(const BlockInfo &info, FluidBlock2D &b) const {
    map<int, const VelocityBlock *>::iterator it =
        desiredVels.find(info.blockID);
...
void I2D_FloatingObstacleOperator::_dist(const double x2[2], const double x1[2],
                                         double d[2]) const {
  d[0] = x2[0] - x1[0];
  d[1] = x2[1] - x1[1];
}

double I2D_FloatingObstacleOperator::_angleVectors(const double v1[2],
                                                   const double v2[2]) const {
...
bool I2D_FloatingObstacleOperator::_isRight(const double x1[2],
                                            const double dir[2],
                                            const double x2[2]) const {
  const double alpha = -M_PI / 2.0;
  const double n[2] = {dir[0] * cos(alpha) - dir[1] * sin(alpha),
                       dir[0] * sin(alpha) + dir[1] * cos(alpha)};
  double d[2];
  _dist(x2, x1, d);
  return ((d[0] * n[0] + d[1] * n[1]) >= 0.0);
}

int I2D_FloatingObstacleOperator::_discretizeRange(const double value,
...
void I2D_FloatingObstacleOperator::computeDragAndStuff(const Real time,
                                                       const Real charLength,
                                                       const Real charVel) {
  if (charVel <= 0.0) {
    printf("Something wrong with characteristic velocity, charVel=%e!\n",
           charVel);
    abort();
  }
  if (charLength <= 0.0) {
    printf("Something wrong with characteristic lenght, charLength=%e!\n",
           charLength);
    abort();
  }

  typedef FloatingObstacleOperatorStuff::Diagnostic
      DiagType; // stores force, area and grid points of each object
  ...
  for (map<int, const VelocityBlock *>::iterator it = desired_velocity.begin();
    ...
    diagnostics[it->first] = DiagType(); // create information container (force,
                                         // area, grid points) for each object

  FloatingObstacleOperatorStuff::ComputeDiagnostics getDiag(
      diagnostics, desired_velocity, penalization.getLambda(), Uinf);
  block_processing.process(
      vInfo, grid.getBlockCollection(),
      getDiag); // compute force for each object and assign it to object

  DiagType global;
  for (map<int, DiagType>::iterator it = diagnostics.begin();
...
bool I2D_FloatingObstacleOperator::choose(
    const double t,
...
void I2D_FloatingObstacleOperator::stopTest(
    const double t,
    map<string, vector<I2D_FloatingObstacleOperator *>> *_data) {
  if (policy != NULL) {
...
void I2D_FloatingObstacleOperator::learn(
    const double t, map<string, vector<I2D_FloatingObstacleOperator *>> *_data,
    string name) {
  if (policy != NULL) {
    assert((*policy) != NULL);
...
std::vector<Real> I2D_FloatingObstacleOperator::getMass() {
  ...
  Real totalmass = 0;
  for (map<int, Real>::iterator it = local_data.begin(); it != local_data.end();
       it++)
    totalmass += it->second;
