...
struct DissipationPower : I2D_GradOfVector_4thOrder {
  ...
  DissipationPower(Real viscosity, Real density,
                   map<int, Real> &local_dissipation)
      : viscosity(viscosity), density(density), t(0),
        local_dissipation(local_dissipation) {
  ...
  DissipationPower(const DissipationPower &copy)
      : viscosity(copy.viscosity), density(copy.density), t(0),
        local_dissipation(copy.local_dissipation) {
    stencil_start[0] = stencil_start[1] = -2;
    stencil_end[0] = stencil_end[1] = +3;
    stencil_start[2] = 0;
    stencil_end[2] = 1;
  ...
  struct Assign {
    static inline void stream(Real &out, Real in) { out = in; }
  };
  struct Add2Square {
    static inline void stream(Real &out, Real in) { out += 2.0 * in * in; }
  };
...
struct FluidKineticEnergy {
  ...
  FluidKineticEnergy(Real density, map<int, Real> &local_fluidKinetic)
      : density(density), local_fluidKinetic(local_fluidKinetic) {}

  FluidKineticEnergy(const FluidKineticEnergy &c)
      : density(c.density), local_fluidKinetic(c.local_fluidKinetic) {}
...
void I2D_ComputeEfficiency::compute(
    I2D_FloatingObstacleOperator *floatingObstacle, double t, double dt,
    double startTime, double endTime) {
  assert(endTime >= startTime);

  if (t >= startTime && t <= endTime) {
    I2D_FloatingObstacleVector *vec =
        static_cast<I2D_FloatingObstacleVector *>(floatingObstacle);
    ...
    for (map<int, Real>::iterator it = local_data.begin();
         it != local_data.end(); it++)
      it->second = 0;

    ComputeEfficiency::DissipationPower dissipation(viscosity, density,
                                                    local_data);
    block_processing.process<I2D_VectorBlockLab<Streamer_Velocity, 2>::Lab>(
        vInfo, coll, binfo, dissipation);
    ...
    for (int i = 0; i < masses.size() - 1; ++i)
      fprintf(ppFile, "%e %e %e ", masses[i], infoShapes[i][0],
              infoShapes[i][1]);
