...
bool PlanBuilder::_isclose_box(const tBox &box, const BlockInfo &info,
  ...
  const Real radius_source = sqrt(pow(box.h[0], 2) + pow(box.h[1], 2));
...
bool PlanBuilder::_is_intersecting(const tBox &box,
...
bool PlanBuilder::_ws_box_barnes_hut_and(const tBox &source_box,
                                         const BlockInfo target_block,
                                         tBox::Btype theta) const {
  bool bResult = false;
  ...
  assert(bResult == false || bResult == _ws_box_barnes_hut_and_DUMB(
                                            source_box, target_block, theta));

  return bResult;
}

bool PlanBuilder::_ws_box_barnes_hut_and_DUMB(const tBox &source_box,
                                              const BlockInfo &target_block,
                                              tBox::Btype theta) const {
    ...
    while (it1 != NULL && (it1->nParticles > 0)) {
      ...
      if (!is_close && current.parent != NULL) {
        ...
        if (current.parent == NULL ||
            !(_ws_box_barnes_hut_and(*current.parent, info, _THETA))) {
