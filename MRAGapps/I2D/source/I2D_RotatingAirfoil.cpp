...
Real I2D_RotatingAirfoil::NACA4gen::getYUpperProfile(Real xx[2],
                                                     Real &dist) const {
  ...
  const int midIdx =
      floor(xx[0] / dx); // get base point before sought x-coordinate x[0]
  ...
  Real myDist =
      std::numeric_limits<Real>::max(); // ensure assignment in for loop (myDist
                                        // for sure smaller than max value of
                                        // Real)
...
Real I2D_RotatingAirfoil::NACA4gen::getYLowerProfile(Real xx[2],
                                                     Real &dist) const {
...
Real I2D_RotatingAirfoil::NACA4gen::sdf(Real xo[2]) const {
  ...
  if (xo[0] < -2.0 * epsilon) {
    return sqrt(pow(xo[0], 2) + pow(xo[1], 2));
  } // return distance to origin

  if (xo[0] > chord) {
    return sqrt(pow(xo[0] - chord, 2) + pow(xo[1], 2));
  } // return distance to trailing edge

  if (xo[0] >= -2.0 * epsilon && xo[0] < 0.05 * leadingEdgeRadius) {
  ...
  const Real D =
      min(distU, distL); // positive inside body, negative outside body

  return s * D;
}

I2D_RotatingAirfoil::NACA4gen::NACA4gen(Real chord_in, unsigned int nPanels_in,
                                        bool isFiniteTE, Real epsilon_in,
                                        const int _d1, const int _d2,
                                        const int _d3d4)
...
assert(d2>=0 && d2<=9);
  ...
  // assure assignment of one of the boxes (values are > 0 but smaller than max
  // of Real for sure)

  maxBoxX = 0.0;
  minBoxX = std::numeric_limits<Real>::max(); // maximum value for a variable of
                                              // type Real
  maxBoxY = 0.0;
...
// -------------------END OF MEMBER FUNCTIONS OF CLASS
// "NACA4gen"--------------------------------

// -------------------MEMBER FUNCTIONS OF CLASS "DISCRETISED WING"
// --------------------------------

void I2D_RotatingAirfoil::DiscretizedWing::check_mapping()
    const // check if O2W and W2O are inverse matrices
{
  Real A[3][3];
...
void I2D_RotatingAirfoil::DiscretizedWing::_w2o(const Real xw[2],
                                                Real xo[2]) const {
  xo[0] = W2O[0][0] * xw[0] + W2O[0][1] * xw[1] + W2O[0][2];
  xo[1] = W2O[1][0] * xw[0] + W2O[1][1] * xw[1] + W2O[1][2];
}

void I2D_RotatingAirfoil::DiscretizedWing::_o2w(const Real xo[2],
                                                Real xw[2]) const {
...
I2D_RotatingAirfoil::DiscretizedWing::DiscretizedWing(
    Real _tx, Real _ty, Real width, Real epsilon, Real _angle, Real _self_angle,
    const int d1, const int d2, const int d3d4, const Real l)
    :

      naca(width, 5000, false, epsilon, d1, d2, d3d4), scaling(1.0),
      epsilon(epsilon), width(width), tx(_tx), ty(_ty), s(1.0),
      angular_velocity(0), rho(1.0), m(0.0), J(0.0), l(0.0), vx(0.0), vy(0.0),
      self_angle(_self_angle), angle(_angle)
...
void I2D_RotatingAirfoil::DiscretizedWing::update_all(const double dt,
                                                      const double t,
                                                      const double perist,
                                                      const double _D,
                                                      const double _theta) {
...
void I2D_RotatingAirfoil::DiscretizedWing::getAerodynamicCenter(
    Real cor[2]) const // calculates position and orientation of airfoil
{
  cor[0] = 0.25 * width;
  cor[1] = 0.0;
...
void I2D_RotatingAirfoil::DiscretizedWing::bbox(const Real eps, Real xmin[2],
                                                Real xmax[2]) const {
  assert(eps > 0);
  ...
  xmin[0] = min((Real)min((Real)min((Real)xw1[0], (Real)xw2[0]), (Real)xw3[0]),
                (Real)xw4[0]); // xmin
  xmin[1] = min((Real)min((Real)min((Real)xw1[1], (Real)xw2[1]), (Real)xw3[1]),
                (Real)xw4[1]); // ymin
  xmax[0] = max((Real)max((Real)max((Real)xw1[0], (Real)xw2[0]), (Real)xw3[0]),
                (Real)xw4[0]); // xmax
  xmax[1] = max((Real)max((Real)max((Real)xw1[1], (Real)xw2[1]), (Real)xw3[1]),
                (Real)xw4[1]); // ymax
...
Real I2D_RotatingAirfoil::DiscretizedWing::_mollified_heaviside(
    const Real x, const Real eps) const {
  const Real alpha = M_PI * min(1., max(0., (x + 0.5 * eps) / eps));
  return 0.5 + 0.5 * cos(alpha);
}

Real I2D_RotatingAirfoil::DiscretizedWing::sample(Real p[2], const Real eps)
...
// -------------------END OF MEMBER FUNCTIONS OF CLASS "DISCRETISED
// WING"---------------------------------

// -------------------MEMBER FUNCTIONS OF CLASS "TOO MANY
// AIRFOILS"---------------------------------------

I2D_RotatingAirfoil::ToomanyAirfoils::ToomanyAirfoils(
    const Real _xm, const Real _ym, const Real _D, const Real _c, const int d1,
    const int d2, const int d3d4, const Real _l, const Real epsilon)
    :

      theta(0.0), xm(_xm), ym(_ym), D(_D), rho(1.0), m(0.0), J(0.0),
      angular_velocity(0.0), epsilon(epsilon)

{
  wing_1 = new I2D_RotatingAirfoil::DiscretizedWing(
      _xm + _D / 2, _ym, _c, epsilon, 0.0, -120., d1, d2, d3d4, _l);
  wing_2 = new I2D_RotatingAirfoil::DiscretizedWing(
      _xm - (cos(M_PI / 3) * _D / 2.0), _ym + (sin(M_PI / 3) * _D / 2.0), _c,
      epsilon, 120.0, 0.0, d1, d2, d3d4, _l);
  wing_3 = new I2D_RotatingAirfoil::DiscretizedWing(
      _xm - (sin(M_PI / 6) * _D / 2.0), _ym - (cos(M_PI / 6) * _D / 2.0), _c,
      epsilon, 240.0, 120.0, d1, d2, d3d4, _l);
}

Real I2D_RotatingAirfoil::ToomanyAirfoils::sample(Real p[2], const Real eps)
    const // calculate characteristic function at point (p[0],p[1])
{
...
Real I2D_RotatingAirfoil::ToomanyAirfoils::_mollified_heaviside(
    const Real x, const Real eps) const {
  const Real alpha = M_PI * min(1., max(0., (x + 0.5 * eps) / eps));
  return 0.5 + 0.5 * cos(alpha);
}

void I2D_RotatingAirfoil::ToomanyAirfoils::bbox(const Real eps, Real xmin[2],
                                                Real xmax[2]) const {
...
void I2D_RotatingAirfoil::ToomanyAirfoils::rotate(Real v[2],
                                                  Real _theta) const {
...
void I2D_RotatingAirfoil::ToomanyAirfoils::getAerodynamicCenter(
    Real cor[2]) const {
...
void I2D_RotatingAirfoil::ToomanyAirfoils::restart(
    FILE *f) // TO DO: change this
{
  float val;
...
// -------------------END OF MEMBER FUNCTIONS OF CLASS "TOO MANY
// AIRFOILS"--------------------------------
...
struct FillBlocks {
  ...
  FillBlocks(Real eps, I2D_RotatingAirfoil::ToomanyAirfoils *_pterygio)
      : eps(eps) {
    pterygio = _pterygio;
  }

  FillBlocks(const FillBlocks &c) : eps(c.eps) { pterygio = c.pterygio; }

  static bool _is_touching(Real eps,
                           const I2D_RotatingAirfoil::ToomanyAirfoils *wing,
...
struct ComputeAll {
  ...
  ComputeAll(double eps, I2D_RotatingAirfoil::ToomanyAirfoils *_pterygio,
    ...
    if (FillBlocks::_is_touching(eps, pterygio, info)) {
      ...
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
          ...
          mass += Xs;
          J += Xs *
               (pow((p[0] - pterygio->xm), 2) + pow((p[0] - pterygio->ym), 2));
          omegabar += Xs * ((b(ix, iy).u[1] + Uinf[1]) * (p[0] - pterygio->xm) -
                            (b(ix, iy).u[0] + Uinf[0]) * (p[1] - pterygio->ym));
        }
...
struct FillVelblocks {
  ...
  FillVelblocks(vector<pair<BlockInfo, VelocityBlock *>> &workitems, double eps,
                I2D_RotatingAirfoil::ToomanyAirfoils *_pterygio)
...
I2D_RotatingAirfoil::I2D_RotatingAirfoil(
    ...
    const Real _D, const Real _c, const int d1, const int d2, const int d3d4,
    const Real _l, const Real _eps, const Real Uinf[2],
...

{
  this->Uinf[0] = Uinf[0];
  this->Uinf[1] = Uinf[1];
  airfoil = new I2D_RotatingAirfoil::ToomanyAirfoils(_xm, _ym, _D, _c, d1, d2,
                                                     d3d4, _l, eps);
}
...
void I2D_RotatingAirfoil::refresh(const double t, string filename) {
  ...
  for (int i = 0; i < N; i++) {
    fprintf(f, "%e %e %e %e %e %e %e %e %e %e \n", dataVect[i][0],
            dataVect[i][1], dataVect[i][2], dataVect[i][3], dataVect[i][4],
            dataVect[i][5], dataVect[i][6], dataVect[i][7], dataVect[i][8],
            dataVect[i][9], dataVect[i][10], dataVect[i][11]);
  }
  fclose(f);
}
...
void I2D_RotatingAirfoil::update(
  ...
  fprintf(ppFile, "%e %e %e %e %e %e %e %e %e %e %e %e\n", this->dimT, t,
          airfoil->xm, airfoil->ym, airfoil->theta, airfoil->angular_velocity,
          airfoil->J, airfoil->m, airfoil->rho, this->Cd);
  fflush(ppFile);
