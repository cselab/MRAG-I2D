...
// @param Uinf
// @param penalization
I2D_PassiveTracer::I2D_PassiveTracer(ArgumentParser &parser, Grid<W, B> &grid,
                                     const Real _xm, const Real _ym,
                                     const Real D, const Real eps,
                                     const Real Uinf[2],
...
void I2D_PassiveTracer::update(
  ...
  for (map<long int, vector<Real>>::iterator it = particles.begin();
       it != particles.end(); ++it) {
    ...
    Real startBlock[2] = {myblock.i[0] * dx,
                          myblock.i[1] *
                              dx}; /// actual location of the start of the block
    Real endBlock[2] = {
        startBlock[0] + dx,
