...
I2D_FlowPastFloatingObstacle::I2D_FlowPastFloatingObstacle(const int argc,
                                                           const char **argv)
  ...
  if (sOBSTACLE == "floatingCyl") {
    ...
    shapesMap[sOBSTACLE].push_back(new I2D_FloatingCylinder(
        parser, *grid, XPOS, YPOS, D, epsilon, Uinf, *penalization));
    floatingObstacle = new I2D_FloatingObstacleVector(
        parser, *grid, epsilon, Uinf, *penalization, shapesMap, D,
        modv); // ObstacleVector enables single object drag evaluation
  } else if (sOBSTACLE == "floatingCylArray") {
    // Pack objects here
    map<string, vector<I2D_FloatingObstacleOperator *>> shapesMap;
    shapesMap[sOBSTACLE] = vector<I2D_FloatingObstacleOperator *>();
    shapesMap[sOBSTACLE].push_back(new I2D_FloatingCylinder(
        parser, *grid, 0.3, 0.5, D, epsilon, Uinf, *penalization));
    shapesMap[sOBSTACLE].push_back(new I2D_FloatingCylinder(
        parser, *grid, 0.7, 0.5, D, epsilon, Uinf, *penalization));
    floatingObstacle = new I2D_FloatingObstacleVector(
        parser, *grid, epsilon, Uinf, *penalization, shapesMap, D,
        modv); // ObstacleVector enables single object drag evaluation
  } else if (sOBSTACLE == "wheels") {
    floatingObstacle = new I2D_RotatingWheelObstacle(
        parser, *grid, epsilon, D / 2, Uinf, *penalization);
  }

  if (floatingObstacle != NULL)
    obstacle = floatingObstacle;

  // Note Mattia:	As in I2D_FlowPastFixedObstacle, this check must be
  // commented out because
  //				the constructor of this class, is used by other
  //study cases with different 				obstacles. The same way this class uses the
  //constructor of I2D_FlowPastFixedObstacle if (obstacle == NULL)
  //{
