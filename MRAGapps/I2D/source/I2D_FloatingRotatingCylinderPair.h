...
public:
  ...
  public:
    double angle, xm, ym, D, width, angular_velocity, vx, vy, m, J, rho, vdefx,
        vdefy, Uinf[2];
    double GammaLeft, GammaRight; // strength
    double timePrefac;
    double burgersTime; // this is the time with which the sine wave has evolved
                        // according to burgers equation

    RotatingCylinderPair(Real xm, Real ym, Real D, Real width, Real angle_rad,
                         Real GammaLeft, Real GammaRight,
                         Real burgersTime = 0.0);
  ...
  I2D_FloatingRotatingCylinderPair(
      ...
      I2D_PenalizationOperator &penalization, const int LMAX, const int ID = 0,
