...
template <typename Streamer, int components> struct I2D_VectorBlockLab {
public:
  ...
  public:
    Lab()
        : m_state(Uninitialized), m_weightPool(NULL), m_weightPoolSize(0),
          m_valuePool(NULL), m_valuePoolSize(0), row_size(0), base_offset(0),
        ...
        for (int iz = 0; iz < nZ; iz++)
          for (int iy = 0; iy < nY; iy++) {
            ...
            for (int ix = 0; ix < nX; ix++) {
              ...
              Streamer::template operate<components>(ptrSource[ix], target_time,
                                                     output);
      ...
      // 2.
      {
...

        {
          ...
          for (int i = 0; i < nValuePoolSize; ++i) {
            ...
            Streamer::template operate<components>((*b)[(*p).index],
                                                   target_time, *e);
    ...
    template <int component> Real get(int ix, int iy = 0) const {
