...
/**
 * creates a non-rotated NACA profile at (0,0), the leading edge is the center
 * of rotation geometry can be specified by the chord length and the NACA digits
 * d1, d2, d3d4
 *
 */
class NACA4gen {
 ...
 * and codomain [rotated airfoil at (tx, ty)] basically each point of the
...
public:
  Real tx, ty, angle, vx, vy, angular_velocity, rho, m,
      J; // (tx,ty): position of airfoil center

  DiscretizedWing(Real xm, Real ym, Real width, Real angle_deg, Real epsilon,
                  const int d1, const int d2, const int d3d4)
      : naca(width, 5000, false, epsilon, d1, d2, d3d4), scaling(1.0),
        epsilon(epsilon), width(width), tx(xm), ty(ym), angle(angle_deg),
...
struct FillBlocks {
  ...
  FillBlocks(Real eps, AirfoilStuff::DiscretizedWing *_shape) : eps(eps) {
    shape = _shape;
  }

  FillBlocks(const FillBlocks &c) : eps(c.eps) { shape = c.shape; }

  static bool _is_touching(Real eps, const AirfoilStuff::DiscretizedWing *wing,
...
struct GetNonEmpty {
  ...
  GetNonEmpty(Real eps, AirfoilStuff::DiscretizedWing *_shape,
              map<int, bool> &b2nonempty)
...
struct FillVelblocks {
  ...
  FillVelblocks(vector<pair<BlockInfo, VelocityBlock *>> &workitems, double eps,
                AirfoilStuff::DiscretizedWing *_shape)
...
I2D_ImposedAirfoil::I2D_ImposedAirfoil(
    ...
    const Real _D, const Real _angle, const Real _vx, const Real _vy,
    const int d1, const int d2, const int d3d4, const Real _eps,
    const Real Uinf[2], I2D_PenalizationOperator &penalization)
    : I2D_FloatingObstacleOperator(parser, grid, D, _eps, Uinf, penalization),
      vx_imposed(_vx), vy_imposed(_vy) {
  this->Uinf[0] = Uinf[0];
  this->Uinf[1] = Uinf[1];
  wing = new AirfoilStuff::DiscretizedWing(_xm, _ym, _D, _angle, eps, d1, d2,
                                           d3d4); // get rotated NACA profile
}
...
void I2D_ImposedAirfoil::update(
