...
public:
  ...
  NACA4gen(Real chord_in, unsigned int nPanels_in, bool isFiniteTE,
    ...
    // assure assignment of one of the boxes (values are > 0 but smaller than
...
public:
  DiscretizedWing(Real xm, Real ym, Real width, Real angle_deg, Real epsilon)
      : naca(width, 5000, false, epsilon), scaling(1.0), epsilon(epsilon),
        width(width) {
    const Real tx = xm;
    const Real ty = ym;
  ...
  void getTrailingBleedsLocations(int bleeIDX, Real &lenghtBleed,
                                  Real &widthBleed, Real &angleBleed,
                                  Real cmBleed[2], Real outletStart[2]) {
    ...
    Real lenghtTube = sqrt((inletw[0] - outletw[0]) * (inletw[0] - outletw[0]) +
                           (inletw[1] - outletw[1]) * (inletw[1] - outletw[1]));
  ...
  void getRearBleedsLocations(int bleeIDX, Real &lenghtBleed, Real &widthBleed,
                              Real &angleBleed, Real cmBleed[2],
                              Real outletStart[2]) {
    Real inlet01 = 0.596 * width;
...
struct FillCylBleed {
  ...
  FillCylBleed(Real support_radius, Real smooth_radius, Real smoothing_length,
               Real sphere_position[2])
      : support_radius(support_radius), smooth_radius(smooth_radius),
  ...
  FillCylBleed(const FillCylBleed &c)
      : support_radius(c.support_radius), smooth_radius(c.smooth_radius),
        smoothing_length(c.smoothing_length) {
  ...
  bool _is_touching(const BlockInfo &info)
      const // true if (at least a part of) the shape lies within the block
  {
    Real min_pos[2], max_pos[2];

    info.pos(min_pos, 0, 0); // minimal physical coordinates of block
    info.pos(max_pos, FluidBlock2D::sizeX - 1,
             FluidBlock2D::sizeY - 1); // maximal physical coordinates of block

    Real intersection[2][2] = {
        max(min_pos[0], sphere_box[0][0]),
        min(max_pos[0], sphere_box[0][1]),
        max(min_pos[1], sphere_box[1][0]),
        min(max_pos[1], sphere_box[1][1]),
    };
    ...
    if (_is_touching(info)) {
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
          ...
          b(ix, iy).tmp *= 1.0 - this->mollified_heaviside(r - smooth_radius,
                                                           smoothing_length);
        }
    }
  }
...
struct FillBlocksBleed {
  ...
  FillBlocksBleed(Real length, Real width, Real epsilon, Real cm[2],
                  Real angle) {
    m_length = length;
    m_width = width;
    m_epsilon = epsilon;
    ...
    _compute_vertices(v1, v2, v3, v4, nor1, nor2, m_cm, m_length, m_width,
                      m_angle);
    _find_rectangle_box();
  }

  FillBlocksBleed(const FillBlocksBleed &c)
      : m_length(c.m_length), m_width(c.m_width), m_epsilon(c.m_epsilon),
        m_angle(c.m_angle) {
    m_cm[0] = c.m_cm[0];
    m_cm[1] = c.m_cm[1];
  ...
  bool _is_touching(const BlockInfo &info) const {
    ...
    Real intersection[2][2] = {
        max(min_pos[0], rectangle_box[0][0]),
        min(max_pos[0], rectangle_box[0][1]),
        max(min_pos[1], rectangle_box[1][0]),
        min(max_pos[1], rectangle_box[1][1]),
    };
  ...
  Real mollified_heaviside(const Real x, const Real eps) const {
    const Real alpha =
        ...
        min(1.,
            max(0., (x + 0.5 * eps) /
                        eps)); // x negative inside box, positive outside box
    return 0.5 + 0.5 * cos(alpha); // outside: 0, inside: 1, boundary: mollified
                                   // heavyside
  }

  void _find_rectangle_box() {
    rectangle_box[0][0] =
        min(min(min(v1[0], v2[0]), v3[0]), v4[0]) - m_epsilon; // xmin
    rectangle_box[0][1] =
        max(max(max(v1[0], v2[0]), v3[0]), v4[0]) + m_epsilon; // xmax
    rectangle_box[1][0] =
        min(min(min(v1[1], v2[1]), v3[1]), v4[1]) - m_epsilon; // ymin
    rectangle_box[1][1] =
        max(max(max(v1[1], v2[1]), v3[1]), v4[1]) + m_epsilon; // ymax
  }
  ...
  void _compute_vertices(Real *v1, Real *v2, Real *v3, Real *v4, Real *nor1,
                         Real *nor2, const Real *cm, const Real length,
                         const Real width, const Real angle) {
    nor1[0] = 0.0;
    nor1[1] = 0.0;
    nor2[0] = 0.0;
    nor2[1] = 0.0;
    ...
    _compute_norm(cm1, cm, nor1,
                  nor2); // calculate specific block unit vectors nor1 and nor2
  ...
  Real _compute_char_func_box(const Real *v1, const Real *v2, const Real *v3,
                              const Real *v4, const Real *nor1,
                              const Real *nor2, const Real *pos,
                              const Real length, const Real width) const {
    Real dist = 0.0;
    ...
    if (distA <= width &&
        distB <= width) // y-pos inside y-range rectangular box
    {
    ...
    if (distC > length ||
        distD > length) // x-pos outside x-range rectangular box
    {
      dist = -1e30;
    }
  ...
  inline void
  operator()(const BlockInfo &info,
             FluidBlock2D &b) const // calculates characteristic function
  {
    if (_is_touching(info)) {
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
          ...
          Real dist = _compute_char_func_box(v1, v2, v3, v4, nor1, nor2, p,
                                             m_length, m_width);

          b(ix, iy).tmp *= 1.0 - this->mollified_heaviside(
                                     -dist, m_epsilon); // outside:1, inside: 0
        }
    }
  }
};
...
struct FillChiBlocks {
  ...
  bool _is_touching(const BlockInfo &info) const {
    ...
    WingStuff::wing->bbox(eps, wing_min,
                          wing_max); // store shapes' xmin, ymin in wing_min and
                                     // shapes' xmax, ymax in wing_max
  ...
  void
  operator()(blocked_range<int> range) const // computes characteristic function
  {
      ...
      if (_is_touching(info)) {
        for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
          for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
            ...
            chi_block.chi[iy][ix] =
                this->mollified_heaviside(WingStuff::wing->sdf(p), eps);
          }
      } else {
        for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
          for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
            ...
            assert(this->mollified_heaviside(WingStuff::wing->sdf(p), eps) <
                   1e-10);
          }
      }
    }
...
struct UpdateFluidBlocks {
  ...
  UpdateFluidBlocks(map<int, ChiBlock *> &cached_blocks)
      : cached_blocks(cached_blocks) {}
  UpdateFluidBlocks(const UpdateFluidBlocks &c)
...
I2D_WingObstacleOperator::I2D_WingObstacleOperator(Grid<W, B> &grid,
  ...
  WingStuff::wing = new WingStuff::DiscretizedWing(
      cm[0], cm[1], D, angle, smoothing_length); // get rotated NACA profile
}
...
void I2D_WingObstacleOperator::characteristic_function() {
  ...
  for (vector<BlockInfo>::iterator it = vInfo.begin(); it != vInfo.end();
       it++) {
  ...
  vector<int> toerase;
  for (map<int, ChiBlock *>::iterator it = cached_blocks.begin();
  ...
  tbb::parallel_for(blocked_range<int>(0, misses.size()), fill,
                    auto_partitioner());
  ...
  if (plainAirfoil == false) {
    ...
    for (unsigned int i = 0; i < 3; i++) {
      ...
      WingStuff::wing->getRearBleedsLocations(bleedToOut[i], lenghtBleed,
                                              widthBleed, angleBleed, cmBleed,
                                              outletStart);
      WingStuff::FillCylBleed fillCylBleed(widthBleed / 2.0 + smoothing_length,
                                           widthBleed / 2.0, smoothing_length,
                                           outletStart);
  ...
  for (unsigned int i = 0; i < activatedBleeds.size(); i++) {
    ...
    Real cmBleed[2] = {0.0, 0.0};
    Real outletStart[2] = {0.0, 0.0};
    WingStuff::wing->getRearBleedsLocations(activatedBleeds[i], lenghtBleed,
                                            widthBleed, angleBleed, cmBleed,
                                            outletStart);
    WingStuff::FillBlocksBleed fillBleed(lenghtBleed, widthBleed,
                                         smoothing_length, cmBleed, angleBleed);
    block_processing.process(vInfo, coll, fillBleed);
    WingStuff::FillCylBleed fillCylBleedLower(
        widthBleed / 2.0 + smoothing_length, widthBleed / 2.0, smoothing_length,
        cmBleed);
    block_processing.process(vInfo, coll, fillCylBleedLower);

    if (plainAirfoil == true) {
      WingStuff::FillCylBleed fillCylBleedUpper(
          widthBleed / 2.0 + smoothing_length, widthBleed / 2.0,
          smoothing_length, outletStart);
  ...
  for (unsigned int i = 0; i < activatedTrailingBleeds.size(); i++) {
    ...
    Real cmBleed[2] = {0.0, 0.0};
    Real outletStart[2] = {0.0, 0.0};
    WingStuff::wing->getTrailingBleedsLocations(
        activatedTrailingBleeds[i], lenghtBleed, widthBleed, angleBleed,
        cmBleed, outletStart);
    WingStuff::FillBlocksBleed fillBleed(lenghtBleed, widthBleed,
                                         smoothing_length, cmBleed, angleBleed);
    block_processing.process(vInfo, coll, fillBleed);
