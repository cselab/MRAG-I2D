...
struct ALIGN_ATTRIBUTE I2D_Layer {
  ...
  inline I2D_Layer<_sizeX, _sizeY, 1> &
  getSubLayer() // DO IT AT YOUR RISK!!!!!!!
  {
  ...
  template <int dim> void clear(Real val) {
    for (int iy = 0; iy < sizeY; iy++)
      for (int ix = 0; ix < sizeX; ix++)
        data[dim][iy][ix] = val;
  }
  ...
  template <int iDim> Real *getPlane() { return (Real *)&data[iDim][0][0]; }
  ...
  void deserialize(string sFileName) {
    ...
    //printf("deserialized, checking norm errors (che palle): Linf norm error =
...
#include <xmmintrin.h>
template <typename LayerT> LayerT *allocate() {
