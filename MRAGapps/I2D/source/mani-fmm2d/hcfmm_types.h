...
// Bounding box type:
template <class BaseType, int sDim> struct bbox {
...
template <class Particle> struct p_ind {
  tBoxKey key;
  Particle *p;

  // Bless Valgrind (don't need to change anything here):
  // ok I don't like this, but because I had problems with using
  // vector:push_back(), I am overwriting the constructor /destructors:
  //
  //	p_ind(): key(0), p(NULL) {
  //	//debug:
  ...
  //	~p_ind() {
  //		//debug:
  //		std::cout << "destroying an index, pointing to: " << p
  //<<std::endl;
  //
  //		key=0; p=NULL;
  //
...
template <class Particle>
tBoxKey getboxKey(Particle *p,
                  bbox<typename Particle::BaseType, Particle::dim> &bounds) {
  tBoxKey res = 0;
  for (int d = 0; d < Particle::dim; ++d) {
    (p->x[d] <
     (bounds.upper[d] - bounds.lower[d]) / (typename Particle::BaseType(2)))
        ? res = res
        : res += (1 << d);
  }
...
/*
 template <class Particle>
 p_ind<Particle> createpIndex(Particle* in_p, bbox<typename
 Particle::BaseType,Particle::dim>& bounds)
 {
 p_ind<Particle> res;
 res.p=in_p;
 res.key=0;
 for (int d=0; d<Particle::dim;++d)
 {
 (in_p->x[d]<(bounds.upper[d]-bounds.lower[d])/(typename
 Particle::BaseType(2)))?res.key=res.key:res.key+=(1<<d);
 }
 ...
 */
template <class Particle>
p_ind<Particle> createpIndex(Particle *in_p,
                             typename Particle::BaseType *center,
                             typename Particle::BaseType *h) {
...
//	//std::cout  << "[m,fac(2m),fac(m),(1<<m)]: [" << m <<"," << tmp1
//<<","<< tmp2 <<","<< tmp3 <<"]" <<std::endl; 	return
//NumType(tmp1)/(NumType(tmp2*tmp3));
//	//return factorial(2*m)/NumType((1<<m)*factorial(m));
// }

template <typename NumType, int _order> inline NumType sqrtfac(int n, int m) {
  ...
  /*
   ...
   while((cNum[k]!=0) && (p<k/2)  ) //if we have an non-zero entry and p is
   between 2 and k/2
   {
   ...
   //a different way would be to find the ggT and eliminate as many terms as
   possible -->would only use integer as long as possible for (int
   i=1;i<=cmax;++i){ Result+=cNum[i]*log(NumType(i));
   }
  ...
  // IMPROVE THIS: don't really need to calculate the high factorials (worst
  // case up to 2*n) but only n
...
template <typename NumType>
inline void
cart2sph(NumType *in_x, NumType *x0,
         NumType *out_x) // r,theta,phi !!!!! it is r,theta,phi NOT r,phi,theta
{
  NumType R(0);
  NumType dx[3] = {0, 0, 0};
  for (int d = 0; d < 3; ++d) {
  ...
  if (R > NumType(0)) {
    R = sqrt(R);
    out_x[0] = R;
    // out_x[1]=atan2(dx[2],sqrt(dx[0]*dx[0]+dx[1]*dx[1]));//acos((in_x[2]-x0[2])/R);
    // //theta, the angle between 0->pi
...
inline tint
pm1n(const tint &n) //(return -1^n in a more efficient way (hopefully))
{
  return 1 - 2 * (n % 2);
}

// calculates the Term involving Fractions of Factorials, which is used in the
// shifting formula: FracAnm=A_n^m*A_(j-n)^(k-m)/A_j^k Have simplified the term
// as much as possible with Mathematica: Sqrt[(j - k)! (j + k)!]/ ( Sqrt[(j + k
// - m - n)! (j - k + m - n)!] * Sqrt[(-m + n)! (m + n)!]) template <class
// ReturnType,int _order> ReturnType calcFracAnm(int j, int k, int n, int m)
//{
//
//	const int max_int=2*_order;
//	const int Nn=2;
...
//
//
//	//Further clean up the integerlist (something like a prime
//factorization) 	for (int k=cmax;k>3;k--) //Starting from the biggest numbers
//	{
//		int p=2; //smallest divisor is 2
//		while((cNum[k]!=0) && (p<k/2)  ) //if we have an non-zero entry
//and p is between 2 and k/2
//		{
//			if (k%p==0)                  //if it k is dividable by p
//			{
//				cNum[p]+=cNum[k];  //split it up into pieces and
//add it to the other stuff. 				cNum[k/p]+=cNum[k]; 				cNum[k]=0;
//			}
//			p++;
//		}
//	}
...
//
//	//This implementation uses the logarithmic.--> have always small number
//	//a different way would be to find the ggT and eliminate as many terms
//as possible -->would only use integer as long as possible 	for (int
//i=1;i<=cmax;++i){ 		Result+=cNum[i]*log(ReturnType(i));
//	}
//	//std::cout << "FoF:  before exp: " << Result << std::endl;
//	return sqrt(exp(Result));
// }
