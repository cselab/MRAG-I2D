...
template <typename tExpansions, int _maxlevel> class Box {
public:
  // typedefs:
  ...
  int level;
  Particle *vparticles;
  int nParticles; // if leaf: number of particles in vparticles, else, number of
                  // particles contained in children.
  ...
  Real getExpansionsCenter(int d) const { return expansions.Center[d]; }
  ...
  // constructor:
  Box()
      : isleaf(false), level(0), nParticles(0), TotalMass(0), vparticles(NULL),
        parent(NULL), got_expansions(false), got_COM(false), expansions(),
        maxlevelinuse(0), toDo_direct(), toDo_far(), id(0) {
  ...
  Box(const Box<tExpansions, maxlevel> &B)
      : level(B.level), isleaf(B.isleaf), vparticles(B.vparticles),
        parent(B.parent), nParticles(B.nParticles), expansions(B.expansions),
...
public:
  ...
  inline operator tBox *() const { return toDo.front(); }
  ...
  inline bool operator!=(const tBox *other) const { return !(*this == other); }
...
struct box_ref {
  ...
  ~box_ref() { p_Box = NULL; }
  // operator=:
  box_ref &operator=(const box_ref &B) {
    p_Box = B.p_Box;
    expanded = B.expanded;
    return *this;
  }
...
template <class tBox> struct box_walker {
  typedef typename tBox::Particle Particle;
  ...
  box_walker(const box_walker &B)
      : _rootBox(B._rootBox), direct_list(B.direct_list), ws_list(B.ws_list),
...
void box_walker<tBox>::_collectSplit(typename tBox::Btype *tpx) // tpoint x.
{
  ...
  ws_list.clear();
  direct_list.clear();
  // std::cout << "[box_walker: _collectSplit: ] collecting elements"
  // <<std::endl;
  ...
  while (it1 != work_list.end()) {
    ...
    if (ws_barnes_hut(it1->p_Box, tpx) &&
        it1->p_Box->nParticles >= minParticles) {
      ws_list.push_back(it1->p_Box);
    } else {
      if (it1->expanded == false) // we have not looked at the children, and it
                                  // this box is not well separated
      {
        // std::cout << "I am at level " << it1->level << "adding children to
        // toDo-list" <<std::endl;
        current = it1->p_Box;
        for (int kb = 0; kb < (1 << Particle::dim); ++kb) {
          if (current->children[kb] != NULL) {
            if (ws_barnes_hut(current->children[kb], tpx) &&
                it1->p_Box->nParticles >=
                    minParticles) // the kid is well separated
            {
              ws_list.push_back(
                  current->children[kb]); //-->we add it to the list and
                                          //continue with next kid
            } else {
              if (current->children[kb]
                      ->isleaf) // it is not well-separated and a leaf
              {
                direct_list.push_back(
                    current->children[kb]); // we want to interact with the
                                            // particles of that leaf directly
              } else // it is not well-separated and not a leaf, so we need to
                     // loop over the children of this kid. -->put it in the
                     // toDo list.
              {
  ...
  // std::cout << "Close: " << direct_list.size() << " Far: " << ws_list.size()
  // << std::endl; work_list=res;
...
template <class tBox> void box_walker<tBox>::_collect() {
...
#ifdef VERBOSE
      std::cout << "I am at level " << it1->p_Box->level
                << "adding children to toDo-list" << std::endl;
#endif
      current = it1->p_Box;
      for (int kb = 0; kb < (1 << Particle::dim); ++kb) {
        if (current->children[kb] != NULL) {
...
void box_walker<tBox>::callbottomup(void (*p_func)(tBox *)) {
  ...
  _collect();
  // note: the list should be automatically sorted in the way we collect the
  // work.
  it1 = work_list.end();
