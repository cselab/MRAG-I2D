...
template <typename Particle, int _order> struct _VortexExpansions {
  ...
  void clear() {
    for (int pd = 0; pd < Particle::pdim;
         ++pd) // HOLLY CRAP: when you put _order here instead of
               // Particle::pdim, you can look for a long time (bless valgrind)
    {
  ...
  void print() {
    for (int n = 0; n < _order; ++n) {
      for (int d = 0; d < Particle::pdim; ++d) {
        // std::cout << "expansion [" << d << "][" << n << "]="<<values[d][n]
        // <<std::endl;
        std::cout << "expansion [" << d << "][" << n
                  << "]=" << values[d][n].real() << " " << values[d][n].imag()
                  << std::endl;
...
#if 0 // this is an implementation based on Rick Beatson's and Leslie Gerards
      // "Short course on Fast Multipole Methods" 
...
template <typename Particle, int _order>
void _VortexExpansions<Particle, _order>::calculateExpansions(
    Particle *in_particles, int nParticles) {
  // ExpansionsValueType Cnm[Particle::pdim][_order*(_order+2)];
  ExpansionsValueType rp, prod,
      ZM; // Complex Distance relative to center: (Z_m-Z_M)
...
/*
 * Shift the Coefficients of a child to the location of the parent and adds it
 * to the parent.
 **/

template <typename Particle, int _order>
void _VortexExpansions<Particle, _order>::gatherExpansions(
    _VortexExpansions<Particle, _order> *in_expansions) {
  // std::cout << "SHOULD NOT BE HERE" <<std::endl;
  unsigned long int bcoeff(0);
  ExpansionsValueType rb, prod, csum;
  rb = ExpansionsValueType(in_expansions->Center[0], in_expansions->Center[1]) -
       ExpansionsValueType(this->Center[0], this->Center[1]);

  for (int j = 0; j < _order;
       ++j) // looping to get all the parents expansions filled up.
  {
    // for each value a_k wee need to loop from 0..k, and sum up.
    for (int pd = 0; pd < Particle::pdim; ++pd) {
      csum = ExpansionsValueType(0, 0);
      ...
      {
        bcoeff = binomial(j, k);
        csum += in_expansions->values[pd][k] * prod *
                ExpansionsValueType(bcoeff, 0);
...
template <typename Particle, int _order>
void _VortexExpansions<Particle, _order>::evaluateExpansions(
    typename Particle::BaseType *location,
    typename Particle::RHSType *out_RHS) {
  ExpansionsValueType csum, rp, prefac, prod;

  rp = ExpansionsValueType(location[0], location[1]) -
       ExpansionsValueType(this->Center[0], this->Center[1]);
