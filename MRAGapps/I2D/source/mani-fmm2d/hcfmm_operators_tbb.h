...
#pragma once
template <class BaseType, class Particle, int sDim> struct getBoundingBox_TBB {
  Particle *vParticles;
  bbox<BaseType, sDim> curBbox;
  int nParticles;

  getBoundingBox_TBB(Particle *_vParticles, bbox<BaseType, sDim> _BBox,
                     int _nParticles)
      : vParticles(_vParticles), curBbox(_BBox), nParticles(_nParticles) {}
  ...
  getBoundingBox_TBB(getBoundingBox_TBB &x, tbb::split)
      : nParticles(x.nParticles), vParticles(x.vParticles), curBbox(x.curBbox) {
  }
