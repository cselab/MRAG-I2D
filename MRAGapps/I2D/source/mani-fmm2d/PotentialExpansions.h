...
template <typename Ntype> void dumpData(Ntype *data, char *fname) {
...
void calcYnm(NumType phi, NumType Pnm[_order][_order],
  ...
  for (int n = 0; n < _order; ++n) {
    ...
    for (int m = 1; m <= n; ++m) {
      ...
      Ynm[_map(n, m)] = sqrtfac<NumType, _order>(n, m) * Pnm[n][m] *
...
template <typename Particle, int _order> struct _PotentialExpansions {
      ...
      for (int m = -n; m <= n; ++m)
        for (int d = 0; d < Particle::pdim; ++d)
          std::cout << "expansion [" << d << "][" << n << "][" << m
                    << "]=" << values[d][_map(n, m)] << std::endl;
  }
};

template <typename Particle, int _order>
void _PotentialExpansions<Particle, _order>::calculateExpansions(
...
template <typename Particle, int _order>
void _PotentialExpansions<Particle, _order>::gatherExpansions(
    _PotentialExpansions<Particle, _order> *in_expansions) {
  Btype Pnm[_order][_order];
  ExpansionsValueType Ynm[_order * (_order + 2)];
        ...
        for (int n = 0; n <= j; ++n) {
          for (int m = -n; m <= n; ++m) {
            ...
            if (abs(pr) >= abs(qr)) {
              ...
              csum += in_expansions->values[pd][_map(pr, qr)] *
                      pow(CI, tmpexp) * prod * Ynm[_map(n, -m)] * tmp1;
            }
          }
          prod *= val;
        }
...
template <typename Particle, int _order>
void _PotentialExpansions<Particle, _order>::evaluateExpansions(
  ...
  for (int n = 0; n < _order; ++n) {
    for (int m = -n; m <= n; ++m)
      for (int d = 0; d < Particle::pdim; ++d)
        out_RHS->x[d] +=
            std::real(this->values[d][_map(n, m)] / prod * Ynm[_map(n, m)]);
