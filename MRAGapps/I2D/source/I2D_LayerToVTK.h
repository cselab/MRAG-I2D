...
void dumpLayer2VTK(int istep, string sFileNamePattern,
                   const I2D_Layer<sizeX, sizeY, 1> &scalar_field) {
  char buf[500];
  ...
  for (int iy = 0; iy < sizeY; iy++)
    for (int ix = 0; ix < sizeX; ix++)
      grid->SetScalarComponentFromFloat(ix, iy, iz, 0,
                                        scalar_field.read(ix, iy, 0));
...
template <int sizeX, int sizeY>
void dumpLayer2VTK(int istep, string sFileNamePattern,
                   const I2D_Layer<sizeX, sizeY, 2> &scalar_field) {
  char buf[500];
  ...
  for (int iy = 0; iy < sizeY; iy++)
    for (int ix = 0; ix < sizeX; ix++) {
      grid->SetScalarComponentFromFloat(ix, iy, iz, 0,
                                        scalar_field.read(ix, iy, 0));
      grid->SetScalarComponentFromFloat(ix, iy, iz, 1,
                                        scalar_field.read(ix, iy, 1));
    }
