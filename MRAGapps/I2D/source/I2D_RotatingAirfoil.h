...
public:
  ...
  private:
    ...
    // maxBoxX: stores physical coordinate of maximal base point in x; the
    // others: analogue
  ...
  public:
    ...
    DiscretizedWing(Real _tx, Real _ty, Real width, Real epsilon, Real _angle,
                    Real _self_angle, const int d1, const int d2,
    ...
    void update_all(double dt, double t, const double peristrofiki,
                    const double _D, const double _theta);
    void getAerodynamicCenter(
  ...
  I2D_RotatingAirfoil(Grid<W, B> &grid, ArgumentParser &parser, const Real _xm,
                      const Real _ym, const Real _D, const Real _c,
                      const int d1, const int d2, const int d3d4, const Real _l,
                      const Real _eps, const Real Uinf[2],
