...
PF_Solver::PF_Solver(int argc, const char **argv)
    : parser(argc, argv), policy(NULL), collection(NULL), unitVelocity(0.0),
      ISLABFRAME(true), FITSELECT(1) {
  // Parse input
...
void PF_Solver::_prepareAgents() {
  ...
  collection = new PF_AgentVector(parser, LR, GREEDYEPS, SMOOTH, ISCONTROLLED,
                                  shapesMap);
...

        {
                ...
                for (int i = 0; i < nGrid; i++)
                {
                        ...
                        for (int j = 0; j < nGrid; j++)
                        {
                                ...
                                PF_Tracer * object = new PF_Tracer(parser,
location, counterID); map<string, vector<PF_Agent*> >::iterator it =
shapesMap.find(name); if (it == shapesMap.end()) shapesMap[name] =
vector<PF_Agent*>(); shapesMap[name].push_back(object); counterID++;
                        }
                }
        }
...
void PF_Solver::_computeVelocity() {
  ...
  for (int n = 0; n < (int)targets.size(); n++) {
    ...
    for (int j = 0; j < (int)vortices.size(); j++) {
      ...
      velocity +=
          (j == n) ? (Real)(0)
                   : -gammaj * (Y + I * X) / (Real)(2 * M_PI * (X * X + Y * Y));
    }
    velocities.push_back(velocity); /// beware, these are conjugate velocities
  }

  vector<complex<Real>>::iterator itVel = velocities.begin();
  for (vector<pair<Real, Real>>::iterator itTarg = targets.begin();
       itTarg != targets.end(); ++itTarg, ++itVel) {
...
void PF_Solver::_computeVelocityTracers() {
  ...
  for (int n = 0; n < (int)tracerPoints.size(); n++) {
    ...
    for (int j = 0; j < (int)vortices.size(); j++) {
      ...
      Real X = xn - xj;
      Real Y = yn - yj;
      velocity += -exagerateFactor * gammaj * (Y + I * X) /
                  (Real)(2 * M_PI * (X * X + Y * Y));
    }
    velocities.push_back(velocity); /// beware, these are conjugate velocities
  }
  ...
  vector<complex<Real>>::iterator itVel = velocities.begin();
  for (vector<pair<Real, Real>>::iterator itTarg = tracerPoints.begin();
       itTarg != tracerPoints.end(); ++itTarg, ++itVel) {
...
double PF_Solver::_setTimeStep() {
  ...
  double dt = max(dtMin, min(dtMax, max((Real)0, minDist - DMIN) *
                                        (dtMax - dtMin) / (DMAX - DMIN)));

  if ((Real)dt == dtMin)
    for (vector<pair<Real, Real>>::iterator it = storeIndex.begin();
         it != storeIndex.end(); ++it) {
...
#ifdef _RL_VIZ
void PF_Solver::_paintSquareCanvas(double edgeLength,
                                   double bottomLeftCorner[2],
                                   double bgColor[3], bool isOutlinedBlack) {
  ...
  //	GLfloat lightColorExit[] = {(GLfloat)bgColor[0], (GLfloat)bgColor[1],
  //(GLfloat)bgColor[2], 1}; 	glLightfv(GL_LIGHT0, GL_DIFFUSE, lightColorExit);
  glPushMatrix();
...
void PF_Solver::_paintBox(Real edgeLength, Real bottomLeftCorner[2],
                          Real bgColor[3]) {
  const double square[5][2] = {
      {bottomLeftCorner[0], bottomLeftCorner[1]},
      {bottomLeftCorner[0] + edgeLength, bottomLeftCorner[1]},
...
void PF_Solver::_paintSphere(Real x, Real y, Real radius, Real r, Real g,
                             Real b) const {
...
void PF_Solver::_paint(const double time) {
  ...
  _getTargetSchoolInfo(centerOfSchool,
                       edgeLength); // todo remove this from here to prevent
                                    // extra computation when painting
  Real bottomLeftCorner[2] = {centerOfSchool[0] - 0.5 * edgeLength,
                              centerOfSchool[1] - 0.5 * edgeLength};
  ...
  if (ISLABFRAME) {
    ...
    if (ISUSINGTRACERS)
      tracers->paint(1.0 / edgeLength, centerOfSchool);
  }
 ...
 * @param initialTime
 */
void PF::PF_Solver::_paintText(float x, float y, const char *text, float r,
...
void PF::PF_Solver::_checkStatus(double time, double &timeAtLastValidRefresh) {
  ...
  if (nTimesLearned < NLEARNINGLEVELS) {
    if (time - timeAtLastValidRefresh > LEARNINGTIME) {
      ...
      printf("** Finished learning stage %d/%d. New values for LR = %e, "
             "GREEDYEPS = %e, LEARNINGTIME = %0.2f.\n",
  ...
  // AVERAGING STATE
    ...
    if (time - timeAtLastValidRefresh >
        FITNESSTIME *
            FITNESSBUFFER) // finished with one averaging run, save fitness
    {
      _computeTimeAveragedFitness(time);

      currentFitness =
          (nCompletedAveraging * currentFitness + timeAveragedSchoolFitness) /
          (nCompletedAveraging + 1); // running average
      ...
      Real sigmasum = 0;
      for (int i = 0; i < nAveragesSoFar; ++i) {
        sigmasum += (currentFitnesses[i] - thisAverage) *
                    (currentFitnesses[i] - thisAverage);
      }
      ...
      for (int i = 0; i < n; ++i) {
        currentIndividualFitness[i] =
            (nCompletedAveraging * currentIndividualFitness[i] +
             avgFitness[i]) /
            (nCompletedAveraging + 1); // running average
      ...
      for (int j = 0; j < n; j++) {
        Real sum = 0.0;
        for (int i = 0; i < nAveragesSoFar; ++i) {
          const vector<double> &fitnessForThisSet =
              currentIndividualFitnesses[i];
        ...
        sigma = sqrt(sigmasum / nAveragesSoFar);
        fprintf(fid, "%e %e %e %e\n", coordinates[j].first,
                coordinates[j].second, thisAverage, sigma);
      }
      fclose(fid);
      ...
      printf(
          "** Finished averaging completely %d times (out of %d, NAVG = %d).\n",
      ...
      printf("** LR = %e. FITNESSTIME = %0.2f, FITNESSBUFFER = %0.2f.\n", LR,
             FITNESSTIME, FITNESSBUFFER);
      printf("** currentFitness = %e, crashPenalty = %e, "
             "penaltyForBeingTooClose = %e.\n",
             currentFitness, (double)crashPenalty, penaltyForBeingTooClose);
...
void PF::PF_Solver::_checkStatusPostAction(double time, double dt, bool valid,
                                           double &timeAtLastValidRefresh,
                                           double &timeAtLastNonValidRefresh) {
  if (ISCONTROLLED) {
    if (!valid || dt <= 1e-4) // || time step is too small -> probably collision
    {
      if (solverState == averaging) {
        ...
        timeAtLastValidRefresh = time;
        printf(
            "** WARNING: Screwed up %d times so far (out of %d, NAVG = %d) \n",
            nAveragingScrewUps, nTimesAveraged, NAVG);
      }

      printf("** NON VALID: Refresh at t = %e using %s.\n", time,
             isUsingSoftReset ? "soft resets" : "hard resets");
      ...
      if (solverState == learning) {
        ...
        if (time - timeAtLastNonValidRefresh < minTimeBetweenRefresh) {
          nTeribleRefreshes++;
          printf("** That is %d terrible refreshes with minTimeBetweenRefresh "
                 "= %0.4f\n",
                 nTeribleRefreshes, minTimeBetweenRefresh);
        }

        if (nTeribleRefreshes == nMaxTerribleRefresh) {
          printf("** Reached nMaxTerribleRefresh = %d. Agents are probably "
                 "close given the decision making time.\n",
                 nMaxTerribleRefresh);
          FILE *fid;
          fid = fopen("fitness", "w");
          fprintf(
              fid, "%e \n",
              1e8 - time); // CAN'T LEARN AT ALL -> ABORT (subtract time to rank
                           // better, if the simulation goes longer = better)
          fclose(fid);
          abort();
        }
      }
...
void PF::PF_Solver::_checkSettings() {
  ...
  if (FITNESSTIME * FITNESSBUFFER > timeToReachOtherSide) {
    printf("** ABORTING: FITNESSTIME * FITNESSBUFFER > timeToReachOtherSide! "
           "Check your settings!\n");
...
void PF::PF_Solver::_computeTimeAveragedFitness(double time) {
  ...
  //	for (int i = 0; i < n; i++)
  //		fprintf(fid, "%e %e %e\n", coordinates[i].first,
  //coordinates[i].second, avgFitness[i]); 	fclose(fid);
...
void PF::PF_Solver::_getTargetSchoolInfo(Real x[2], Real &edgeLength) {
  ...
  edgeLength =
      xrange > yrange ? (windowScale * xrange) : (windowScale * yrange);
