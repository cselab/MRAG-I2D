...
class PF_DipoleEight : public PF_Agent {
public:
  PF_DipoleEight(MRAG::ArgumentParser &parser, const Real _D, const Real _V,
                 const Real _acc, const Real _radius,
                 const complex<Real> _locationCenter, const Real _dir[2],
                 const Real lr, const Real greedyEps, bool isSmooth,
                 bool isControlled, const Real T = 0, const int _ID = 0,
                 RL::RL_TabularPolicy **_policy = NULL, const int seed = 0);
  virtual ~PF_DipoleEight();
...
protected:
  ...
  Real followedPoint[2]; // point swimmer creates for itself to travel so that
                         // he does not do something out of its capabilities
  Real targetPoint[2]; // point corresponding to RIGID advection of the lattice
                       // in the specified direction of travel
  Real dir[2];
  Real deltaStir;
  Real deltaSpeed;
  ...
  enum { straight, turnRight, turnLeft } pathAction;
  bool isInBox;
  Real boxLength;
