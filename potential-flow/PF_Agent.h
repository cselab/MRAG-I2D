...
public:
  ...
  PF_Agent(MRAG::ArgumentParser &parser, const Real lr, const Real greedyEps,
           bool isSmooth, bool isControlled, string _name = string(),
           const int _ID = 0, RL::RL_TabularPolicy **_policy = NULL,
           const int seed = 0);
  virtual ~PF_Agent();
  ...
  virtual void update(const Real dt, const Real time,
                      map<string, vector<PF_Agent *>> *collection = NULL) = 0;
  virtual void
  updatePostVelocityUpdate(const Real dt, const Real time,
                           map<string, vector<PF_Agent *>> *collection = NULL) {
  };
  virtual void
  storePosition(vector<pair<Real, Real>> &position,
                map<pair<Real, Real>, PF_Agent *> *positionAgent = NULL) {};
  ...
  virtual void
  getTargets(vector<pair<Real, Real>> &targets,
             map<pair<Real, Real>, PF_Agent *> *targetsAgent = NULL) {};
  ...
  bool mapStart(const Real time,
                map<string, vector<PF_Agent *>> *collection = NULL);
  void setReward(const Real time);
  void mapTest(const Real time, map<string, vector<PF_Agent *>> *collection);
  void stopTest(const Real time,
                map<string, vector<PF_Agent *>> *collection = NULL);

  virtual bool choose(const Real time,
                      map<string, vector<PF_Agent *>> *collection = NULL);
  virtual void learn(const Real time,
                     map<string, vector<PF_Agent *>> *collection = NULL,
  ...
  virtual void reward(const Real time,
                      map<string, vector<PF_Agent *>> *collection = NULL) {};
  virtual void fitness() {};
  virtual void error() {};
  virtual bool mapState(vector<int> &state,
                        map<string, vector<PF_Agent *>> *collection = NULL) {
    return true;
  };
...
#ifdef _RL_VIZ
  void _paintSphere(Real x, Real y, Real radius, Real r = 0.0, Real g = 0.0,
                    Real b = 0.0) const;
  void _paintX(Real x, Real y, Real radius, Real r, Real g, Real b) const;
  void _paintTriangle(Real xc, Real yc, Real radius, Real direction, Real r,
                      Real g, Real b) const;
  void _paintCone(Real xc, Real yc, Real radius, Real direction, Real r, Real g,
                  Real b) const;
  void _drawCircle(Real radius, Real xc = 0.5, Real yc = 0.5, Real r = 0.0,
                   Real g = 0.0, Real b = 0.0) const;
  void _drawFullCircle(Real radius, Real xc = 0.5, Real yc = 0.5, Real r = 0.0,
                       Real g = 0.0, Real b = 0.0) const;
...
protected:
  ...
  int _discretize(const Real signal, const Real minsignal, const Real maxsignal,
                  const int nlevels, const bool deadEnd = false) const;
  int _discretizeRange(const Real value, const Real minvalue,
