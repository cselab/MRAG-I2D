...
template <class tExpansions, int _maxlevel> struct boxBuilder {
  typedef typename tExpansions::ParticleType Particle;
  void buildBoxes(Particle *in_p, int nParticles,
...
void boxBuilder<tExpansions, _maxlevel>::printStats(
    Box<tExpansions, _maxlevel> *rootBox) {
  ...
  while (it1 != NULL) {
    std::cout << "Level: " << it1->level << " nParticles: " << it1->nParticles
              << " Leaf: " << it1->isleaf << std::endl;
    ...
    std::cout << "]" << std::endl;
    std::cout << "Radius: " << it1->expansions.Radius
              << " TotalMass: " << it1->TotalMass << std::endl;
    it1++;
  }
}
