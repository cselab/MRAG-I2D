...
public:
  Refiner_Greedy(int nMaxLevelJump = 1, int nMaxLevel = -1)
      : Refiner(nMaxLevelJump, nMaxLevel) {}
  ...
  virtual RefinementPlan *createPlan(const HierarchyType &hierarchy,
    ...
    // 2. iterate over the leafs: if a leaf should be refined (and the jump
    // resolution is not too high), create new children leaves
    // 3. return it
    ...
    for (HierarchyType::const_iterator it = hierarchy.begin();
      ...
      bool bJumpInLevelTooHigh = false;
      {
        ...
        for (vector<GridNode *>::const_iterator n = neighbors.begin();
             n != neighbors.end() && bJumpInLevelTooHigh == false; n++)
          bJumpInLevelTooHigh |=
              (abs((*n)->level - candidate_level) > m_nMaxLevelJump);
      }
      ...
      if (node->shouldBeRefined && !bJumpInLevelTooHigh &&
        ...
        // vRefinements.push_back(singleRefinement);
