...
public:
  QuadTree(const unsigned int _maxLevel, const double _width,
  ...
  //-------------------------------------------------------------------------------
  // Macro to traverse a quadtree from a specified cell (typically the root
  // cell) to a leaf cell by following the x and y locational codes, xLocCode
  // and yLocCode. Upon entering, cell is the specified cell and nextLevel is
  // one less than the level of the specified cell. Upon termination, cell is
  // the leaf cell and nextLevel is one less than the level of the leaf cell.
  //-------------------------------------------------------------------------------
  // BUG: in their implementation the pointer was not modified
  //-------------------------------------------------------------------------------
  void traverse(QtBox **cell, unsigned int &nextLevel,
                const unsigned int xLocCode, const unsigned int yLocCode);
  ...
  // BUG: in their implementation the pointer was not modified
  //-------------------------------------------------------------------------------
  void traverseToLevel(QtBox **cell, unsigned int &nextLevel,
                       const unsigned int xLocCode, const unsigned int yLocCode,
  ...
  // corresponding x or y locational code by binaryDiff (determined by XOR'ing
  // the appropriate pair of x or y locational codes). Upon entering, cell is
  // the specified cell and cellLevel is the cell's level. Upon termination,
  // cell is the common ancestor and cellLevel is the common ancestor's level.
  //-------------------------------------------------------------------------------
  void getCommonAncestor(QtBox **cell, unsigned int &cellLevel,
  ...
  // defined by its bottom-left vertex v0 and its top-right vertex v1, where v0
  // and v1 lie in [0,1)x[0,1).
  //-------------------------------------------------------------------------------
  ...
  // cell. Periodic tree is assumed, therefore the returned pointer cannot be
  // NULL. To each box a bool is associated: true if the box is a ghost, false
  // if the box is adjacent within the unit square doamain
  //-------------------------------------------------------------------------------
  ...
  locateLeftNeighborsPeriodic(QtBox *cell);
  ...
  //-------------------------------------------------------------------------------
  std::vector<std::pair<QtBox *, bool>>
  locateRightNeighborsPeriodic(QtBox *cell);
  ...
  //-------------------------------------------------------------------------------
  std::vector<std::pair<QtBox *, bool>>
  locateBottomNeighborsPeriodic(QtBox *cell);
  ...
  // The first entry in the main pair is the vector containing all adjacent
  // blocks. The second entry in the main pair is a vector of pairs containing
  // all non existing (outside square unit domain) ghosts. The first entry of
  // this second pair is the non-existing ghost indices, while the second is the
  // non-existing ghost indices, re-mapped periodically
  //-------------------------------------------------------------------------------
  std::pair<std::vector<I3>, std::vector<std::pair<I3, I3>>>
  locateNeighborhoodMRAG(QtBox *cell);

  //-------------------------------------------------------------------------------
  // Get MRAG neighbors
  //-------------------------------------------------------------------------------
  std::vector<std::pair<std::vector<I3>, std::vector<std::pair<I3, I3>>>>
  getNeighborsMRAG(std::vector<I3> const &input);
};
