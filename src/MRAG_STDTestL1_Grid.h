...
class MRAG_STDTestGrid
    : public MRAG_STDTestL1<Wavelets, Block> // LEVEL 1: crashes, memory leaks
{
protected:
  bool run(int nX, int nY, int nZ) {
    try {
      ...
      MRAG::Grid<Wavelets, Block> *grid =
          new MRAG::Grid<Wavelets, Block>(nX, nY, nZ);
      delete grid;

      cout << endl;
    } catch (...) {
      cout << "exception here at: nX: " << nX << " nY: " << nY << " nZ: " << nZ
           << endl;
      abort();
    }
