...
struct RefinerInspector {
  ...
  RefinerInspector(const GridNode *initial_node, const HierarchyType &hierarchy,
                   const NeighborhoodType &neighborhood, set<int> &black_list,
                   const int max_jump, const int max_level)
      : requested_node(initial_node), black_list(black_list),
        hierarchy(hierarchy), neighborhood(neighborhood), max_jump(max_jump),
        max_level(max_level) {}
  ...
  void inspect() {
    ...
    // requested_node->index[1], requested_node->index[2],
    // requested_node->level, to_refine.size());
  }

  bool _step_inspect(const GridNode *node) {
    const bool bInsideBlackList =
        (black_list.find(node->blockID) != black_list.end());
    if (bInsideBlackList)
      return false;
  ...
  static GridNode *_findRealNode(const HierarchyType &hierarchy,
                                 const NeighborhoodType &neighborhood,
                                 GridNode *node) {
    // this function finds the "true" grid node when the input grid node is a
    // ghost

    NeighborhoodType::const_iterator itNode =
        neighborhood.find(const_cast<GridNode *>(node));
...
class Refiner_BlackList : public Refiner {
  ...
  bool _check(const HierarchyType &hierarchy,
              const NeighborhoodType &neighborhood, set<int> black_list,
              SetOfNodes &to_refine) {
    for (SetOfNodes::iterator it = to_refine.begin(); it != to_refine.end();
         ++it) {
      ...
      NeighborhoodType::const_iterator itNeighbors =
          neighborhood.find(const_cast<GridNode *>(*it));
      assert(itNeighbors != neighborhood.end());
      vector<GridNode *> neighbors = itNeighbors->second;

      for (vector<GridNode *>::iterator itNeighbor = neighbors.begin();
           itNeighbor != neighbors.end(); ++itNeighbor) {
        const GridNode *neighbor = RefinerInspector::_findRealNode(
            hierarchy, neighborhood, *itNeighbor);
        ...
        if (!bToRefine)
          ...
          assert(abs(neighbor->level + 1 - ((*it)->level + 1)) <=
                 m_nMaxLevelJump);
      }
    }
...
public:
  Refiner_BlackList(int nMaxLevelJump = 1, int nMaxLevel = -1)
      : Refiner(nMaxLevelJump, nMaxLevel), black_list(NULL) {}

  void set_blacklist(set<int> *black_list) { this->black_list = black_list; }
  ...
  virtual RefinementPlan *createPlan(const HierarchyType &hierarchy,
    ...
    if (black_list == NULL)
      return Refiner::createPlan(hierarchy, neighborhood, vProcessingDirections,
                                 vRefinements);
    else {
      ...
      for (HierarchyType::const_iterator it = hierarchy.begin();
        ...
        if (node != NULL && !node->isEmpty && node->shouldBeRefined &&
            (m_nMaxLevel == -1 ? true : node->level < m_nMaxLevel))
          requested_nodes.insert(node);
      }

      SetOfNodes final_list;

      for (SetOfNodes::iterator it = requested_nodes.begin();
           it != requested_nodes.end(); ++it) {
        RefinerInspector inspector(*it, hierarchy, neighborhood, *black_list,
                                   this->m_nMaxLevelJump, this->m_nMaxLevel);
        inspector.inspect();

        final_list.insert(inspector.to_refine.begin(),
                          inspector.to_refine.end());
      }
      ...
      {
        int nNewChildren = 0;
        for (SetOfNodes::const_iterator it = final_list.begin();
             it != final_list.end(); ++it) {
