...
public:
  ...
  PlanBuilder(Plan *_plan, tBox *const _root_node, BlockInfo *_target_blocks,
              int _num_target_blocks)
      : m_plan_ptr(_plan), m_root_node(_root_node),
        m_target_blocks(_target_blocks),
        m_num_target_blocks(_num_target_blocks),
        m_root_source_particle(&m_root_node->vparticles[0]) {
    assert(m_plan_ptr != NULL && m_root_node != NULL &&
           m_target_blocks != NULL);
...
protected:
  bool _isclose_box(const tBox &sourceBox, const BlockInfo &info,
                    tBox::Btype _theta) const;
  bool _is_intersecting(const tBox &box, const BlockInfo &info) const;
  bool _ws_box_barnes_hut_and(const tBox &source_box,
                              const BlockInfo target_block,
                              tBox::Btype theta) const;
  bool _ws_box_barnes_hut_and_DUMB(const tBox &source_box,
                                   const BlockInfo &target_block,
                                   tBox::Btype theta) const;
};
