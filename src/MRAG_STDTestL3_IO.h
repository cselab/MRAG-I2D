...
protected:
  ...
  bool run(void) {
    try {
      // Test 1:	I am printing out a grid (culo) with its structure, then
      // I read it, then I print it again (culoCopy) to see if
      //			culo and culoCopy are the same

      // Test 2: I refine one block of the grid and I print it out as a VTK
      // format.
            ...
            for (int iy = 0; iy < TBlock::sizeY; iy++)
              for (int ix = 0; ix < TBlock::sizeX; ix++) {
                ...
                if (block(ix, iy, iz) != InitialData) {
                  printf("Something went wrong: at x=[ %g,%g,%g], initial: %g, "
                         "readin: %g \n",
