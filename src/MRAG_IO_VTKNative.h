...
void IO_VTKNative<TWavelets, TBlock, nChannels, iChannelStart>::Write(
    GridType &inputGrid, BoundaryInfo &bInfo, string fileName_) {
#ifndef _MRAG_VTK
  (void)inputGrid;
  (void)bInfo;
  (void)fileName_; // VTK output disabled
#else
  // Open output file
      ...
      for (int i = 0; i < vInfo.size(); i++) {
        ...
        if (TBlock::sizeZ == 1) {
          ...
          for (int iy = 0; iy < TBlock::sizeY; iy++)
            for (int ix = 0; ix < TBlock::sizeX; ix++) {
              const float rValue =
                  (lab(ix, iy, iz).giveMe(ichannel + iChannelStart, h));
              *vpts++ = rValue;
              ++icount;
            }
        }
