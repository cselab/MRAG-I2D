...
public:
  enum ETimeInterval {
    ...
    ETimeInterval_EndB,
    ETimeInterval_Start,
    ETimeInterval_StartB,
    ETimeInterval_Intermediate1,
    ETimeInterval_Intermediate2
  };
...
protected:
  struct StackItem {
    ...
    double t0;
    ETimeInterval type;
    StackItem(int l, double t, ETimeInterval type_)
        : level(l), t0(t), type(type_) {}
...
private:
  ...
  SpaceTimeSorter(const SpaceTimeSorter &)
      : m_bInSession(false), m_bConnected(false), m_bVerbose(false),
        ...
        m_stackRecursions(), m_maxLevel(0) {
    abort();
  }

  SpaceTimeSorter &operator=(const SpaceTimeSorter &) {
    abort();
    return *this;
  }
...
public:
  SpaceTimeSorter()
      : m_bInSession(false), m_bConnected(false), m_bVerbose(false),
        m_refBlockAtLevel(NULL), m_nRecursions(0), m_t0(0), m_dt(0),
        m_stackRecursions(), m_maxLevel(0), m_startLevel(0),
        m_nNofPerformedBlockOperations(0), m_nNofExpectedBlockOperations(0) {}
  ...
  virtual int estimateNofBlockOperations(int nRecursionsPerLevel,
...

  {
    ...
    do {
      ...
      if (false)
        printf(
            "Estimate: L: %d,  t:FAKE,  dt:FAKE (%s), so far: %d block-ops\n",
  ...
  virtual bool getBlocks(int &level, double &timeStep, double &currentTime,
    ...
    if (m_bVerbose)
      printf("L: %d,  t:%e,  dt:%e (%s)\n", l, t, dt,
             type == ETimeInterval_Start ? "Start" : "End");
    ...
    if (type == ETimeInterval_Start) {
      ...
      if (l < m_maxLevel) {
        ...
        for (int i = m_nRecursions - 1; i >= 0; i--)
          m_stackRecursions.push(
              StackItem(l + 1, t + i * children_dt, ETimeInterval_Start));
      }
    }
  ...
  void startSession(double timeStep, int nRecursionsPerLevel, double timeStart,
                    int startLevel) {
    assert(m_bConnected && !m_bInSession);
    ...
    m_nNofExpectedBlockOperations =
        estimateNofBlockOperations(nRecursionsPerLevel, startLevel);
