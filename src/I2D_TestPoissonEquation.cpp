...
I2D_TestPoissonEquation::I2D_TestPoissonEquation(const int argc,
                                                 const char **argv)
    : parser(argc, argv) {
    ...
    for (int iz = 0; iz < B::sizeZ; iz++)
      for (int iy = 0; iy < B::sizeY; iy++)
        for (int ix = 0; ix < B::sizeX; ix++) {
          ...
          b(ix, iy, iz).omega = PoissonEquationIC::_fill(p[0], p[1]);
          ; // z;//_fill(p[0], p[1], p[2]);//p[2];
          b(ix, iy, iz).u[0] =
              0.0; // x*x*x;//PoissonEquationIC::_fill(p[0], p[1], p[2]);
          b(ix, iy, iz).u[1] =
              0.0; // y*y;//PoissonEquationIC::_fill(p[0], p[1],
                   // p[2]);//sqrt(pow(p[0]-0.25, 2) + pow(p[1]-0.25, 2) +
                   // pow(p[2]-0.25, 2))<0.1;
          b(ix, iy, iz).tmp = -313;
        }
  }
