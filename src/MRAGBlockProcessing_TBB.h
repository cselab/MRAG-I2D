...
template <typename BlockType, template <typename BB> class Lab,
          typename Collection, typename ProcessingMT, int nSlots>
...
public:
  BlockProcessingMT_TBB(
      concurrent_bounded_queue<Lab<BlockType> *> &availableLabs,
      const BlockInfo *ptrInfos_, Collection &collection_,
    ...
    for (int i = 0; i < nSlots; i++) {
      ...
      lab->prepare(collection_, boundaryInfo_, processing_.stencil_start,
                   processing_.stencil_end);
  ...
  BlockProcessingMT_TBB(const BlockProcessingMT_TBB &p)
      : collection(p.collection), boundaryInfo(p.boundaryInfo),
        processing(p.processing), ptrInfos(p.ptrInfos),
        m_availableLabs(p.m_availableLabs) {}

private:
  // forbidden
  BlockProcessingMT_TBB &operator=(const BlockProcessingMT_TBB &p) {
    abort();
    return *this;
  }
}; /* BlockProcessingMT_TBB */
...
/**
 * Functor to actually perform the operations on the blocks.
 * See MRAG::Multithreading::DummySimpleBlockFunctor for a sample ProcessingMT
 * type.
 */
...
public:
  BlockProcessingMT_Simple_TBB(const BlockInfo *ptrInfos_,
                               ProcessingMT &processing_)
      : processing(processing_), ptrInfos(ptrInfos_) {}
  ...
  operator=(const BlockProcessingMT_Simple_TBB &p) {
    abort();
    return *this;
  }
 ...
 */
template <typename BlockType> class BlockProcessing_TBB {
...
protected:
  ...
  static void _getResources(concurrent_bounded_queue<Lab *> &resources,
                            const int nSlots) {
    // 0. checks
    ...
    map<string, vector<void *>>::const_iterator it =
        s_cachedResources.find(key);
    ...
    if (bCacheMiss) {
      ...
      for (vector<void *>::const_iterator itElem = it->second.begin();
  ...
  template <typename Collection>
  static void _releaseBlockPointers(vector<BlockInfo> &vInfo,
                                    Collection &collection) {
    for (vector<BlockInfo>::const_iterator it = vInfo.begin();
         it != vInfo.end(); it++)
...
public:
  ...
  /**
   * Process blocks in parallel using parallel_for (see tbb-doc) to split up the
   * work.
   ...
   *                      Optional: if not set, auto_partitioner (see tbb-doc)
   * will be used.
   */
  ...
  static void process(vector<BlockInfo> &vInfo, Collection &c, Processing &p,
    ...
    if (bAutomatic)
      parallel_for(blocked_range<size_t>(0, vInfo.size()), body,
                   auto_partitioner());
    else
  ...
  static void process(vector<BlockInfo> &vInfo, Collection &c, BoundaryInfo &b,
                      Processing &p, int nGranularity = -1) {
    ...
    BlockProcessingMT_TBB<BlockType, Lab, Collection, Processing, nSlots> body(
        resources, infos, c, b, p);
...
template <typename BlockType> int BlockProcessing_TBB<BlockType>::s_nBlocks = 0;
 ...
 */
template <typename BlockType, template <typename Btype> class LabType,
          int nSlots>
class BlockProcessing_Pipeline_TBB : public BlockProcessing_TBB<BlockType> {
  LabType<BlockType> m_LabSlots[nSlots];

  template <typename Processing> struct Token {
    BlockInfo *info;
    ...
    Token()
        : info(NULL), lab(NULL), processing(NULL), block(NULL), slotID(-1) {}
  ...
  template <typename Processing> class Filter_PickABlock : public tbb::filter {
  ...
  public:
    ...
    Filter_PickABlock(LabType<BlockType> *labSlots, const BlockInfo *vInfo,
                      const int nBlocks, Processing &p)
        : filter(false), ptrCurrentInfo(vInfo), availableSlots(),
          availableBlocks() {
    ...
    void *operator()(void *) {
      ...
      if (!availableBlocks.try_pop(iBlock))
        return NULL;
      assert(iBlock >= 0);
  ...
  private:
    // forbidden
    Filter_PickABlock(const Filter_PickABlock &p)
        : filter(false), ptrCurrentInfo(NULL), availableSlots(),
          availableBlocks() {
      abort();
    }

    Filter_PickABlock &operator=(const Filter_PickABlock &p) {
      abort();
      return *this;
    }
  };

  template <typename Processing> class Filter_Process : public tbb::filter {
  public:
  ...
  template <typename Processing> class Filter_Release : public tbb::filter {
    concurrent_bounded_queue<int> &availableSlots;

  public:
    Filter_Release(Filter_PickABlock<Processing> &pickAblock)
        : filter(false), availableSlots(pickAblock.availableSlots) {}
...
public:
  ...
  /**
   * Alias for process<LabType>(vInfo, c, b, p, nGranularity) of
   * BlockProcessing_TBB.
   * @see BlockProcessing_TBB::process()
   */
  template <typename Processing, typename Collection>
  static void process(vector<BlockInfo> &vInfo, Collection &c, BoundaryInfo &b,
                      Processing &p, int nGranularity = -1) {
    BlockProcessing_TBB<BlockType>::template process<LabType>(vInfo, c, b, p,
                                                              nGranularity);
  }

  /**
   * Alias for process<LabType>(vInfo, c, p, nGranularity) of
   * BlockProcessing_TBB.
   * @see BlockProcessing_TBB::process()
   */
  template <typename Processing, typename Collection>
  ...
  /**
   * Process blocks in parallel using a pipeline (see tbb-doc) to split up the
   * work.
   * @param vInfo         Info of all the blocks (to be processed) in the grid
   *                      (e.g. result of Grid::getBlocksInfo()).
   * @param c             Collection of all the blocks (to be processed) in the
  ...
  void pipeline_process(vector<BlockInfo> &vInfo, Collection &c,
    ...
    const BlockInfo *infos = BlockProcessing_TBB<BlockType>::_prepareBlockInfos(
        vInfo, c); // new BlockInfo[nBlocks];
