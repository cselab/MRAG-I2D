...
public:
  ...
  void fitness(I2D_FloatingObstacleOperator *floatingObstacle, double time,
    ...
    if (time >= startTime && time <= endTime) {
      ...
      const Real dist =
          sqrt((xStart - xx) * (xStart - xx) + (yStart - yy) * (yStart - yy));
...
I2D_SmartInteractions::I2D_SmartInteractions(const int argc, const char **argv)
    : I2D_FluidMediatedInteractions(argc, argv), efficiency(NULL), policy(NULL),
      TSTARTFTLE(0), TENDFTLE(0), TSTARTEFF(0), TENDEFF(0) {
  // Reset stuff set by studycases
  _dispose();
  // assert(floatingObstacle==obstacle);
  // if(floatingObstacle!=NULL && floatingObstacle==obstacle){ delete
  // floatingObstacle; floatingObstacle = NULL; obstacle = NULL;}
  ...
  // This must be deleted and reallocated in refresh!! Why? Look at the second
  // input!!
  // efficiency = new I2D_ComputeEfficiency(*grid, floatingObstacle, nu);
}

I2D_SmartInteractions::~I2D_SmartInteractions() { _dispose(); }
...
void I2D_SmartInteractions::_dispose() {
  ...
  assert(floatingObstacle == obstacle);
  if (floatingObstacle != NULL && floatingObstacle == obstacle) {
    delete floatingObstacle;
    floatingObstacle = NULL;
    obstacle = NULL;
  }
...
void I2D_SmartInteractions::_prepareAgents() {
  if (parser("-obstacle").asString() == "heterogeneous") {
    ...
    I2D_ObjectFactory factory(*grid, charLength, XPOS, YPOS, epsilon, Uinf,
                              *penalization, LMAX);
    factory.create(parser, shapesMap, true, &policy);

    // Compute characteristic velocity (the highest characteristic velocity
    // among the shapes and also Uinf is considered)
    ...
    floatingObstacle = new I2D_FloatingObstacleVector(
        parser, *grid, epsilon, Uinf, *penalization, shapesMap, charLength,
        charVel);
  } else {
  ...
  while (true) {
    ...
    for (int i = 0; i < ADAPTFREQ; i++) {
...

      {
        ...
        const Real nondim_factor =
            sqrt(Uinf[0] * Uinf[0] + Uinf[1] * Uinf[1]) * 2 / D;
