...
void RL_MultiTable::save(string name) {
  ...
  for (map<vector<int>, double>::iterator it = data.begin(); it != data.end();
       it++) {
    for (int i = 0; i < DIM; i++)
      out << (*it).first[i] << "  ";
  ...
  // if( ptr != NULL){ std::cout << command << std::endl; std::cout <<
  // "successful policy backup!" << std::endl; } else{ std::cout << command <<
  // std::endl; std::cout << "policy backup failed" << std::endl; }
  pclose(ptr);
  ...
  printf("counter=%d, ndata=%d, nlines=%d, nlinesBackup=%d\n", counter, ndata,
         nlines, nlinesBackup);
  if (ndata != nlines || nlines != counter || counter != ndata ||
      nlinesBackup != nlines) {
...
void RL_MultiTable::restart(string name) {
  const int DIM = dim.size();
  if (DIM == 0) {
    printf("Policy dimension was not set!\n");
    abort();
  }
  ...
  if (nlinesBackup >= 1) {
    ...
    if (in.good()) {
      ...
      in >> dimSignature;
      if (DIM != dimSignature) {
        printf("Saved policy and current policy do not match in "
               "dimensionality!\n");
        abort();
      } else {
        printf("dimSignature=%d\n", dimSignature);
      }
      ...
      printf("counter=%d, ndata=%d, nlinesBackup=%d\n", counter, ndata,
             nlinesBackup);
      if (ndata != nlinesBackup || nlinesBackup != counter ||
          counter != ndata) {
        printf("Something went fucking wrong!\n");
        abort();
      }
