...
template <>
inline void Streamer_OmegaAndVelocity::operate<3>(FluidElement2D &input,
                                                  Real time, Real output[3]) {
...
class I2D_ParticleBlockLab
    : public I2D_VectorBlockLab<Streamer_OmegaAndVelocity, 3>::Lab<BlockType> {
public:
  I2D_CoreParticles pcore;

  void load(const BlockInfo &info) {
    I2D_VectorBlockLab<Streamer_OmegaAndVelocity, 3>::Lab<BlockType>::load(
        info);
    ...
    if (info.index[0] == 0) {
      for (int iy = this->m_stencilStart[1]; iy < this->m_stencilEnd[1]; iy++)
        for (int ix = this->m_stencilStart[0]; ix < 0; ix++)
          this->m_sourceData[0][this->base_offset + ix + iy * this->row_size] =
              0;
    }
  }
};