...
struct BoundaryBlockPack {
  ...
  __device__
    ...
    int dir[3] = {B::sizeX > 1 ? floor(input_pos[0] / (float)B::sizeX) : 0,
    ...
    const int size[3] = {
        dir[0] < 0 ? -stencil_start[0]
                   : (dir[0] == 0 ? B::sizeX : stencil_end[0] - 1),
        dir[1] < 0 ? -stencil_start[1]
                   : (dir[1] == 0 ? B::sizeY : stencil_end[1] - 1),
        dir[2] < 0 ? -stencil_start[2]
                   : (dir[2] == 0 ? B::sizeZ : stencil_end[2] - 1)};
    ...
    offset = (input_pos[0] - s[0]) + (input_pos[1] - s[1]) * size[0] +
...
__device__
#endif
    inline void constructGhosts(BoundaryBlockPack &pack, int ipos[3],
                                ElementType &poolElement) {
  int code, ig;
