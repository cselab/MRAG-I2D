...
protected:
  ...
  int _discretize1SS(const Real signal, const Real minsignal,
                     const Real maxsignal, const int nlevels,
                     const bool deadEnd) const;

  // Discretize and account for two special states when the signal is beyond the
  // cutoff and when it s below a certain value, i.e. distance < 0 and therefore
  // agent inside another object. The priority is given to the inside obstacle
  // state.
  int _discretize2SS(const Real signal, const Real minsignal,
                     const Real maxsignal, const int nlevels,
                     const bool deadEnd = false,
                     const bool inside = false) const;

public:
  int ID;
  string name;

  RL_Agent(MRAG::ArgumentParser &parser, string _name = string(),
           const int _ID = 0, RL::RL_TabularPolicy **_policy = NULL,
           const int seed = 0)
      : name(_name), ID(_ID), policy(_policy), policystore(NULL), shared(true),
  ...
  bool startChoose(const double t,
  ...
  virtual bool choose(const double t,
                      map<string, vector<RL_Agent *>> *_data = NULL);
  virtual void learn(const double t,
                     map<string, vector<RL_Agent *>> *_data = NULL,
                     string name = string());

  // Methods re-implementable by all agents
  virtual void update(const double dt, const double t,
                      map<string, vector<RL_Agent *>> *_data = NULL,
                      string filename = string()) {};
  virtual void reward(const double t,
                      map<string, vector<RL_Agent *>> *_data = NULL) {};
