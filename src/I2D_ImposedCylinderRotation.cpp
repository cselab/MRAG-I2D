...
I2D_ImposedCylinderRotation::I2D_ImposedCylinderRotation(
    ...
    const Real _radiusRot, const Real _T, const Real _D, const Real _align,
    const Real eps, const Real Uinf[2], I2D_PenalizationOperator &penalization)
    : I2D_ImposedCylinder(parser, grid, _xm, _ym, 0.0, 0.0, _D, eps, Uinf,
                          penalization),
      XCM(_xm), YCM(_ym), radiusRot(_radiusRot), T(_T), align(_align) {}
...
void I2D_ImposedCylinderRotation::update(
  ...
  if (filename == std::string()) {
    ...
    ppFile =
        fopen("update_I2D_ImposedCylinderRotation.txt", t == 0.0 ? "w" : "a");
    assert(ppFile != NULL);
  } else {
