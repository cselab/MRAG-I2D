  ...
  inline void operator()(Lab &lab, const BlockInfo &info,
    ...
    for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
      for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
        const Real dwdx[2] = {2 * lab(ix + 1, iy) + 3 * lab(ix, iy) -
                                  6 * lab(ix - 1, iy) + lab(ix - 2, iy),
                              -lab(ix + 2, iy) + 6 * lab(ix + 1, iy) -
                                  3 * lab(ix, iy) - 2 * lab(ix - 1, iy)};

        const Real dwdy[2] = {2 * lab(ix, iy + 1) + 3 * lab(ix, iy) -
                                  6 * lab(ix, iy - 1) + lab(ix, iy - 2),
                              -lab(ix, iy + 2) + 6 * lab(ix, iy + 1) -
                                  3 * lab(ix, iy) - 2 * lab(ix, iy - 1)};

        const Real u[2] = {Uinf[0] + out(ix, iy).u[0],
                           Uinf[1] + out(ix, iy).u[1]};

        out.external_data[iy][ix] =
            factor *
...
struct GetUMax {
  ...
  GetUMax(map<int, Real> &local_max_velocities, Real Uinf[2])
      : local_max_velocities(local_max_velocities) {
    this->Uinf[0] = Uinf[0];
    this->Uinf[1] = Uinf[1];
  }
...
Real I2D_AdvectionOperator::compute_maxvel() {
  ...
  Real maxvel = 0;
  for (map<int, Real>::iterator it = velocities.begin(); it != velocities.end();
       it++)
...
struct AdvectionSimple {
  ...
  AdvectionSimple(Grid<W, B> &grid, BlockProcessing &block_processing,
                  Real Uinf[2], int &rhscounter_)
      : grid(grid), block_processing(block_processing),
        rhscounter(rhscounter_) {
    this->Uinf[0] = Uinf[0];
    this->Uinf[1] = Uinf[1];
  }

  template <template <typename BT> class CorrectLab> void integrate(double dt) {
    vector<BlockInfo> vInfo = grid.getBlocksInfo();
...
struct AdvectionLTS {
  ...
  AdvectionLTS(Grid<W, B> &grid, BlockProcessing &block_processing,
               Real Uinf[2], int &rhscounter, Real CFL, Real maxvel,
               const bool bSmartTrick)
      : grid(grid), block_processing(block_processing), rhscounter(rhscounter),
        CFL(CFL), maxvel(maxvel), bSmartTrick(bSmartTrick) {
  ...
  ~AdvectionLTS() { stsorter.disconnect(); }
...
void I2D_AdvectionOperator::perform_timestep(double dt) {
  ...
  if (state != Ready) {
    ...
    if (dt > val) {
      printf("oops.  I2D_AdvectionOperator::perform_timestep: dt=%e > largest "
             "dt allowed %e\n",
             dt, val);
    }
  ...
  if (bUseLTS) {
    printf("LTS!\n");
    AdvectionLTS lts(grid, block_processing, Uinf, rhscounter, CFL, tmp_maxvel,
                     true);
