...
I2D_FloatingObstacleVector::I2D_FloatingObstacleVector(
    ArgumentParser &parser, Grid<W, B> &grid, const Real eps,
    const Real Uinf[2], I2D_PenalizationOperator &penalization,
    map<string, vector<I2D_FloatingObstacleOperator *>> _data, Real charLength,
    Real charVel)
    : I2D_FloatingObstacleOperator(parser, grid, 1, eps, Uinf, penalization),
      data(_data), arrayVelBlock(NULL), charLength(charLength),
      charVel(charVel) {
  for (map<string, vector<I2D_FloatingObstacleOperator *>>::iterator it =
           data.begin();
       it != data.end(); ++it)
    for (vector<I2D_FloatingObstacleOperator *>::iterator it2 =
             it->second.begin();
...
I2D_FloatingObstacleVector::~I2D_FloatingObstacleVector() {
  for (vector<I2D_FloatingObstacleOperator *>::iterator it = agents.begin();
       it != agents.end(); ++it)
    if ((*it) != NULL) {
      delete (*it);
      (*it) = NULL;
...
Real I2D_FloatingObstacleVector::getD() const {
  printf("The call to this method is for an eterogeneous collection of "
         "obstacle, not implemented yet!\n");
  abort();
  return 1.0;
}
...
Real I2D_FloatingObstacleVector::getDrag() const {
  Real SumcD = 0.0;
  for (vector<I2D_FloatingObstacleOperator *>::const_iterator it =
           agents.begin();
       it != agents.end(); ++it)
    SumcD += (*it)->getDrag();
...
void I2D_FloatingObstacleVector::update(
...
void I2D_FloatingObstacleVector::computeDesiredVelocity(const double t) {
  ...
  // Clean up obstacle vector desired velocity (TODO: blocks could be
  // allocated/deallocated sync with refinement/compression)
  ...
  set<int> idx;
  for (vector<map<int, const VelocityBlock *>>::iterator ito =
           vectorDesiredVelocities.begin();
       ito != vectorDesiredVelocities.end(); ito++)
    for (map<int, const VelocityBlock *>::iterator it = (*ito).begin();
         it != (*ito).end(); it++)
      idx.insert(it->first);
  ...
  for (map<int, VelocityBlock *>::iterator it = gmap.begin(); it != gmap.end();
       it++)
...
void I2D_FloatingObstacleVector::save(const double t, string filename) {
  if (filename == "") {
    printf("What?? I2D_FloatingObstacleVector does not have the step_id "
           "foffuckffake!\n");
    abort();
  }
...
void I2D_FloatingObstacleVector::restart(const double t, string filename) {
  if (filename != "") {
    printf(
        "What?? I2D_FloatingObstacleVector does not restart foffuckffake! \n");
    abort();
  }
...
void I2D_FloatingObstacleVector::refresh(const double t, string filename) {
  if (filename != "") {
    printf(
        "What?? I2D_FloatingObstacleVector does not refresh foffuckffake! \n");
    abort();
  }
...
void I2D_FloatingObstacleVector::computeDragAndStuff(const Real time,
...
bool I2D_FloatingObstacleVector::choose(
    const double t,
    map<string, vector<I2D_FloatingObstacleOperator *>> *_data) {
  bool valid = true;
  for (vector<I2D_FloatingObstacleOperator *>::iterator it = agents.begin();
...
void I2D_FloatingObstacleVector::learn(
...
void I2D_FloatingObstacleVector::reward(
    const double t,
    map<string, vector<I2D_FloatingObstacleOperator *>> *_data) {
  for (vector<I2D_FloatingObstacleOperator *>::iterator it = agents.begin();
       it != agents.end(); ++it)
