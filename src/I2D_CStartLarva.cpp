...
I2D_CStartLarva::CStartLarva::CStartLarva(
    double xm, double ym, double _D, double Tprep, double Tprop, double phase,
    double tau, double angle_rad, vector<double> BASELINE,
    vector<double> CURVATURE, double angleInSpace_rad, double eps,
    const int LMAX, const bool isSharp)
    : Tprep(Tprep), Tprop(Tprop),
      StefanFish(xm, ym, _D, Tprop, phase, tau, angle_rad, BASELINE, CURVATURE,
                 angleInSpace_rad, eps, LMAX, isSharp) {
...
double I2D_CStartLarva::CStartLarva::_getWidth(const double &ss) const {
  ...
  const double a =
      (-2.0 * (WT - WH) - WT * (ST - SB)) / ((ST - SB) * (ST - SB) * (ST - SB));
...
void I2D_CStartLarva::CStartLarva::updateInSpace(const double t) {
  ...
  _startWave(time);
  _schedulerBaseline(time, timeTransition, baseline, 6, S, N,
                     mBaselineInterpolated, mBaselineDerivative);
  _schedulerCurvature(time, timeTransition, curvature, 6, S, N, K,
...
I2D_CStartLarva::I2D_CStartLarva(
    ArgumentParser &parser, Grid<W, B> &grid, const Real _xm, const Real _ym,
    const Real D, const Real Tprep, const Real Tprop, const Real phase,
    const Real tau, const Real angle, vector<double> BASELINE,
    vector<double> CURVATURE, const Real angleInSpace, const Real eps,
    const Real Uinf[2], I2D_PenalizationOperator &penalization, const int LMAX,
    const int ID, RL::RL_TabularPolicy **policy)
    : I2D_StefanFish(parser, grid, _xm, _ym, D, Tprop, phase, tau, angle,
                     BASELINE, CURVATURE, angleInSpace, eps, Uinf, penalization,
                     LMAX, ID, policy) {
  assert(shape != NULL);
  if (shape != NULL) {
    delete shape;
  ...
  shape =
      new CStartLarva(_xm, _ym, D, Tprep, Tprop, phase, tau, angle, BASELINE,
                      CURVATURE, angleInSpace, eps, LMAX, isSharp);
