...
template <typename TWavelets, typename TBlock, typename TProjector,
          int nChannels>
IO_VTK<TWavelets, TBlock, TProjector, nChannels>::IO_VTK() {}
...
template <typename TWavelets, typename TBlock, typename TProjector,
          int nChannels>
void IO_VTK<TWavelets, TBlock, TProjector, nChannels>::Write(
    GridType &inputGrid, string fileName) {
  // Open output file
  fileName += ".vtk";
  ...
  if (TBlock::sizeZ == 1) {
    totalNumberOfPoints =
        vInfo.size() * (TBlock::sizeX + 1) * (TBlock::sizeY + 1);
  ...
  for (int i = 0; i < vInfo.size(); i++) {
    ...
    // don't need that for locations of points: TBlock& block =
  ...
  output << "CELLS " << totalNumberOfCells << " "
         << totalNumberOfCells * (verticesPerCell + 1) << std::endl;
        ...
        for (int iy = 0; iy < TBlock::sizeY; iy++)
          for (int ix = 0; ix < TBlock::sizeX; ix++) {
            ...
            for (int j = 0; j < verticesPerCell; j++) {
              ...
              int iyy = iy + shift[1];
              int izz = iz + shift[2];
              int pointIndex = counter * (TBlock::sizeX + 1) *
                                   (TBlock::sizeY + 1) * (TBlock::sizeZ + 1) +
  ...
  lab.prepare(inputGrid.getBlockCollection(), inputGrid.getBoundaryInfo(),
              steStart, steEnd);
  for (int i = 0; i < vInfo.size(); i++) {
    BlockInfo &info = vInfo[i];
    lab.load(info);
    // using the blocklab, don't need this: \TBlock& block =
    // inputGrid.getBlockCollection()[info.blockID];
    if (TBlock::sizeZ > 1) {
      for (int iz = 0; iz < TBlock::sizeZ + 1; iz++)
        for (int iy = 0; iy < TBlock::sizeY + 1; iy++)
          for (int ix = 0; ix < TBlock::sizeX + 1; ix++) {
            output << TProjector::template Project<ElementType, nChannels>(
                          lab(ix, iy, iz))
                   << "  ";
          }
      output << std::endl;
...
template <typename TWavelets, typename TBlock, typename TProjector,
          int nChannels>
void IO_VTK<TWavelets, TBlock, TProjector, nChannels>::Read(GridType &inputGrid,
                                                            string fileName) {
