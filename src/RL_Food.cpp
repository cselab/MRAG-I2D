...
RL_Food::RL_Food(MRAG::ArgumentParser &parser, const Real _x, const Real _y,
                 const Real _D, const int _ID, const int seed,
                 RL_TabularPolicy **_policy)
    : x(_x), y(_y), D(_D), count(1), rng(seed),
      RL_Agent(parser, "RL_Food", _ID, NULL, seed) {
  // printf("seed = %f\n", rng.uniform());
  // exit(0);
}
...
void RL_Food::update(const double dt, const double t,
                     map<string, vector<RL_Agent *>> *_data, string filename) {
