    ...
    if (region[0][1] > region[0][0])
      for (int iy = 0; iy < B::sizeY; iy++)
        for (int ix = region[0][0]; ix < region[0][1]; ix++)
          streamer::stream(out(ix, iy),
                           factor *
                               (-3 * lab.template get<component>(ix, iy) +
                                4 * lab.template get<component>(ix + 1, iy) -
                                lab.template get<component>(ix + 2, iy)));

    for (int iy = 0; iy < B::sizeY; iy++)
      for (int ix = region[1][0]; ix < region[1][1]; ix++)
        streamer::stream(out(ix, iy),
                         factor * (lab.template get<component>(ix + 1, iy) -
                                   lab.template get<component>(ix - 1, iy)));
      ...
      for (int iy = region[0][0]; iy < region[0][1]; iy++)
        for (int ix = 0; ix < B::sizeX; ix++)
          streamer::stream(out(ix, iy),
                           factor *
                               (-3 * lab.template get<component>(ix, iy) +
                                4 * lab.template get<component>(ix, iy + 1) -
                                lab.template get<component>(ix, iy + 2)));

    for (int iy = region[1][0]; iy < region[1][1]; iy++)
      for (int ix = 0; ix < B::sizeX; ix++)
        streamer::stream(out(ix, iy),
                         factor * (lab.template get<component>(ix, iy + 1) -
                                   lab.template get<component>(ix, iy - 1)));
...
struct I2D_GradOfVector_4thOrder {
    ...
    if (region[0][1] > region[0][0])
      for (int iy = 0; iy < B::sizeY; iy++)
        for (int ix = region[0][0]; ix < region[0][1]; ix++)
          streamer::stream(out(ix, iy),
                           factor *
                               (-25 * lab.template get<component>(ix, iy) +
                                48 * lab.template get<component>(ix + 1, iy) -
                                36 * lab.template get<component>(ix + 2, iy) +
                                16 * lab.template get<component>(ix + 3, iy) -
                                3 * lab.template get<component>(ix + 4, iy)));
    ...
    if (region[1][1] > region[1][0])
      for (int iy = 0; iy < B::sizeY; iy++)
        for (int ix = region[1][0]; ix < region[1][1]; ix++)
          streamer::stream(out(ix, iy),
                           factor *
                               (-3 * lab.template get<component>(ix - 1, iy) -
                                10 * lab.template get<component>(ix, iy) +
                                18 * lab.template get<component>(ix + 1, iy) -
                                6 * lab.template get<component>(ix + 2, iy) +
                                lab.template get<component>(ix + 3, iy)));

    for (int iy = 0; iy < B::sizeY; iy++)
      for (int ix = region[2][0]; ix < region[2][1]; ix++)
        streamer::stream(out(ix, iy),
                         factor * (lab.template get<component>(ix - 2, iy) -
                                   8 * lab.template get<component>(ix - 1, iy) +
                                   8 * lab.template get<component>(ix + 1, iy) -
                                   lab.template get<component>(ix + 2, iy)));
  ...
  void _dfdx_ptr(Lab &lab, const BlockInfo &info, Real *const ptr) const {
    ...
    if (region[0][1] > region[0][0])
      for (int iy = 0; iy < B::sizeY; iy++)
        for (int ix = region[0][0]; ix < region[0][1]; ix++)
          streamer::stream(ptr[iy * B::sizeX + ix],
                           factor *
                               (-25 * lab.template get<component>(ix, iy) +
                                48 * lab.template get<component>(ix + 1, iy) -
                                36 * lab.template get<component>(ix + 2, iy) +
      ...
      for (int iy = region[0][0]; iy < region[0][1]; iy++)
        for (int ix = 0; ix < B::sizeX; ix++)
          streamer::stream(out(ix, iy),
                           factor *
                               (-25 * lab.template get<component>(ix, iy) +
                                48 * lab.template get<component>(ix, iy + 1) -
                                36 * lab.template get<component>(ix, iy + 2) +
                                16 * lab.template get<component>(ix, iy + 3) -
                                3 * lab.template get<component>(ix, iy + 4)));
    ...
    if (region[1][1] > region[1][0])
      for (int iy = region[1][0]; iy < region[1][1]; iy++)
        for (int ix = 0; ix < B::sizeX; ix++)
          streamer::stream(out(ix, iy),
                           factor *
                               (-3 * lab.template get<component>(ix, iy - 1) -
                                10 * lab.template get<component>(ix, iy) +
                                18 * lab.template get<component>(ix, iy + 1) -
                                6 * lab.template get<component>(ix, iy + 2) +
                                lab.template get<component>(ix, iy + 3)));

    for (int iy = region[2][0]; iy < region[2][1]; iy++)
      for (int ix = 0; ix < B::sizeX; ix++)
        streamer::stream(out(ix, iy),
                         factor * (lab.template get<component>(ix, iy - 2) -
                                   8 * lab.template get<component>(ix, iy - 1) +
                                   8 * lab.template get<component>(ix, iy + 1) -
                                   lab.template get<component>(ix, iy + 2)));
