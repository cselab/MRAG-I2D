...
template <typename DataType> class HuffmanEncoder : public Encoder<DataType> {
  ...
  struct TranslationItem {
    ...
    TranslationItem(const TranslationItem &item)
        : p(item.p), encoding(item.encoding), symbol(item.symbol) {}

  private:
    TranslationItem &operator=(TranslationItem &item) {
      abort();
      return *this;
    }
  };
  ...
  struct PQitem {
    ...
    PQitem(const PQitem &item)
        : p(item.p), symbolsAndPrefixes(item.symbolsAndPrefixes) {}

    PQitem(const PQitem &item1, const PQitem &item2)
        : p(0), symbolsAndPrefixes() {
      item1.add_bit(0);
      item2.add_bit(1);

      p = item1.p + item2.p;

      symbolsAndPrefixes.resize(item1.symbolsAndPrefixes.size() +
                                item2.symbolsAndPrefixes.size());
      set_union(item1.symbolsAndPrefixes.begin(),
                item1.symbolsAndPrefixes.end(),
                item2.symbolsAndPrefixes.begin(),
                item2.symbolsAndPrefixes.end(), symbolsAndPrefixes.begin());
    }
  };
  ...
  struct LUT // used in the _translate_back (decompression)
  {
    ...
    struct Item {
      ...
      Item(const Item &item)
          : next_lut(item.next_lut), symbol(item.symbol),
      ...
      void printItem() const {
        if (symbol != NULL)
          ...
          printf("symbol=another LUT (0x%x), length = nan\n",
                 (unsigned int)(long unsigned int)next_lut);
        }
      }
    };
    ...
    void printLUT() const {
      printf("LUT (0x%x):\n", (unsigned int)(long unsigned int)this);
      for (int i = 0; i < nEntries; i++) {
      ...
      for (typename map<DataType, BitStream>::const_iterator it =
        ...
        // A.
        {
          for (int c = 0; c < nof_chunks - 1; c++) {
            const unsigned int chunk_data =
                bits.get_bits(c * chunk_size, chunk_size);
        ...
        // B.
        {
          const int remaining_bits =
              bits.getLength() - chunk_size * (nof_chunks - 1);
          const int lshift = chunk_size - remaining_bits;
          const unsigned int prefix =
              bits.get_bits((unsigned int)(bits.getLength() - remaining_bits),
                            remaining_bits);
          const unsigned int postfixes = 1 << lshift;
          for (unsigned int i = 0; i < postfixes; i++) {
            typename LUT<chunk_size>::Item &item =
                (*currLUT)[prefix + (i << remaining_bits)];
...
public:
  HuffmanEncoder()
      : Encoder<DataType>(), m_vCheat(), m_encodedStream(),
        m_bPathologicCase(false), m_PathologicSymbol(NULL),
        m_entropyPerDigit(HUGE_VAL), m_maxDigitEncodingLength(0),
        m_encodedTable_Symbols(), m_encodedTable_Encodings() {}
  ...
  void encode(const vector<DataType> &stream, const int nSymbols) {
    ...
    if (!m_bPathologicCase) {
      _computeTranslationTable(mapFrequencies, mapTranslationTable,
                               m_maxDigitEncodingLength);
      _translate(m_entropyPerDigit, stream, mapTranslationTable,
                 m_encodedStream);
      _encodeTranslationTable(mapTranslationTable, m_encodedTable_Symbols,
                              m_encodedTable_Encodings);

      const float uncompressedMB =
          stream.size() * sizeof(DataType) / 1024. / 1024.;
      const double compression_rate = uncompressedMB / getMemorySize();

      if (Encoder<DataType>::bVerbose)
        printf("HuffmanEncoder Compression Rate %f  (%.2f KB instead of %.2f "
               "KB)\n",
               compression_rate, getMemorySize() * 1024.,
               uncompressedMB * 1024.);

      /*printf("%s, Compression Rate: %f, est. size %d, actualsize = %d (msg
         symbols: %d, symbols in alphabet %d)\n",
                 estimated_encoding_bits>=compressed_bits?"Successiful
         Estimated": "Failed Estimation", compression_rate,
         compressed_bits/8./1024., uncompressed_bits/8./1024.,
                 estimated_encoding_bits, encodedStream.getLength(),
         stream.size(), mapTable.size());*/
    } else {
  ...
  float getMemorySize() const {
    ...
    return (sizeof(HuffmanEncoder) + sBytes) / (1024. * 1024.) +
           m_encodedTable_Symbols.getMemorySize();
  }

private:
  // forbidden
  HuffmanEncoder(const HuffmanEncoder &h)
      : Encoder<DataType>(), m_encodedStream(), m_bPathologicCase(false),
        m_PathologicSymbol(NULL), m_entropyPerDigit(HUGE_VAL) {
    abort();
  }

  HuffmanEncoder &operator=(const HuffmanEncoder &h) {
    abort();
    return *this;
  }
};
