...
PF_Dipole::PF_Dipole(MRAG::ArgumentParser &parser, const Real _D, const Real _V,
                     const complex<Real> _locationCenter, const Real _alpha,
                     const Real lr, const Real greedyEps, bool isSmooth,
                     bool isControlled, const Real T, const int _ID,
                     RL::RL_TabularPolicy **_policy)
    : PF_Agent(parser, lr, greedyEps, isSmooth, isControlled, "PF_Dipole", _ID,
               _policy),
      D(_D / (2 * sqrt(2 * M_PI))), V(_V), locationCenter(_locationCenter),
      alpha(_alpha), gammaRight(0), gammaLeft(0), deadDipole(false),
      maxDomainRadius(0.5) {
  cutoff = 10 * D;
  gamma = 2 * M_PI * V * D;
...
void PF_Dipole::update(const Real dt, const Real time,
...
void PF_Dipole::getTargets(vector<pair<Real, Real>> &targets,
...
bool PF_Dipole::mapState(vector<int> &state,
                         map<string, vector<PF_Agent *>> *collection) {
  ...
  Real dist = sqrt(myX[0] * myX[0] + myX[1] * myX[1]);
  dist = (dist == 0.0) ? 1.0 : dist;
  Real radialDir[2] = {myX[0] / dist, myX[1] / dist};
  radialDir[0] =
      ((radialDir[0] == 0.0) && (radialDir[1] == 0.0)) ? 1.0 : radialDir[0];
  radialDir[1] =
      ((radialDir[0] == 0.0) && (radialDir[1] == 0.0)) ? 0.0 : radialDir[1];
  ...
  //		const Real target[2] =
  //{coord[0].first-center[0],coord[0].second-center[1]}; 		Real d[2];
  //		_dist(target, myX, d);
...
void PF_Dipole::_mapForMaxRadius(int &idxDist, int &idxAngle,
                                 map<string, vector<PF_Agent *>> *collection) {
  // Gather my info
  const Real center[2] = {0.5, 0.5};
  Real x = real(locationCenter);
  ...
  Real dir[2];
  _normalize(dist, dir);
  const Real target[2] = {maxDomainRadius * dir[0] + center[0],
                          maxDomainRadius * dir[1] + center[1]};
...
void PF_Dipole::mapAction(int action, Real time) {
  ...
  //	if(out)
  //	{
  //		out << time << " " << action << " " << gammaRight << " " <<
  //gammaLeft << " " << myAction << " " << myPreviousAction << endl;
  //		out.close();
  //	}
  //	else
...
void PF_Dipole::perform(const Real time) {
  ...
  //	if(out)
  //	{
  //		out << time << " " << gammaRight << " " << gammaLeft << " " <<
  //diffGammaR << " " << myAction << " " << myPreviousAction << endl;
  //		out.close();
  //	}
  //	else
  //		cerr << "Problem opening the file !" << endl;
...
void PF_Dipole::reward(const Real time,
                       map<string, vector<PF_Agent *>> *collection) {
  ...
  if (time > (learningTimer + learningInterval) && status == Waiting) {
    ...
    const Real xx[2] = {x, y};
    const Real dist =
        sqrt((xx[0] - 0.5) * (xx[0] - 0.5) + (xx[1] - 0.5) * (xx[1] - 0.5));
    ...
    //			b->storePosition(coord);
    //			Real angle = b->alpha;
    //			const Real turnSens = ((coord[0].first > 0.5 && angle >
    //0.) || (coord[0].first < 0.5 && angle < 0.))?-1:1; 			turn += turnSens;
    //		}
    //
    //		const Real sensRight = (turn <= 0)?false:true;
    //		const Real sensDipole = ((xx[0] > 0.5 && alpha > 0.) || (xx[0] <
    //0.5 && alpha < 0.))?false:true; 		reward += (sensRight == sensDipole)?0.5:0;
    ...
    //			const Real signIdI = _modv(d);
    //			reward +=
    //(signIdI<=D)?(rewardAvoid)/(D/3.)*signIdI+2*rewardAvoid/3.:rewardAvoid;
    //		}
...
void PF_Dipole::saveData(Real time) {
  ...
  if (out) {
    Real velDipole = sqrt(vx * vx + vy * vy);
    //		out << time << " " << real(locationCenter) << " " <<
    //imag(locationCenter) << " " << real(locationRightVortex) << " " <<
    //imag(locationRightVortex) << " " << real(locationLeftVortex) << " " <<
    //imag(locationLeftVortex) << " " << alpha << " " << D << " " << velDipole
    //<< endl;
    out << time << " " << real(locationCenter) << " " << imag(locationCenter)
        ...
        << " " << velDipole << endl;
    //		out << real(locationCenter) << " " << imag(locationCenter) <<
    //endl;
    out.close();
