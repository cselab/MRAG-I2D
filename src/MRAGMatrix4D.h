...
public:
  ...
  Matrix4D(unsigned int nSizeX, unsigned int nSizeY, unsigned int nSizeZ,
           unsigned int nSizeW);
  ~Matrix4D() { delete[] m_pData; }

  DataType &Access(unsigned int ix, unsigned int iy, unsigned int iz,
                   unsigned int iw) const;
...
void Matrix4D<DataType>::_Setup(unsigned int nSizeX, unsigned int nSizeY,
                                unsigned int nSizeZ, unsigned int nSizeW) {
  m_vSize[0] = nSizeX;
  m_vSize[1] = nSizeY;
  m_vSize[2] = nSizeZ;
  m_vSize[3] = nSizeW;
...
template <class DataType>
Matrix4D<DataType>::Matrix4D(unsigned int nSizeX, unsigned int nSizeY,
                             unsigned int nSizeZ, unsigned int nSizeW)
...
template <class DataType>
Matrix4D<DataType>::Matrix4D(FILE *f, bool bSwapBytes)
    : m_pData(NULL), m_nElements(0), m_nElementsPerSlice(0),
      m_nElementsPerCube(0) {
...
template <class DataType>
inline DataType &Matrix4D<DataType>::Access(unsigned int ix, unsigned int iy,
                                            unsigned int iz,
                                            unsigned int iw) const {
#ifdef _DEBUG
  if (!(ix < m_vSize[0]) || !(iy < m_vSize[1]) || !(iz < m_vSize[2]) ||
      !(iw < m_vSize[3]))
    printf("excpetion %d %d %d %d\n", ix, iy, iz, iw);
  assert(ix < m_vSize[0]);
  assert(iy < m_vSize[1]);
  assert(iz < m_vSize[2]);
  assert(iw < m_vSize[3]);
  ...
  return m_pData[iw * m_nElementsPerCube + iz * m_nElementsPerSlice +
                 iy * m_vSize[0] + ix];
}

template <class DataType>
inline DataType &Matrix4D<DataType>::LinAccess(unsigned int i) const {
#ifdef _DEBUG
...
template <class DataType>
inline unsigned int Matrix4D<DataType>::getNumberOfElements() const {
  return m_nElements;
}
...
template <class DataType>
inline void Matrix4D<DataType>::operator=(DataType val) {
...
template <class DataType>
void Matrix4D<DataType>::Deserialize(FILE *f, bool bSwapBytes) {
  /*	if (bSwapBytes)
        {
                const unsigned int cElementSize = sizeof(DataType);
                unsigned char * buf = new unsigned char
     [sizeof(Matrix4D<DataType>)]; fread((void*)buf, sizeof(Matrix4D<DataType>),
     1, f); SwapBytes<unsigned int>(buf, sizeof(Matrix4D<DataType>));
        ...
        fread((void*)this, sizeof(Matrix4D<DataType>), 1, f);*/
  {
    ...
    // if(bSwapBytes) SwapBytes<unsigned int>((unsigned char *)buf,
    // (int)4*nIntMembers);
    m_pData = NULL;
    m_vSize[0] = buf[1];
    m_vSize[1] = buf[2];
