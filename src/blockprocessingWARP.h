...
template <int blockSize, int maximumDistance = 1> struct CLFParticle {
...
#pragma mark ParticleSampler
  struct ParticleSampler {
    ...
    void _computeWeights(const Real xp[2]) {
      ...
      Real tx[4] = {
          fabs(xp[0] - iap[0] - (0.5 + -2)), fabs(xp[0] - iap[0] - (0.5 + -1)),
          fabs(xp[0] - iap[0] - (0.5 + 0)), fabs(xp[0] - iap[0] - (0.5 + 1))};
      ...
      Real ty[4] = {
          fabs(xp[1] - iap[1] - (0.5 + -2)), fabs(xp[1] - iap[1] - (0.5 + -1)),
          fabs(xp[1] - iap[1] - (0.5 + 0)), fabs(xp[1] - iap[1] - (0.5 + 1))};
    ...
    void m2p(Real x[2], Layer<startLayer, endLayer, components> &workingplace,
             Real sample[components]) {
      ...
      const int start[2] = {max(kernel_start, startLayer - iap[0]),
                            max(kernel_start, startLayer - iap[1])};
      const int end[2] = {min(kernel_end, endLayer - iap[0]),
                          min(kernel_end, endLayer - iap[1])};
      ...
      for (int ic = 0; ic < components; ic++)
        for (int sy = start[1]; sy < end[1]; sy++)
          for (int sx = start[0]; sx < end[0]; sx++) {
            ...
            sample[ic] += wx[sx - kernel_start] * wy[sy - kernel_start] *
                          workingplace(iap[0] + sx, iap[1] + sy, ic);
          }
    ...
    template <int startLayer, int endLayer, int components>
    void p2m(Real value[components], Real x[2],
             Layer<startLayer, endLayer, components> &workingplace) {
      static const int kernel_start = -2;
      static const int kernel_end = 2;
      ...
      for (int ic = 0; ic < components; ic++)
        for (int sy = start[1]; sy < end[1]; sy++)
          for (int sx = start[0]; sx < end[0]; sx++)
            workingplace(iap[0] + sx, iap[1] + sy, ic) +=
                value[ic] * wx[sx - kernel_start] * wy[sy - kernel_start];
    }
  };
...
#pragma mark AdvectParticles
  struct AdvectParticles {
    ...
    template <typename WP> void advectGridPoints(WP &wp) const {
    ...
    template <typename WP> void advectParticles(WP &wp) const {
      for (int iy = startParticle; iy < endParticle; iy++)
        for (int ix = startParticle; ix < endParticle; ix++) {
          Real x_current[2] = {wp.x(ix, iy, 0), wp.x(ix, iy, 1)};
    ...
    void finalize(Block &block, WP &wp) const {
      for (int iy = 0; iy < Block::sizeY; iy++)
        for (int ix = 0; ix < Block::sizeX; ix++) {
          ...
          //wp.w(ix,iy,1)); assert(wp.w(ix,iy,1) > 1 - 1e-5);
        }
    }

    template <typename BlockType>
    inline void operator()(BlockLabWARP<BlockType> &lab, const BlockInfo &info,
                           BlockType &o) const {
  ...
  struct AdvectParticlesRK3 : public AdvectParticles {
    ...
    AdvectParticlesRK3(const AdvectParticlesRK3 &copy)
        : AdvectParticles(copy.dt) {}

    template <typename WP> void thirdStep(WP &wp) const {
      ...
      for (int iy = startParticle; iy < endParticle; iy++)
        for (int ix = startParticle; ix < endParticle; ix++) {
          ...
          wp.x(ix, iy, 0) =
              ix + 0.5 +
              1. / 6 *
                  (wp.u(ix, iy, 0) + 4 * wp.tmp(ix, iy, 0) + new_velocity[0]) *
                  dt_;
          wp.x(ix, iy, 1) =
              iy + 0.5 +
              1. / 6 *
                  (wp.u(ix, iy, 1) + 4 * wp.tmp(ix, iy, 1) + new_velocity[1]) *
                  dt_;
        }
    }
