...
BlockCollection<BlockType_>::BlockCollection()
    : m_blockIDToBlockPointers(), m_blockIDToChunck(), m_setChunks(), m_trash(),
      m_currentChunk(NULL), m_nAvailableBlocksInCurrentChunk(0), allocator() {}

template <typename BlockType_> BlockCollection<BlockType_>::~BlockCollection() {
  clear();
}

template <typename BlockType_> void BlockCollection<BlockType_>::_trash() {
  // old: if (m_trash.size()>(int)ceil(m_setChunks.size()*0.35))

  // new policy in megabytes
  const long unsigned int maxCapacity = 50 << 20;
  const long unsigned int currentSize =
      m_trash.size() * sizeof(BlockType_) * nChunkSize;

  if (currentSize > maxCapacity) {
    //		printf("Too Many blocks in the trash (%.2f MB, max
...
inline vector<int> BlockCollection<BlockType_>::_allocateBlockInChunk(
  ...
  if (inoutAvailableBlocksInCurrentChunk == 0)
    return vIDs;

  const int nAllocations =
      std::min(inoutAvailableBlocksInCurrentChunk, inoutRequestedBlocks);
  vIDs.resize(nAllocations);

  const int startID =
      chunk->startID + nChunkSize - inoutAvailableBlocksInCurrentChunk;
...
inline vector<int> BlockCollection<BlockType_>::_allocateChunks(
  ...
  outAvailableBlocksInCurrentChunk =
      nChunks * nChunkSize - inoutRequestedBlocks;
  ...
  assert(
      _implies(m_nAvailableBlocksInCurrentChunk == 0, m_currentChunk == NULL));
  assert(
      _implies(m_currentChunk == NULL, m_nAvailableBlocksInCurrentChunk == 0));
