...
/**
 * creates a non-rotated NACA profile at (-l,0) such that the center of rotation
 * lies at (0,0) geometry can be specified by the chord length, the x-offset of
 * the center of rotation and the NACA digits d1, d2, d3d4
 *
 */
class NACA4gen {
private:
  ...
  // maxBoxX: stores physical coordinate of maximal base point in x; the others:
  // analogous
  ...
  const Real l;           // x-offset center of rotation from leading edge
...
public:
  ...
  Real getNoseDistance(Real xx[2]) const {
    assert((xx[0] + l) >= -2.0 * epsilon &&
           (xx[0] + l) < 0.05 * leadingEdgeRadius);

    Real dist =
        sqrt((leadingEdgeCenter[0] - xx[0]) * (leadingEdgeCenter[0] - xx[0]) +
             (leadingEdgeCenter[1] - xx[1]) * (leadingEdgeCenter[1] - xx[1])) -
        leadingEdgeRadius;
  ...
  /**
   * - @ return: interpolates y coordinate of upper airfoil shape at position
   * xx[0]
   * - @ dist: calculates minimal distance between point xx and the upper sample
   * points
   *
  ...
  Real getYUpperProfile(Real xx[2], Real &dist) const {
    // nPanels = number of base points --> prerequisite: nPanels == xu.size ==
    // yu.size
    assert(xu.size() == yu.size());
    assert(xu.size() == nPanels);
    assert((xx[0] + l) >= 0.0 && (xx[0] + l) <= chord);

    const Real dx = chord / ((Real)nPanels - 1.0); // interval length
    const int midIdx = floor(
        (xx[0] + l) / dx); // get base point before sought x-coordinate x[0]
    // base points to iterate over
    const int startIdx =
        ((midIdx - maxDiffX) < 0)
            ? 0
            : (midIdx - maxDiffX); // symmetrical profiles: maxDiff = 10
    const int endIdx =
        ((midIdx + maxDiffX) > (int)(nPanels)) ? nPanels : (midIdx + maxDiffX);
    ...
    const Real v = yu[lowIdx] + (yu[highIdx] - yu[lowIdx]) /
                                    (xu[highIdx] - xu[lowIdx]) *
                                    (xx[0] - xu[lowIdx]);

    // base points to iterate over
    const int startIdxEps =
        ((lowIdx - maxDiffEpsilon) < 0) ? 0 : (lowIdx - maxDiffEpsilon);
    const int endIdxEps = ((highIdx + maxDiffEpsilon) > (int)(nPanels))
                              ? nPanels
                              : (highIdx + maxDiffEpsilon);
    ...
    Real myDist =
        std::numeric_limits<Real>::max(); // ensure assignment in for loop
                                          // (myDist for sure smaller than max
                                          // value of Real)
  ...
  /**
   * - @ return: interpolates y coordinate of lower airfoil shape at position
   * xx[0]
   * - @ dist: calculates minimal distance between point xx and the lower sample
   * points
   *
   * input:	xx[2]  	arbitrary point
   *			dist	variable, where minimal distance is written to
  ...
  Real getYLowerProfile(Real xx[2], Real &dist) const {
    ...
    const int midIdx = floor((xx[0] + l) / dx);
    ...
    const Real v = yl[lowIdx] + (yl[highIdx] - yl[lowIdx]) /
                                    (xl[highIdx] - xl[lowIdx]) *
                                    (xx[0] - xl[lowIdx]);
   ...
   * 			distance negative if point inside body, positive if
   * point outside body
   *
   * input:	xo[2]	arbitrary point
   */
  Real sdf(Real xo[2]) const {
    // xo not within [-l,chord-l]
    if ((xo[0] + l) < -2.0 * epsilon) {
      return sqrt(pow((xo[0] - l), 2) + pow(xo[1], 2));
    } // return distance to origin

    if ((xo[0] + l) > chord) {
      return sqrt(pow((xo[0] - chord + l), 2) + pow(xo[1], 2));
    } // return distance to trailing edge

    if ((xo[0] + l) >= -2.0 * epsilon &&
        (xo[0] + l) < 0.05 * leadingEdgeRadius) {
      return getNoseDistance(xo);
    } // return distance to leading edge
    ...
    const Real s = (Real)(xo[1] < low || xo[1] > high) * 2.0 -
                   1.0; // s=-1, if xo inside body, s=1 if xo outside body
    const Real D = min(distU, distL);

    return s * D; // negative inside body, positive outside body
  }

  NACA4gen(Real chord_in, unsigned int nPanels_in, bool isFiniteTE,
           Real epsilon_in, const int _d1, const int _d2, const int _d3d4,
           const Real _l)
      : d1(_d1), d2(_d2), d3d4(_d3d4), l(_l) {
    ...
    for (unsigned int i = 0; i < nPanels; i++) {
      ...
      yt.push_back((t / 0.2) *
                   (a0 * sqrt(x[i]) + a1 * x[i] + a2 * x[i] * x[i] +
                    a3 * x[i] * x[i] * x[i] + a4 * x[i] * x[i] * x[i] * x[i]));
    }
    ...
    maxDiffX = ceil(diffX / dx) +
               10; // symmetrical profiles: maxDiffX = 10; ceil(x) returns the
                   // smallest int value not smaller than x
    ...
    // 4. shift basepoints [0,chord] -> [-l, chord-l], such that center of
    // rotation lies in [0,0]
    for (unsigned int i = 0; i < nPanels; i++) {
      x[i] -= l;
      xu[i] -= l;
      xl[i] -= l;
    ...
    // assure assignment of one of the boxes (values are > -l but smaller than
    // max of Real for sure)
    maxBoxX = -l;
    minBoxX = std::numeric_limits<Real>::max(); // maximum value for a variable
                                                // of type Real
    maxBoxY = 0.0;
    ...
    leadingEdgeCenter[0] = (leadingEdgeRadius - l) * cos(noseAngle);
    leadingEdgeCenter[1] = (leadingEdgeRadius - l) * sin(noseAngle);
...
/**
 * Provides transformation matrices between domain [unrotated airfoil at (0,0)]
 * and codomain [rotated airfoil at (tx-l, ty)] basically each point of the
 * codomain is mapped on the domain, where all functions involving the shape are
 * evaluated
 */
class DiscretizedWing {
   ...
   * 			xo[2]	variable where coordinates of xw[2] in domain
   * are written to
   */
   ...
   * 			xw[2]	variable where coordinates of xo[2] in codomain
   * are written to
   */
...
public:
  Real tx, ty, angle, vx, vy, angular_velocity, rho, m,
      J; // (tx-l,ty): coordinates of airfoil's leading edge

  DiscretizedWing(Real xm, Real ym, Real width, Real angle_rad, Real epsilon,
                  const int d1, const int d2, const int d3d4, const Real l)
      : naca(width, 5000, false, epsilon, d1, d2, d3d4, l), scaling(1.0),
        epsilon(epsilon), width(width), tx(xm), ty(ym), angle(angle_rad),
        s(1.0), vx(0.0), vy(0.0), angular_velocity(0.0), rho(1.0), m(0.0),
        J(0.0) {
    ca = cos(angle);
    sa = sin(angle);
   ...
   * tx, ty and angle are updated explicitely in I2D_PitchingAirfoil::update,
   ...
   */
  Real sample(Real p[2], const Real eps)
      const // calculate characteristic function at point (p[0],p[1])
  {
    Real dist = sdf(p);
...
/**
 * Identical to FillBlocks of the other fluid mediated obstacles (Imposed
 * Cylinder, Imposed Ellipse, ...)
 */
struct FillBlocks {
  Real eps;
  PitchingStuff::DiscretizedWing *shape;

  FillBlocks(Real eps, PitchingStuff::DiscretizedWing *_shape) : eps(eps) {
    shape = _shape;
  }

  FillBlocks(const FillBlocks &c) : eps(c.eps) { shape = c.shape; }

  static bool _is_touching(Real eps, const PitchingStuff::DiscretizedWing *wing,
                           const BlockInfo &info) {
    Real wing_min[2], wing_max[2];
    wing->bbox(eps, wing_min, wing_max); // store shapes' xmin, ymin in wing_min
                                         // and shapes' xmax, ymax in wing_max
...
/**
 * Identical to GetNonEmpty of the other fluid mediated obstacles (Imposed
 * Cylinder, Imposed Ellipse, ...)
 */
struct GetNonEmpty {
  Real eps;
  ...
  GetNonEmpty(Real eps, PitchingStuff::DiscretizedWing *_shape,
              map<int, bool> &b2nonempty)
      : eps(eps), b2nonempty(b2nonempty) {
    shape = _shape;
  }
  ...
  inline void operator()(const BlockInfo &info, FluidBlock2D &b) const {
    ...
    if (FillBlocks::_is_touching(
            eps, shape, info)) // does shape lie (partially) within the block?
    {
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
          Real p[2];
          ...
          const Real Xs = shape->sample(
              p, eps); // calculate characteristic function (outside body 0,
                       // inside body 1, between: mollified heaviside
          bNonEmpty |= Xs > 0; // bNonEmpty is true, if it was true before or if
                               // Xs>0 or if both are true
        }
...
/**
 * Identical to FillVelblocks of the other fluid mediated obstacles (Imposed
 * Cylinder, Imposed Ellipse, ...)
 */
struct FillVelblocks {
  ...
  FillVelblocks(vector<pair<BlockInfo, VelocityBlock *>> &workitems, double eps,
                PitchingStuff::DiscretizedWing *_shape)
...
// TODO: why is D and not _D used within constructor, why is Uinf assigned
// again?
I2D_PitchingAirfoil::I2D_PitchingAirfoil(
    Grid<W, B> &grid, ArgumentParser &parser, const Real _xm, const Real _ym,
    const Real _D, const Real _l, const int d1, const int d2, const int d3d4,
    const Real _plungeAmplitude, const Real _plungePeriod,
    const Real _plungePhase, const Real _pitchAmplitude,
    const Real _pitchPeriod, const Real _pitchPhase, const Real _eps,
    const Real Uinf[2], I2D_PenalizationOperator &penalization)
    : I2D_FloatingObstacleOperator(parser, grid, D, _eps, Uinf, penalization),
      plungePeriod(_plungePeriod), plungeAmplitude(_plungeAmplitude),
      plungePhase(_plungePhase), pitchPeriod(_pitchPeriod),
      pitchAmplitude(_pitchAmplitude), pitchPhase(_pitchPhase), D_char(_D),
      l(_l), xcrot0(_xm), ycrot0(_ym) {
  this->Uinf[0] = Uinf[0];
  this->Uinf[1] = Uinf[1];
  const Real y_plunge0 = ycrot0 + plungeAmplitude * sin(plungePhase);
  const Real angle0 = pitchAmplitude * sin(pitchPhase);
  wing = new PitchingStuff::DiscretizedWing(xcrot0, y_plunge0, _D, angle0, eps,
                                            d1, d2, d3d4,
                                            l); // get rotated NACA profile
}
...
Real I2D_PitchingAirfoil::getModulusMaxVel() {
  Real translation = fabs(2.0 * plungeAmplitude * M_PI / plungePeriod);
  Real rotation = fabs(pitchAmplitude * M_PI / pitchPeriod * D_char);
...
void I2D_PitchingAirfoil::characteristic_function() {
  vector<BlockInfo> vInfo = grid.getBlocksInfo();
  const BlockCollection<B> &coll = grid.getBlockCollection();

  PitchingStuff::FillBlocks fill(eps, wing);
  block_processing.process(vInfo, coll, fill);
}

void I2D_PitchingAirfoil::update(
    const double dt, const double t, string filename,
    map<string, vector<I2D_FloatingObstacleOperator *>> *_data) {
  // update location of the body explicitely
  wing->tx = xcrot0;
  wing->ty = ycrot0 +
             plungeAmplitude * sin(2.0 * M_PI / plungePeriod * t + plungePhase);
  wing->angle = pitchAmplitude * sin(2.0 * M_PI / pitchPeriod * t + pitchPhase);
  ...
  if (filename == std::string()) {
    ...
    ppFile = fopen("update_I2D_PitchingAirfoil.txt", t == 0.0 ? "w" : "a");
    assert(ppFile != NULL);
  } else {
  ...
  fprintf(ppFile, "%e %e %e %e %e %e %e %e %e %e %e %e\n", this->dimT, t,
          wing->tx, wing->ty, wing->vx, wing->vy, wing->angle,
          wing->angular_velocity, wing->J, wing->m, wing->rho, this->Cd);

  // Close file
  fclose(ppFile);
}

void I2D_PitchingAirfoil::save(const double t, string filename) {
  FILE *ppFile = NULL;

  if (filename == std::string()) {
    // If the string is not set I write my own file
    ppFile = fopen("restart_I2D_PitchingAirfoil.txt", "w");
    assert(ppFile != NULL);
  } else {
    // If string is set I open the corresponding file
...
void I2D_PitchingAirfoil::restart(const double t, string filename) {
  FILE *ppFile = NULL;

  if (filename == std::string()) {
    // If the string is not set I write my own file
    ppFile = fopen("restart_I2D_PitchingAirfoil.txt", "r");
    assert(ppFile != NULL);
  } else {
    // If string is set I open the corresponding file
...
void I2D_PitchingAirfoil::refresh(const double t, string filename) {
  // Open file stream
  ifstream filestream;
  if (filename == std::string()) {
    // If the string is not set I write my own file
    filestream.open("restart_I2D_PitchingAirfoil.txt");
    if (!filestream.good()) {
  ...
  if (filename == std::string()) {
    // If the string is not set I write my own file
    f = fopen("restart_I2D_PitchingAirfoil.txt", "r");
    assert(f != NULL);
  } else {
    // If string is set I open the corresponding file
  ...
  if (filename == std::string()) {
    // If the string is not set I write my own file
    f = fopen("restart_I2D_PitchingAirfoil.txt", "w");
    assert(f != NULL);
  } else {
    // If string is set I open the corresponding file
...
void I2D_PitchingAirfoil::_setMotionPattern(const Real t) {
  Real factor1 = 2.0 * M_PI / plungePeriod;
  Real factor2 = 2.0 * M_PI / pitchPeriod;
  wing->vx = 0.0;
  wing->vy = plungeAmplitude * factor1 * cos(factor1 * t + plungePhase);
  wing->angular_velocity =
      pitchAmplitude * factor2 * cos(factor2 * t + pitchPhase);
}

void I2D_PitchingAirfoil::computeDesiredVelocity(const double t) {
  _setMotionPattern(t);
  ...
  PitchingStuff::GetNonEmpty getNonEmpty(eps, wing, nonempty);
  ...
  PitchingStuff::FillVelblocks fillvelblocks(velblocks, eps, wing);
