 ...
 */
/*
// Wim: change this to _SP_COMP_, so only define when single precision
#ifndef _MRAG_REAL_
#define _MRAG_REAL_ double
#endif
*/
#ifndef _SP_COMP_
#define _MRAG_REAL_ double
...
struct I3 {
  ...
  bool operator<(const I3 &a) const {
    return (i[0] < a.i[0] || (i[0] == a.i[0] && i[1] < a.i[1]) ||
...
struct I4 {
  ...
  I4(int x, int y, int z, int l) : x(x), y(y), z(z), l(l) {}

  I4(const I4 &a) : x(a.x), y(a.y), z(a.z), l(a.l) {
    x = a.x;
    y = a.y;
    z = a.z;
    l = a.l;
  ...
  bool operator<(const I4 &i) const {
    return (l < i.l || (l == i.l && x < i.x) ||
            (l == i.l && x == i.x && y < i.y) ||
            (l == i.l && x == i.x && y == i.y && z < i.z));
  }
...
/**
 * Helper structure to pass info of a block (like ID, position, level and a
...
struct BlockInfo {
  ...
  BlockInfo(int ID, const int idx[3], int l, const Real pos[3],
...
struct Subspace {
  ...
  void setup(const int idx[3], const Integer stencil_start[3],
             const Integer stencil_end[3], const int block_size[3]) {
    start[0] = idx[0] < 0
                   ? 0
                   : (idx[0] == 0 ? -stencil_start[0]
                                  : (block_size[0] - stencil_end[0] + 1));
    start[1] = idx[1] < 0
                   ? 0
                   : (idx[1] == 0 ? -stencil_start[1]
                                  : (block_size[1] - stencil_end[1] + 1));
    start[2] = idx[2] < 0
                   ? 0
                   : (idx[2] == 0 ? -stencil_start[2]
                                  : (block_size[2] - stencil_end[2] + 1));

    end[0] = idx[0] < 0 ? -stencil_start[0]
                        : (idx[0] == 0 ? (block_size[0] - stencil_end[0] + 1)
                                       : block_size[0]);
    end[1] = idx[1] < 0 ? -stencil_start[1]
                        : (idx[1] == 0 ? (block_size[1] - stencil_end[1] + 1)
                                       : block_size[1]);
    end[2] = idx[2] < 0 ? -stencil_start[2]
                        : (idx[2] == 0 ? (block_size[2] - stencil_end[2] + 1)
                                       : block_size[2]);
  }

  template <typename Integer>
  int computeByteSize(const Integer stencil_start[3],
                      const Integer stencil_end[3],
                      const int sizeofElement) const {
...
struct UniformPartition {
  ...
  template <typename Integer>
  UniformPartition(const Integer s[3], const Integer e[3],
                   const int nThreads[3], const int *vMaxWorkPerThread = NULL) {
    setup(s, e, nThreads, vMaxWorkPerThread);
  }

  template <typename Integer>
  void setup(const Integer s[3], const Integer e[3], const int nThreads[3],
             const int *vMaxWorkPerThread = NULL) {
    ...
    if (vMaxWorkPerThread != NULL) {
      work_per_thread[0] =
          min(vMaxWorkPerThread[0], max(1, total_work[0] / nThreads[0]));
      work_per_thread[1] =
          min(vMaxWorkPerThread[1], max(1, total_work[1] / nThreads[1]));
      work_per_thread[2] =
          min(vMaxWorkPerThread[2], max(1, total_work[2] / nThreads[2]));
    } else {
    ...
    passes_per_dim[0] =
        (int)ceil(total_work[0] / (double)(work_per_thread[0] * nThreads[0]));
    passes_per_dim[1] =
        (int)ceil(total_work[1] / (double)(work_per_thread[1] * nThreads[1]));
    passes_per_dim[2] =
        (int)ceil(total_work[2] / (double)(work_per_thread[2] * nThreads[2]));
  }
  ...
  __device__
#endif
      bool findBlockWorkingArea(const int passID, int base_index[3],
                                int end_index[3]) {
    if (passID >= passes())
      return false;

    const int pass[3] = {passID % passes_per_dim[0],
                         (passID / passes_per_dim[0]) % passes_per_dim[1],
                         (passID / (passes_per_dim[0] * passes_per_dim[1]))};

    const int offset[3] = {work_per_thread[0] * (threads[0] * pass[0] + 0),
    ...
    end_index[0] = start[0] + min(offset[0] + threads[0] * work_per_thread[0],
                                  (int)total_work[0]);
    end_index[1] = start[1] + min(offset[1] + threads[1] * work_per_thread[1],
                                  (int)total_work[1]);
    end_index[2] = start[2] + min(offset[2] + threads[2] * work_per_thread[2],
                                  (int)total_work[2]);
  ...
  __device__
#endif
      bool findWorkingInterval(const int thread_index[3], const int passID,
                               int s[3], int e[3]) {
    if (passID >= passes())
      return false;
...
template <int s, int e, int components = 1> struct Layer {
