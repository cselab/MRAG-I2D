...
public:
  ...
  public:
    double angle, xm, ym, xcrot, ycrot, D, aspectRatio, angular_velocity, vx,
        vy, m, J, rho;

    Ellipse();
    Ellipse(Real xm, Real ym, Real xcrot, Real ycrot, Real D, Real angle_rad,
            Real aspectRatio);
  ...
  I2D_ImposedEllipse(ArgumentParser &parser, Grid<W, B> &grid, const Real _xm,
                     const Real _ym, const Real _xcrot, const Real _ycrot,
                     const Real _vx, const Real _vy, const Real _D,
                     const Real aspectRatio, const Real eps, const Real Uinf[2],
