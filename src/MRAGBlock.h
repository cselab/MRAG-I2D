 ...
 * Data is aligned s.t. one should loop first (outermost) in z, then in y, then
 * (innermost) in x. DataType needs the following:
 *  - Default constructor (compiler-generated one may be enough)
 *  - Copy constructor (compiler-generated one may be enough)
 *  - DataType operator*(const DataType&, Real) (outside of class as we need
 * const(!)...) (needed to refine/compress grids and to load ghosts)
...
struct Block {
  ...
  __device__
    ...
    if (cDoDebugChecks) {
      ...
      assert((DataType *)&data[iz][iy][ix] ==
             (DataType *)data + cSizeX * cSizeY * iz + cSizeX * iy + ix);
    }
#endif
  ...
  /**
   * Get a single element from the block. Does just the same as operator() but
   * with less checks.
   *
   * @param ix    Index in x-direction (0 <= ix < cSizeX).
   * @param iy    Index in y-direction (0 <= iy < cSizeY).
  ...
  inline static int getMemSize() {
    return cSizeX * cSizeY * cSizeZ * sizeof(DataType);
  }
};
