...
class MRAG_STDTestL2_Compression
    : public MRAG_STDTestL2<Wavelets,
                            Block> // LEVEL 2: correct data representation,
                                   // correct data-processing
{
protected:
  MRAG_STDTestL2_Compression() : MRAG_STDTestL2<Wavelets, Block>() {}
...
public:
    ...
    for (int i = 0; i < vInfo.size(); i++) {
      ...
      for (int iy = 0; iy < Block::sizeY; iy++)
        for (int ix = 0; ix < Block::sizeX; ix++) {
          ...
          // block(ix,iy) = bSetToOne? 1 :
          // exp(-pow(2.0*sin(2.0*x[0]*M_PI),1.0));
          block(ix, iy) = bSetToOne ? 1
                                    : exp(-pow(2.0 * sin(2.0 * x[0] * M_PI) *
                                                   sin(2.0 * x[1] * M_PI),
                                               1.0));
  ...
  void _babak_fwt(Grid<Wavelets, Block> &grid) {
    ...
    for (int i = 0; i < vInfo.size(); i++) {
      ...
      vMaxDetails[i] =
          max(report.getCoeffMaxMag(1, 0), report.getCoeffMaxMag(0, 1));
      vMaxDetails[i] = max(vMaxDetails[i], report.getCoeffMaxMag(1, 1));
      vMinDetails[i] =
          min(report.getCoeffMinMag(1, 0), report.getCoeffMinMag(0, 1));
      ...
      if (vMaxDetails[i] > globmax)
        globmax = vMaxDetails[i];
      if (vMinDetails[i] < globmin)
        globmin = vMinDetails[i];
    }
  ...
  vector<double> _fwt(Grid<Wavelets, Block> &grid) {
    ...
    for (int i = 0; i < vInfo.size(); i++) {
      ...
      vMaxDetail[i] =
          max(report.getCoeffMaxMag<0>(1, 0), report.getCoeffMaxMag<0>(0, 1));
      vMaxDetail[i] = max(vMaxDetail[i], report.getCoeffMaxMag<0>(1, 1));
      vMinDetail[i] =
          min(report.getCoeffMinMag<0>(1, 0), report.getCoeffMinMag<0>(0, 1));
      ...
      // get the global maximum and minimum
      if (vMaxDetail[i] > globmax)
        globmax = vMaxDetail[i];
      if (vMinDetail[i] < globmin)
        globmin = vMinDetail[i];
    }
    ...
    for (int i = 0; i < vInfo.size(); i++) {
      ...
      for (int iy = 0; iy < Block::sizeY; iy++)
        for (int ix = 0; ix < Block::sizeX; ix++) {
          ...
          double exactValue;
          exactValue =
              exp(-pow(2.0 * sin(2.0 * x[0] * M_PI) * sin(2.0 * x[1] * M_PI),
                       1.0)); // sin(2.0*x[0]*M_PI);
          // exactValue = 1.0;
          error = max(error, fabs(exactValue - block(ix, iy)));
          fprintf(file, "%f %f %f %f\n", x[0], x[1], exactValue, block(ix, iy));
          if (f != error)
            printf("%d %d %d %d -> error =%f\n", ix, iy, info.index[0],
                   info.index[1], error);
  ...
  virtual bool run() {
    ...
    try {
      ...
      for (comprule = 0.0; comprule < 1.0; comprule += 0.1) {
        ...
        NpBeforeRef =
            myGrid->getBlocksInfo().size() * Block::sizeX * Block::sizeY;
        MRAG_STDTestL2_Compression<Wavelets, Block>::_refineUpto(*myGrid,
                                                                 whichlevel);

        printf("Done Refinement\n");
        MRAG_STDTestL2_Compression<Wavelets, Block>::_babak_computation(
            *myGrid);
