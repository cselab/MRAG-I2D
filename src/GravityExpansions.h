...
template <int _order> inline int _map(const int &n, const int &m) {
  int res = n * n + (m + n);
...
template <typename Ntype, int _order> void dumpData(Ntype *data, char *fname) {
...
void calcPnm(typename Particle::BaseType Pnm_out[_order][_order], const typename
Particle::BaseType  &theta_in)
{
...
// NEW implementation based on Kautzleben 1965 (Aeronomie, BandI) (digged out
// from Bosch 2000)
template <class Particle, int _order>
void calcPnm(typename Particle::BaseType Pnm_out[_order][_order],
             const typename Particle::BaseType &theta_in) {
  typedef typename Particle::BaseType Btype;
  Btype sine, cosine;
  sine = sin(theta_in);
  cosine = cos(theta_in);
  ...
  for (int n = 2; n < _order; ++n) {
    Pnm_out[n][0] = (2 * n - 1) / Btype(n) * cosine * Pnm_out[n - 1][0] -
                    (n - 1) / Btype(n) * Pnm_out[n - 2][0];
    Pnm_out[n][n - 1] = (2 * n - 1) * sine * Pnm_out[n - 1][n - 2];

    for (int m = 1; m < n - 1; ++m) {
      Pnm_out[n][m] =
          (2 * n - 1) * sine * Pnm_out[n - 1][m - 1] + Pnm_out[n - 2][m];
    }
...
// TODO: improve this withoug using a temporary Pnmtmp. (derive formula directly
// including pm1n)
template <typename Particle, int _order>
void calcDPnm(typename Particle::BaseType DPnm_out[_order][_order],
              const typename Particle::BaseType Pnm_in[_order][_order]) {
  typename Particle::BaseType Pnmtmp[_order][_order];
  typename Particle::BaseType prefac;

  for (int n = 0; n < _order; ++n) {
    for (int m = 0; m <= n; ++m) {
      Pnmtmp[n][m] = pm1n(m) * Pnm_in[n][m]; //(-1^m-factor) //copying to Pnmtmp
                                             //which included (-1)^m
    }
  }
  ...
  for (int n = 1; n < _order; ++n) {
    ...
    for (int m = 1; m < n; ++m) {
      prefac = (n + m) * (n - m + 1);
      DPnm_out[n][m] =
          pm1n(m) * 0.5 * (prefac * Pnmtmp[n][m - 1] - Pnmtmp[n][m + 1]);
    }
...
void calcYnm(NumType phi, NumType Pnm[_order][_order],
             std::complex<NumType> Ynm[_order * (_order + 2)]) {

  for (int n = 0; n < _order; ++n) {
    Ynm[_map<_order>(n, 0)] =
        Pnm[n][0]; // special case n=0: sqrtfac=1, cos(0)=1, sin(0)=0

    for (int m = 1; m <= n; ++m) {
      Ynm[_map<_order>(n, m)] = sqrtfac<NumType, _order>(n, m) * Pnm[n][m] *
                                exp(m * phi * std::complex<NumType>(0, 1));
...
// require 3 Functions:
// void calculateExpansions (tParticle* in_particles, tExpansions*
// out_expansions) void gatherExpansions (tExpansions* in_expansions,
// tExpansions* out_expansions) void evaluateExpansions (tExpansions*
// in_expansions, tRHS* out_RHS)

template <typename Particle, int _order> struct _GravityExpansions {
  ...
  void _calcNablaCnm(ExpansionsValueType NablaCnm[3], Btype Pnm[_order][_order],
                     Btype DPnm[_order][_order],
                     ExpansionsValueType Ynm[_order * (_order + 2)],
                     const Btype rp[3], const int &n, const int &m);
    ...
    for (int n = 0; n < _order; ++n) {
      for (int m = -n; m <= n; ++m) {
        for (int d = 0; d < Particle::pdim; ++d) {
          std::cout << "expansion [" << d << "][" << n << "][" << m
                    << "]=" << values[d][_map<_order>(n, m)] << std::endl;
        }
      }
    }
  }
...
// it calculates the coefficients to express the derivative d()/dx,d()/dy,d()/dz
// of Phi=laplace(U) which can then be multiplied with the coefficients Mnm[:]
// perhaps a macro would be better, and storing sin(phi),sin(theta) etc..
template <typename Particle, int _order>
inline void _GravityExpansions<Particle, _order>::_calcNablaCnm(
    ExpansionsValueType NablaCnm[3],
    typename Particle::BaseType Pnm[_order][_order],
    typename Particle::BaseType DPnm[_order][_order],
    ExpansionsValueType Ynm[_order * (_order + 2)],
    const typename Particle::BaseType rp[3], const int &n, const int &m) {
  // abort();
  ...
  if ((fabs(theta) < epstheta) || (fabs(M_PI - theta) < epstheta)) {
    ...
    // Check if singularity is at pi or at 0 (has effect on sign of some of the
    // terms)
    int signum;
    ...
    dUdtheta = sqrtfac<typename Particle::BaseType, _order>(n, m) *
               exp(phi * tmpZ) * DPnm[n][abs(m)];
    NablaCnm[0] = (ExpansionsValueType)(sin(theta) * cos(phi)) * dUdr +
                  (ExpansionsValueType)(cos(theta) * cos(phi)) * dUdtheta -
                  (ExpansionsValueType)(sin(phi)) * dUdphi;
    NablaCnm[1] = (ExpansionsValueType)(sin(theta) * sin(phi)) * dUdr +
                  (ExpansionsValueType)(cos(theta) * sin(phi)) * dUdtheta +
                  (ExpansionsValueType)(cos(phi)) * dUdphi;
    NablaCnm[2] = (ExpansionsValueType)(cos(theta)) * dUdr -
                  (ExpansionsValueType)(sin(theta)) * dUdtheta;
    NablaCnm[0] *= C0;
    NablaCnm[1] *= C0;
    NablaCnm[2] *= C0;
    // std::cout << "NablaC (r,theta,phi,n,m): (" <<R << "," << theta << "," <<
    // phi << ","<< n << "," << m << " ) " <<  " ( " << NablaCnm[0] <<  "," <<
    // NablaCnm[1] << "," << NablaCnm[2] << ")" <<std::endl;
  }
}

template <typename Particle, int _order>
void _GravityExpansions<Particle, _order>::calculateExpansions(
    Particle *in_particles, int nParticles) {
  ExpansionsValueType Ynm[_order * (_order + 2)];
  ...
  for (int i = 0; i < nParticles; ++i) {
    ...
    //			{
    //				std::cout << n << " " << m  << " " <<
    //Ynm[_map<_order>(n,m)] <<std::endl;
    //			}
    // calc Cnm (expansions) //there seems to be a difference in Epton/Dembart
    // to Greengard. the whole term i^-m*Anm/(-1)^n is missing in Eptons
    // formulation for Cnm.
    prod = Btype(1);
    val = rp[0];
    for (int n = 0; n < _order; ++n) {
      for (int m = -n; m <= n;
           ++m) // doing -n,n to follow Matlab Matlab implementation (might undo
                // for performance, later)
      {
        for (int d = 0; d < Particle::pdim; ++d) {
          ...
          // afterMatlab:
          this->values[d][_map<_order>(n, m)] +=
              (p[i].w[d] * prod * Ynm[_map<_order>(n, -m)]);
        }
      }
      prod = prod * val;
      // debug:
      // std::cout << "debugging expansions:  i=" << i << ", n=" << n << ",
      // prod(rp^n)= " <<prod << " w[i]" << p[i].w[0] << std::endl;
    }
...
// reread...done
// reread2--> found bug in cart2sph->changed order of r,phi,theta to
// r,theta,phi!

// Another Implementation, after reading deeply into the papers, doing a
// Matlab-prototype ... Wed. 1.April.2009
template <typename Particle, int _order>
void _GravityExpansions<Particle, _order>::gatherExpansions(
    _GravityExpansions<Particle, _order> *in_expansions) {
  Btype Pnm[_order][_order];
  ExpansionsValueType Ynm[_order * (_order + 2)];
  std::complex<Btype> csum;
  Btype rb[Particle::dim]; // spherical coordinates of childbox. relative to
                           // center.
  ...
  // compute sum of expansions. (using Greengard,Rohklin 1997, note that a naive
  // implementation of their formula would lead to negative factorials!) outer
  // loops for Mjk
  val = rb[0];
  for (int j = 0; j < _order; ++j) {
    for (int k = -j; k <= j; ++k) {
        ...
        for (int n = 0; n <= j; ++n) {
          for (int m = -n; m <= n; ++m) {
            ...
            if (abs(pr) >= abs(qr)) {
              ...
              csum += in_expansions->values[pd][_map<_order>(pr, qr)] *
                      pow(CI, tmpexp) * prod * Ynm[_map<_order>(n, -m)] * tmp1;
            }
          }
          prod *= val;
        }
...
template <typename Particle, int _order>
void _GravityExpansions<Particle, _order>::evaluateExpansions(
    typename Particle::BaseType *location,
    typename Particle::RHSType *out_RHS) {
  ...
  Btype DPnm[_order][_order];
  ExpansionsValueType
      NablaCnm[3]; // the Nablaoperator in 3D-Cartesian coordinates  (simplifies
                   // to coefficients in Spherical harmonics)
  Btype rp[Particle::dim]; // spherical coordinates. relative to center.
  ...
  // debug:
  // std::cout << "calculating spherical coordinates relative to center: [" <<
  // this->Center[0] << " " << this->Center[1] <<" "<<this->Center[2] << "]"
  // <<std::endl;
  //	std::cout << "evaluating at location: " << location[0] << ", "<<
  //location[1] <<", "<< location[2] <<std::endl;
  cart2sph(location, this->Center, rp);
  //	std::cout << "rp: [" << rp[0] << " " << rp[1] <<" "<<rp[2]<< "]"
  //<<std::endl;
  ...
  for (int n = 0; n < _order; ++n) {
    ...
    for (int m = -n; m <= n; ++m) {
      ...
      // void _calcNablaCnm(Btype NablaCnm[3],Btype Pnm[_order][_order], Btype
      // DPnm[_order][_order], Btype Ynm[_order*(_order+2)],   const Btype
      // rp[3], const int &n, const int& m );

      out_RHS->x[0] +=
          factorPI * std::real(NablaCnm[0] *
                               this->values[0][_map<_order>(
                                   n, m)]); // assume only one field quantitiy.
      out_RHS->x[1] +=
          factorPI *
          std::real(NablaCnm[1] * this->values[0][_map<_order>(n, m)]);
      out_RHS->x[2] +=
          factorPI *
          std::real(NablaCnm[2] * this->values[0][_map<_order>(n, m)]);
      // debug:
      //			std::cout << "NablaCnm (r,theta,phi,n,m): (" <<
      //rp[0] << "," << rp[1] << "," << rp[2] << ","<< n << "," << m << " ) " <<
      //" ( " << NablaCnm[0] <<  "," << NablaCnm[1] << "," << NablaCnm[2] << ")"
      //<<std::endl; 			std::cout <<  "Contrib. (r,theta,phi,n,m): (" <<rp[0] <<
      //"," << rp[1] << "," << rp[2] << ","<< n << "," << m << " ) " <<  " ( "
      //<< std::real(NablaCnm[0]*this->values[0][_map<_order>(n,m)]) <<  "," <<
      //std::real(NablaCnm[1]*this->values[0][_map<_order>(n,m)]) << "," <<
      //std::real(NablaCnm[2]*this->values[0][_map<_order>(n,m)]) << ")"
      //<<std::endl; 			std::cout << "Psum. (r,theta,phi,n,m): (" <<rp[0] << "," <<
      //rp[1] << "," << rp[2] << ","<< n << "," << m << " ) " <<  " ( " <<
      //out_RHS->x[0] <<  "," << out_RHS->x[1] << "," << out_RHS->x[2] << ")"
      //<<std::endl;
    }
  }
  ...
  //	//debug:
  //	std::cout << "Outer Expansion Onm(theta,phi): theta: " << rp[1] << "
  //phi:" << rp[2] <<std::endl; 	for (int n=0;n<_order;++n)
  //	{
  //		for (int m=-n;m<=n;++m)
  //		{
  //			std::cout << n << ", " << m << ": " <<
  //Outer[_map<_order>(n,m)] <<std::endl;
  //		}
  //	}
  ...
  //			for (int d=0;d<Particle::pdim;++d)
  //			{
  //				//ToDo: why is it Outer(n,-m)-->it is like that
  //in Epton/dembart eq. 2.39.
  ...
  for (int pd = 0; pd < Particle::pdim; ++pd) {
    out_RHS->x[pd] +=
        std::real(this->values[pd][0]); // for testing reasons:mass summation
  }
#endif
}
