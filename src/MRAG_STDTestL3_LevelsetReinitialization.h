...
inline Real lvlstinit_projector_impl(const LevelsetPoint &t) {
...
class ComputeRHS {
  inline const double phi_WENO(const double a, const double b, const double c,
    ...
    const double w[2] = {alpha[0] / sum_alpha, alpha[2] / sum_alpha};

    return 1. / 3 * w[0] * (a - 2 * b + c) +
           1. / 6 * (w[1] - 0.5) * (b - 2 * c + d);
  }

  inline const double compute_sign1(const double phi, const double h,
                                    const double grad_phi_mag) const {
    return phi / sqrt(phi * phi + h * h);
  }

  inline const double compute_sign2(const double phi, const double h,
                                    const double grad_phi_mag) const {
  ...
  inline const double compute_sign3(const double phi, const double h,
                                    const double grad_phi_mag) const {
    return 2 * H_h(phi, h) - 1;
  }

  inline const double H_GODUNOV(const double spacing, const double phi,
                                const double u_plus, const double u_minus,
                                const double v_plus,
  ...
  template <int index, typename Field>
  inline const double Dplus(Field &f, const int ix, const int iy) const {
    return (f(ix + (int)(index == 0), iy + (int)(index == 1), 0).phi -
            f(ix, iy, 0).phi);
  }

  template <int index, typename Field>
  inline const double Dminus(Field &f, const int ix, const int iy) const {
    return (f(ix, iy, 0).phi -
            f(ix - (int)(index == 0), iy - (int)(index == 1), 0).phi);
  }

  template <int index, typename Field>
  inline const double DplusDminus(Field &f, const int ix, const int iy) const {
    return (f(ix + (int)(index == 0), iy + (int)(index == 1), 0).phi +
            f(ix - (int)(index == 0), iy - (int)(index == 1), 0).phi -
            2 * f(ix, iy, 0).phi);
  }
  ...
  const double mainRHS(Field &f, const double spacing, const int ix,
                       const int iy) const {
    const double u_common_term =
        1. / (12 * spacing) *
        (-Dplus<0>(f, ix - 2, iy) + 7 * Dplus<0>(f, ix - 1, iy) +
         7 * Dplus<0>(f, ix, iy) - Dplus<0>(f, ix + 1, iy));

    const double v_common_term =
        1. / (12 * spacing) *
        (-Dplus<1>(f, ix, iy - 2) + 7 * Dplus<1>(f, ix, iy - 1) +
         7 * Dplus<1>(f, ix, iy) - Dplus<1>(f, ix, iy + 1));

    const double phi_weno_x[2] = {
        phi_WENO(DplusDminus<0>(f, ix - 2, iy) / spacing,
                 DplusDminus<0>(f, ix - 1, iy) / spacing,
                 DplusDminus<0>(f, ix, iy) / spacing,
                 DplusDminus<0>(f, ix + 1, iy) / spacing),
        phi_WENO(DplusDminus<0>(f, ix + 2, iy) / spacing,
                 DplusDminus<0>(f, ix + 1, iy) / spacing,
                 DplusDminus<0>(f, ix, iy) / spacing,
    ...
    const double phi_weno_y[2] = {
        phi_WENO(DplusDminus<1>(f, ix, iy - 2) / spacing,
                 DplusDminus<1>(f, ix, iy - 1) / spacing,
                 DplusDminus<1>(f, ix, iy) / spacing,
                 DplusDminus<1>(f, ix, iy + 1) / spacing),
        phi_WENO(DplusDminus<1>(f, ix, iy + 2) / spacing,
                 DplusDminus<1>(f, ix, iy + 1) / spacing,
                 DplusDminus<1>(f, ix, iy) / spacing,
    ...
    return -H_GODUNOV(spacing, f(ix, iy).phi_0, u_plus, u_minus, v_plus,
                      v_minus);
  }
...
public:
  ...
  inline void operator()(LabType &i, const BlockInfo &info,
                         BlockType &o) const {
    typedef BlockType B;
...
class STDTestL3_LevelsetReinitialization // LEVEL 3: correct 2D simulations,
...
protected:
  ...
  STDTestL3_LevelsetReinitialization()
      : grid(8, 8), refiner(), compressor(), lab(), blockfwt(),
        viewer(true, true), time(0) {
  ...
  template <int dir> static inline const double HeavySide(double x0, double x) {
  ...
  template <bool bPerturbation> static float _ic_func(float x[3]) {
    ...
    4*epsilon/(16*M_PI)*sin(4*M_PI*d*sin(5*teta)/epsilon);
    ...
    if (bPerturbation) {
      ...
      const double val =
          HeavySide<1>(0.5 - hw, x[0]) * HeavySide<-1>(0.5 + hw, x[0]) *
          HeavySide<1>(0.5 - hw, x[1]) * HeavySide<-1>(0.5 + hw, x[1]);

      return val * (-2.) + 1.;
    } else {
      double p[2] = {fabs(x[0] - 0.5), fabs(x[1] - 0.5)};

      int c[2] = {(int)(p[0] >= hw), (int)(p[1] >= hw)};

      const bool bInside = (c[0] + c[1] == 0);
      const double dInside = -min(hw - p[0], hw - p[1]);
      const double dOutside =
          sqrt(c[0] * pow(hw - p[0], 2) + c[1] * pow(hw - p[1], 2));
      ...
      for (int iz = 0; iz < BlockType::sizeZ; iz++)
        for (int iy = 0; iy < BlockType::sizeY; iy++)
          for (int ix = 0; ix < BlockType::sizeX; ix++) {
            ...
            const double val = HeavySide<1>(0.5 - hwidth, x[0]) *
                               HeavySide<-1>(0.5 + hwidth, x[0]) *
                               HeavySide<1>(0.5 - hwidth, x[1]) *
                               HeavySide<-1>(0.5 + hwidth, x[1]);
  ...
  static double _computeError(Grid<Wavelets, BlockType> &grid, double time) {
      ...
      for (int iz = 0; iz < BlockType::sizeZ; iz++)
        for (int iy = 0; iy < BlockType::sizeY; iy++)
          for (int ix = 0; ix < BlockType::sizeX; ix++) {
            ...
            const double dError =
                _eta(block(ix, iy, iz).phi, smallest_scale) *
                fabs(block(ix, iy, iz).phi - _ic_func<false>(x));
    ...
    if (counter >= 2 && counter % 1 == 0) {
      ...
      fprintf(fgnuplot, "plot \"Error.dat\" using 1:2  title 'error of the
      computed solution' with linespoint\n"); fprintf(fgnuplot, "set terminal
  ...
  bool run() {
    ...
    BoundaryInfo *boundaryInfo = grid.createBoundaryInfo(
        computeRhs.stencil_start, computeRhs.stencil_end);

    vector<BlockInfo> vInfo = grid.getBlocksInfo();

    for (int i = 0; i < 100; i++) {
      BlockProcessing::process<BlockLab>(vInfo, grid.getBlockCollection(),
                                         *boundaryInfo, computeRhs);
  ...
  static void display(void) {
    ...
    singletone->run();
    singletone->viewer.drawContent(singletone->grid,
                                   singletone->grid.getBlockCollection());
...
template <typename Wavelets>
STDTestL3_LevelsetReinitialization<Wavelets>
    *STDTestL3_LevelsetReinitialization<Wavelets>::singletone;
} // namespace MRAG
