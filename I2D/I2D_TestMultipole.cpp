...
I2D_TestMultipole::I2D_TestMultipole(const int argc, const char **argv)
...
void I2D_TestMultipole::_refine(bool bUseIC) {
  if (!parser("-uniform").asBool()) {
    while (true) {
      ...
      //_getBoundaryBlockIDs();
      //			((Refiner_BlackList*)refiner)->set_blacklist(&boundary_blocks);
      //			const int refinements =
      //Science::AutomaticRefinement<0,2>(*grid, fwt_omega,
      //parser("-rtol").asDouble(), parser("-lmax").asInt(), 1, NULL,
...
void I2D_TestMultipole::_computeError() {
  ...
  for (int i = 0; i < vInfo.size(); i++) {
    ...
    for (int iy = 0; iy < B::sizeY; iy++)
      for (int ix = 0; ix < B::sizeX; ix++) {
        ...
        const Real err = std::sqrt(
            std::pow(b(ix, iy).u[0] - exactsol[i].u_exact[iy][ix][0], 2) +
            std::pow(b(ix, iy).u[1] - exactsol[i].u_exact[iy][ix][1], 2));
        ...
        const Real err_ex =
            std::sqrt(std::pow(b(ix, iy).u[0] - exactVel[0], 2) +
                      std::pow(b(ix, iy).u[1] - exactVel[1], 2));
  ...
  const double nofgridpoints =
      grid->getBlocksInfo().size() * (double)(B::sizeX * B::sizeY * B::sizeZ);
