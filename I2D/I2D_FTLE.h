...
protected:
  ...
  void _extractSubsed(double tStart, double tEnd,
  ...
  void _computeFwdFTLE(double tStart, double tEnd, int idx,
                       map<string, double> &mapping);
  void _save(string filename);
  void _save();
  void _restart();
  void _refine();
  double _advectParticles(map<long int, vector<Real>> &particles,
                          vector<Real> &dts, map<string, double> &subset);
  void _FTLE(string initialField, double T,
             map<long int, vector<Real>> &particles, vector<Real> &dts,
             map<I3, unsigned int> &code);
  void _extractSubset(double tStart, double tEnd,
                      const map<string, double> &mapping,
                      map<string, double> &subset);
  void _createParticleSet(string restart,
