...
public:
  ...
  void evaluate(TargetPoint *inTargets, int nTargets,
                const typename TargetPoint::BaseType theta = _THETA);

  // private:
  void _interact(TargetPoint *inTargets, tRHS *rhs_out,
                 const typename Particle::BaseType theta)
      const; //, size_t &cnt_box, size_t& cnt_direct
  inline void _interactDirect(TargetPoint *inTargets, tBox *p_Box,
                              tRHS *rhs_out) const;
  inline void _interactBox(TargetPoint *inTargets, tBox *p_Box,
                           tRHS *rhs_out) const;
  tRHS traverseRecursive(TargetPoint *inTarget, tBox *node,
                         const typename tExpansions::Btype theta) const;
};

template <typename tBox> struct recursiveListBuilder {
  ...
  struct InteractionListElement {
    ...
    //			InteractionListElement( const InteractionListElement&
    //rhs):
  ...
  struct binpred {
    ...
    bool operator()(const InteractionListElement &lhs,
                    const InteractionListElement &rhs) {
  ...
  bool recursiveBuild(tBox *target_rootBox, InteractionListElement iElement,
                      typename tBox::Particle::BaseType theta) {

    // std::cout << "Size of incoming nearBoxList= "
    // <<iElement.nearBoxList.size() <<std::endl;

    tBox *current;
    tBox *child;
    int isclose;
    // if we have reached the leaf of the target-Tree:
    // go through all near_list and traverse down to source-leafs and put into
    // direct or far, at the end, near must be empty
    if (target_rootBox->isleaf) {
      while (!iElement.nearBoxList.empty()) {
        ...
        if (isclose == 0) {
          iElement.farBoxList.push_back(
              current); // it is far now (because we are in a different
                        // targetBox)
        } else {
          if (current->isleaf) {
            if (isclose ==
                2) // it is very close (all have to do direct interactions)
            {
              iElement.directBoxList.push_back(
                  current); // it is close and source is also leaf -> direct
            } else {
              iElement.closeBoxList.push_back(current);
            }
          } else {
            for (int kb = 0; kb < _nc; ++kb) // it is near but, not a leaf ->
                                             // put children in near list.
            {
      ...
      // std::cout << "reprocessed close-> corrected " << cnt_corr << " size
      // direct: " <<iElement.directBoxList.size() << " size close: " <<
      // iElement.closeBoxList.size()  <<std::endl; put it in the Leaf-List
    ...
    // call this function on all children handing over the current iElement.
      ...
      while (cnt < cSize &&
             it1 != iElement.nearBoxList.end()) // loop once through near list.
      {
      ...
      for (int kb = 0; kb < _nc; ++kb) {
        if (target_rootBox->children[kb] != NULL) {
          tmpbol2 =
              recursiveBuild(target_rootBox->children[kb], iElement, theta);
  ...
  recursiveListBuilder(tBox *target_rootBox, tBox *source_rootBox,
                       typename tBox::Particle::BaseType theta) {
    ...
    while (it1 != InteractionList.end()) {
      ...
      for (it2 = it1->directBoxList.begin(); it2 != it1->directBoxList.end();
           ++it2) {
        cdir += (it1->Leaf->nParticles) * (*it2)->nParticles;
      }
      for (it2 = it1->closeBoxList.begin(); it2 != it1->closeBoxList.end();
           ++it2) {
        cdir += (it1->Leaf->nParticles) * (*it2)->nParticles;
      }
    ...
    std::cout << "Planned Interactions: box: " << cbox << "direct: " << cdir
              << std::endl;
  }
};
...
#ifndef _TRECURSIVE
template <typename tExpansions, typename TargetPoint, int _maxlevel>
void Evaluator_serial<tExpansions, TargetPoint, _maxlevel>::evaluate(
    TargetPoint *inTargets, int nTargets,
    const typename TargetPoint::BaseType theta) {
  ...
  for (int i = 0; i < nTargets; ++i) // this is probably the most expensive Loop
  {
    //		int k=nTargets/10;
    //		(k==0)?k=1:k=k;
    //		if(i%k==0) std::cout << "Calculating... Passed " << i << " of "
    //<< nTargets <<std::endl;
    _interact(&(inTargets[i]), &(inTargets[i].RHS), theta);
  }
  //	std::cout << "Standard evaluate: I had " << cnt_box << "
  //box_interactions " <<  cnt_direct << " direct interactions" <<std::endl;
  //	std::cout << "global counters: box: " <<global_box << " direct: " <<
  //global_direct  << " self: " << global_self<< std::endl;
}
#else
template <typename tExpansions, typename TargetPoint, int _maxlevel>
void Evaluator_serial<tExpansions, TargetPoint, _maxlevel>::evaluate(
  ...
  for (int i = 0; i < nTargets; ++i) // this is probably the most expensive Loop
  {
    //			int k=nTargets/10;
    //			(k==0)?k=1:k=k;
    //			if(i%k==0) std::cout << "Calculating... Passed " << i <<
    //" of " << nTargets <<std::endl;
...
void Evaluator_serial<tExpansions, TargetPoint, _maxlevel>::evaluate(
  ...
  // global_direct=0;
#ifndef _FMMSILENT
  std::cout << "Evaluating SERIAL, LOOP CHANGED -> theta is: " << theta
            << std::endl;
#endif
  ...
  while (it1 != NULL) {
    ...
    for (int i = 0; i < nTargets;
         ++i) // this is probably the most expensive Loop
    {
      if (current->parent == NULL ||
          !(ws_barnes_hut(
              current->parent, inTargets[i].x,
              theta))) // when we were already well separated from the parent,
                       // we should not further interact.
      {
        if (ws_barnes_hut(current, inTargets[i].x, theta)) {
          it1->expansions.evaluateExpansions(&(inTargets[i].x[0]),
                                             &(inTargets[i].RHS));
#ifdef VERBOSE
          cnt_box++;
#endif
        ...
        {
          if (!it1->isleaf) // Case 2: its not a leaf so we further traverse
                            // into the tree (summing up the children and
                            // bailing out)
          {
            canRemove = false;
...
#ifdef #VERBOSE
  std::cout << "Evaluated with Tloopc-> box interactions: " << cnt_box
            << " direct: " << cnt_direct << std::endl;
#endif
  //		std::cout << "global counters: box: " <<global_box << " direct:
  //" << global_direct << std::endl;
}
...
//		global_self=0;
#ifndef _FMMSILENT
  std::cout << "Evaluating SERIAL,USING BOX STRUCTURE FOR TARGETS , theta is: "
            << theta << std::endl;
#endif
#ifdef VERBOSE
  int cnt_direct(0);
  ...
  for (it1 = iList.InteractionList.begin(); it1 != iList.InteractionList.end();
       ++it1) {
    for (it2 = it1->farBoxList.begin(); it2 != it1->farBoxList.end(); ++it2) {
      for (int i = 0; i < it1->Leaf->nParticles; ++i) {
        (*it2)->expansions.evaluateExpansions(it1->Leaf->vparticles[i].x,
                                              &(it1->Leaf->vparticles[i].RHS));
        cnt_box++;
      }
    }
    ...
    for (it2 = it1->closeBoxList.begin(); it2 != it1->closeBoxList.end();
      ...
      for (int i = 0; i < it1->Leaf->nParticles; ++i) {
        if (ws_barnes_hut(*it2, it1->Leaf->vparticles[i].x, theta)) {
          ...
          (*it2)->expansions.evaluateExpansions(
              it1->Leaf->vparticles[i].x, &(it1->Leaf->vparticles[i].RHS));
          cnt_corrected++;
          cnt_box++;
        } else {
          _interactDirect(&(it1->Leaf->vparticles[i]), (*it2),
                          &(it1->Leaf->vparticles[i].RHS));
  ...
  std::cout << "finished with evaluate-serial, Interactions with Boxes: "
            << cnt_box << " direct: " << cnt_direct
            << " corrected: " << cnt_corrected << std::endl;
  //		std::cout << "global counters: box: " <<global_box << " direct:
  //" << global_direct  << " self: " << global_self<< std::endl;
  ...
  //				{
  //					if(!ws_barnes_hut(current,
  //currentLeaf->vparticles[i].x, theta))
  //					{
  //						std::cout << "Something went
  //wrong:  " <<std::endl; 						exit(1);
  //					}
  ...
  //				if(current->isleaf)
  //				{
  //				//Direct interactions with particles in here and
  //brothers particles: 				for (int i=0;i<currentLeaf->nParticles;++i)
  //				{
  //					//this->_interactDirect(&(inTargets[i]),it1,&(inTargets[i].RHS));
  //
  ...
  //		}
  //
  //		std::cout << "finished with evaluate-serial, Interactions with
  //Boxes: " << cnt_box << " direct: " <<cnt_direct << std::endl;
  //
}
...
bool recursivelyPutLists(tBox *node, std::list<tBox *> toHandle,
                         const typename tBox::Btype theta) {
  ...
  std::cout << "Level: " << node->level << " toHandle.size= " << toHandle.size()
            << std::endl;

  if (toHandle.empty()) {
    return true; // nothing to do, everything is done on the parents basis
                 // already
  }
  ...
  if (node->isleaf) {
    while (it1 != toHandle.end()) {
      ...
      if (!isclose) // it is far away -> so these guys (in it1) can interact
                    // with me as a box.
      {
  ...
  else { // ok, this is difficult: we can either go down in the targets tree or
         // in the source tree, criteria should probably be the level together
         // with isclose==1

    // since we are now a level down compared to the call from above, the first
    // thing is to check the "tohandle" again:
    while (it1 != toHandle.end()) {
      isclose = isclose_box_barnes_hut(node, *it1, theta);
      if (!isclose) {
        node->toDo_far.push_back(*it1);
        ...
        if (node->level >= (*it1)->level &&
            isclose == 2) // no way that i am going to be far for them, so treat
                          // this at my childrens level.
        // if(node->level>(*it1)->level)
        {
          it1++;
          // std::cout << "Check four 670" <<std::endl;

        } else { // mmh, might be that i appear far to some of those childrens
                 // boxes. if we do that, we take it out from toHandle, put
                 // possibly put the children into the list.
          ...
          while (it_targets != NULL) {
            ...
            if (!isclose) {
              ...
              if (current->isleaf) // i appear close to a leaf -> all those
                                   // particles have to interact directly with
                                   // mines.
              {
                node->toDo_direct.push_back(current);
                it_targets.advanceRemove();
                ...
                if (node->level >= current->level &&
                    isclose == 2) // no way that i am going to be far for them,
                                  // so treat this at my childrens level.
                // if(isclose==2)
                // if(node->level>current->level)
                {
          ...
          // since we have split the parent into childrens, we need to delete it
          // from the toHandle list.
          tmpit = it1;
          it1++;
          toHandle.erase(tmpit);
        }
    ...
    // checked everything here in to handle->so lets give it down to the
    // children.
    res = true;
    bool tmpbol;
...
void Evaluator_serial<tExpansions, TargetPoint, _maxlevel>::evaluate(
...
#ifndef _FMMSILENT
  std::cout << "Evaluating SERIAL,USING BOX STRUCTURE Building lists at the "
               "sources , theta is: "
            << theta << std::endl;
#endif
...
#ifdef VERBOSE
  std::cout
      << "recursive interaction listPutting (put them at the treenodes) took: "
      << tdur << std::endl;
#endif
  ...
  while (it1 != NULL) {
    ...
    while (listIterator != it1->toDo_direct.end()) {
      ...
      for (int i = 0; i < (*listIterator)->nParticles; ++i) {
        _interactDirect(&((*listIterator)->vparticles[i]), it1,
                        &((*listIterator)->vparticles[i].RHS));
      }
...
#ifdef VERBOSE
  std::cout << "finished with evaluate-serial, Interactions with Boxes: "
            << cnt_box << " direct: " << cnt_direct << std::endl;
#endif
}
...
template <typename tExpansions, typename TargetPoint, int _maxlevel>
void Evaluator_serial<tExpansions, TargetPoint, _maxlevel>::_interact(
    TargetPoint *inTarget, typename TargetPoint::RHSType *rhs_out,
    const typename Particle::BaseType theta)
    const //,size_t &cnt_box, size_t &cnt_direct
{
  if (this->rootBox->nParticles < 1)
  ...
  while (it1 != NULL) {
    tBox *current = it1;
    if (ws_barnes_hut(current, inTarget->x,
                      theta)) // it is far away  we can use the multipoles
    {
...
// template <typename tExpansions, typename TargetPoint,  int _maxlevel>
// void
// Evaluator_serial<tExpansions,TargetPoint,_maxlevel>::_interact(TargetPoint*
// inTarget, typename TargetPoint::RHSType *rhs_out)
//{
//	typedef typename box_walker<Box<tExpansions,_maxlevel> >::t_p2Box_list
//t_p2Box_list; 	typedef typename box_walker<Box<tExpansions,_maxlevel>
//>::t_p2Box_it t_p2Box_it;
//
...
////	std::cout << "Close (direct) : " << my_walker.direct_list.size() <<
///std::endl; /	std::cout << "Far (use expansions) : " <<
///my_walker.ws_list.size() << std::endl;
//
//
...
template <typename tExpansions, typename TargetPoint, int _maxlevel>
void Evaluator_serial<tExpansions, TargetPoint, _maxlevel>::_interactDirect(
    TargetPoint *inTarget, Box<tExpansions, _maxlevel> *p_Box,
    typename TargetPoint::RHSType *rhs_out) const {
  if (this->rootBox->nParticles < 1)
    return;
...
// recursive function:
template <typename tExpansions, typename TargetPoint, int _maxlevel>
typename TargetPoint::RHSType
Evaluator_serial<tExpansions, TargetPoint, _maxlevel>::traverseRecursive(
    TargetPoint *inTarget, tBox *node,
    const typename tExpansions::Btype theta) const {
  assert(node != NULL);
  static const int nc = (1 << Particle::dim);
  tRHS res;
  ...
  {
    if (!node->isleaf) // Case 2: its not a leaf so we further traverse into the
                       // tree (summing up the children and bailing out)
    {
      for (int kb = 0; kb < nc; ++kb) {
      ...
      return res;
    } else // Case 3: we have reached the bottom of the tree carry out direct
           // interactions.
    {
