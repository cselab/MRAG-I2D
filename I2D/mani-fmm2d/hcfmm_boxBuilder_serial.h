...
template <typename tExpansions, int _maxlevel>
int boxBuilder_serial<tExpansions, _maxlevel>::_getMaxLevelinuse(
    Box<tExpansions, _maxlevel> *rootBox) {
...
template <typename tExpansions, int _maxlevel>
void boxBuilder_serial<tExpansions, _maxlevel>::buildBoxes(
    Particle *in_p, int in_nParticles, Box<tExpansions, _maxlevel> *rootBox) {
  ...
  if (in_nParticles < 1) {
#ifndef _FMMSILENT
    std::cout << "[boxBuilder]: no Source particles found. creating empty box"
              << std::endl;
#endif
    return;
  }
  bbox<Btype, Particle::dim> bounds =
      getBoundingBox<Btype, Particle, Particle::dim>(in_p, in_nParticles);
...
#ifndef _FMMSILENT
  std::cout << "Having " << rootBox->nParticles << " items in initial Box"
            << std::endl;
...
#ifndef _FMMSILENT
  std::cout << "Max Level in use is: " << rootBox->maxlevelinuse << "of "
            << _maxlevel << "possible" << std::endl;
#endif
}

template <typename tExpansions, int _maxlevel>
void boxBuilder_serial<tExpansions, _maxlevel>::_split(
    Box<tExpansions, _maxlevel> *rootBox, int levels) {
  ...
  if (rootBox->level + 1 < levels) {
    ...
    for (int i = 0; i < rootBox->nParticles; ++i) {
      p_ind<Particle> dummy =
          createpIndex(&(tmpArr[i]), rootBox->center, rootBox->h);
...
#ifdef VERBOSE
      std::cout << "particle " << i << ": pos: [" << rootBox->vparticles[i].x[0]
                << " " << rootBox->vparticles[i].x[1] << " "
                << rootBox->vparticles[i].x[2] << " ] key: " << pmap[i].key
                << std::endl;
#endif
    }
    // delete[] vtmp; //THIS DOES INVALIDATE THE CONTIGUITY of THE BASe-ARRAY!:
    delete[] tmpArr; // This should be ok, because it is an array that we
    ...
    {
            p_ind<Particle>
    tmpk1=createpIndex(&(rootBox->vparticles[i-1]),rootBox->center,rootBox->h);
            p_ind<Particle> tmpk2=createpIndex(&(rootBox->vparticles[i]),
    rootBox->center,rootBox->h);
            //tmpk1.key=ot_hkey_TruncateAtDepth(tmpk1.key,1);
            //tmpk2.key=ot_hkey_TruncateAtDepth(tmpk2.key,1);
            std::cout << i << ": " << rootBox->vparticles[i].x[0] << "," <<
    rootBox->vparticles[i].x[1] << "," << rootBox->vparticles[i].x[2] << " : "
    << pmap[i].key << "---"; if ( tmpk1 <= tmpk2 ) std::cout << "ok.
    "<<std::endl; else std::cout << "error!: prev: "<<tmpk1.key << " cur: "
    <<tmpk2.key <<std::endl;
    ...
    */
    ...
    while (kb < (1 << Particle::dim)) {
      if (children_count[kb] == 0) {
        ...
        // this->children[kb]->makeChild(children_count[kb],
        // &(this->vparticles[kids_st]), this);
        ...
        for (int d = 0; d < Particle::dim; ++d) {
          ...
          rootBox->children[kb]->center[d] =
              rootBox->center[d] + Btype(kbits[d]) * rootBox->h[d] / Btype(4);
        }
        ...
        std::cout << " ] " << std::endl;
        std::cout << "number of particles of child" << kb << ": "
                  << rootBox->children[kb]->nParticles << std::endl;
#endif
        kids_st = kids_nd;
      }
      kb++;
    ...
    if (rootBox->level + 1 < levels) {
      ...
      for (int kb = 0; kb < (1 << Particle::dim); ++kb) {
        if (rootBox->children[kb] != NULL) {
...
#ifdef VERBOSE
            std::cout << "contains less than " << smax
                      << "Particles, not splitting." << std::endl;
#endif
          }
        } else {
#ifdef VERBOSE
          ...
          std::cout
              << kb
              << " is a empty box(no particles in this sector):  not splitting."
              << std::endl;
#endif
        }
      }
...
#ifdef VERBOSE
    std::cout << "not splitting, reached maxlevel (from the beginning)"
              << std::endl;
#endif
    rootBox->isleaf = true;
#ifdef VERBOSE
...
template <typename tExpansions, int _maxlevel>
void boxBuilder_serial<tExpansions, _maxlevel>::_computeRadius(
    Box<tExpansions, _maxlevel> *rootBox) {
  ...
  if (rootBox->isleaf) // compute based on particles
  {
    ...
    for (int i = 0; i < rootBox->nParticles; ++i) {
      tmpR = 0;
      for (int d = 0; d < Particle::dim; ++d) {
        tmpR += (rootBox->COM[d] - rootBox->vparticles[i].x[d]) *
                (rootBox->COM[d] - rootBox->vparticles[i].x[d]);
      }
      tmpR = sqrt(tmpR);
    ...
    for (int kb = 0; kb < (1 << Particle::dim); ++kb) {
      if (rootBox->children[kb] != NULL) {
        ...
        for (int d = 0; d < Particle::dim; ++d) {
          ...
          // wim: replaced with correct expression: need to take radius of
          // children into account when computing parent radius
          tmpR += (rootBox->COM[d] - rootBox->children[kb]->COM[d] -
                   rootBox->children[kb]->expansions.Radius) *
                  (rootBox->COM[d] - rootBox->children[kb]->COM[d] -
                   rootBox->children[kb]->expansions.Radius);
        }
        tmpR = sqrt(tmpR);
        rootBox->expansions.Radius = max(rootBox->expansions.Radius, tmpR);
...
template <typename tExpansions, int _maxlevel>
void boxBuilder_serial<tExpansions, _maxlevel>::_calculateCOM(
    Box<tExpansions, _maxlevel> *rootBox) {
  ...
  if (!(rootBox->isleaf)) // gather from children
  {
    ...
    for (int kb = 0; kb < (1 << Particle::dim); ++kb) {
      if (rootBox->children[kb] != NULL) {
        ...
        assert(currentMass >= Btype(0)); // TODO: what happens if mass is zero->
                                         // the center will be zero?
        assert(rootBox->children[kb]->got_COM == true);
        for (int d = 0; d < Particle::dim; ++d) {
          rootBox->COM[d] +=
              rootBox->children[kb]->TotalMass * rootBox->children[kb]->COM[d];
        }
  ...
  } else // calculate based on particles.
  {
    for (int i = 0; i < rootBox->nParticles; ++i) {
      ...
      for (int pd = 0; pd < Particle::pdim; ++pd) {
        currentMass +=
            (rootBox->vparticles[i].w[pd]) * (rootBox->vparticles[i].w[pd]);
      }
  ...
  //		{
  //			std::cout << "[boxBuilder_serial:] got COM: [ " <<
  //rootBox->COM[0]<< " " << rootBox->COM[1] << " " << rootBox->COM[2] << " ]"
  //<< std::endl; 			std::cout << "[boxBuilder_serial:] TotalMass: " <<
  //rootBox->TotalMass << std::endl;
  //		}
  //
  //		//debug:...:
  //		double COM_serial[3];
  //		computeCOM_SingleCPU(rootBox->vparticles, rootBox->nParticles,
  //COM_serial); 		std::cout << "[boxBuilder_serial:] got COM(based on children):
  //[ " << rootBox->COM[0]<< " " << rootBox->COM[1] << " " << rootBox->COM[2] <<
  //" ]" << std::endl;
  //        std::cout << "[boxBuilder_serial:] SingleCPU: [ " << COM_serial[0]<<
  //        " " << COM_serial[1] << " " << COM_serial[2] << " ]" << std::endl;
  //
}

template <typename tExpansions, int _maxlevel>
void boxBuilder_serial<tExpansions, _maxlevel>::generateExpansions(
    Box<tExpansions, _maxlevel> *rootBox) {
  ...
  // ToDo: check whether the work_list assures that the expansions of the
  // children are always calculated (should be like that, since thats the way we
  //build up the work_list. to be sure I added a boolean.)
  ...
  while (up_it != box_iterator.work_list.begin()) {
    ...
    if (up_it->p_Box->isleaf == true) {
      ...
      up_it->p_Box->expansions.calculateExpansions(up_it->p_Box->vparticles,
                                                   up_it->p_Box->nParticles);
    ...
    // 3) shift expansions of children to center of current Box and sum up.
      ...
      //				{
      //				std::cout <<
      //"[generateExpansions]handling branch" << std::endl; 				std::cout << "I
      //SHOULD NOT BE HERE in TEST 4 (set maxlevel=0)" <<std::endl; 				std::cout <<
      //"leaf-status: " << up_it->p_Box->isleaf <<std::endl; 				std::cout <<
      //"test-true: "  << true << std::endl;
      //				}
      ...
      for (int kb = 0; kb < (1 << Particle::dim); ++kb) {
        if (up_it->p_Box->children[kb] != NULL) {
          ...
          up_it->p_Box->expansions.gatherExpansions(
              &(up_it->p_Box->children[kb]->expansions));
        }
      }
      up_it->p_Box->got_expansions = true;
    }
  ...
  if (rootBox->nParticles > 0) {
    ...
    std::cout << "TotalMass of Root: [ " << rootBox->TotalMass << "]"
              << std::endl;
  }
#endif
}
