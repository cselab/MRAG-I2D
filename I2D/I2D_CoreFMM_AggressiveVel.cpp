...
struct AggressiveExpansion
    : _VortexExpansions<VelocitySourceParticle, _ORDER_> {
  ...
  void evaluateExpansions(Real *location, VelocityRHS *out_RHS) {
    const ExpansionsValueType rp =
        ...
        ExpansionsValueType(this->Center[0], this->Center[1]);
...
typedef HCFMM::boxBuilder_serial<AggressiveExpansion, _FMM_MAX_LEVEL_>
    tBoxBuilder;

class Measurements {
public:
  Measurements(int num_target_blocks)
      : num_direct_evals(num_target_blocks),
        num_indirect_evals(num_target_blocks) {}
...
struct VelocityEvaluator {
  ...
  VelocityEvaluator(tBox *rootNode, const Real inv_scaling,
                    Measurements &_measurements)
      : rootNode(rootNode), inv_scaling(inv_scaling), destblocks(NULL),
        measurements(_measurements) {}

  VelocityEvaluator(const VelocityEvaluator &c)
      : rootNode(c.rootNode), inv_scaling(c.inv_scaling),
        destblocks(c.destblocks), measurements(c.measurements) {}

  // we want to be able to distinguish between far(0) close(1) and very close(2)
  bool _isclose_box(tBox *sourceBox, const BlockInfo &info,
                    tBox::Btype _theta) const {
    const Real h_block = pow(0.5, info.level);

    const Real blockCenter[2] = {(0.5 + info.index[0]) * h_block,
                                 (0.5 + info.index[1]) * h_block};

    const Real b2b_dist =
        sqrt(pow(sourceBox->expansions.Center[0] - blockCenter[0], 2) +
             pow(sourceBox->expansions.Center[1] - blockCenter[1], 2));

    const Real radius_source =
        sqrt(pow(sourceBox->h[0], 2) + pow(sourceBox->h[1], 2));

    const Real radius_target = h_block * sqrt(2.0);
    const Real denom =
        max(b2b_dist - radius_target, std::numeric_limits<Real>::epsilon());
  ...
  bool _is_intersecting(tBox &box, const BlockInfo &info) const {
    ...
    const Real intersection[2] = {
        min(max_pos[0], (Real)(box.center[0] + box.h[0] * 0.5)) -
            max(min_pos[0], (Real)(box.center[0] - box.h[0] * 0.5)),
        min(max_pos[1], (Real)(box.center[1] + box.h[1] * 0.5)) -
            max(min_pos[1], (Real)(box.center[1] - box.h[1] * 0.5))};
    ...
    for (int iblock = range.begin(); iblock < range.end(); iblock++) {
      ...
      while (it1 != NULL && (it1->nParticles > 0)) {
        ...
        if (!is_close && current->parent != NULL) {
          const bool parent_is_close =
              _isclose_box(current->parent, info, _THETA);

          if (parent_is_close) // if we were already well separated from parent,
                               // we should skipt this.
            ...
            for (int iy = 0; iy < B::sizeY; iy++)
              for (int ix = 0; ix < B::sizeX; ix++) {
                ...
                if (current->parent == NULL ||
                    !(ws_barnes_hut(current->parent, target_pos, _THETA))) {
                  VelocityRHS rhs;
          ...
          for (int iy = 0; iy < B::sizeY; iy++)
            for (int ix = 0; ix < B::sizeX; ix++) {
              ...
              if (current->parent == NULL ||
                  !(ws_barnes_hut(current->parent, target_pos, _THETA))) {
                // when we were already well separated from the parent, we
                // should not further interact.
                if (ws_barnes_hut(current, target_pos, _THETA)) {
                  ...
                  if (!it1->isleaf)
                    // Case 2: its not a leaf so we further traverse into the
                  ...
                  else // its close and a leaf ->interactDirectly with particles
                  {
                    ...
                    if (_is_intersecting(*current,
                                         info)) // tBox current and this block
                                                // are intersecting
                      for (int i = 0; i < nof_sources; ++i) {
                        Real r[2] = {target_pos[0] - p[i].x[0],
      ...
      printf("Done with %d %d %d l=%d\n", info.index[0], info.index[1],
             info.index[2], info.level);
#endif
    }
  }
};

void I2D_CoreFMM_AggressiveVel::solve(const Real theta, const Real inv_scaling,
  ...
  if (b_verbose) {
    ...
    const double evaluations_wallclock_time =
        (end - start_before_evaluations).seconds();
    const double expansions_wallclock_time =
        (start_before_evaluations - start_before_expansions).seconds();

    const double num_effective_interactions =
        (double)nparticles * nblocks * _BLOCKSIZE_ * _BLOCKSIZE_;

    double effective_direct_gflops, effective_indirect_gflops,
        effective_total_gflops;
    ...
    effective_indirect_gflops =
        num_indirect_evals * (16 + 14 * _ORDER_) * 1. / 1e9;
    actual_indirect_gflops =
        num_indirect_evals * (20 + 14 * _ORDER_) * 1. / 1e9;

    effective_total_gflops =
        effective_direct_gflops + effective_indirect_gflops;
    ...
    file << setprecision(6) << scientific << num_effective_interactions << "\t"
         << effective_total_gflops << "\t" << actual_total_gflops << "\t"
         << evaluations_wallclock_time << "\t" << expansions_wallclock_time
         << "\t" << tree_wallclock_time << "\n";
    file.close();
  }
  delete rootBox;
