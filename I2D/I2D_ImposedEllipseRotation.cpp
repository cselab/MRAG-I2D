...
I2D_ImposedEllipseRotation::I2D_ImposedEllipseRotation(
    ...
    const Real _xcrot, const Real _ycrot, const Real _T, const Real _D,
    const Real _aspectRatio, const Real eps, const Real Uinf[2],
    I2D_PenalizationOperator &penalization)
    : I2D_ImposedEllipse(parser, grid, _xm, _ym, _xcrot, _ycrot, 0.0, 0.0, _D,
                         _aspectRatio, eps, Uinf, penalization),
      XCROT(_xcrot), YCROT(_ycrot), T(_T), aspectRatio(_aspectRatio), D(_D) {}
...
void I2D_ImposedEllipseRotation::update(
  ...
  if (filename == std::string()) {
    ...
    ppFile =
        fopen("update_I2D_ImposedEllipseRotation.txt", t == 0.0 ? "w" : "a");
    assert(ppFile != NULL);
  } else {
