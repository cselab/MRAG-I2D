 ...
 *
 *  This class is the same as the I2D_LinkedBodies class. It just adds (if
 * possible) a last plate for which the vertical position of the second
 * extremity is constrained to be alligned with the extremity of the leading
 * edge (cf Ingo experiments).
 */
...
public:
  I2D_Ingo(Grid<W, B> &grid, const Real length, const Real width,
           const Real position[2], vector<Real> anglesIn, const Real epsilon)
