...
struct TBB_map_start {
  ...
  TBB_map_start(Real t, vector<PF_Agent *> &agents,
                map<string, vector<PF_Agent *>> *agentCollection,
                vector<bool> &valids)
      : t(t), agents(agents), agentCollection(agentCollection), valids(valids) {
  }
...
struct TBB_choose_no_shared {
  ...
  TBB_choose_no_shared(Real t, vector<PF_Agent *> &agents,
                       map<string, vector<PF_Agent *>> *agentCollection,
                       vector<bool> &valids)
      : t(t), agents(agents), agentCollection(agentCollection), valids(valids) {
  }
...
struct TBB_save {
  ...
  TBB_save(vector<PF_Agent *> &agents, vector<string> &names)
      : agents(agents), names(names) {}
...
struct TBB_restart {
  ...
  TBB_restart(vector<PF_Agent *> &agents, vector<string> &names)
      : agents(agents), names(names) {}
...
struct TBB_learn_no_shared_test {
  ...
  TBB_learn_no_shared_test(Real t, vector<PF_Agent *> &agents,
                           map<string, vector<PF_Agent *>> *agentCollection,
                           vector<string> &names)
      : t(t), agents(agents), agentCollection(agentCollection), names(names) {}
...
struct TBB_map_test {
  ...
  TBB_map_test(Real t, vector<PF_Agent *> &agents,
               map<string, vector<PF_Agent *>> *agentCollection)
      : t(t), agents(agents), agentCollection(agentCollection) {}
...
struct TBB_reward {
  ...
  TBB_reward(Real t, vector<PF_Agent *> &agents,
             map<string, vector<PF_Agent *>> *agentCollection)
      : t(t), agents(agents), agentCollection(agentCollection) {}
...
struct TBB_fitness {
  ...
  TBB_fitness(vector<PF_Agent *> &agents) : agents(agents) {}
...
struct TBB_error {
  ...
  TBB_error(vector<PF_Agent *> &agents) : agents(agents) {}
...
struct TBB_update {
  ...
  TBB_update(Real dt, Real t, vector<PF_Agent *> &agents,
             map<string, vector<PF_Agent *>> *agentCollection)
      : dt(dt), t(t), agents(agents), agentCollection(agentCollection) {}
...
struct TBB_updatePostVelocityUpdate {
  ...
  TBB_updatePostVelocityUpdate(Real dt, Real t, vector<PF_Agent *> &agents,
                               map<string, vector<PF_Agent *>> *agentCollection)
      : dt(dt), t(t), agents(agents), agentCollection(agentCollection) {}
...
PF_AgentVector::PF_AgentVector(MRAG::ArgumentParser &parser, const Real lr,
                               const Real greedyEps, bool isSmooth,
                               bool isControlled,
                               map<string, vector<PF_Agent *>> collection)
    : PF_Agent(parser, lr, greedyEps, isSmooth, isControlled),
      agentCollection(collection) {
  shared = parser("-shared").asBool();

  // Prepare stuff for tbb
  for (map<string, vector<PF_Agent *>>::iterator it = agentCollection.begin();
       it != agentCollection.end(); ++it)
    for (vector<PF_Agent *>::iterator it2 = it->second.begin();
...
PF_AgentVector::~PF_AgentVector() {
  for (vector<PF_Agent *>::iterator it = agents.begin(); it != agents.end();
       ++it)
    if ((*it) != NULL) {
      delete (*it);
      (*it) = NULL;
...
int PF_AgentVector::numberOfAgents() { return (int)agents.size(); }

void PF_AgentVector::update(const Real dt, const Real time,
                            map<string, vector<PF_Agent *>> *collection) {
  TBB_update tbb_update(dt, time, agents, &agentCollection);
  tbb::parallel_for(tbb::blocked_range<size_t>(0, agents.size()), tbb_update,
                    tbb::auto_partitioner());
}

void PF_AgentVector::updatePostVelocityUpdate(
    const Real dt, const Real time,
    map<string, vector<PF_Agent *>> *collection) {
  TBB_updatePostVelocityUpdate tbb_updatePostVelocityUpdate(dt, time, agents,
                                                            &agentCollection);
  tbb::parallel_for(tbb::blocked_range<size_t>(0, agents.size()),
                    tbb_updatePostVelocityUpdate, tbb::auto_partitioner());
}
...
bool PF_AgentVector::choose(const Real time,
                            map<string, vector<PF_Agent *>> *collection) {
  bool valid = true;

  if (!shared) {
    TBB_choose_no_shared choose_no_shared(time, agents, &agentCollection,
                                          valids);
    tbb::parallel_for(tbb::blocked_range<size_t>(0, agents.size()),
                      choose_no_shared, tbb::auto_partitioner());
  } else {
    TBB_map_start tbb_map_start(time, agents, &agentCollection, valids);
    tbb::parallel_for(tbb::blocked_range<size_t>(0, agents.size()),
                      tbb_map_start, tbb::auto_partitioner());
...
void PF_AgentVector::learn(const Real time,
                           map<string, vector<PF_Agent *>> *collection,
                           string filename) {
  if (!shared) {
    // Everything parallel PARALLEL
    TBB_learn_no_shared_test tbb_learn_no_shared_test(
        time, agents, &agentCollection, name_learning);
    tbb::parallel_for(tbb::blocked_range<size_t>(0, agents.size()),
                      tbb_learn_no_shared_test, tbb::auto_partitioner());
  } else {
    // Set reward SERIAL
    for (vector<PF_Agent *>::iterator it = agents.begin(); it != agents.end();
         ++it)
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, agents.size()),
                      tbb_map_test, tbb::auto_partitioner());
...
void PF_AgentVector::reward(const Real time,
                            map<string, vector<PF_Agent *>> *collection) {
  TBB_reward tbb_reward(time, agents, &agentCollection);
  tbb::parallel_for(tbb::blocked_range<size_t>(0, agents.size()), tbb_reward,
                    tbb::auto_partitioner());
}

void PF_AgentVector::fitness() {
  TBB_fitness tbb_fitness(agents);
  tbb::parallel_for(tbb::blocked_range<size_t>(0, agents.size()), tbb_fitness,
                    tbb::auto_partitioner());
}

void PF_AgentVector::error() {
  TBB_error tbb_error(agents);
  tbb::parallel_for(tbb::blocked_range<size_t>(0, agents.size()), tbb_error,
                    tbb::auto_partitioner());
}
...
void PF_AgentVector::savePolicy(string name) {
  if (!shared) {
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, agents.size()), tbb_save,
                      tbb::auto_partitioner());
  } else {
    unsigned int counter = 0;
...
void PF_AgentVector::restartPolicy(string name) {
  if (!shared) {
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, agents.size()), tbb_restart,
                      tbb::auto_partitioner());
  } else {
    unsigned int counter = 0;
    vector<PF_Agent *>::iterator it = agents.begin();
...
void PF_AgentVector::storePosition(
...
void PF_AgentVector::getTargets(
