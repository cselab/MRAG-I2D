...
struct FillBlocksRectangle {
  ...
  FillBlocksRectangle(Real length, Real width, Real epsilon, Real cm[2],
                      Real angle) {
    m_length = length;
    m_width = width;
    m_epsilon = epsilon;
    ...
    if (_is_touching(info)) {
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
          ...
          b(ix, iy).tmp =
              max(I2D_WingAngleOfAttack::mollified_heaviside(-dist, m_epsilon),
                  b(ix, iy).tmp);
        }
    }
  }
...
struct FillBlocks {
  ...
  FillBlocks(const FillBlocks &c) : radius(c.radius), epsilon(c.epsilon) {
    ...
    if (_is_touching(info)) {
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
          ...
          b(ix, iy).tmp = max(
              I2D_WingAngleOfAttack::mollified_heaviside(r - radius, epsilon),
              b(ix, iy).tmp);
        }
    }
  }
...
Real I2D_WingAngleOfAttack::mollified_heaviside(const Real x, const Real eps) {
  const Real alpha = M_PI * min(1., max(0., (x + 0.5 * eps) / eps));

  return 0.5 + 0.5 * cos(alpha);
}

void I2D_WingAngleOfAttack::characteristic_function() {
  ...
  for (unsigned int i = 0; i < anglesPlusOne.size(); i++) {
    if (anglesPlusOne[i] < 0.0) {
      anglesPlusOne[i] += 360.0;
    }
  ...
  // For leading edge as rotation axe :
  for (unsigned int k = 0; k < anglesPlusOne.size(); k++) {
    LinksStuff::FillBlocks fill(width / 2.0, epsilon, cm);
    block_processing.process(vInfo, coll, fill);
    cm[0] += length * cos(anglesPlusOne[k] - (angleAttack * M_PI / 180.0));
    cm[1] += length * sin(anglesPlusOne[k] - (angleAttack * M_PI / 180.0));
  }
  ...
  for (unsigned int k = 0; k < anglesPlusOne.size() - 1; k++) {
    LinksStuff::FillBlocksRectangle fillRectangle(
        length, width, epsilon, cm,
        anglesPlusOne[k] - (angleAttack * M_PI / 180.0));
    block_processing.process(vInfo, coll, fillRectangle);
    cm[0] += length * cos(anglesPlusOne[k] - (angleAttack * M_PI / 180.0));
    cm[1] += length * sin(anglesPlusOne[k] - (angleAttack * M_PI / 180.0));
  }

  /* //good for corrugated bodies :
  //Positioning at the center of the chord
  for(int k=0;k<IndexCenterOfChord;k++)
  {
  ...
  cm[0] = CM[0];
  cm[1] = CM[1];
  X = fabs(xChord-cm[0])*(1-cos(angleAttack*M_PI/180.0));
  Y = X/tan((angleAttack*M_PI/180.0)/2);
  cm[0] += X;
  cm[1] += Y;

  for(int k=0;k<anglesPlusOne.size();k++)
  {
          LinksStuff::FillBlocks fill(width/2.0, epsilon, cm);
          block_processing.process(vInfo, coll, fill);
          cm[0] += length*cos(anglesPlusOne[k]-(angleAttack*M_PI/180.0));
  ...
  for(int k=0;k<anglesPlusOne.size()-1;k++)
  {
          LinksStuff::FillBlocksRectangle fillRectangle(length, width, epsilon,
  cm, anglesPlusOne[k]-(angleAttack*M_PI/180.0));
          block_processing.process(vInfo, coll, fillRectangle);
          cm[0] += length*cos(anglesPlusOne[k]-(angleAttack*M_PI/180.0));
          cm[1] += length*sin(anglesPlusOne[k]-(angleAttack*M_PI/180.0));
  ...
  //Good for flat linked bodies with center of mass as rotation axe:
  ...
  for(int k=1;k<anglesPlusOne.size();k++)
  {
          if(k<=4)
          {
                  cm[0] -= X;
  ...
  cm[0] += X;
  cm[1] += Y;
  LinksStuff::FillBlocksRectangle fillRectangle(length, width, epsilon, cm,
  anglesPlusOne[0]-(angleAttack*M_PI/180.0)); block_processing.process(vInfo,
  coll, fillRectangle);

  for(int k=1;k<anglesPlusOne.size()-1;k++)
  {
          if(k<=4)
          {
                  cm[0] -= X;
