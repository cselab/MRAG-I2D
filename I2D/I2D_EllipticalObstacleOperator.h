...
public:
  I2D_EllipticalObstacleOperator(Grid<W, B> &grid, const Real semiMajorAxis,
                                 const Real aspectRatio, const Real angle,
                                 const Real position[2], const Real epsilon)
      : grid(grid), semiMajorAxis(semiMajorAxis), aspectRatio(aspectRatio),
        angle(angle), epsilon(epsilon) {
