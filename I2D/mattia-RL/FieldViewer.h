...
public:
  ...
  Viewer() : texture_data(NULL), texture_handle(0) { size[0] = size[1] = 0; }
  ...
  void allocate_texture(int sizeX, int sizeY) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, size[0], size[1], 0, GL_RED,
                 GL_FLOAT, 0);
  ...
  void upload_texture() {
    ...
    glBindTexture(GL_TEXTURE_2D, texture_handle);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, size[0], size[1], GL_LUMINANCE,
                    GL_FLOAT, texture_data);
    glBindTexture(GL_TEXTURE_2D, 0);
  }
};
