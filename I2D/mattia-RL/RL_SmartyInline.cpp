...
RL_SmartyInline::RL_SmartyInline(MRAG::ArgumentParser &parser, const Real _x,
    ...
    : x(_x), y(_y), D(_D), T(1), maxDomainRadius(0.45),
      RL_Agent(parser, "RL_SmartyInline", _ID, _policy, seed) {
  modv = D / 5.0;
...
bool RL_SmartyInline::mapState(vector<int> &state,
...
void RL_SmartyInline::reward(const Real t,
                             map<string, vector<RL_Agent *>> *_data) {
...
void RL_SmartyInline::update(const Real dt, const Real t,
                             map<string, vector<RL_Agent *>> *_data,
                             string filename) {
  x += vx * dt;
  y += vy * dt;
