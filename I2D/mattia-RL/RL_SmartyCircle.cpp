...
RL_SmartyCircle::RL_SmartyCircle(MRAG::ArgumentParser &parser, const Real _x,
                                 const Real _y, const Real _D, const int _ID,
                                 RL_TabularPolicy **_policy, const int seed)
    : x(_x), y(_y), D(_D), T(1), maxDomainRadius(0.45),
      RL_Agent(parser, "RL_SmartyCircle", _ID, _policy, seed) {
  modv = D / 5.0;
  vx = modv;
  vy = 0;
...
bool RL_SmartyCircle::mapState(vector<int> &state,
...
void RL_SmartyCircle::reward(const Real t,
                             map<string, vector<RL_Agent *>> *_data) {
...
void RL_SmartyCircle::update(const Real dt, const Real t,
                             map<string, vector<RL_Agent *>> *_data,
                             string filename) {
  x += vx * dt;
  y += vy * dt;
