...
RL_SmartyDodger::RL_SmartyDodger(MRAG::ArgumentParser &parser, const Real _x,
                                 const Real _y, const Real _D, const Real _T,
                                 const int _ID, RL_TabularPolicy **_policy,
                                 const int seed)
    : x(_x), y(_y), D(_D), T(_T),
      RL_Agent(parser, "RL_SmartyDodger", _ID, _policy, seed) {
  modv = D / 5.0;
  vx = modv;
  vy = 0;
...
void RL_SmartyDodger::_mapForMaxRadius(const Real cutoff,
                                       const int idxDistLevel,
  ...
  const Real target[2] = {maxDomainRadius * dir[0] + center[0],
                          maxDomainRadius * dir[1] + center[0]};
...
void RL_SmartyDodger::_mapForColumns(const Real cutoff, const int idxDistLevel,
                                     const int idxAngleLevel, int &idxDist,
                                     int &idxAngle,
                                     map<string, vector<RL_Agent *>> *_data) {
  // Gather my info (always double check with getInfo!!)
  ...
  for (vector<RL_Agent *>::iterator it = columns.begin(); it != columns.end();
       ++it) {
...
void RL_SmartyDodger::_mapForDynamicColumns(
    const Real cutoff, const int idxDistLevel, const int idxAngleLevel,
    int &idxDist, int &idxAngle, map<string, vector<RL_Agent *>> *_data) {
  // Gather my info (always double check with getInfo!!)
  const Real myX[2] = {x, y};
  const Real myV[2] = {vx, vy};
  ...
  Real minDist = numeric_limits<Real>::max();
  for (vector<RL_Agent *>::iterator it = Dcolumns.begin(); it != Dcolumns.end();
       ++it) {
...
bool RL_SmartyDodger::mapState(vector<int> &state,
                               map<string, vector<RL_Agent *>> *_data) {
  assert(_data != NULL);
  assert((*_data).size() != 0);
  ...
  // Map wall
  {
    ...
    _mapForMaxRadius(cutoffWall, signature[0], signature[1], idxDistMaxRadius,
                     idxAngleMaxRadius, _data);
  ...
  // Map columns
  {
    ...
    _mapForColumns(cutoffColumns, signature[2], signature[3], idxDistMaxColumns,
                   idxAngleMaxColumns, _data);
  ...
  // Map dynamic columns
  {
    ...
    _mapForDynamicColumns(cutoffDynamicColumns, signature[4], signature[5],
                          idxDistMaxDynamicColumns, idxAngleMaxDynamicColumns,
                          _data);
...
void RL_SmartyDodger::reward(const double t,
                             map<string, vector<RL_Agent *>> *_data) {
  ...
  if ((t > (learningTimer + learningInterval)) && status == Waiting) {
    ...
    // Wall
    {
      ...
      assert(b != NULL);
      const Real dist =
          max(0.0, sqrt((x - 0.5) * (x - 0.5) + (y - 0.5) * (y - 0.5)) -
                       b->innerD / 2);
    ...
    // Columns
    {
      ...
      // Find the closest
      for (vector<RL_Agent *>::iterator it = columns.begin();
           it != columns.end(); ++it) {
        RL_Column *b = static_cast<RL_Column *>(*it);
        const Real target[2] = {b->x, b->y};
        Real d[2];
    ...
    // DynamicColumns
    {
      ...
      // Find the closest
      for (vector<RL_Agent *>::iterator it = Dcolumns.begin();
           it != Dcolumns.end(); ++it) {
        RL_DynamicColumn *b = static_cast<RL_DynamicColumn *>(*it);
        const Real target[2] = {b->x, b->y};
        Real d[2];
...
void RL_SmartyDodger::update(const double dt, const double t,
                             map<string, vector<RL_Agent *>> *_data,
                             string filename) {
  Real const xold = x;
  Real const yold = y;
