...
private:
    ...
    for (int iter = 0; iter < iterations; iter++) {
      ...
      for (int iy = 0; iy < sizeX; iy++)
        for (int ix = 1; ix < sizeX - 1; ix++) {
          ...
          data[ix + sizeX * iy] =
              (tmp[ix - 1 + sizeX * iy] + tmp[ix + 1 + sizeX * iy]) * 0.25f +
              tmp[ix + sizeX * iy] * 0.5f;
        }
      ...
      for (int iy = 0; iy < sizeX; iy++)
        for (int ix = 0; ix < sizeX; ix++) {
          ...
          const Real myleft =
              (ix == 0) ? tmp[ix + sizeX * iy] : tmp[ix - 1 + sizeX * iy];
          const Real myright = (ix == sizeX - 1) ? tmp[ix + sizeX * iy]
                                                 : tmp[ix + 1 + sizeX * iy];

          data[ix + sizeX * iy] =
              (myleft + myright) * 0.25f + tmp[ix + sizeX * iy] * 0.5f;
        }
      ...
      for (int iy = 1; iy < sizeX - 1; iy++)
        for (int ix = 0; ix < sizeX; ix++) {
          ...
          data[ix + sizeX * iy] =
              (tmp[ix + sizeX * (iy - 1)] + tmp[ix + sizeX * (iy + 1)]) *
                  0.25f +
              tmp[ix + sizeX * iy] * 0.5f;
        }
      ...
      for (int iy = 0; iy < sizeX; iy++)
        for (int ix = 0; ix < sizeX; ix++) {
          ...
          const Real mydown =
              (iy == 0) ? tmp[ix + sizeX * iy] : tmp[ix + sizeX * (iy - 1)];
          const Real myup = (iy == sizeX - 1) ? tmp[ix + sizeX * iy]
                                              : tmp[ix + sizeX * (iy + 1)];

          data[ix + sizeX * iy] =
              (mydown + myup) * 0.25f + tmp[ix + sizeX * iy] * 0.5f;
        }
    }
  ...
  Real _sample(const Real x, const Real y) const {
    ...
    const Real wx[4] = {_bsp4(xr + 1), _bsp4(xr + 0), _bsp4(xr - 1),
                        _bsp4(xr - 2)};
    const Real wy[4] = {_bsp4(yr + 1), _bsp4(yr + 0), _bsp4(yr - 1),
                        _bsp4(yr - 2)};
...
public:
  ...
  Wheel(Real xm, Real ym, Real width, Real angle_rad, const int sizeX = 512)
      : sizeX(sizeX), data(NULL), N(sizeX * sizeX), angle(angle_rad),
        width(width), xm(xm), ym(ym), J(0), angular_velocity(0), area(0) {
    assert(sizeX > 4);
    ...
    if (sWHEELTYPE == "cookie") {
      ...
      for (int iy = 0; iy < sizeX; iy++)
        for (int ix = 0; ix < sizeX; ix++) {
          ...
          const double t =
              -((double)(vx * x1) - (double)(vy * y0) - (double)(vx * x0) +
                sqrt((double)(-2 * vx * x1 * vy * y0 + 2 * vy * y0 * vx * x0 -
                              vx * vx * y0 * y0 + 4 * vx * vx * R * R +
                              2 * vy * vy * x0 * x1 - vy * vy * x1 * x1 -
                              vy * vy * x0 * x0 + 4 * vy * vy * R * R))) /
              (double)(vx * vx + vy * vy) / 0.2e1;

          const double xc = (x0 + x1) * 0.5 + vx * t;
          const double yc = (y0 + y1) * 0.5 + vy * t;

          data[ix + sizeX * iy] =
              pow(xc - r * cos(alpha), 2) + pow(yc - r * sin(alpha), 2) <
              R * R; // < 0);
        }
      ...
      for (int iy = 0; iy < sizeX; iy++)
        for (int ix = 0; ix < sizeX; ix++) {
          ...
          const double f = y0; // pow(x-x0,2)*(y0/(5*(x1-x0)*(x2-x0))) +
                               // (x-x0)*(-y0/(5*(x1-x0))) + y0;
...

          {
            ...
            const double yg =
                1 / (R * 2) * pow(xg - x1, 2) +
                (y0 / (x2 - x1) - (x2 - x1) / (R * 2)) * (xg - x1);
            const double fprime =
                1 / R * (xg - x1) + (y0 / (x2 - x1) - (x2 - x1) / (R * 2));
            ...
            const double check_dist =
                sqrt(pow(xg - xfit, 2) + pow(yg - yfit, 2));
          ...
          const bool b3 =
              (x >= x1 && x <= x2 && y <= f &&
               y >= 1 / (R * 2) * pow(x - x1, 2) +
                        (y0 / (x2 - x1) - (x2 - x1) / (R * 2)) * (x - x1));
          data[ix + sizeX * iy] =
              (b1 || b2 || b3) && bExtra; // pow(xc - r*cos(alpha),2) + pow(yc -
                                          // r*sin(alpha), 2) < R*R;// < 0);
        }
    } else {
  ...
  void update_angle(double torque, double dt) {
...

    {
      ...
      fprintf(f, "%e %e %e %e %e %e\n", angle, angular_velocity, torque,
              (torque / J) * (dt / area), torque * dt, 1. / J / area);
      fclose(f);
    }
  }
  ...
  Real sample(const Real x_, const Real y_) const {
    ...
    if (xobject[0] < -0.5 - myh || xobject[0] > 0.5 + myh)
      return 0;
    if (xobject[1] < -0.5 - myh || xobject[1] > 0.5 + myh)
      return 0;

    return _sample((xobject[0] + 0.5) * (sizeX - 1),
                   (xobject[1] + 0.5) * (sizeX - 1));
  }
...
public:
  ...
  static void update_angle(vector<double> omegabar, double dt, double t) {
    ...
    //-------------------------------------- the following part is a request
    //from petros
    ...
    output << endl;
    //-------------------------------------- the preceding part is a request
    //from petros
  }
};
...
struct FillBlocks {
  ...
  static bool _is_touching(Real eps, const Wheel &wheel,
...
struct ComputeOmegaBar {
  ...
  ComputeOmegaBar(Real eps, map<int, vector<double>> &b2sum, Real Uinf[2],
                  map<int, bool> &b2nonempty)
  ...
  ComputeOmegaBar(const ComputeOmegaBar &c)
      ...
      if (FillBlocks::_is_touching(eps, mywheel, info)) {
        ...
        for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
          for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
            ...
            s += Xs * ((b(ix, iy).u[1] + Uinf[1]) * (p[0] - mywheel.xm) -
                       (b(ix, iy).u[0] + Uinf[0]) * (p[1] - mywheel.ym));
          }
...
struct FillVelblocks {
  ...
  FillVelblocks(vector<pair<BlockInfo, VelocityBlock *>> &workitems)
      : workitems(workitems) {}
...
struct NonEmpty {
  ...
  NonEmpty(Real eps, map<int, bool> &b2nonempty)
...
I2D_RotatingWheelObstacle::I2D_RotatingWheelObstacle(
    ArgumentParser &parser, Grid<W, B> &grid, const Real smoothing_length,
    const Real radius, const Real Uinf[2],
    I2D_PenalizationOperator &penalization)
    : I2D_FloatingObstacleOperator(parser, grid, 2 * radius, smoothing_length,
                                   Uinf, penalization),
      grid(grid), smoothing_length(smoothing_length), radius(radius),
  ...
  if (!input.good()) {
    ...
    RotatingWheels::Wheel::wheels.push_back(
        new RotatingWheels::Wheel(0.5, 0.5, 2 * radius, -M_PI * 0.36));
    RotatingWheels::Wheel::wheels.push_back(new RotatingWheels::Wheel(
        0.5 + d * cos(a), 0.5 + d * sin(a), 2 * radius, -M_PI * -1.3));
    RotatingWheels::Wheel::wheels.push_back(
        new RotatingWheels::Wheel(0.5 + d * cos(2 * a), 0.5 + d * sin(2 * a),
                                  2 * radius, -M_PI / 2 * 2.34));

    return;
  }

  //-------------------------------------- the following part is a request from
  //petros
  ...
  for (int i = 0; i < NWHEELS; i++) {
    ...
    cout << "New wheel (x, y, alpha) = " << x << ", " << y << ", " << alpha
         << " degrees." << endl;

    RotatingWheels::Wheel::wheels.push_back(
        new RotatingWheels::Wheel(x, y, 2 * radius, alpha));
  }

  //-------------------------------------- the preceding part is a request from
  //petros
}
...
void I2D_RotatingWheelObstacle::characteristic_function() {
  ...
  for (vector<RotatingWheels::Wheel *>::const_iterator it =
           RotatingWheels::Wheel::wheels.begin();
       it != RotatingWheels::Wheel::wheels.end(); ++it)
    assert(*it != NULL);
...
void I2D_RotatingWheelObstacle::update(
    const double dt, const double t, string,
  ...
  RotatingWheels::ComputeOmegaBar compute(smoothing_length, integrals, Uinf,
                                          nonempty);
  block_processing.process(vInfo, coll, compute);
