 ...
 *
 *	This operator integrate the diffusion term with a fourth order
 * finite-difference scheme and RK2. It uses LTS (if the dt is big enough).
 * Tested!
 *
 *	perform_timestep():
 *	IN: omega[0-2]
...
public:
  I2D_SolverOperator(Grid<W, B> &grid, double viscosity,
                     I2D_VelocityOperator *velsolver_)
      : grid(grid), viscosity(viscosity), velsolver(velsolver_) {
...
public:
  I2D_SolverOperator_4thOrder(Grid<W, B> &grid, double viscosity,
                              I2D_VelocityOperator *velsolver_)
      : I2D_SolverOperator(grid, viscosity, velsolver_) {}
