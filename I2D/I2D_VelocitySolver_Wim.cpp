...
struct myExpansions : _VortexExpansions<VelocitySourceParticle, _ORDER_> {
  void evaluateExpansions(Real *location, VelocityRHS *out_RHS,
                          const Real theta, Real &errorBound) {
...
#pragma unroll
    ...
    errorBound = 1.0 / std::abs(rp) * std::pow(theta, _ORDER_) /
                 (1 - theta); // prefactor A multiplication done outside
  }
};
...
struct Core_VelocitySolver {
  ...
  int _createSourceParticles(tParticle *&sources, const Real tolParticle,
    ...
    static const int BS =
        FluidBlock2D::sizeZ * FluidBlock2D::sizeY * FluidBlock2D::sizeX;
    ...
    {
      int n = 0;
      for (vector<BlockInfo>::iterator itBlock = vInfo.begin();
           itBlock != vInfo.end(); ++itBlock) {
        FluidElement2D *const e = &coll[itBlock->blockID](0, 0, 0);
        for (int i = 0; i < BS; i++)
          n += (int)(fabs(e[i].omega) > tolParticle);
      }
  ...
  struct VelocityEvaluatorWim {
    ...
    VelocityEvaluatorWim(tBox *rootNode, const Real inv_scaling,
                         const Real theta)
    ...
    bool isClosePoint(Real srcBoxCtr[2], Real srcBoxRad,
                      const Real trgPos[2]) const {
      const Real p2b_dist =
          std::sqrt((srcBoxCtr[0] - trgPos[0]) * (srcBoxCtr[0] - trgPos[0]) +
                    (srcBoxCtr[1] - trgPos[1]) * (srcBoxCtr[1] - trgPos[1]));

      const Real denom =
          std::max(p2b_dist, std::numeric_limits<Real>::epsilon());
      return (srcBoxRad >= theta * denom);
    }

    bool isIntersectingPoint(Real srcBoxCtr[2], Real srcBoxRad,
                             const Real trgBoxCtr[2]) const {
      const Real intersection[2] = {
          min(trgBoxCtr[0], srcBoxCtr[0] + srcBoxRad) -
              max(trgBoxCtr[0], srcBoxCtr[0] - srcBoxRad),
          min(trgBoxCtr[1], srcBoxCtr[1] + srcBoxRad) -
              max(trgBoxCtr[1], srcBoxCtr[1] - srcBoxRad)};

      return intersection[0] >= 0 && intersection[1] >= 0;
    }

    bool isCloseBox(Real srcBoxCtr[2], Real srcBoxRad, const Real trgBoxCtr[2],
                    const Real halfTrgBoxWidth) const {
      ...
      const Real dx =
          x < x0 ? (x0 - x)
                 : (x > x1 ? (x - x1) : 0); // can replace this with min/max
      const Real dy = y < y0 ? (y0 - y) : (y > y1 ? (y - y1) : 0);
      const Real min_p2bDist = std::sqrt(dx * dx + dy * dy);

      const Real denom =
          std::max(min_p2bDist, std::numeric_limits<Real>::epsilon());
      return (srcBoxRad >= theta * denom);
    }

    bool isIntersectingBox(Real srcBoxCtr[2], Real srcBoxRad,
                           const Real trgBoxCtr[2],
                           const Real halfTrgBoxWidth) const {
      const Real intersection[3] = {
          std::min(trgBoxCtr[0] + halfTrgBoxWidth, srcBoxCtr[0] + srcBoxRad) -
              std::max(trgBoxCtr[0] - halfTrgBoxWidth,
                       srcBoxCtr[0] - srcBoxRad),
          std::min(trgBoxCtr[1] + halfTrgBoxWidth, srcBoxCtr[1] + srcBoxRad) -
              std::max(trgBoxCtr[1] - halfTrgBoxWidth,
                       srcBoxCtr[1] - srcBoxRad),
      };

      return intersection[0] >= 0 && intersection[1] >= 0;
    }

    std::pair<Real, Real> evaluateDirect(tBox *srcBox,
                                         const Real trgPos[2]) const {
      Real u = 0, v = 0;
    ...
    //            Real evaluateDirectAlgebraic(tBox * srcBox, const Real
    //            trgPos[3], const Real eps_in=-1) const
    //            {
    //                Real phi=0;
    //                const tParticle * const p = srcBox->vparticles;
    //                const Real epsSq = eps_in > 0 ? eps_in*eps_in :
    //                EpsilonSmoothKernel::epsSq; const Real threeHalfepsSq
    //                = 1.5*epsSq;
    //
    //                for(int i=0;i<srcBox->nParticles;i++)
    //                {
    //                    const Real distSq=(
    //                    (p[i].x[0]-trgPos[0])*(p[i].x[0]-trgPos[0]) +
    //                                       (p[i].x[1]-trgPos[1])*(p[i].x[1]-trgPos[1])
    //                                       +
    //                                       (p[i].x[2]-trgPos[2])*(p[i].x[2]-trgPos[2]));
    //                    phi+=p[i].w[0]*(distSq + threeHalfepsSq)/((distSq +
    //                    epsSq)*std::sqrt(distSq + epsSq));
    //                }
    //                return phi;
    //            }
      ...
      for (int iy = 0; iy < B::sizeY; iy++)
        for (int ix = 0; ix < B::sizeX; ix++) {
          ...
          while (srcBox != NULL && (srcBox->nParticles > 0)) {
            ...
            const bool isClose =
                isClosePoint(srcBox->expansions.Center,
                             srcBox->expansions.Radius, target_pos);
            const bool isIntersecting =
                isIntersectingPoint(srcBox->expansions.Center,
                                    srcBox->expansions.Radius, target_pos);
            if (isClose || isIntersecting) {
              ...
              VelocityRHS rhs;
              Real errorBound = 0.0;
              srcBox->expansions.evaluateExpansions(target_pos, &rhs, theta,
                                                    errorBound);
              b(ix, iy).u[0] += rhs.x[0];
              b(ix, iy).u[1] += rhs.x[1];
              b(ix, iy).tmp +=
                  errorBound * srcBox->TotalMass / (2.0 * M_PI) * inv_scaling;
            }
...
public:
  Core_VelocitySolver(Grid<W, B> &grid) : grid(grid) {}
  ...
  int execute(BlockProcessing &block_processing, double tolParticle,
    ...
    const int nof_sources =
        _createSourceParticles(sources, tolParticle, in_scaling_factor);
...
void I2D_VelocitySolver_Wim::compute_velocity() {
  ...
  const long long nsources =
      core.execute(block_processing, tolParticle, scaling_factor, theta);
