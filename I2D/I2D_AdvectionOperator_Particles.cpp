...
struct GetGradUMax : I2D_GradOfVector_4thOrder {
  ...
  GetGradUMax(map<int, Real> &local_max_gradu)
      : local_max_gradu(local_max_gradu), t(0) {
  ...
  struct TmpMax {
    static inline void stream(FluidElement2D &out, Real in) {
      out.tmp = max((Real)fabs(in), out.tmp);
    }
  };
...
Real I2D_AdvectionOperator_Particles::estimate_largest_dt() {
  ...
  block_processing.process<I2D_VectorBlockLab<Streamer_Velocity, 2>::Lab>(
      vInfo, coll, binfo, compute_max_val);

  Real maxval = 0;
  for (map<int, Real>::iterator it = max_grad_u.begin(); it != max_grad_u.end();
       it++)
...
void I2D_AdvectionOperator_Particles::perform_timestep(double dt) {
  ...
  block_processing.process<I2D_ParticleBlockLab>(vInfo, coll, binfo,
                                                 push_remesh);
