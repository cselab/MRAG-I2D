...
I2D_ImposedEllipse::Ellipse::Ellipse(Real xm, Real ym, Real xcrot, Real ycrot,
                                     Real D, Real angle_rad, Real aspectRatio)
    : angle(angle_rad), D(D), xm(xm), ym(ym), xcrot(xcrot), ycrot(ycrot),
      aspectRatio(aspectRatio), angular_velocity(0), vx(0), vy(0), rho(1.0) {
  m = rho * M_PI * (D / 2.0) * (D / 2.0);
  J = 0.25 * M_PI * (D / 2.0) * (D / 2.0) * aspectRatio *
      ((D / 2.0) * (D / 2.0) * (1 + (aspectRatio * aspectRatio)));
}

Real I2D_ImposedEllipse::Ellipse::_mollified_heaviside(const double dist,
...
/*
const I2D_ImposedEllipse::Ellipse::Ellipse&
I2D_ImposedEllipse::Ellipse::operator=(const Ellipse& c)
{
...
Real I2D_ImposedEllipse::Ellipse::sample(const Real x_, const Real y_,
  ...
  const double radius =
      a * b /
      sqrt(b * b * cos(alpha) * cos(alpha) + a * a * sin(alpha) * sin(alpha));
  const double dist =
      sqrt((x_ - xm) * (x_ - xm) + (y_ - ym) * (y_ - ym)) - radius;
  ...
  // const double radius = a*b/sqrt(b*b*cos(alpha)*cos(alpha) +
  // a*a*sin(alpha)*sin(alpha)); const double dist = sqrt( (xe)*(xe) + (ye)*(ye)
  // ) - radius;
...
void I2D_ImposedEllipse::Ellipse::bbox(const Real eps, Real xmin[2],
                                       Real xmax[2]) const {
  xmin[0] = xm - D / 2.0;
  xmax[0] = xm + D / 2.0;
...
struct FillBlocks {
  ...
  FillBlocks(Real eps, I2D_ImposedEllipse::Ellipse *_shape) : eps(eps) {
    shape = _shape;
  }

  FillBlocks(const FillBlocks &c) : eps(c.eps) { shape = c.shape; }

  static bool _is_touching(Real eps, const I2D_ImposedEllipse::Ellipse *wheel,
...
struct GetNonEmpty {
  ...
  GetNonEmpty(Real eps, I2D_ImposedEllipse::Ellipse *_shape,
              map<int, bool> &b2nonempty)
      ...
      for (int iy = 0; iy < FluidBlock2D::sizeY; iy++)
        for (int ix = 0; ix < FluidBlock2D::sizeX; ix++) {
          ...
          const Real Xs = shape->sample(
              p[0], p[1],
              eps); // calculate characteristic function (outside body 0, inside
                    // body 1, between: mollified heaviside
...
struct FillVelblocks {
  ...
  FillVelblocks(vector<pair<BlockInfo, VelocityBlock *>> &workitems, double eps,
                I2D_ImposedEllipse::Ellipse *_shape)
...
I2D_ImposedEllipse::I2D_ImposedEllipse(ArgumentParser &parser, Grid<W, B> &grid,
                                       const Real _xm, const Real _ym,
                                       const Real _xcrot, const Real _ycrot,
                                       const Real _vx, const Real _vy,
                                       const Real _D, const Real aspectRatio,
...
void I2D_ImposedEllipse::update(
