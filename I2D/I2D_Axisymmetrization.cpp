...
public:
  ...
  void compute(Grid<W, B> &grid, Real t) {
    ...
    file << setprecision(6) << scientific << t << "\t" << max_omega << "\t"
         << Gamma << "\t" << nu_eff << "\t" << Re_eff << "\t" << lambda_eff
         << "\t" << enstrophy << "\n";
...
public:
  ...
  Real getOmega(Real x, Real y) {
    const Real r = std::sqrt(m_AR * m_AR * (x - m_cx) * (x - m_cx) +
                             (y - m_cy) * (y - m_cy));
    const Real z = r / m_r0;
    return (z <= 1.0 ? m_lambda *
                           (1.0 - std::exp(-(m_q / z) * std::exp(1. / (z - 1))))
                     : 0.0);
  }

  Real getMaxOmega() { return m_lambda * scaleFactT; }
};
...
I2D_Axisymmetrization::I2D_Axisymmetrization(const int argc, const char **argv)
    : I2D_Test(), parser(argc, argv), t(0), step_id(0), velsolver(NULL),
      advection(NULL), initialCondition(NULL) {
  printf("////////////////////////////////////////////////////////////\n");
...
set<int> I2D_Axisymmetrization::_getBoundaryBlockIDs() {
  ...
  if (sRIGID_INLET_TYPE == "rigid_frame") // RIGID AT EACH BOUNDARY
    ...
    printf("I2D_Axisymmetrization::_getBoundaryBlockIDs: the chosen "
...
void I2D_Axisymmetrization::_tnext(Real &tnext, Real &tnext_dump, Real &tend) {
...

  {
    ...
    const Real delta =
        nondim_factor_time /
        DUMPFREQ; // wim: changed this to DUMPFREQ dumps per non-dim time. dunno
                  // what was the shit above.. wtf mattia
  ...
  // 2.
  {
    ...
    fprintf(f, "stepid=%d\tT=%e\tDT=%e\t", step_id, tnext * nondim_factor_time,
            (tnext - t) * nondim_factor_time);
    for (int i = 0; i < tnext_candidates.size(); i++)
    ...
    for (int i = 0; i < ADAPTFREQ; i++) {
...

      {
        ...
        if (f != NULL) {
          ...
          fprintf(f, "%d\t%e\t%e\t%d\t%d\n", step_id, wallclock, Tcurr, nblocks,
                  advection->get_nofrhs());
